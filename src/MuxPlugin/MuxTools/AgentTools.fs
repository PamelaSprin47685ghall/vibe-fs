                  ...
                  async {
                      let! reports =
                          intentsArr |> Array.map (fun intent ->
                              ...
                              delegateToSubAgent config "exec" prompt "Editor" (Some (createObj [ "experiments", box (experimentsFor Editor) ]))
                              |> Async.AwaitPromise) |> Async.Parallel
                      return String.concat "\n---\n" (List.ofArray reports)
                  } |> Async.StartAsPromise
      condition = None }

let greperTool : ToolDefinition =
    { name = "greper"
      description = greper
                  ...
                  async {
                      let! reports =
                          intents |> Array.map (fun intent ->
                              delegateToSubAgent config "explore" intent "Greper" (Some (createObj [ "experiments", box (experimentsFor Greper) ]))
                              |> Async.AwaitPromise) |> Async.Parallel
                      return String.concat "\n---\n" (List.ofArray reports)
                  } |> Async.StartAsPromise
      condition = None }

let reverieTool : ToolDefinition =
    { name = "reverie"
      description = reverie
      ...
      execute = fun config args ->
          if strField config "workspaceId" = None then resolveStr "reverie requires workspaceId"
              ...
              delegateToSubAgent config "explore" prompt "Reverie" (Some (createObj [ "experiments", box (experimentsFor Reverie) ]))
      condition = None }

let browserTool : ToolDefinition =
    { name = "browser"
      description = browser
      ...
      execute = fun config args ->
          if strField config "workspaceId" = None then resolveStr "browser requires workspaceId"
              ...
              let intent = defaultArg (strField args "intent") ""
              delegateToSubAgent config "explore" intent "Browser" (Some (createObj [ "experiments", box (experimentsFor Browser) ]))
      condition = None }
