...
let private normalizeIPv6 (ip: string) =
    ...
    if lower.StartsWith("::ffff:", StringComparison.Ordinal) then
        ...
        if v4part.IndexOf '.' >= 0 then v4part else lower
    else
        lower
...
let private isPrivateIPv6 (ip: string) =
    ...
    then
        ...
        normalized.StartsWith("fe8", StringComparison.Ordinal)
...
let private looksLikeIPv4 (ip: string) =
    ...
    parts.Length = 4
    && parts
       |> Array.forall (fun part -> part.Length > 0 && part.Length <= 3 && part |> Seq.forall Char.IsDigit)

let private looksLikeIPv6 (ip: string) = ip.Contains ':'

let private isIpBlocked (ip: string) =
    if looksLikeIPv4 ip then isPrivateIPv4 ip
    elif looksLikeIPv6 ip then isPrivateIPv6 ip
    else true
...
let private hostForValidation (url: string) (parsedHost: string) =
    if urlContainsBlockedIpv6Literal url then
        ...
        parsedHost = "["
        || (parsedHost.StartsWith("[", StringComparison.Ordinal)
            && not (parsedHost.EndsWith("]", StringComparison.Ordinal)))
    then
        let lower = url.ToLowerInvariant()
