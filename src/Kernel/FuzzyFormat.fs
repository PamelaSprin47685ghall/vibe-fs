...
let fileAnnotation (item: FileAnnotation option) : string =
    ...
    | Some i ->
        ...
        | _ ->
            let frecency =
                i.totalFrecencyScore
                |> Option.orElse i.accessFrecencyScore
...
let formatFindOutput (result: FindResult option) : string =
    ...
    | Some r ->
        if r.items.IsEmpty then
            ...
            String.concat
                "\n"
                ([ $"{total} matching {plural} ({r.totalFiles} total indexed)"; "" ]
                 @ (r.items |> List.map (fun m -> $"{m.relativePath}{fileAnnotation m.annotation}")))
