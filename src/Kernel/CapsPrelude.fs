...
## 关于工具纠偏（amend）
- 工具调用失败、参数错误或发现方向走错时，可通过在当前工具调用参数中附加 `amend = N` ($N > 0$) 启动纠偏。
- `amend = N` 将在消息投影中回溯并剪除最近的 $N$ 组工具调用链（含调用、返回及中间推理），防历史污染。
- 纠偏本身作为新事实起点保留在消息历史中。严禁对正常调用无故附加 `amend`。
...
- 禁止临时测试：修 bug/探边界不得依赖一次性脚本、未入库用例。调试过程永久化→把复现路径与期望断言写成正式回归用例。
