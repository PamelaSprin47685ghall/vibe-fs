...
let internal fuzzyFindSpec: ToolSpec =
    { name = "fuzzy_find"
      ...
      paramDocs =
        map
            [ "pattern", """Plain fuzzy file path text to search for. Pass a real JSON array of strings for parallel search; never pass a stringified JSON string. Correct: ["src","build"]. Wrong: "[\"src\",\"build\"]" (a string, not an array)."""
...
let internal fuzzyGrepSpec: ToolSpec =
    { name = "fuzzy_grep"
      ...
      paramDocs =
        map
            [ "pattern", """Search pattern. Pass a real JSON array of strings for parallel search; never pass a stringified JSON string. Required on the first call. Correct: ["StateMachine","EventLog"]. Wrong: "[\"StateMachine\",\"EventLog\"]" (a string, not an array)."""
