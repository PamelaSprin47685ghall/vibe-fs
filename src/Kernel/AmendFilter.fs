...
/// Extract all callIDs from a message's ToolParts.
...
let popOneToolCall (messages: Message<'raw> list) : (Message<'raw> list * Message<'raw> list) =
    ...
    | _ ->
        let lastToolCallIdx =
            messages
            |> List.indexed
            |> List.tryFindBack (fun (_, msg) ->
                msg.info.role = Assistant && List.exists partIsTool msg.parts)
            |> Option.map fst
        ...
        | Some idx ->
            ...
            let endIdx =
                if Set.isEmpty callIDs then
                    ...
                    |> List.filter (fun (i, m) ->
                        ...
                        && (Set.isEmpty callIDs || List.isEmpty (getCallIDs m) || List.exists (fun cid -> Set.contains cid callIDs) (getCallIDs m)))
                    |> List.map fst
                    |> function
                        | [] -> idx
            ...
            let after = if endIdx >= messages.Length - 1 then [] else messages.[endIdx + 1..]
