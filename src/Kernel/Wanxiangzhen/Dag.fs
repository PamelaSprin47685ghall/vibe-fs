module Wanxiangshu.Kernel.Wanxiangzhen.Dag
...
let addTask (task: SquadTask) (dag: Dag) : Dag =
    { dag with
        Tasks = dag.Tasks.Add(task.Id, task) }

let updateTask (taskId: string) (f: SquadTask -> SquadTask) (dag: Dag) : Dag =
    match dag.Tasks.TryFind taskId with
    | Some task ->
        { dag with
            Tasks = dag.Tasks.Add(taskId, f task) }
    | None -> dag

let findTask (taskId: string) (dag: Dag) : SquadTask option = dag.Tasks.TryFind taskId

let isReady (task: SquadTask) (dag: Dag) : bool =
    task.Status = Pending
    && task.DependsOn
...
let readyTasks (dag: Dag) : SquadTask list =
    dag.Tasks
    |> Map.toList
    |> List.map snd
...
let formatDag (dag: Dag) : string =
    ...
    if dag.RootRequirement <> "" then
        sb.Append(sprintf "\nRequirement: %s" dag.RootRequirement) |> ignore

    if dag.Tasks.IsEmpty then
        sb.Append("\n(no tasks)") |> ignore
    else
        sb.Append("\n") |> ignore
        ...
        for t in dag.Tasks |> Map.toList |> List.map snd |> List.sortBy (fun t -> t.Id) do
            let deps =
                if t.DependsOn = [] then
                    ...
                    t.DependsOn |> String.concat ", "

            sb.Append(sprintf "\n  [%s] %s | deps: %s | %s" t.Id t.Title deps (statusToString t.Status))
            |> ignore
