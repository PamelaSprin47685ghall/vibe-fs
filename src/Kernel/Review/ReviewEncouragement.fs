...
/// Instruction to immediately call todowrite and continue, used in WIP ack.
let updateTodoAndContinue: string =
    "Immediately call todowrite to record what is complete and the next "
    + "concrete steps, then continue working. Do not stop or wait after this acknowledgment."

/// Todo nudge — reminds the model to update the plan.
let todoNudge: string =
    "You are making progress. There are still incomplete todos. "
    + "Take the time you need, update the plan, and continue one concrete "
...
/// and instructs it to call todowrite and continue.
let wipAcknowledgment: string =
    ...
    + "Immediately call todowrite to record what is complete and the next concrete\n"
    + "steps, then continue working. Do not stop or wait after this acknowledgment.\n\n"
...
let needsRevisionVerdict: string =
    ...
    + "feedback item carefully, update todowrite, and continue for as long as needed.\n"
...
let terminatedVerdict: string =
    ...
    + "With-Review Mode remains active. Update todowrite, verify the current state,\n"
