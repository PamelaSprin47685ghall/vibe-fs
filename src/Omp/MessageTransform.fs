...
let private defaultMagicSession = MagicSession(omp)
    ...
    promise {
        if Dyn.isNullish entriesObj || not (Dyn.isArray entriesObj) then return entriesObj :?> obj array
            ...
            if entriesArr.Length = 0 then return entriesArr
                ...
                let backlog = defaultMagicSession.GetOrRebuildBacklog(sessionId, cleaned)
                let afterMagic = projectMagicFor defaultMagicSession.Host cleaned backlog false sessionId
...
let compactingHandler (_magicSession: MagicSession) (_input: obj) (_output: obj) : JS.Promise<unit> =
    Promise.lift ()
