...
let private runExecutorJob (pi: obj) (ctx: obj) (parentId: string) (childId: string) (options: ExecuteOptions) (signal: obj) =
    promise {
        let runWork =
            sessionExecutor.EnqueuePerSession(
                ...
                fun () ->
                    promise {
                        ...
                        if parentId <> "" then appendRunnerLog ompScope parentId output
                        return r
                    })
        let onSignalAbort () =
            if parentId <> "" then abortRunnerJob ompScope parentId |> ignore
            if childId <> "" then abortExecutorRun childId
        return! raceWithAbortSignal signal onSignalAbort runWork
    }

let private summarizeOutput (pi: obj) (childSession: obj) (output: string) (lang: string) (program: string) (deps: string list) (mode: string) (what: string) =
    promise {
        let summaryPrompt = executorSummarizerPrompt what output lang program deps "executor" mode
...
let private executeExecutor (pi: obj) (_id: string) (params': obj) (signal: obj) (_onUpdate: obj) (ctx: obj) =
    promise {
        ...
        let (lang, program, what, timeoutType, mode, deps, cwd) = parseExecutorParams params' ctx
