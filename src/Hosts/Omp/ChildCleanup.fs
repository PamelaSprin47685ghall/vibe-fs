...
/// R-03 fix: review / executor child sessions MUST be torn down on
...
/// teardown.
///
/// CleanupChildSession is best-effort: every operation is wrapped in
/// its own try/with so a single failure (e.g. the host already
/// aborted the session internally) does not prevent the others from
/// running.  Callers MUST invoke it from a finally block or as the
/// last statement of every branch.
