module Wanxiangshu.Hosts.Omp.MessagingCodecEncode
...
let encodeMessage (msg: Message<obj>) : obj =
    if isNull msg.raw then
        ...
        box (
            createObj
                [ "info", box (createObj [ "id", box msg.info.id; "role", box (encodeRole msg.info.role) ])
                  "parts", box partsObj ]
        )
    else
        ...
        let partsUnchanged =
            not (Dyn.isNullish rawParts)
            && Dyn.isArray rawParts
        ...
        if partsUnchanged then
            ...
            Dyn.withKey msg.raw "parts" (box encodedParts)
