        ...
        if not (Dyn.isNullish isIdleVal) && Dyn.typeIs isIdleVal "boolean" then
            ...
            if not (Dyn.isNullish isBusyVal) && Dyn.typeIs isBusyVal "boolean" then
                ...
                if not (Dyn.isNullish statusVal) && Dyn.typeIs statusVal "string" then
                    ...
                    match status with
                    | "idle" | "closed" | "completed" | "done" | "stopped" -> Some Stopped
                    | "busy" | "running" | "active" | "pending" -> Some StillRunning
                    | _ -> None
                else
        ...
        try
            ...
            if not (Dyn.isNullish statusFn) && Dyn.typeIs statusFn "function" then
                ...
                if Dyn.typeIs resp "string" then
                    ...
                    match status with
                    | "idle" | "closed" | "completed" | "done" | "stopped" -> return Some Stopped
                    | "busy" | "running" | "active" | "pending" -> return Some StillRunning
                    | _ -> return None
                else
...
let private tryClosePiSession (sessionApi: obj) (sessionId: SessionId) : JS.Promise<QuiescenceStatus option> =
    promise {
        let arg = box {| sessionId = SessionId.value sessionId |}
        let fnNames = [| "sessionDelete"; "deleteSession"; "sessionClose"; "closeSession"; "delete"; "close" |]
                    ...
                    let! dataOpt =
                        promise {
                            if not (Dyn.isNullish sessionApi) then
                                ...
                                if not (Dyn.isNullish sm) then return Some(Dyn.get sm "messages")
                                else return None
                        }
                    match dataOpt with
                    | Some data when not (Dyn.isNullish data) && Dyn.isArray data ->
                        ...
                        for msg in msgs do
                            ...
                            if not (Dyn.isNullish info) then
                                ...
                                if cId1 = target || cId2 = target then found <- true
                        if found then return DispatchStatus.Accepted OrderedTurnMarkerObserved
                        else return DispatchStatus.Unknown
