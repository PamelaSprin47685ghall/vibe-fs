module Wanxiangshu.Hosts.Omp.AgentConfig
...
let private ompBuiltinAgentSpecs =
    [ { name = "manager"
        defaultMode = "primary"
        systemPrompt = managerSystemPromptFor omp
        defaultMcps = [||] }
      { name = "build"
        defaultMode = "primary"
        systemPrompt = ""
...
let private mapToolNames (f: string -> 'a) : obj =
    Array.append (allToolNames omp) [| "methodology" |]
...
let private withRoleDefaultsFor (name: string) (userAgent: obj) : obj =
    ...
    let prompt =
        if userPrompt <> "" then
            ...
            spec |> Option.map (fun s -> s.systemPrompt) |> Option.defaultValue ""
    ...
    let mode =
        if userMode <> "" then
            ...
            spec |> Option.map (fun s -> s.defaultMode) |> Option.defaultValue "subagent"

    let primaryDefaultMode =
        if primaryAliases |> Set.contains name then
            "primary"
        else
            "subagent"
    ...
    let mcps =
        if Dyn.isNullish userMcps then
            spec
            |> Option.map (fun s ->
                if s.defaultMcps.Length = 0 then
                    emptyJsArray
                else
                    box s.defaultMcps)
