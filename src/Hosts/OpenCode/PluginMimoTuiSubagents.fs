module Wanxiangshu.Hosts.Opencode.PluginMimoTuiSubagents
...
let private dateNow () : float = float (getTimestampMs ())
...
let private toOption (child: obj) : obj =
    ...
    box
        {| title = label
           value = id
           description = relTime (numField child "time" "updated") |}
    ...
    promise {
        ...
        return
            if Dyn.isNullish data then
                ...
                unbox<obj[]> data
                |> Array.filter (fun c -> isVisibleSubagent c && not (isCheckpointWriterSession c))
    }
...
let private showSubagentDialog (api: obj) (sessionID: string) (visible: obj[]) : unit =
    ...
    let onSelect =
        System.Func<obj, unit>(fun opt ->
            api?route?navigate ("session", box {| sessionID = Dyn.str opt "value" |})
            |> ignore
    ...
    api?ui?dialog?replace (System.Func<obj>(fun () -> api?ui?DialogSelect (props)))
    |> ignore
...
let registerCommands (api: obj) : unit =
