module Wanxiangshu.Hosts.Mux.Fallback.Hook
...
open Wanxiangshu.Hosts.Mux.Fallback.Executor
...
let private translateError (rawEvent: obj) : FallbackEvent option =
...
let private extractSessionID (rawEvent: obj) : string =
...
let private isSessionError (rawEvent: obj) : bool =
    let t = Dyn.str rawEvent "type"
    t = "error" || t = "session.error"

let private extractContinuationIdentity (rawEvent: obj) : (string * int) option =
    let props = Dyn.get rawEvent "properties"
    let props = if Dyn.isNullish props then rawEvent else props
...
let private extractHostRunId (rawEvent: obj) : string option =
    let props = Dyn.get rawEvent "properties"
    let props = if Dyn.isNullish props then rawEvent else props
    let tid = Dyn.str props "turnId"
    let tid = if tid <> "" then tid else Dyn.str props "turnID"
...
let muxEventTranslator: IEventTranslator =
    { new IEventTranslator with
        member _.TranslateError(rawEvent: obj) : FallbackEvent option = translateError rawEvent

        member _.ExtractSessionID(rawEvent: obj) : string = extractSessionID rawEvent
        ...
        member _.ExtractContinuationIdentity(rawEvent: obj) = extractContinuationIdentity rawEvent

        member _.ExtractHostRunId(rawEvent: obj) = extractHostRunId rawEvent
