module Wanxiangshu.Hosts.Mux.WrappersReview
...
let mkFileReadCapture (hostReadExec: HostFunctionCapture) : obj =
    let wrapperFn =
        System.Func<obj, obj, obj>(fun (hostTool: obj) (_config: obj) ->
            ...
            let execFn =
                System.Func<obj, obj, JS.Promise<string>>(fun (_args: obj) (_opts: obj) -> Promise.lift "disabled")
                        ...
                        createObj
                            [ "verdict",
                              box (
                                  createObj
                                      [ "type", box "string"
                                        "enum", box [| "PERFECT"; "REVISE" |]
                                        "description",
                                        box
                                            "PERFECT accepts the work; REVISE requests revision with actionable feedback." ]
                              )
                              "feedback",
                              box (
                                  createObj
    ...
    let wrapperFn =
        System.Func<obj, obj, obj>(fun (tool: obj) (config: obj) ->
            if decodeSubagentRole config <> "reviewer" then
                ...
                createObj
                    [ "description", box (Dyn.get definition "description")
