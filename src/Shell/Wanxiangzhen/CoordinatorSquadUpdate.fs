...
let private parseEventsArgs (args: obj) : obj list option =
    ...
    let arrOpt =
        if isArray eventsRaw then Some (eventsRaw :?> obj array)
...
let validateTasksArrayShape (events: obj list) : SquadUpdateOutcome option =
    ...
    |> List.tryFind (fun e ->
        ...
        && let t = get e "tasks" in isNullish t || not (isArray t))
                ...
                let extracted =
                    ...
                    |> List.map (fun t ->
                        (get t "taskId" |> fun v -> if isNullish v then None else Some (string v)),
                         ...
                         else (dr :?> obj array) |> Array.map string |> Array.toList)
    ...
    promise {
        ...
        let dangling =
            ...
            |> List.collect (fun (id, _, _, deps) ->
                deps |> List.filter (fun d -> not (Set.contains d allIds)) |> List.map (fun d -> id, d))
        if dangling <> [] then return Error (DependencyErrors dangling)
        else
            ...
            | None ->
                ...
                | Ok () ->
                    ...
                    let resultText =
                        ...
                        else sprintf "%d task(s) created, scheduler notified." (List.length createdTasks)
