...
let private setState (scope: RuntimeScope) (s: RunnerState) =
    scope.Add(runnerStateKey, box s)
...
let registerActiveRunnerSession (scope: RuntimeScope) (sessionId: string) : unit =
    if sessionId <> "" then updateState scope (fun s -> { s with ActiveSessions = Set.add sessionId s.ActiveSessions })

let unregisterActiveRunnerSession (scope: RuntimeScope) (sessionId: string) : unit =
    if sessionId <> "" then updateState scope (fun s -> { s with ActiveSessions = Set.remove sessionId s.ActiveSessions })

let registerRunnerChild (scope: RuntimeScope) (parentSessionId: string) (childSessionId: string) (dispose: unit -> unit) : unit =
    if parentSessionId <> "" && childSessionId <> "" then
        updateState scope (fun s -> { s with
...
let appendRunnerLog (scope: RuntimeScope) (sessionId: string) (chunk: string) : unit =
    if sessionId <> "" && chunk <> "" then
        let prev = Map.tryFind sessionId (getState scope).LogBuffers |> Option.defaultValue ""
        updateState scope (fun s -> { s with LogBuffers = Map.add sessionId (prev + chunk) s.LogBuffers })
...
let private abortRunnerJobCore (scope: RuntimeScope) (sessionId: string) : unit =
    ...
    updateState scope (fun s -> { s with Jobs = Map.remove sessionId s.Jobs; LogBuffers = Map.remove sessionId s.LogBuffers })
