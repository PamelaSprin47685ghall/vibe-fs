...
let translateJsError (error: obj) : DomainError =
    ...
    let rec classify (value: obj) (seen: obj list) =
        if isNullish value then SystemPanic "Null error context"
            ...
            | MessageAborted | SessionBusy | TaskWaitBackgrounded | ClientCancellation _ as resolved -> resolved
            | _ ->
                ...
                if not (isNullish nested) then classify nested seenNext
                    ...
                    if not (isNullish data) && typeIs data "object" then classify data seenNext
                        ...
                        if not (isNullish cause) then classify cause seenNext
                            ...
                            elif hasNonEmpty value "statusCode" && not (isNullish config) && hasNonEmpty config "url" then
                            ...
                            elif not (isNullish response) && hasNonEmpty response "status" && not (isNullish config) && hasNonEmpty config "url" then
                            ...
                            elif hasNonEmpty value "field" && hasNonEmpty value "expected" && hasNonEmpty value "actual" then
