...
let appendWorkBacklogCommitted
    ...
    appendAndCache
        workspaceRoot
        (buildEvent sessionID eventKindWorkBacklogCommitted payload (getTimestampMs().ToString()))
...
let appendLoopActivatedOrFail (workspaceRoot: string) (sessionID: string) (task: string) : JS.Promise<unit> =
    ...
    appendAndCacheOrFail
        workspaceRoot
        (buildEvent sessionID eventKindLoopActivated payload (getTimestampMs().ToString()))
...
let appendReviewVerdictOrFail
    ...
    appendAndCacheOrFail
        workspaceRoot
        (buildEvent sessionID eventKindReviewVerdict payload (getTimestampMs().ToString()))
...
let appendAssistantCompleted
    ...
    appendAndCache
        workspaceRoot
        (buildEvent sessionID eventKindAssistantCompleted payload (getTimestampMs().ToString()))
