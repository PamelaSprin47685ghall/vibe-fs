...
let resolveFindSearchStateForPattern (pattern: string) (params': FuzzyFindParams) (opts: SearchOptions)
...
let private fuzzyFindMulti (patterns: string list) (params': FuzzyFindParams) (opts: SearchOptions) : JS.Promise<SearchOutcome> =
    promise {
        let searchPath = resolveFuzzySearchPath params'.path opts.cwd
        let externalBasePath = if searchPath.external then Some searchPath.basePath else None
