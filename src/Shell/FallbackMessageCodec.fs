...
/// Check if a user message was auto-generated by the fallback recovery mechanism.
let isSystemForcedUserMessage (msg: obj) : bool =
    if isNull msg || Dyn.isNullish msg then
        false
    else
        let parts = Dyn.get msg "parts"
        if isNull parts || not (Dyn.isArray parts) then
            false
        else
            let arr = parts :?> obj array
            arr
            |> Array.exists (fun part ->
                let t = Dyn.str part "type"
                let text = Dyn.str part "text"
                t = "text" && (
                    text = "continue"
                    || text.Contains("You produced the tool call as raw text instead of properly dispatching")
                ))

/// Find the last user message in the message list and extract its specified model if present.
/// Excludes system-forced user messages (like "continue" or recovery text).
        ...
        |> Array.tryPick (fun msg ->
            ...
            if isNull info || Dyn.isNullish info then
                ...
                if role <> "user" then
                    ...
                    if isSystemForcedUserMessage msg then
                        None
                    else
