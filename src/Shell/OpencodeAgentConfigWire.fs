...
let disableMimoWorkflowToolsForAgents (agentMap: obj) : obj =
    for name in Dyn.keys agentMap do
        ...
        let permissions =
            ...
            mergeConfigObj (if Dyn.isNullish current then emptyObj () else current) (createObj [ "workflow", box "deny" ])
        let tools =
            let current = Dyn.get agentObj "tools"
            mergeConfigObj (if Dyn.isNullish current then emptyObj () else current) (createObj [ "workflow", box false ])
...
let applyAgentConfigFor
    ...
    let mergedMcp = if Dyn.isNullish configMcp then mcps else mergeConfigObj configMcp mcps
    ...
    let finalAgents =
        ...
        |> fun builtAgents ->
            ...
            | Opencode
            | Mux -> builtAgents
    mergeConfigObj prepared (box {| agent = finalAgents; mcp = mergedMcp |})