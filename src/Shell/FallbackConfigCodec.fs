...
let private splitFrontMatter (content: string) : obj option =
    ...
    if not (trimmed.StartsWith("---")) then None
        ...
        | closeIdx ->
            ...
            try Some (yamlParse yamlText) with _ -> None

let normalizeAgentName (name: string) : string =
    name
    |> Seq.filter (fun c -> not (System.Char.IsWhiteSpace(c) || System.Char.GetUnicodeCategory(c) = System.Globalization.UnicodeCategory.Format))
...
let private parseChain (entries: obj list) : FallbackChain =
    entries |> List.map parseModelEntry

let emptyConfig : FallbackConfig =
    { DefaultChain = []; AgentChains = Map.ofList []; MaxRetries = 2; LoopMaxContinues = 3 }

let extractFallbackConfig (frontmatter: obj) : FallbackConfig option =
    if isNullish frontmatter then None
    else
            ...
            let defaultChain =
                ...
                | Some defaultObj ->
                    ...
                    | :? System.Collections.IEnumerable as arr ->
                        parseChain (arr |> Seq.cast<obj> |> Seq.toList)
                    | _ -> []
            let agentChains =
                ...
                | Some agentsObj when Dyn.typeIs agentsObj "object" ->
                    ...
                    |> Array.fold (fun acc key ->
                        ...
                        | _ -> acc) Map.empty
                | _ -> Map.empty
            Some { DefaultChain     = defaultChain
