...
let requireWarnTddOnArgs (tool: string) (args: obj) : Result<unit, string> =
    if not (WarnTdd.isModificationTool tool) then Result.Ok ()
    else
        let raw = DynField.strField args "warn_tdd" |> Option.defaultValue ""
        match WarnTdd.parseWarnTdd raw with
        | Some _ -> Dyn.deleteKey args "warn_tdd"; Result.Ok ()
        | None -> Result.Error (wireDomainFailure tool (InvalidIntent(tool, "warn_tdd", "required — acknowledge TDD + Kolmolgorov discipline")))
...
let requireWarnOnArgs (tool: string) (args: obj) : Result<unit, string> =
    if not (WarnTdd.isWarnRequiredTool tool) then Result.Ok ()
    else
        ...
        if WarnTdd.parseWarn raw then
            ...
            Result.Error (wireDomainFailure tool (InvalidIntent(tool, "warn", "required — acknowledge this task cannot be done with other tools")))
