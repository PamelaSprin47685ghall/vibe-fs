...
type ReadHit = { msgIndex: int; partIndex: int; payload: ReadPayload }
...
let processDedupHits
    ...
    for i in 0 .. messages.Length - 1 do
        ...
        for j in 0 .. parts.Length - 1 do
            ...
            | None -> ()
    let nextSeen, (outputs, replaced) = foldDedup seenByPath (List.ofSeq hits |> List.map (fun h -> h.payload))
    let verdicts =
        replaced
