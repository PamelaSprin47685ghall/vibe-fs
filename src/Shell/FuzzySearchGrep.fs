...
let private runGrepWithFinder
    ...
    if not (Dyn.truthy (Dyn.get raw "ok")) then
        ...
        { output = buildGrepOutput body resolved.regexError nextIterator
          isError = false }
                        ...
                        try
                            ...
                            | Ok state ->
                                ...
                                if not (Dyn.truthy (Dyn.get raw "ok")) then
                                    ...
                                    return
                                        (pat,
                                         { output = buildGrepOutput body resolved.regexError ""
                                           isError = false },
                                         resolved.regexError)
                        with ex ->
