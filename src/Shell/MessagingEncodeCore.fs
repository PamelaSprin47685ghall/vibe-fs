module Wanxiangshu.Shell.MessagingEncodeCore
...
let rawOutputMatchesState (rawPart: obj) (state: ToolState<obj>) : bool =
    ...
    if isNullish rawOutput then state.output = ""
    elif typeIs rawOutput "string" then string rawOutput = state.output
    else str rawOutput "content" = state.output

let encodeToolPartState (rawPart: obj) (state: ToolState<obj>) : obj =
    let rawOutput = get rawPart "output"
    let nextOutput =
        if isNullish rawOutput || typeIs rawOutput "string" then box state.output
        else withKey rawOutput "content" (box state.output)
