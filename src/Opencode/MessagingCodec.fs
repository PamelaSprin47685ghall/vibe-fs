    ...
    { GetParts = fun msg -> decodePartsFromArray (Dyn.get msg "parts")
      ...
      MessageIsError =
        fun m ->
            ...
            if Dyn.isNullish i then
                false
            else
                let v = Dyn.get i "isError" in not (Dyn.isNullish v) && (v :?> bool)
...
let decodeMessage msg =
    Wanxiangshu.Shell.MessagingDecodeCore.decodeMessage opencodeAdapters "" msg

let decodeMessages msgs =
    Wanxiangshu.Shell.MessagingDecodeCore.decodeMessages opencodeAdapters "" msgs
