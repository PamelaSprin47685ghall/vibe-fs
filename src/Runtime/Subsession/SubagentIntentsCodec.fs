module Wanxiangshu.Runtime.SubagentIntentsCodec
...
let private requireNonEmpty (field: string) (value: string) (tool: string) : Result<string, string> =
    if System.String.IsNullOrWhiteSpace value then
        Result.Error $"Invalid LLM input for {tool}: {field} is required"
    else
        Result.Ok value
...
let private foldArrayResult<'T> (decode: obj -> Result<'T, string>) (arr: obj array) : Result<'T list, string> =
    arr
    |> Array.fold (fun acc item -> acc |> Result.bind (fun xs -> decode item |> Result.map (fun x -> x :: xs))) (Ok [])
    |> Result.map List.rev
...
let private decodeCoderTarget (t: obj) : Result<CoderTarget, string> =
    if not (Dyn.typeIs t "object") then
        ...
        let fileResult =
            if file = "" then
                Result.Error "Invalid LLM input for coder: each target requires file"
            else
                Result.Ok file

        let guideResult =
            if guide = "" then
                Result.Error "Invalid LLM input for coder: each target requires guide"
            else
                Result.Ok guide
...
let private parseCoderTargets (targets: obj) : Result<CoderTarget list, string> =
    if Dyn.isNullish targets || not (Dyn.isArray targets) then
        ...
        if arr.Length = 0 then
            ...
            foldArrayResult decodeCoderTarget arr
...
let private parseIntentCore<'specific, 'intent>
    (tool: string)
    (item: obj)
    (decodeSpecific: obj -> Result<'specific, string>)
    (makeIntent: string -> string -> 'specific -> 'intent)
    : Result<'intent, string> =
    if not (Dyn.typeIs item "object") then
        Result.Error $"Invalid LLM input for {tool}: each intent must be an object"
    else
        ...
        Ok(fun o b s -> makeIntent o b s) <*> objective <*> background <*> specific
...
let private parseIntentList<'T>
    (parser: obj -> Result<'T, string>)
    (errorPrefix: string)
    (intents: obj)
    : Result<'T list, string> =
    if not (Dyn.isArray intents) then
        Result.Error $"Invalid LLM input for {errorPrefix}: intents must be an array"
    else
        let arr = intents :?> obj array

        if arr.Length = 0 then
            Result.Error $"Invalid LLM input for {errorPrefix}: intents must be a non-empty array"
        else
            foldArrayResult parser arr
...
let private joinIntentUiLabel<'T>
    ...
    (getObjective: 'T -> string)
    (intents: obj)
    : Result<string, string> =
    parser intents
    |> Result.map (fun list -> list |> List.map getObjective |> String.concat "; ")
