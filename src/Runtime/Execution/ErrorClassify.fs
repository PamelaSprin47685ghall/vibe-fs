module Wanxiangshu.Runtime.ErrorClassify
...
let private strOpt (o: obj) (key: string) : string option =
    match opt o key with
    | Some v -> Some(string v)
    | None -> None

let private hasNonEmpty (o: obj) (key: string) : bool =
    match opt o key with
    | Some v -> not (isNullish v) && string v <> ""
    | None -> false

let private classifyLeafDetails (value: obj) (message: string) (config: obj) (response: obj) : DomainError =
    ...
    then
        NetworkTransportFailure(str config "url", opt value "statusCode" |> Option.map unbox<int>, message)
    // Network: response.status + config.url
        ...
        && hasNonEmpty config "url"
    then
        NetworkTransportFailure(str config "url", opt response "status" |> Option.map unbox<int>, message)
...
let rec private classify (value: obj) (seen: obj list) =
            ...
            if not (isNullish nested) then
                ...
                if not (isNullish data) && typeIs data "object" then
                    ...
                    if not (isNullish cause) then
                        ...
                        classifyLeafDetails value message config response

let translateJsError (error: obj) : DomainError = classify error []
