...
/// Resolve the sessionID of a session event. For lifecycle-style events,
/// `info.id` is checked first and preferred over other carriers. If `info.id`
/// is missing or for non-lifecycle events, fall back through the known carriers
/// in priority order.
