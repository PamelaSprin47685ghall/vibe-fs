module Wanxiangshu.Runtime.Wanxiangzhen.GitShell
...
let private runStdout (cwd: string) (args: string array) : string =
    ...
    if st <> 0 then
        failwith (readResult result "stderr")
    else
        (readResult result "stdout").TrimEnd()
...
let tryRun (cwd: string) (args: string array) : Result<string, string> =
    ...
    if st <> 0 then
        Error((readResult result "stderr").TrimEnd())
    else
        Ok((readResult result "stdout").TrimEnd())
...
let revParseBranch (cwd: string) : string =
    runStdout cwd [| "rev-parse"; "--abbrev-ref"; "HEAD" |]
...
let verifyBaseBranch (cwd: string) (baseBranch: string) : Result<string, string> =
    try
        let resolved =
            runStdout cwd [| "rev-parse"; "--verify"; "refs/heads/" + baseBranch |]
...
let mergeBaseIsAncestor (cwd: string) (ancestor: string) (descendant: string) : bool =
    try
        runStdout cwd [| "merge-base"; "--is-ancestor"; ancestor; descendant |]
        |> ignore
