module Wanxiangshu.Runtime.Wanxiangzhen.HttpServer
...
let private readBody (req: obj) : JS.Promise<string> =
    Promise.create (fun resolve reject ->
        ...
        req?on ("data", System.Func<obj, unit>(fun chunk -> chunks.Add(string chunk)))
        |> ignore

        req?on ("end", System.Func<unit, unit>(fun () -> resolve (chunks |> System.String.Concat)))
        |> ignore

        req?on ("error", System.Func<obj, unit>(fun e -> reject (exn (string e))))
        |> ignore)
...
let startServer (token: string) (handler: RouteHandler) : JS.Promise<StartedServer> =
    Promise.create (fun resolve reject ->
        let server =
            createServer (
                System.Func<obj, obj, unit>(fun req res ->
                    (promise {
        ...
        server?once ("error", System.Func<obj, unit>(fun e -> reject (exn (string e))))
        |> ignore
