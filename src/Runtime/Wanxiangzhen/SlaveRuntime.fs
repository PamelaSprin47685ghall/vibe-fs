module Wanxiangshu.Runtime.Wanxiangzhen.SlaveRuntime
...
let doneBeacon (cfg: SlaveConfig) : JS.Promise<unit> =
    promise {
        ...
        let init =
            createObj [ "method", box "POST"; "headers", box (authHeaders cfg); "body", box "{}" ]
...
let querySquad (cfg: SlaveConfig) (query: string) : JS.Promise<string> =
    promise {
        let url =
            if query = "state" then
                cfg.CoordinatorUrl + "/state"
            else
                cfg.CoordinatorUrl + "/task/" + query
...
let slaveToolDefs (cfg: SlaveConfig) : obj =
    let submitDef =
        createObj
            [ "description",
              box
                  "Submit completed work to squad coordinator for fast-forward merge into the integration branch. Prerequisites: changes committed, review passed (if /loop is available). Success → merged. Failure → rebase needed."
              "execute",
              box (System.Func<obj, obj, JS.Promise<string>>(fun (_args: obj) (_ctx: obj) -> formatSubmitResult cfg)) ]
    ...
    let queryDef =
        createObj
            [ "description", box "Query squad coordinator for current DAG state or a specific task's details."
              ...
              "execute",
              box (
                  System.Func<obj, obj, JS.Promise<string>>(fun (args: obj) (_ctx: obj) ->
                      let q = str args "query"
                      querySquad cfg q)
              ) ]

    createObj [ "submit_to_squad", box submitDef; "query_squad", box queryDef ]
