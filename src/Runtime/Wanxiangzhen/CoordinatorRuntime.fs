module Wanxiangshu.Runtime.Wanxiangzhen.CoordinatorRuntime
...
let rec waitForPidDeath (deps: CoordinatorDeps) (pid: int) (remaining: int) : JS.Promise<unit> =
    if remaining <= 0 then
        Promise.lift ()
    elif not (deps.IsPidAlive pid) then
        Promise.lift ()
    else
        promise {
...
let cleanupTask (rt: CoordinatorRuntime) (task: SquadTask) : unit =
    task.WorktreePath
    |> Option.iter (fun p ->
        match rt.Deps.TryWorktreeRemoveForce rt.ProjectRoot p with
        | Error e when not (e.Contains "does not exist" || e.Contains "not found") -> rt.GitError <- Some e
        | _ -> ())

    task.BranchName
    |> Option.iter (fun b ->
        match rt.Deps.TryBranchDeleteForce rt.ProjectRoot b with
        | Error e when not (e.Contains "not found" || e.Contains "does not exist") -> rt.GitError <- Some e
        | _ -> ())
