...
module HorizonSurface =
    ...
    let private agentLines language (value: obj) =
        ...
        let work =
            match workPath with
            | "latest" -> ProviderProse.render language HorizonTool.Path.LatestWork (Map [ "label", label; "record", text (value?record) ])
    ...
    let render (agents: obj array) (ptys: obj array) : string =
        ...
        if List.isEmpty lines then
            ToolHostCodec.tomlObjectWithInstructions (ProviderProse.instructionLines language HorizonTool.Path.EmptyRoster Map.empty) []
        else
            ToolHostCodec.tomlObjectWithInstructions lines []

    let unavailable () : string =
        ToolHostCodec.tomlObjectWithInstructions [ ProviderProse.render ProviderLanguage.English HorizonTool.Path.UnavailableFromContext Map.empty ] []

    let cannotBeSeen () : string =
        ToolHostCodec.tomlObjectWithInstructions [ ProviderProse.render ProviderLanguage.English HorizonTool.Path.CannotBeSeen Map.empty ] []
