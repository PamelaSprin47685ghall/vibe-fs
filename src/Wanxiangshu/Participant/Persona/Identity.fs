...
module ParticipantIdentity =
    ...
    let private roleLabelOfKind kind =
        match kind with
        | ManagedRole role -> ManagedAgentCatalog.roleLabel role
    ...
    let private parseRoleBackedLabels
        ...
        match ManagedAgentCatalog.tryParseTier tierLabel, ManagedAgentCatalog.tryParseRole roleLabel with
    ...
    let private validateLegacyPeerRoleTier
        ...
        let expectedTier = initialTier evidence |> ManagedAgentCatalog.tierLabel
