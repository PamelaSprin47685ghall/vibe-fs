...
type InquiryRuntime(store: IEventStore) =
    let inFlight =
        Dictionary<string, (string * int option * string option) * Task<obj>>()
        ...
        task {
            ...
            do!
                transact (fun current ->
                    ...
                    | Some(_, budget) when
                        Set.contains key budget.ChargedWork
                        || budget.UsedTurns >= TurnBudget.safetyLimit
                        ->
                        Ok None
                    | Some(root, _) ->
                        Ok(
            ...
            return
                ...
                | None -> true
                | Some(_, budget) ->
                    Inquiry.budgetIsOpen (current ()) invocationId
                    && Set.contains key budget.ChargedWork
        }
        ...
        transact (fun current ->
            let event =
                if isCancelled () || not (Inquiry.budgetIsOpen current invocationId) then
                    ...
                    resultEvent current invocationId observation
    ...
    let defaultTarget invocationId root (expectation: TurnExpectation) =
        if root = invocationId then
            TurnBudget.defaultExpected
        else
            expectation.ExpectedTurns
    ...
    let validateBudgetArguments current invocationId expected requestedRoot =
        ...
        | Some _ -> Ok None
        | None when expected.IsSome || requestedRoot.IsSome ->
            Error "historical inquiry cannot acquire a new expectTurns budget"
        | None -> Ok None
    ...
    let initialize (current: Inquiry.Current) invocationId question expected budgetRoot =
        ...
        | Some entry when Inquiry.question entry <> question ->
            Error "Sphinx invocation identity was reused for a different question"
    ...
    let start invocationId arguments observe isCancelled =
        ...
        let execute () =
            task {
                try
                    return! run invocationId question expected root observe isCancelled
                finally
                    inFlight.Remove invocationId |> ignore
            }
        ...
        | true, (known, _) when known <> arguments ->
            task {
                return
                    raise (
                        ArgumentException
                            "Sphinx invocation identity was reused with different question or budget arguments"
                    )
            }
        | true, (_, work) -> work
