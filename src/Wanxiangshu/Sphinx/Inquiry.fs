...
module Inquiry =
    ...
    let budgetReport current invocationId =
        match Map.tryFind invocationId current.Entries |> Option.bind (fun e -> e.CompletionBudget) with
        | Some report -> Some report
        | None ->
            rootBudget current invocationId
            |> Option.map (fun (root, budget) ->
                { Root = root; Expectation = budget.Expectation; UsedTurns = budget.UsedTurns })
    ...
    let private inheritedExpectation current root expected =
        ...
        | Some { Progress = Working _ }, Some(_, budget)
            when expected |> Option.exists (fun target -> target <> budget.Expectation.ExpectedTurns) ->
    ...
    let transition (current: Current) invocationId kind (data: obj) =
        ...
        EventEnvelope.normalize
            { baseEvent with
                Parents = baseEvent.Parents @ (budget |> Option.map (snd >> fun b -> b.Frontier) |> Option.toList) @ childHead }
    ...
    let private budgetBinding current (event: EventEnvelope) expectation root =
        match root with
        | None -> Ok(OwnBudget { Expectation = expectation; UsedTurns = 0; ChargedWork = Set.empty; Frontier = event.EventId })
    ...
    let private nativeStart (current: Current) (event: EventEnvelope) data =
        ...
        |> Result.bind (fun (text, expectation, root) ->
            ...
            |> Result.bind (fun _ ->
                if String.IsNullOrWhiteSpace text || expectation.TurnPrice <= 0.0
                   || Double.IsNaN expectation.TurnPrice || Double.IsInfinity expectation.TurnPrice
                   || expectation.CalibrationSamples < 0 then
                    Error "invalid native Sphinx budget or question"
                else
                    budgetBinding current event expectation root |> Result.bind (fun budget ->
                        let state = State.create (text.Trim())
                        { state with Budget = { state.Budget with MaxYields = TurnBudget.safetyLimit; Expectation = Some expectation } }
                        |> Policy.decide |> progress |> Result.map (fun next -> next, budget))))
    ...
    let private stopped state data =
        ...
        |> Result.bind (fun reason ->
            if String.IsNullOrWhiteSpace reason then Error "stop reason required"
            else Ok(Complete(Policy.canonicalAnswer reason state)))
    ...
    let private advance (current: Current) invocationId entry kind data =
        ...
        | Working(state, _), "observed" ->
            observationState current invocationId state |> Result.bind (fun admitted ->
    ...
    let private charge current invocationId (entry: Entry) data =
        let decoder = Decode.object (fun get -> get.Required.Field "invocation" Decode.string, get.Required.Field "workId" Decode.string)
        Decode.fromValue "$.data" decoder data
        |> Result.bind (fun (childId, key) ->
            ...
            | Working _, OwnBudget budget, Some { Progress = Working(state, _) }, Some(root, _)
                when root = invocationId && key = workId childId state ->
                if Set.contains key budget.ChargedWork then Error "work identity already charged"
                elif budget.UsedTurns >= TurnBudget.safetyLimit then Error "root work budget exhausted"
                else Ok(entry.Progress, OwnBudget { budget with UsedTurns = budget.UsedTurns + 1; ChargedWork = Set.add key budget.ChargedWork })
    ...
    let private recordCompletion current invocationId (answer: CanonicalAnswer) =
        ...
        let updated = { current with Entries = Map.add invocationId { terminal with CompletionBudget = report } current.Entries }
        ...
        | OwnBudget _, Some report when answer.StopReason = "turn-price" && report.UsedTurns >= TurnBudget.minimum ->
            ...
            let previous = Map.tryFind key updated.Calibration |> Option.defaultValue TurnBudget.emptyCalibration
            { updated with Calibration = Map.add key (TurnBudget.observe report.Expectation report.UsedTurns previous) updated.Calibration }
        | _ -> updated

    let private finishEvent current invocationId (event: EventEnvelope) entry =
        let updated = { current with Entries = Map.add invocationId entry current.Entries }
        let updated =
            match rootBudget updated invocationId with
            | None -> updated
            | Some(root, budget) ->
                let owner = Map.find root updated.Entries
                { updated with Entries = Map.add root { owner with Budget = OwnBudget { budget with Frontier = event.EventId } } updated.Entries }
        match entry.Progress with
        | Working _ -> updated
        ...
        |> Result.bind (fun (invocationId, revision, kind, data) ->
            ...
            then
                ...
                let next =
                    ...
                    | Some entry, _ -> advance current invocationId entry kind data |> Result.map (fun next -> next, entry.Budget)

                next
                |> Result.map (fun (value, budget) ->
                    finishEvent current invocationId event
                        { Revision = revision; Head = event.EventId; Progress = value; Budget = budget; CompletionBudget = None }))

    let rule: IntegrationRule =
        { Name = currentKey
          Initial = box { Entries = Map.empty; Calibration = Map.empty }
