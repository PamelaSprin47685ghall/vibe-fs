...
module Value =
    ...
    let private answerLossForRoot (state: EpistemicState) (root: RootContract) =
    ...
    let currentAnswerLoss (state: EpistemicState) =
        ...
        | None -> 1.0
        | Some root -> answerLossForRoot state root

    let stopUtility state = -currentAnswerLoss state

    let private discountForDependency (state: EpistemicState) dependency =
