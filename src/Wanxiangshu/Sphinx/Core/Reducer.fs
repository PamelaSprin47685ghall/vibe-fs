...
module Reducer =
    ...
    let private validBudget budget =
        budget |> Map.forall (fun _ value -> finiteNonnegative value)
        ...
        |> List.fold
            (fun result entry ->
                ...
                |> Result.bind (fun locked ->
                    if
                        String.IsNullOrWhiteSpace entry.Plugin.Id
                        || String.IsNullOrWhiteSpace entry.Plugin.Release
                        || String.IsNullOrWhiteSpace entry.Plugin.AbiHash
                    then
    ...
    let private checkSuccessorPlanned (state: InquiryState) (successor: WorkId) =
    ...
    let private validTransition (state: InquiryState) (work: WorkSpec) (current: WorkState) (next: WorkState) =
        let sameAttempt (proof: LeaseProof) = proof.Attempt = work.Attempt
        ...
        | Leased previous, Executing proof when
            sameAttempt proof
        ...
        | Executing previous, WorkState.InputRequired proof when
            sameAttempt proof
            && proof.Fence = previous.Fence
            && proof.Session = previous.Session
            ->
            Ok()
        | WorkState.InputRequired previous, Executing proof when
            sameAttempt proof
            && proof.Fence = previous.Fence
            && proof.Session = previous.Session
            ->
            Ok()
        | (Leased previous | Executing previous | WorkState.InputRequired previous), WorkState.Cancelled proof when
            proof.Attempt = previous.Attempt
            ->
            Ok()
        | Executing previous, Succeeded proof when proof.Attempt = previous.Attempt -> Ok()
        | Executing previous, WorkState.Failed proof when proof.Attempt = previous.Attempt -> Ok()
        ...
        | Ready, Superseded successor -> checkSuccessorPlanned state successor
    ...
    let private advance (event: InquiryEvent) (state: InquiryState) =
        { state with
            Revision = event.Revision
            EventHead = event.Id }

    let private verifyChain (state: InquiryState) (event: InquiryEvent) =
    ...
    let private patchGraph (patch: GraphPatch) (state: InquiryState) =
        let nodes =
            patch.RemoveNodes
            |> List.fold (fun graph nodeId -> Map.remove nodeId graph) state.Graph
            |> fun graph ->
                patch.UpsertNodes
                |> List.fold (fun current node -> Map.add node.Id node current) graph
        ...
        let edges =
            ...
            |> fun graph ->
                patch.UpsertEdges
                |> List.fold (fun current edge -> Map.add edge.Id edge current) graph
    ...
    let private planWork (specs: WorkSpec list) (state: InquiryState) =
        specs
        |> List.fold
            (fun result spec ->
                result
    ...
    let private staleWorkError (current: WorkItem) (nextState: WorkState) (fromState: string) =
        if stateTag current.State = "Succeeded" && stateTag nextState = "Succeeded" then
            ...
            error "stale-work-state" (sprintf "expected %s but work is %s" fromState (stateTag current.State))
    ...
    let private transitionWork (spec: WorkSpec) (fromState: string) (nextState: WorkState) (state: InquiryState) =
        ...
        | Some current when stateTag current.State <> fromState -> staleWorkError current nextState fromState
        | Some current ->
            checkSpec current spec nextState
            |> Result.bind (fun () -> validTransition state spec current.State nextState)
            |> Result.map (fun () ->
                { state with
                    Work = state.Work |> Map.add spec.Id { Spec = spec; State = nextState } })

    let private requireDeterministicGuarantee (message: string) (guarantee: CertificateGuarantee option) =
        match guarantee with
        | Some(DeterministicInclusion _) -> Ok()
        | Some _ -> error "missing-guarantee" message
        | None -> Ok()

    let private requireCoverageGuarantee (message: string) (guarantee: CertificateGuarantee option) =
        match guarantee with
        | Some(ProbabilisticCoverage _) -> Ok()
        | _ -> error "missing-coverage" message

    let private requireOrdinalGuarantee (message: string) (guarantee: CertificateGuarantee option) =
        match guarantee with
        | Some(OrdinalModel _) -> Ok()
        | _ -> error "missing-guarantee" message

    let private checkResidualGuarantee (guarantee: CertificateGuarantee option) =
        match guarantee with
        | Some ResidualOnly -> Ok()
        | Some _ -> error "missing-guarantee" "residual slot carries an unrelated guarantee"
        | None -> Ok()

    let private checkGuarantee (slot: string) (certificate: ValueCertificate) =
        match slot with
        | "exact" when certificate.Exact.IsSome ->
            requireDeterministicGuarantee
                "exact slot requires a deterministic inclusion guarantee"
                (Map.tryFind "exact" certificate.Guarantees)
        | "bound" when certificate.LowerEnvelope.IsSome || certificate.UpperEnvelope.IsSome ->
            requireDeterministicGuarantee
                "bound slot requires a deterministic inclusion guarantee"
                (Map.tryFind "bound" certificate.Guarantees)
        | "sample" when certificate.SampleSummary.IsSome ->
            requireCoverageGuarantee
                "sample slot requires a probabilistic coverage guarantee"
                (Map.tryFind "sample" certificate.Guarantees)
        | "latent" when certificate.LatentPosterior.IsSome ->
            requireCoverageGuarantee
                "latent slot requires a probabilistic coverage guarantee"
                (Map.tryFind "latent" certificate.Guarantees)
        | "ordinal" when not (List.isEmpty certificate.OrdinalConstraints) ->
            requireOrdinalGuarantee
                "ordinal slot requires an ordinal guarantee"
                (Map.tryFind "ordinal" certificate.Guarantees)
        | "residual" when certificate.Residual.IsSome ->
            checkResidualGuarantee (Map.tryFind "residual" certificate.Guarantees)
        | _ -> Ok()

    let private checkCertificate certificate =
        [ "exact"; "bound"; "sample"; "latent"; "ordinal"; "residual" ]
        |> List.fold (fun result slot -> result |> Result.bind (fun () -> checkGuarantee slot certificate)) (Ok())

    let private debitResource (resource: string) (amount: float) (budget: ResourceBudget) =
    ...
    let private debitBudget (debit: ResourceBudget) (state: InquiryState) =
        if not (validBudget debit) then
            ...
            |> Map.fold
                (fun result resource amount ->
                    result |> Result.bind (fun budget -> debitResource resource amount budget))
                (Ok state.Budget)
            |> Result.map (fun budget -> { state with Budget = budget })

    let private applyPluginSetBound (state: InquiryState) (entries: PluginLockEntry list) =
        if sameLock entries state.PluginLock then
            Ok state
        else
            error "plugin-swapped" "plugin lock is immutable"

    let private checkObservationSchema (item: WorkItem) (binding: ProtocolBinding) =
    ...
    let private insertObservation (state: InquiryState) (binding: ProtocolBinding) =
        ...
        | Some _ -> error "observation-conflict" "an observation already exists for this work attempt"
        | None ->
            Ok
                { state with
                    Observations = Map.add key binding.Payload.CanonicalPayload state.Observations }

    let private applyObservation (state: InquiryState) (binding: ProtocolBinding) =
        ...
        | Some item ->
            checkObservationSchema item binding
            |> Result.bind (fun () -> insertObservation state binding)

    let private applyCertificate (state: InquiryState) (patch: CertificatePatch) =
        let certificate: ValueCertificate = patch.Certificate
    ...
    let private applyStatusChange (state: InquiryState) (status: InquiryStatus) =
    ...
    let private applyAnswer (state: InquiryState) (answer: JsonEnvelope) =
    ...
    let private applyExisting (state: InquiryState) (event: InquiryEvent) =
        verifyChain state event
        |> Result.bind (fun () ->
            match event.Body with
            | InquiryCreated _ -> error "duplicate-inquiry" "inquiry is already created"
            | PluginSetBound entries -> applyPluginSetBound state entries
            ...
            | ObservationAccepted binding -> applyObservation state binding
            | CertificatePatched patch -> applyCertificate state patch
            | BudgetDebited debit -> debitBudget debit state
            | InquiryStatusChanged status -> applyStatusChange state status
            | AnswerCommitted answer -> applyAnswer state answer)
        |> Result.map (advance event)

    let private createFromOrigin (event: InquiryEvent) (entries: PluginLockEntry list) (budget: ResourceBudget) =
    ...
    let apply (state: InquiryState option) (event: InquiryEvent) =
        match state, event.Body with
        | None, InquiryCreated(root, entries, budget) -> createFromOrigin event entries budget
        | None, _ -> error "missing-inquiry" "first event must create the inquiry"
        | Some current, _ -> applyExisting current event

    let fold (events: InquiryEvent list) =
        events
        |> List.fold
    ...
    let private nodeView (node: GraphNode) : obj =
        box
            {| id = NodeId.value node.Id
               ...
               schema =
                {| id = node.Payload.Schema.Id
                   hash = node.Payload.Schema.Hash |}
    ...
    let private edgeView (edge: HyperEdge) : obj =
        box
            {| id = EdgeId.value edge.Id
               ...
               payload =
                edge.Payload
                |> Option.map (fun envelope -> envelope.CanonicalPayload)
                |> Option.toObj
               payloadSchema =
                edge.Payload
    ...
    let private workView item =
        let fence: obj =
            ...
            | Leased proof
            | Executing proof
            | WorkState.InputRequired proof -> box proof.Fence
            | _ -> null
        ...
        let session: obj =
            ...
            | Leased proof
            | Executing proof
            | WorkState.InputRequired proof -> proof.Session |> Option.map box |> Option.toObj
            | _ -> null

        let completionAttempt: obj =
            match item.State with
            | Succeeded proof
            | WorkState.Failed proof
            | WorkState.Cancelled proof -> box proof.Attempt
            | _ -> null
        ...
        let completionEvent: obj =
            ...
            | Succeeded proof
            | WorkState.Failed proof
            | WorkState.Cancelled proof -> proof.EventId |> Option.map (EventId.value >> box) |> Option.toObj
            | _ -> null
        ...
        let completionDetail: obj =
            ...
            | Succeeded proof
            | WorkState.Failed proof
            | WorkState.Cancelled proof ->
                proof.Detail
                |> Option.map (fun envelope -> box envelope.CanonicalPayload)
                |> Option.toObj
            | _ -> null
    ...
    let semanticHash state =
        state |> semanticView |> CoreHash.canonicalSha256
