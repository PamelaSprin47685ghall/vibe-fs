...
module GecHost =
    ...
    let private faultView (fault: HostFault) : obj =
        box
            {| ok = false
               error = box {| code = code fault; message = message fault |} |}

    let private requireText (fault: HostFault) (value: string) : Result<string, HostFault> =
        if String.IsNullOrWhiteSpace value then Error fault else Ok value
    ...
    let planOpenCodeDispatch (input: obj) : obj =
        ...
        let attempt =
            if isNullish attemptRaw then -1 else unbox<int> attemptRaw

        let depthRaw : obj = input?depth

        if attempt < 0 then
            faultView HostFault.MissingAttempt
        else
            match planChild (text input?workId) attempt (snapshotHashOf input) (parentSessionOf input) (intField 1 depthRaw) with
            | Ok plan -> box {| ok = true; child = childView plan |}
    ...
    let abortOpenCodeWork (input: obj) : obj =
        ...
        |> function
            | Ok (wid, cid) -> box {| ok = true; aborted = true; workId = wid; childSessionId = cid |}
            | Error fault -> faultView fault
        ...
        let rec loop index prev accIds accKinds =
            if index >= events.Length then
                ...
                if String.IsNullOrWhiteSpace id then
                    ...
                    match prev with
                    | None -> loop (index + 1) (Some id) (id :: accIds) (kind :: accKinds)
                    | Some expected ->
                        match parentOf event with
                        | Some actual when actual = expected ->
                            loop (index + 1) (Some id) (id :: accIds) (kind :: accKinds)
                        | _ ->
                            Error(
                                HostFault.ParentChainBreak(
                                    sprintf "event %s at index %d expects parent %s" id index expected
                                )
                            )
    ...
    let private distinctWork (ids: string list) : string list =
        let _, ordered =
            ...
            |> List.fold
                (fun (seen, acc) id ->
                    if Set.contains id seen then seen, acc else Set.add id seen, id :: acc)
                (Set.empty, [])
    ...
    let foldHostEvents (input: obj) : obj =
        ...
        if isNullish eventsRaw then
            ...
            let events : obj array = unbox<obj array> eventsRaw
            ...
            | Ok (ids, kinds) ->
                ...
                let facts : obj array =
                    if isNullish factsRaw then [||] else unbox<obj array> factsRaw
                ...
                box
                    {| ok = true
                       graph = box {| eventCount = events.Length; kinds = kinds |> List.toArray |}
                       certificates = createObj []
                       work = box {| ids = workIds |> List.toArray |}
                       budget = box {| debited = debited; totalDebited = total |}
                       status = "active"
