...
module SelfPrediction =
    ...
    let private validateForecast (forecast: Map<string, float>) =
        if Map.isEmpty forecast then
            ...
            match forecast |> Map.tryPick (fun outcome probability -> if finite probability then None else Some outcome) with
            | Some outcome -> Error(NonFiniteProbability outcome)
            | None ->
                match forecast |> Map.tryPick (fun outcome probability -> if probability < 0.0 then Some outcome else None) with
                ...
                | None ->
                    ...
                    if abs (total - 1.0) > simplexTolerance then
                        ...
                        Ok(forecast |> Map.toList |> List.map fst |> List.sort)

    let private digest (workId: string) (outcomes: string list) (forecast: Map<string, float>) =
        let parts = outcomes |> List.map (fun outcome -> outcome + "=" + string forecast.[outcome])

        CoreHash.sha256Hex (workId + "|" + String.concat "," outcomes + "|" + String.concat "," parts)

    let commit (workId: string) (forecast: Map<string, float>) =
        if String.IsNullOrWhiteSpace workId then
            Error BlankWorkId
        else
            match validateForecast forecast with
    ...
    let score (input: ScoreInput) =
        ...
        elif not (Map.containsKey input.Outcome input.Forecast) then
            Error(UnknownOutcome input.Outcome)
        else
            match validateForecast input.Forecast with
            | Error error -> Error error
            | Ok outcomes ->
                if input.Seal.Outcomes <> outcomes
                   || input.Seal.Digest <> digest input.Seal.WorkId outcomes input.Forecast then
                    Error SealMismatch
                else
                    let probability = input.Forecast.[input.Outcome]
    ...
    let assess (input: AssessmentInput) =
        if not input.CommittedBeforeStimulus then
            Error NotCommitted
        elif String.IsNullOrWhiteSpace input.WorkId then
            Error BlankWorkId
        else
            match commit input.WorkId input.Forecast with
            | Error error -> Error error
            | Ok seal ->
                score
                    { Seal = seal
