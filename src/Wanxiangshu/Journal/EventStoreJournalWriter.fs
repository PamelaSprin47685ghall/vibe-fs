        ...
        try
            ...
            Ok
                { BlobRef = blobRef
                  BlobDigest = digest }
        with ex ->
            Error(sprintf "event-store blob write failed: %s" ex.Message)
    ...
    member _.Read(blobRef: BlobRef) : Result<string, string> =
        ...
        if not (relative.StartsWith(prefix, StringComparison.Ordinal)) then
            ...
            if String.IsNullOrWhiteSpace oidHex || oidHex.Contains "/" then
                ...
                | None -> Error(sprintf "event-store blob missing: %s" oidHex)
                | Some bytes ->
                    try
                        Ok(Encoding.UTF8.GetString bytes)
                    with ex ->
                        Error(sprintf "event-store blob read failed: %s" ex.Message)
...
type private UnavailableBlobWriter() =
    interface IBlobWriter with
        member _.Write(_content) =
            Error "EventStore journal writer has no IGitRawStore for blobs"
...
type EventStoreJournalWriter
    ...
    static member create
        ...
        | Ok published ->
            ...
            writer :> IJournalWriter, initEnvelope
    ...
    static member resumeOrCreate
        ...
        | Error msg -> Error { Fact = "Boot"; Reason = msg }
        | Ok prior ->
    ...
    member private this.AppendLocked
        ...
        if poisoned || disposed then
            ...
            | Error err ->
                poisoned <- true
                CommitUnknown(eventId, WriteFailed(EventStoreJournalWriter.formatAppendError err))
    ...
    member this.Append
        ...
        (fact: Fact)
        : CommitResult<Envelope> =
        lock gate (fun () -> this.AppendLocked streamKind providerRun fact)
