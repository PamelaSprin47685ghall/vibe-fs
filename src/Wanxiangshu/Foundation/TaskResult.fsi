...
type TaskResultBuilder =
    ...
    member TryWith: body: (unit -> Task<Result<'value, 'error>>) * handler: (exn -> Task<Result<'value, 'error>>) -> Task<Result<'value, 'error>>

    member TryFinally: body: (unit -> Task<Result<'value, 'error>>) * compensation: (unit -> unit) -> Task<Result<'value, 'error>>
    ...
    member While:
        guard: (unit -> bool) * body: (unit -> Task<Result<unit, 'error>>) ->
        Task<Result<unit, 'error>>

    member For:
        items: seq<'item> * body: ('item -> Task<Result<unit, 'error>>) ->
        Task<Result<unit, 'error>>
