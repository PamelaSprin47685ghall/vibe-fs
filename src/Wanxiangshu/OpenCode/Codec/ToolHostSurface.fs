...
module ToolHostSurface =
    ...
    let private argumentsOf value = unbox<ArgumentsHandle> value |> fun handle -> handle.Value
    let private contextOf value = unbox<ContextHandle> value |> fun handle -> handle.Value

    let arguments (raw: obj) : obj = ArgumentsHandle(HostToolArguments raw) :> obj
    let argumentText (args: obj) (name: string) = (argumentsOf args).Text name
    let argumentOptionalText (args: obj) (name: string) = (argumentsOf args).OptionalText name |> Option.toObj

    let argumentOptionalTexts (args: obj) (name: string) =
        (argumentsOf args).OptionalTexts name
    ...
    let private schemaValue schema : obj =
        emitJsExpr schema "$0.fields[0]"

    let private factory toolModule = ToolHostCodec.factory toolModule

    let schemaString toolModule = ToolHostCodec.stringSchema(factory toolModule) |> schemaValue
    let schemaStringDescribed toolModule description = ToolHostCodec.stringSchemaDescribed description (factory toolModule) |> schemaValue
    let schemaNumber toolModule = ToolHostCodec.numberSchema(factory toolModule) |> schemaValue
    let schemaNumberDescribed toolModule description = ToolHostCodec.numberSchemaDescribed description (factory toolModule) |> schemaValue
    let schemaBool toolModule = ToolHostCodec.boolSchema(factory toolModule) |> schemaValue
    let schemaBoolDescribed toolModule description = ToolHostCodec.boolSchemaDescribed description (factory toolModule) |> schemaValue
    let schemaEnum toolModule values = ToolHostCodec.enumSchema (List.ofArray values) (factory toolModule) |> schemaValue
    let schemaEnumDescribed toolModule values description = ToolHostCodec.enumSchemaDescribed (List.ofArray values) description (factory toolModule) |> schemaValue
    let schemaOptionalEnum toolModule values = ToolHostCodec.optionalEnumSchema (List.ofArray values) (factory toolModule) |> schemaValue
    let schemaOptionalEnumDescribed toolModule values description = ToolHostCodec.optionalEnumSchemaDescribed (List.ofArray values) description (factory toolModule) |> schemaValue
    let schemaManagedOrHandle toolModule values = ToolHostCodec.managedOrHandleSchema (List.ofArray values) (factory toolModule) |> schemaValue
    let schemaOptionalString toolModule = ToolHostCodec.optionalStringSchema(factory toolModule) |> schemaValue
    let schemaOptionalStringDescribed toolModule description = ToolHostCodec.optionalStringSchemaDescribed description (factory toolModule) |> schemaValue
    let schemaOptionalNumber toolModule = ToolHostCodec.optionalNumberSchema(factory toolModule) |> schemaValue
    let schemaOptionalNonNegativeIntegerDescribed toolModule description = ToolHostCodec.optionalNonNegativeIntegerSchemaDescribed description (factory toolModule) |> schemaValue
    let schemaOptionalStringArray toolModule = ToolHostCodec.optionalStringArraySchema(factory toolModule) |> schemaValue
    ...
    let registryNames (toolModule: obj) (names: string array) : obj =
        ...
        box {| names = names; one = value?one; two = value?two |}

    let hide (registry: obj) (name: string) (callback: obj) =
        ToolHostCodec.hide registry name callback
        registry

    let contextDecode (raw: obj) : obj = ContextHandle(ToolHostCodec.decodeContext raw) :> obj
        ...
        if isNull value then
            ...
            let entries =
                ...
                |> Array.choose (fun entry ->
                    ...
                    else Some (string entry?name, tomlValue entry?value))
                |> Array.toList
    ...
    let tomlObjectWithInstructions (instructions: string array) (fields: obj array) : string =
        let values = fields |> Array.toList |> List.map (fun field -> string field?name, tomlValue field?value)
    ...
    let tomlTable (name: string) (entries: obj array array) : string =
        entries
        |> Array.map (fun row -> row |> Array.toList |> List.map (fun field -> string field?name, tomlValue field?value))
        |> Array.toList
