...
module PairProgrammingThoughtSurface =
    ...
    let private journalHandleOf (value: obj) =
        (unbox<JournalHandleBox> value).Value

    let private agentJournalOf (value: obj) =
        (journalHandleOf value).Journal
    ...
    let private projectionFlagsOf (projection: AgentProjectionSet) (session: SessionId) : obj =
        ...
        | None -> box {| guidelines = false; xTrace = false; blog = false |}
        | Some value ->
            box
    ...
    let projectionFlags (journal: obj) (session: string) : obj =
        flagsForJournal journal session
    ...
    let foldAnchoredPair (payload: obj) : obj =
        ...
        | Ok projection -> projectionFlagsOf projection sessionId |> fun flags -> box {| ok = true; flags = flags |}
    ...
    let isPairProgrammingThought raw = PairProgrammingThoughtTransform.isPairProgrammingThought raw
    let providerIdOfMessage raw = PairProgrammingThoughtTransform.providerIdOfMessage raw |> Option.toObj
    let providerIdFromMessages (raw: obj array) = PairProgrammingThoughtTransform.providerIdFromMessages (Array.toList raw) |> Option.toObj

    let skipAutoInjectedRequested (providerId: obj) =
        PairProgrammingThoughtTransform.skipAutoInjectedRequested (if isNull providerId then None else Some(string providerId))
    ...
    let tryInjectWithJournal
        ...
        task {
            let durableJournal =
                if isNullish journal then None else Some(agentJournalOf journal)
            let session = if isNull sessionId then None else Some(string sessionId)
