...
module SphinxExecutionSurface =
    let createWithOwner
        ...
        (invoke: obj -> Task<string>)
        (cancel: string -> Task<unit>)
        (ownerOf: string -> string)
        : obj =
        let port =
            { new ISphinxEngineerPort with
                member _.LogicalOwnerOf present =
                    SessionId.create (ownerOf (SessionId.value present))
    ...
    let run (execution: obj) sessionId invocationId question expectTurns (attachAbort: obj) =
        ...
        (unbox<SphinxExecution> execution)
            .Run(context, invocationId, question, expectTurns)
    ...
    let dispose (execution: obj) =
        (unbox<SphinxExecution> execution).DisposeAsync()
