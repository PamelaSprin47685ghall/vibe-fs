...
module SphinxConfig =
    ...
    let private register (config: obj) =
        if isNull config?command then config?command <- createObj []
    ...
    let private apply (config: obj) =
        ...
        if AblationSettings.allowsToolSchema "sphinx" then register config
        else emitJsStatement config "if ($0.command) delete $0.command.sphinx"

    let configure (config: obj) : unit =
        if not (isNull config) then apply config
