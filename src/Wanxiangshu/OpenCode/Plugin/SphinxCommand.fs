...
module SphinxCommand =
    ...
    let parse (arguments: string) =
        ...
        if text = "" then
            Error "Usage: /sphinx [--expect-turns N] question"
        elif text.StartsWith "--expect-turns" then
            let matched = Regex.Match(text, "^--expect-turns(?:=|\\s+)([0-9]+)\\s+([\\s\\S]+)$")

            if not matched.Success then
                Error "Usage: /sphinx --expect-turns N question (N must be a positive integer)"
            else
                ...
                | true, value when value > 0 ->
                    ...
                    if question = "" then
                        ...
                        TurnBudget.validate value
                        |> Result.map (fun target ->
                            { Question = question
                              ExpectTurns = Some target })
                | _ -> Error "expectTurns must be a positive integer"
        else
            Ok { Question = text; ExpectTurns = None }
    ...
    let questionAndAnswer question (answer: obj) =
        let synthesis: string =
            if isNull answer?synthesis || isNull answer?synthesis?text then
                ...
                string answer?synthesis?text

        let rendered =
            if String.IsNullOrWhiteSpace synthesis then
                pretty answer
            else
                ...
                createObj (
                    ...
                    @ (directory
                       |> Option.map (fun value ->
                           [ "query" ==> createObj [ "directory" ==> value ]; "directory" ==> value ])
                       |> Option.defaultValue [])
                )
    ...
    let private finish (output: obj) =
        ...
        if supportsHandled output then
            ...
            let error =
                Exception "Sphinx completed: question and answer were inserted without starting a new turn."
        ...
        task {
            if isNull input || string input?command <> "sphinx" then
                ...
                if String.IsNullOrWhiteSpace sessionId then
                    invalidArg "sessionID" "current session is required"
