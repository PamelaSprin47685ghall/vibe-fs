...
本动议通过统一的 TOML 视觉记法降低这种歧义。

## 1.2 为什么使用 TOML

本动议选择 TOML，不是因为系统需要 parse TOML，而是因为它对 LLM 有稳定且熟悉的视觉语义：
...
assistant 在原始 transcript 中产生的正文、reasoning 和 tool call 不因本动议而重写。
...
本动议只约束最终进入 LLM 阅读面的文本 body。

### 非 LLM 可见内部数据

Journal facts、projection state、日志、metrics、diagnostics 等若不进入模型上下文，不受本动议约束。
...
## ARCH-010：运行时合成内容的 TOML Instruction/Data 记法
...
若某固定文本拥有自己的 version/digest 合同，必须由该文本的 SSOT owner 按既有规则决定是否 bump；本动议不替各领域预先发明统一 versioning 规则。
...
* [ ] system/developer prompt 未被本动议迁移。
