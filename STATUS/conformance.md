...
| COMPANION-005: BlogFrame 增量投影 | CONFORMANT | `Domain/CompanionProjectionBuilder.fs` `Session/EnforcerHost.fs` | normal = Working Record frames + New Work delta + final instruction；标题消息层包装；resume 经 `rebuildFromContext` 用 `CompanionProjectionBuilder` 重建（非 append raw）；判据：`tests-mjs/Context/companion-projection.test.mjs` 23 项 |
...
| ENFORCER-010/040…047/150/160…163（blog 工具与挂起链 Host 接线） | PARTIAL | `Infrastructure/OpenCode/Tools/BlogTool.fs` `Session/EnforcerHost.fs` `Session/ParkedTransform.fs` `Session/BloggerRuntimeState.fs` `Journal/EnforcementProjection.fs` `Kernel/Roles.fs`（`ToolPermission.Blog`）+ `tests-mjs/Enforcer/parked-transform.test.mjs` + `tests-mjs/Enforcer/blogger-runtime.test.mjs` + `tests-mjs/Journal/blog-entry-committed.test.mjs` + canary `host-transform-capability` | 已闭合（本轮垂直切片）：`BlogEntryCommitted` 原子推进 coverage 并携带 ToolCallIds/scores/evidence（ENFORCER-045）；独立 `EnforcementCycleCommitted` 已删除；`BloggerRuntimeState` 纯转换 + typed request context（ENFORCER-047/050）；BloggerMain terminal 等待已删除（`CompanionHostBlogger.blog` 发送即返回）；resume 经 `rebuildFromContext` 用 `CompanionProjectionBuilder` 重建（ENFORCER-051）；system 唯一来源 `blogger-system.md` 含 blog 协议；单一 coordinator 在 `CompanionTransform.handleCompanionTransform`。未闭合：throttle/nudge 生产接线、完整 crash recovery、squash continuation transform 接线（squash 仍走 terminal wait 提交 `BlogSquashCommitted`） |
