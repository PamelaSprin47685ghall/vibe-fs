...
| COMPANION-004: Y 的 System Prompt | CONFORMANT | `prompts/blogger-system.md` `Tools/PromptAssets.fs` `Tools/StaticTools.fs` | System 唯一来源 `blogger-system.md`（managed-agent config 覆盖注入）；`CompanionPrompt.System` 已删除；工具面 `{ blog }` 在 system 与 permission 收口；判据：`tests-mjs/Plugin/manager-tool-contract.test.mjs`（forbidden no-tools patterns + required blog protocol） |
| COMPANION-005: BlogFrame 增量投影 | PARTIAL | `Domain/CompanionProjectionBuilder.fs` `Session/EnforcerHost.fs` `Session/CompanionHostBlogger.fs` | builder/resume 已存在；生产尚未统一：首次请求仍从 raw user 抽 TOML；Squash 仍拼特殊字符串 prompt + terminal path；Normal/Squash 未完全共用同一 continuation 投影。判据：`tests-mjs/Context/companion-projection.test.mjs` 23 项（纯投影）+ 本轮 C0 红门禁 |
| COMPANION-008: 忙时跳过 | PARTIAL | `Companion.Submit` `Session/BloggerRuntimeState.fs` | `Companion.Submit` 有 `SkippedBusy`，但生产 busy 权威仍是 `inFlightTask`/`HasParked`，`BloggerRuntimeState` 未接生产；InFlight skip 未由 runtime state 掌权。纯转换测试：`tests-mjs/Enforcer/blogger-runtime.test.mjs` |
...
| CTX-006: 恢复槽的两种动作 | PARTIAL | `Domain/RecoverySlot.fs` `OpenCode/XWire.fs` `Session/CompanionHostBlogger.fs` | X probe 已接线；Y squash 仍走 `SubscribeTerminal` + prose terminal 文本，未迁入 blog tool continuation。第 1 层测试含 parked-cursor 反例；生产 squash 工具循环未闭合 |
| CTX-007: Attempt 三结局按 RequestKind 分派 | PARTIAL | `Domain/RecoverySlot.fs` `Session/CompanionHostBlogger.fs` | 纯分派表已测；生产 squash 仍经 terminal 判定 validity 再 `AppendSquash`，未与 Normal 共用 blog continuation。第 1 层测试 9 项 |
...
| CTX-012: 提交语义 | PARTIAL | `OpenCode/XWire.fs` `Journal/BlogProjection.fs` `Session/CompanionHostBlogger.fs` | X `PrefixRebaseCommitted` writer 已闭合；Y squash 永久提交语义在 fold 侧成立，但生产仍 terminal path，未用 typed Squash context + blog tool 提交。第 1 层测试跨 4 文件 |
...
| 条款 | 状态 | 当前代码位置 | 差距 |
|------|------|-------------|------|
| ENFORCER-020…043/080…102/170…172（codec/throttle/nudge/cycle/catalog） | PURE_CORE_ONLY | `Domain/EnforcerCatalog.gen.fs` `EnforcerCodec.fs` `EnforcerThrottle.fs` `EnforcerNudge.fs` `EnforcerCycle.fs` + `tests-mjs/Enforcer/` | 纯领域内核已实现（39 测试，120 项规则目录生成）。ENFORCER-030 exactly-once 与 ENFORCER-042 防御性归并已写入规范；throttle/nudge 生产接线仍未做 |
| ENFORCER-010/040…047/150/160…163（blog 工具与挂起链 Host 接线） | PARTIAL | `Infrastructure/OpenCode/Tools/BlogTool.fs` `Session/EnforcerHost.fs` `Session/ParkedTransform.fs` `Session/BloggerRuntimeState.fs` `Journal/EnforcementProjection.fs` `Kernel/Roles.fs`（`ToolPermission.Blog`）+ `tests-mjs/Enforcer/parked-transform.test.mjs` + `tests-mjs/Enforcer/blogger-runtime.test.mjs` + `tests-mjs/Journal/blog-entry-committed.test.mjs` + canary `host-transform-capability` | PARTIAL 未闭合：`BlogEntryCommitted` 原子推进与 blog 工具/挂起原语已接线；独立 `EnforcementCycleCommitted` 已删除。未闭合：`BloggerRuntimeState` 仅有纯转换、无生产生命周期权威；CurrentRequest/PendingOffer 未拆；多入口（`handleCompanionTransform` / `Submit` / `offerToBlogger`）并存；Squash 仍 `SubscribeTerminal`；首次请求 raw TOML 抽取；`BloggerNeedsReset` 全量 X 旁路仍在；crash recovery / CommitUnknown reconcile / 一次 repair / Park 仅 KnownCommitted 未闭合。不得把纯类型+部分 Normal 接线描述为单一 coordinator 已闭合 |
