...
## 反向缺口：有生产契约但无条款（包 T-5e 发现）

本表的形状假设每份判据都对应一条条款。写 `Parallel.mapBounded` 的测试时出现了相反的
情况：一个跨领域共享的原语，行为契约真实存在，而 `SSOT/` 没有任何条款管它。

```text
有条款覆盖   并发上限、取消传播        VERIFY-004（无界扇出使「慢」与「死」不可区分）
无条款覆盖   结果保序、空输入短路、
             拒绝非正上限、拒绝后
             siblings 继续运行         —
```

测试因此分两种命名：前者用 `VERIFY_004_` 前缀，后者用 `mapBounded_`。不给后者硬套一个
条款 ID，因为那会在测试名里放一个 `SSOT/` 并不背书的断言，而 `ssot-lint` 只读 `SSOT/`
目录——测试名里的伪造引用没有任何机器会发现。

其中一条值得单独记档，因为它反直觉。 拒绝后 siblings 不被取消：`mapBounded` 用
`Promise.all`，所以一个 action 抛出时调用立即 reject，但已获得许可的 action 会在后台
跑到完成。把 reject 当成「不会再发生任何事」的调用方是错的，而且这里没有取消机制能
纠正它——token 传给了每个 action，停止 siblings 是 action 的责任而非组合子的。

这不是缺陷，是 `Promise.all` 的语义。但它是一条没有条款背书、只靠一个测试锁住的行为，
所以下一个读到 reject 的人有权认为语义是相反的。登记为 SSOT 待补项，与那 14 个
`UNVERIFIED` 是不同性质的缺口：那些是判据未产生，这个是规范未表达。
