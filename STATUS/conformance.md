...
| COMPANION-005: BlogFrame 增量投影 | PARTIAL | `Domain/CompanionProjectionBuilder.fs` `Session/EnforcerHost.fs` `Session/BloggerCoordinator.fs` | Normal/Squash 共用 builder；首次与 resume 均从 typed context 重建。Squash 生产已构造 typed context 并经 blog continuation 提交。缺 layer-4 canary 证据，保持 PARTIAL |
| COMPANION-008: 忙时跳过 | PARTIAL | `Session/BloggerRuntimeState.fs` `Session/BloggerCoordinator.fs` | 生产 busy = `BloggerRuntimeState.InFlight`；InFlight 新 material 只 Skip。`Companion.inFlightTask` busy 权威已删。缺 layer-4 竞态 canary，保持 PARTIAL |
...
| CTX-006: 恢复槽的两种动作 | PARTIAL | `Domain/RecoverySlot.fs` `OpenCode/XWire.fs` `Session/BloggerCoordinator.fs` | X probe 已接线；Y squash 已迁入 blog tool continuation（不再 SubscribeTerminal）。armed+odd+frames 时 coordinator 启动 typed Squash。缺 layer-4 证据，保持 PARTIAL |
| CTX-007: Attempt 三结局按 RequestKind 分派 | PARTIAL | `Domain/RecoverySlot.fs` `Session/EnforcerHost.fs` | Normal/Squash 共用 continuation；Squash 提交 `BlogSquashCommitted` 不推进 coverage。一次 repair 生产接线仍缺。第 1 层测试 9 项 + 收敛红测 |
...
| CTX-012: 提交语义 | PARTIAL | `OpenCode/XWire.fs` `Journal/BlogProjection.fs` `Session/EnforcerHost.fs` | X PrefixRebaseCommitted writer 闭合；Y squash 经 tool loop 永久提交、ceil(m/2)、coverage 不变。缺 layer-4 canary，保持 PARTIAL |
...
| ENFORCER-010/040…047/150/160…163（blog 工具与挂起链 Host 接线） | PARTIAL | `Infrastructure/OpenCode/Tools/BlogTool.fs` `Session/EnforcerHost.fs` `Session/ParkedTransform.fs` `Session/BloggerRuntimeState.fs` `Session/BloggerCoordinator.fs` `Journal/EnforcementProjection.fs` + `tests-mjs/Enforcer/*` + canary `host-transform-capability` | C0–C4 已落地：生产 `BloggerRuntimeCell` 权威；CurrentRequest/PendingOffer 双槽；唯一 `BloggerCoordinator.onMainMaterial`；发送前冻结 context；无 BloggerNeedsReset；无 raw TOML 抽取；Squash typed context + blog tool continuation 提交 `BlogSquashCommitted`；仅 KnownCommitted 才 Park；资源上限；Main success 清 fallback；fail 清 child cache；fail-closed frame loader；`CompanionIdentity.newWorkMessageId`。未闭合：interaction/tool-loop 一次 repair 生产接线；CommitUnknown reconcile；durable request materialization 事实；五窗口 crash recovery 全量证据；layer-4 canary 三轮 |
