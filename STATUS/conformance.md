...
| COMPANION-004: Y 的 System Prompt | PARTIAL | `Domain/CompanionPrompt.fs` `prompts/blogger-system.md` | 冻结后 system 唯一来源应为 `blogger-system.md` + ENFORCER-030；`CompanionPrompt.System` 与 no-tools 文案仍待删除；权威工具面 `{ blog }` 未在 prompt asset 收口。旧判据 `COMPANION_004_normal_behaviour_has_a_single_owner_in_the_system_prompt` 已过时 |
| COMPANION-005: BlogFrame 增量投影 | PARTIAL | `Domain/CompanionProjectionBuilder.fs` | 冻结后 normal = Working Record frames + New Work delta + final instruction；标题消息层包装；resume 须重建而非 append raw。现实现仍为 frames + data-only delta、无 instruction 消息。判据需重写 |
...
| 条款 | 状态 | 当前代码位置 | 差距 |
|------|------|-------------|------|
| ENFORCER-020…043/080…102/170…172（codec/throttle/nudge/cycle/catalog） | PURE_CORE_ONLY | `Domain/EnforcerCatalog.gen.fs` `EnforcerCodec.fs` `EnforcerThrottle.fs` `EnforcerNudge.fs` `EnforcerCycle.fs` + `tests-mjs/Enforcer/` | 纯领域内核已实现（39 测试，120 项规则目录生成）。ENFORCER-030 exactly-once 与 ENFORCER-042 防御性归并已写入规范，代码仍按旧「可多调用」文案/路径；throttle/nudge 生产接线仍未做 |
| ENFORCER-010/040…047/150/160…163（blog 工具与挂起链 Host 接线） | PARTIAL | `Infrastructure/OpenCode/Tools/BlogTool.fs` `Session/EnforcerHost.fs` `Session/ParkedTransform.fs` `Journal/EnforcementProjection.fs` `Kernel/Roles.fs`（`ToolPermission.Blog`）+ `tests-mjs/Enforcer/parked-transform.test.mjs` + canary `host-transform-capability` | 已有：blog 工具、{blog} 权限、continuation park/offer、cancel 释放。未闭合（本轮垂直切片目标）：扩展 `BlogEntryCommitted` 原子推进 coverage 并携带 ToolCallIds/scores/evidence；删除独立 `EnforcementCycleCommitted`；单一 `BloggerRuntimeState` + typed request context；删除 BloggerMain terminal 等待；resume 重建 provider view；system/prompt 与 squash 投影收口。throttle/nudge/完整 crash recovery 仍保持后续项 |
