...
| COMPANION-005: BlogFrame 增量投影 | CONFORMANT | `Domain/CompanionProjectionBuilder.fs` `Session/EnforcerHost.fs` `Session/BloggerCoordinator.fs` | Normal/Squash 共用 builder；首次与 resume 从 typed CurrentRequest 重建；无 raw TOML 抽取。layer-4：`host-transform-capability-canary`（Working Record + New Work + instruction、无 raw transcript 泄漏）；`companion-canary` 同 child 两轮投影 |
| COMPANION-008: 忙时跳过 | CONFORMANT | `Session/BloggerRuntimeState.fs` `Session/BloggerCoordinator.fs` | 生产 busy = `BloggerRuntimeState.InFlight`；InFlight 新 material 只 Skip。layer-4：`host-transform-capability-canary` 第三 main turn 单飞（at most one blogger request for third material）；第 1 层：`blogger-runtime.test.mjs` |
...
| CTX-006: 恢复槽的两种动作 | CONFORMANT | `Domain/RecoverySlot.fs` `OpenCode/XWire.fs` `Session/BloggerCoordinator.fs` | X probe 接线；Y squash 经 blog tool continuation（无 SubscribeTerminal）。armed+odd+frames 时 coordinator 启动 typed Squash。第 1 层 recovery-slot；Squash 生产构造 `BloggerRequestContext.Squash` + `BlogSquashCommitted` |
| CTX-007: Attempt 三结局按 RequestKind 分派 | CONFORMANT | `Domain/RecoverySlot.fs` `Session/EnforcerHost.fs` | Normal/Squash 共用 continuation；Squash 不推进 coverage；一次 RepairSpent repair。第 1 层 9 项 + cycle outcome 三态 |
...
| CTX-012: 提交语义 | CONFORMANT | `OpenCode/XWire.fs` `Journal/BlogProjection.fs` `Session/EnforcerHost.fs` | X PrefixRebaseCommitted writer 闭合；Y squash tool-loop 永久提交、ceil(m/2)、coverage 不变。统一 BloggerCycleReceipt；layer-4 BlogEntryCommitted ≥2 + materialize 证据 |
...
| ENFORCER-010/040…047/150/160…163（blog 工具与挂起链 Host 接线） | CONFORMANT | `Infrastructure/OpenCode/Tools/BlogTool.fs` `Session/EnforcerHost.fs` `Session/ParkedTransform.fs` `Session/BloggerRuntimeState.fs` `Session/BloggerCoordinator.fs` `Application/Reconciliation/BloggerCrashRecovery.fs` + `tests-mjs/Enforcer/*` + `host-transform-capability-canary` + `companion-canary` | 生产 `BloggerRuntimeCell`；CurrentRequest/PendingOffer 双槽；唯一 coordinator；发送前 materialize；Squash typed + blog continuation；KnownCommitted 才 Park；资源上限；Main success 清 fallback；fail 清 child cache；fail-closed frame loader；crash recovery gate 不 stomp live CurrentRequest；layer-4 single-flight + materialize |
