...
| join/return 不停 Y | `HandleCompleted` / `HandleRetired` 不碰 Companion；`session.deleted` 才 `DisposeSession` |
...
| Human root / 无 handle | 不用 handle。采用：本 session 作为 parent 时不再有 Active handle，且自身不是任何 parent 的 Active child 不够。更干净：Human root 没有 join 语义；停止条件改为 session.deleted 或显式 「逻辑 run 终态」——见下 |
...
| 传输/Host 层 | Y 的 `TurnFailed` provider error；Host retry | 走既有 Fallback/AABB（cursor odd 槽 + ArmRecovery）；不把「coverage 没动」当成功结束；若 cycle 结束时仍无 commit → 不得静默 Idle 后再 Start 同一 RequestId 材料，除非新的 Main 材料（新 delta）且 main 未 sealed |
...
不要拿失败那一刻 materialize 的 delta 原样再发。  
...
与 rule A 合取：重试前若 `mainSealedForBlogger` → 不重试、不发新请求（丢 InFlight，静默 Sealed）。
...
提交 fatal 时尽量带：`result`, `session_id`, `blogger_session_id`；若需 prev/next ingest，扩展白名单并改 `ctx014.test.mjs`（或塞进 `result` 字符串避免扩表——优先结构化扩表：`cutoff_before` 已有，可复用语义或加 `result` 内嵌）。
