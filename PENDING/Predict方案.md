本文档是 SSOT/14 — Predict & Reduce Strength 的最终修订稿，融合了 ChatGPT 审阅的全部反馈与 Host 调研结论。
它按现有 SSOT 的 Session/Companion、Transform 身份绑定、前缀缓存、Journal Fold 和 fail-closed 原则收敛，
并吸收了以下修改变更：

- 记号统一：X / Y_X / Z_X / P(X) / L(X) / F(X) / Π / σ / ε / ↾，规范位置 SSOT/99
- Replica 晋升为 CanonicalRole（与 Blogger 同构），删除 ExecutionSurface 及其交集代数
- 三会话拓扑：X 拥有两颗卫星 Y_X 与 Z_X，二者皆为叶子；Z_X 无 Companion
- 卫星结构层泛化（SatelliteContract + SatelliteRuntime），策略层不泛化
- 工具权限双层经实读确认无需 ARCH-003 例外；session ruleset + resolveTools 提供第一层过滤
- Z_X 无自身 epoch，继承 X 的前缀；无恢复路径，失败即丢弃决策
- Host canary 清单扩展至 21 项，超时与取消责任归插件侧

# SSOT/14 — Predict & Reduce Strength

**状态：** 最终修订稿（已通过审阅，待 Host canary 和灰度验证）
**条款前缀：** `STRENGTH-`
**关联文档：** SSOT/13 — Projection Algebra（所有 provider-visible projection 的唯一生产路径）
**目标版本：** 首个生产版本
**默认状态：** 关闭；通过 Host canary 和灰度门禁后启用

> **说明：** 根据最终审阅裁决，Projection DSL 已提升为全局架构 SSOT/13。
> 本方案顺延为 SSOT/14，所有涉及 provider-visible bytes 构造的条款统一引用 SSOT/13。
> 旧 SSOT/13 仅保留为迁移输入和审计来源。
...
即 `Z_X` 的可见历史由两部分构成：X 的 cutoff 之后原始历史，与 `L(X)` 的冻结切片（`FrozenB`）。

```text
Z_X 不维护 ActivePrefixEpoch / LatestB / CoverableB / Coverage / FrameEpoch。
禁止将 Y_X 的 FrozenB 直接用于 Z_X——Z_X 继承的是 ε(X) 下的冻结切面，不是 Y_X 的原始状态。
```
...
预测模型和控制器状态**只按主工作会话 X 的 CanonicalRole 分桶**：
...
`StrengthFrameCandidateCommitted` 必须在 Xm transform 返回包含这些帧的最终 projection **之前**写入 Journal。
...
```text
SatelliteTypes.fs             卫星种类、契约与Descriptor（STRENGTH-118）
...
5. 第 K+1 次 Replica transform 默认挂起（不跨 Primary Authority Root 复用）。超时由插件侧实施（STRENGTH-135），10 分钟取值待 C-05 确认。
...
通过 `K≤2`、候选-消费-提升语义、字节成本入账、独立 K2 控制环、慢速负反馈、自动熔断和数据边界门禁，这些风险被约束在工程可接受范围内。超时与取消责任显式归属插件侧（STRENGTH-135），Host 不提供保障。
...
4. STATUS/conformance.md 在同一提交内回写状态，不得延后。
