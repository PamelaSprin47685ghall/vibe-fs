...
| Fork 续做沿用绑定 `deep-*`；禁止 caller `fast-*` 覆盖 | EXEC-002、PROMPT-006 → `tests/unit/session/host-fork-agent.test.mjs`、`tests/unit/tools/fork-tool.test.mjs` |
...
| SyncDelegate Charge/ProviderPrompt split | warm-start provider bytes 可 enrich；Casebook `NoteInspectorPrompt` 与 Opening 仍 byte-exact raw Charge；zero-keyword 两者一致；完整 ProviderRun semantic batch admission 后才 prepare，并按 tool-call 顺序 concat | EXEC-031/032、AGENT-032 |
