...
let ompMessageTransformUsesProjectionPolicy () =
    ...
    check "arch: Omp MessageTransform uses shouldExcludeAgentFromProjection"
        (code.Contains "shouldExcludeAgentFromProjection")
    check "arch: Omp MessageTransform uses MessageTransformPipeline"
        (code.Contains "MessageTransformPipeline")

let ompMessageTransformUsesShellCaps () =
    let code = requireFile "src/Omp/MessageTransform.fs" |> nonCommentCode
    check "arch: Omp MessageTransform opens OmpCaps"
        (code.Contains "OmpCaps")
    check "arch: Omp MessageTransform uses MessageTransformCore"
        (code.Contains "MessageTransformCore")
    check "arch: Omp MessageTransform uses MessageTransformHostEntry"
        (code.Contains "MessageTransformHostEntry")

let ompMessageTransformUsesMessagingCodec () =
    let code = requireFile "src/Omp/MessageTransform.fs" |> nonCommentCode
    check "arch: Omp MessageTransform opens MessagingCodec"
        (code.Contains "MessagingCodec")
    check "arch: Omp MessageTransform no local buildCapsMessages"
...
let ompMessagingCodecUsesShellPartCodec () =
    ...
    check "arch: Omp MessagingCodec defines textFromParts"
        (code.Contains "let private textFromParts")
    check "arch: Omp MessagingCodec defines entries"
        (code.Contains "let entries")
...
let ompHookExecuteUsesSubagentIntentsCodec () =
    ...
    check "arch: Omp HookExecute opens SubagentIntentsCodec"
        (code.Contains "SubagentIntentsCodec")
    check "arch: Omp HookExecute uses joinCoderUiLabel"
        (code.Contains "joinCoderUiLabel")
    check "arch: SubagentIntentsCodec defines joinCoderUiLabel"
        (codec.Contains "let joinCoderUiLabel")

let ompToolsRegisterAll () =
    let code = requireFile "src/Omp/Tools.fs" |> nonCommentCode
    check "arch: Omp Tools defines registerAllTools"
        (code.Contains "let registerAllTools")
    check "arch: Omp Tools registers fuzzy tools"
        (code.Contains "registerFuzzyTools")
    check "arch: Omp Tools registers methodology tools"
        (code.Contains "registerMethodologyTools")
    check "arch: Omp Tools registers executor tools"
        (code.Contains "registerExecutorTools")
    check "arch: Omp Tools registers subagent tools"
        (code.Contains "registerSubagentTools")
...
let ompPluginUsesPluginCore () =
    ...
    check "arch: Omp Plugin opens PluginCore"
        (plugin.Contains "PluginCore")
    check "arch: Omp PluginCore defines CoreServices"
        (core.Contains "type CoreServices")
    check "arch: Omp PluginCore creates review store"
        (core.Contains "createReviewStore")
...
let ompUsesOmpToolSchema () =
    ...
    check "arch: Omp SubagentTools references OmpToolSchema"
        (subagent.Contains "OmpToolSchema")
...
let ompReviewUsesReviewRuntime () =
    ...
    check "arch: Omp ReviewLoop uses ReviewSession"
...
let ompFuzzyToolsUsesShellFinder () =
    ...
    check "arch: Omp FuzzyTools uses FuzzyFinderShell"
        (code.Contains "FuzzyFinderShell")

let ompExecutorUsesShellExecute () =
    let code = requireFile "src/Omp/ExecutorTools.fs" |> nonCommentCode
    check "arch: Omp ExecutorTools uses ToolExecute or Shell executor path"
