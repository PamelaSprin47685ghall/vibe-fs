...
let opencodeReviewUsesToolCopy () =
    ...
    check
        "arch: Opencode ReviewTools uses opencodeSubmitReviewInProgress"
        (code.Contains "opencodeSubmitReviewInProgress")

    check
        "arch: Opencode ReviewTools must not inline you do not need review"
...
let opencodeReviewUsesFromOpencode () =
    ...
    check
        "arch: Opencode ReviewTools uses runtime.Execution for session and directory"
    ...
    check
        "arch: Opencode ReviewTools must not Dyn.str context sessionID"
        (not (code.Contains "Dyn.str context \"sessionID\""))

    check
        "arch: Opencode ReviewTools must not Dyn.str context directory"
...
let opencodeReviewUsesReviewToolsCodec () =
    ...
    check
        "arch: Opencode ReviewTools submit must not Dyn.str args report"
    ...
    check
        "arch: Opencode ReviewTools return must not Dyn.str args verdict"
    ...
    check
        "arch: Opencode ReviewTools return uses Params.returnReviewerVerdict"
        (code.Contains "Params.returnReviewerVerdict")

    check "arch: Opencode ReviewTools opens ToolExecute" (code.Contains "ToolExecute")

    check
        "arch: Opencode ReviewTools submit decode uses wireDecodeFailure submit_review"
        (code.Contains "wireDecodeFailure \"submit_review\"")

    check
        "arch: Opencode ReviewTools return decode uses wireDecodeFailure return_reviewer"
        (code.Contains "wireDecodeFailure \"return_reviewer\"")

    check
        "arch: Opencode ReviewTools client failure uses wireEncodeToolError OpencodeClient"
        (code.Contains "wireEncodeToolError \"OpencodeClient\"")

    check
        "arch: Opencode ReviewTools must not ToolHelpers.formatDomainError submit_review"
        (not (code.Contains "formatDomainError \"submit_review\""))

    check
        "arch: Opencode ReviewTools must not ToolHelpers.formatDomainError return_reviewer"
