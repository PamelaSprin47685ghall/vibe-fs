...
let subagentCapsInjectionSpec () = promise {
    let! workspaceDir = mkdtempAsync "subagent-caps-inj-"
    do! writeFileAsync (unbox<string> (pathModule?join(workspaceDir, "Target.fs"))) "let targetContent = \"hello subagent\""
            ...
            if Dyn.isArray parts then
                for part in unbox<obj array> parts do
                    if Dyn.str part "tool" = "read" then
                        ...
                        if outText.Contains("hello subagent") then foundTargetRead <- true
...
let crossSessionIsolationSpec () = promise {
            ...
            if Dyn.isArray parts then
                for part in unbox<obj array> parts do
                    if Dyn.str part "tool" = "read" then
                        let state = Dyn.get part "state"
                        let outText = Dyn.str state "output"
                        if outText.Contains("let a = 1") then foundA <- true
                        if outText.Contains("let b = 2") then foundB <- true
...
let ompChildSessionObjectiveReRegisterSpec () = promise {
    let scope = RuntimeScope()
    
    let mockSessionManagerType =
        createObj [ "create", box (fun (cwd: string) ->
            createObj [
    ...
    let mockCodingAgent =
        createObj [ "SessionManager", box mockSessionManagerType ]
    ...
    let mockInnerPi =
        createObj [
            "createAgentSession", box (fun (body: obj) ->
                let wrapper =
                    createObj [
                        "session", box (createObj [
                            "sessionManager", box (createObj [
                                "sessionId", box (Some "child-sess-1")
                                "getSessionId", box (Some (fun () -> box "child-sess-1"))
    ...
    let ctx =
        createObj [
            "sessionManager", box (Some (createObj [
    ...
    check "OMP ChildSession automatically replicates temp file registrations to child session ID" (copiedFiles = Some ["OmpTarget.fs"])
}

let opencodeSubsessionParentIDSpec () = promise {
    ...
    let backlogSession = Wanxiangshu.Opencode.BacklogSession.BacklogSession (Wanxiangshu.Kernel.HostTools.Host.Mimocode, scope)
    let reviewStore = createReviewStore ()
    do! Wanxiangshu.Opencode.MessageTransform.messagesTransform registry workspaceDir scope backlogSession reviewStore (box null) input output
        ...
        if Dyn.isArray parts then
            for part in unbox<obj array> parts do
                if Dyn.str part "tool" = "read" then
                    let state = Dyn.get part "state"
                    let outText = Dyn.str state "output"
                    if outText.Contains("let parentVal = 42") then foundParentRead <- true
                    
    check "Opencode messagesTransform correctly uses parent session ID via ResolveSubsessionParentID for temp files injection" foundParentRead
    do! rmAsync workspaceDir
}

let subagentFallbackRawTextSpec () = promise {
            ...
            if Dyn.isArray parts then
                for part in unbox<obj array> parts do
                    if Dyn.str part "tool" = "read" then
                        let state = Dyn.get part "state"
                        let outText = Dyn.str state "output"
                        if outText.Contains("let raw = 100") then foundRawRead <- true
                        
        check "subagent caps injection falls back to raw text key when objective front-matter is missing" foundRawRead
    finally
...
let private mockMuxUserMsg (id: string) (prompt: string) : obj =
    let parts = [| createObj [ "type", box "text"; "text", box prompt; "state", box "done" ] |]
    createObj [ "id", box id; "role", box "user"; "parts", box parts ]

let muxSubsessionParentIDSpec () = promise {
    ...
    let mockWorkspaceEntry =
        createObj [
            "workspace", box (createObj [
                "parentWorkspaceId", box parentSessionID
            ])
        ]
    ...
    let parentOpt = Wanxiangshu.Shell.MuxWorkspaceCodec.tryGetParentWorkspaceId mockDeps childSessionID
    check "parent workspace is parent-ws-A" (parentOpt = Some parentSessionID)
    let isChild = Wanxiangshu.Shell.MuxWorkspaceCodec.isChildWorkspace mockDeps childSessionID
    check "is child workspace" isChild
        
    let prompt = "---\nobjective: Code mux parent\n---\nImplement it"
    let userMsg = mockMuxUserMsg "msg-1" prompt
    let output = createObj [ "messages", box [| userMsg |] ]
    let input = createObj [ "agent", box "build"; "sessionID", box childSessionID; "workspacePath", box workspaceDir ]
        ...
        if Dyn.isArray parts then
            for part in unbox<obj array> parts do
                if Dyn.str part "toolName" = "file_read" then
                    ...
                    if content.Contains("let muxParentVal = 999") then foundMuxParentRead <- true
                    
    check "Mux messagesTransform correctly uses parent workspace ID via tryGetParentWorkspaceId for temp files injection" foundMuxParentRead
    do! rmAsync workspaceDir
}
