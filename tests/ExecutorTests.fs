...
let infiniteStdoutBounded () = promise {
    // Arrange: spawn a child that dumps ~100 MiB of stdout.
    let sid = nextSessionId ()
    let workDir = cwdVal ()
    let! outcome =
        spawnAndRun "node" [| "-e"; hugeOutputScript |] workDir (Some timeoutMs) (Some sid) None
    ...
    check
        ("infiniteStdoutBounded.totalBytes <= " + string outputByteLimit +
         " (got " + string totalBytes + ")")
    ...
    check
        "infiniteStdoutBounded.outcome is not Exited(0,...)"
        (not (isExitedZero outcome))
}
...
let infiniteStderrBounded () = promise {
    // Arrange: spawn a child that dumps ~100 MiB of stderr.
    // The same accumulator flaw applies symmetrically.
    let sid = nextSessionId ()
    let workDir = cwdVal ()
    let! outcome =
        spawnAndRun "node" [| "-e"; scriptErr |] workDir (Some timeoutMs) (Some sid) None
    ...
    check
        ("infiniteStderrBounded.stderr.Length <= " + string outputByteLimit +
         " (got " + string capturedErr.Length + ")")
        (capturedErr.Length <= outputByteLimit)

    check
        "infiniteStderrBounded.outcome is not Exited(0,...)"
        (not (isExitedZero outcome))
}

let smallOutputUnchanged () = promise {
    ...
    let! outcome =
        spawnAndRun "node" [| "-e"; scriptOk |] workDir (Some timeoutMs) (Some sid) None
    ...
    | _ ->
        check "smallOutputUnchanged.expectedExit0" false
}
