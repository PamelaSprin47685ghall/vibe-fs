...
let decodeReadOkWithOptionalPaging () =
    let args = createObj [ "path", box "/tmp/a.txt"; "offset", box 5; "limit", box 20 ]
...
let decodeWriteMissingFilePath () =
    ...
    | Error(InvalidIntent("write", "file_path", "missing required parameter")) -> check "write missing file_path" true
...
let decodeWriteEmptyFilePath () =
    ...
    match decodeWriteArgs args with
    | Error(InvalidIntent("write", "file_path", "must not be empty")) -> check "write empty file_path" true
