...
let opencodeExecutorUsesExecutorToolsCodec () =
    ...
    check "arch: Opencode ExecutorTool uses wireDomainFailure for executor decode" (code.Contains "wireDomainFailure \"Executor\"")
    ...
    check "arch: Opencode ExecutorTool must not Dyn.str args timeout_type" (not (code.Contains "Dyn.str args \"timeout_type\""))
