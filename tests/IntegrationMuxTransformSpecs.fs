...
let muxTopLevelDedupSpec () =
    promise {
                ...
                {| parts =
                    [| box
                           {| ``type`` = "dynamic-tool"
                              ...
                              toolCallId = string callIdCounter |} |] |}
        ...
        check
            "mux top-level dedup replaces repeat content"
            (str output "content" = Wanxiangshu.Kernel.ToolOutputInfo.noChangeEnvelope ())
    }
...
let muxMessagesTransformDedupsRepeatedReadSpec () =
    promise {
        ...
        if isNullish tf then
            ...
            let input =
                createObj [ "agent", box "manager"; "sessionID", box "mux-read-dedup-session" ]
            ...
            check
                "mux messagesTransform dedups repeated plugin read"
                (string secondOutput = Wanxiangshu.Kernel.ToolOutputInfo.noChangeEnvelope ())
    }

let muxMessagesTransformDedupsRepeatedFileReadSpec () =
    promise {
        let reg = sharedMuxRegistration ()
        let tf = muxMessageTransform reg
        ...
        if isNullish tf then
            ...
            check
                "mux messagesTransform dedups repeated file_read"
                (str secondOutput "content" = Wanxiangshu.Kernel.ToolOutputInfo.noChangeEnvelope ())
    }

let muxMessagesTransformDedupsRepeatedReadForTopLevelExecSpec () =
    promise {
        let reg = sharedMuxRegistration ()
        let tf = muxMessageTransform reg
        ...
        if isNullish tf then
            ...
            check
                "mux messagesTransform dedups repeated read for top-level exec"
                (str secondOutput "content" = Wanxiangshu.Kernel.ToolOutputInfo.noChangeEnvelope ())
    }
