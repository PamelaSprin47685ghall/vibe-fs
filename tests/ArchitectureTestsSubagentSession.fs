...
let sessionIoRunSubagentReturnsResult () =
    ...
    check
        "arch: SessionIo runSubagent returns Promise Result"
        (sessionIo.Contains "let runSubagent"
         && sessionIo.Contains "JS.Promise<Result<string, DomainError>>")

    check
        "arch: SessionIo runSubagentWithCleanup returns Promise Result"
        (sessionIo.Contains "let runSubagentWithCleanup")

    check
        "arch: SessionIo must not define private runSubagentCore string wrapper"
...
let commandHooksUsesToolCopyReviewMessages () =
    ...
    check "arch: CommandHooks opens ToolCopy" (code.Contains "ToolCopy")
    check "arch: CommandHooks uses reviewAlreadyActiveMessage" (code.Contains "reviewAlreadyActiveMessage")
    check "arch: ToolCopy defines preReviewCouldNotComplete" (copy.Contains "let preReviewCouldNotComplete")

    check
        "arch: CommandHooks must not inline With-Review Mode is already active"
...
let opencodeSubagentToolsUsesSimpleArgsCodec () =
    ...
    check
        "arch: Opencode SubagentTools must not open SubagentSimpleArgsCodec"
    ...
    check
        "arch: Opencode SubagentTools meditator must not Dyn.str args intent"
...
let opencodeSubagentToolsUsesFromOpencode () =
    ...
    check
        "arch: Opencode SubagentTools must not decodeOpencodeToolContext"
        (not (opencode.Contains "decodeOpencodeToolContext"))

    check
        "arch: Opencode SubagentTools must not define private ToolExecutionContext"
        (not (opencode.Contains "type private ToolExecutionContext"))

    check
        "arch: Opencode SubagentTools must not Dyn.str ctx directory"
