...
let private opencodeRejectedBeforeSendRejectsLateReceipt () =
    promise {
        ...
        check
            ...
            (not (OpencodeHost.PendingTurnReceipt.tryResolve (TurnId.value turnId) (UserMessageObserved "late")))
    }
...
let private routeNoneTurnIdEvidenceNotMergedWhenActive () =
    promise {
        ...
        let! routed = Wanxiangshu.Shell.SubsessionEventRouter.routeToChild "" sid (EvidenceUpdated obs)
