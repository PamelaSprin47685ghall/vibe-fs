...
let encText () =
    ...
    equal "text type" "text" (Dyn.str p.[0] "type")
    equal "text body" "hello" (Dyn.str p.[0] "text")
...
let encToolNoState () =
    ...
    let p = unbox<obj array> (Dyn.get e "parts")
    equal "type" "tool" (Dyn.str p.[0] "type")
    equal "tool" "t" (Dyn.str p.[0] "tool")
    check "no state field" (Dyn.isNullish (Dyn.get p.[0] "state"))
