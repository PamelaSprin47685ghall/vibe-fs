...
let txt (t: string) : Message<obj> =
    { info = mi ()
      parts = [ TextPart t ]
      source = Native
      raw = null }

let tmsg (tn: string) (cid: string) (st: ToolState<obj> option) (raw: obj) : Message<obj> =
    { info = mi ()
      parts = [ ToolPart(tn, cid, st, raw) ]
      source = Native
      raw = raw }

let rmsg (raw: obj) : Message<obj> =
    { info = mi ()
      parts = []
      source = Native
      raw = raw }
...
let encToolStateMatch () =
    ...
    let raw =
        createObj [ "state", box (createObj [ "status", box "running"; "output", box "out"; "error", box "" ]) ]
...
let encToolStateMismatch () =
    ...
    let raw =
        createObj [ "state", box (createObj [ "status", box "running"; "output", box "old"; "error", box "" ]) ]
        ...
        createObj
            [ "message",
              box (
                  createObj
                      [ "role", box "user"
                        "content", box [| createObj [ "type", box "text"; "text", box "hi" ] |] ]
              ) ]

    match decodeEntry "s1" e with
    | None -> check "user→Some" false
    | Some m -> equal "role" User m.info.role

let decAsst () =
    let e =
        createObj [ "message", box (createObj [ "role", box "assistant"; "content", box [||] ]) ]

    match decodeEntry "s1" e with
    | None -> check "asst→Some" false
    | Some m -> equal "role" Assistant m.info.role

let decInfoFallback () =
    let e =
        createObj
            [ "info", box (createObj [ "role", box "user"; "id", box "e1" ])
              "parts", box [| createObj [ "type", box "text"; "text", box "fb" ] |] ]

    match decodeEntry "s1" e with
    | None -> check "info→Some" false
    | Some m -> equal "role" User m.info.role

let decNull () =
    check "null→None" (decodeEntry "s1" null = None)

let decEmptyRole () =
    check
        "emptyRole→None"
        (decodeEntry "s1" (createObj [ "message", box (createObj [ "role", box ""; "content", box [||] ]) ]) = None)

let decArr () =
    let e1 =
        createObj [ "message", box (createObj [ "role", box "user"; "content", box [||] ]) ]
...
let entSM () =
    let sm =
        createObj [ "getEntries", box (fun () -> [| createObj [ "role", box "user" ] |] :> obj) ]

    equal "ent len" 1 (unbox<obj array> (entries (unbox<ISessionManager> sm))).Length

let entEmpty () =
    equal "ent empty" [||] (entries (unbox<ISessionManager> (createObj [])))
        ...
        createObj
            [ "message",
              box (
                  createObj
                      [ "role", box "user"
                        "content", box [| createObj [ "type", box "text"; "text", box "u" ] |] ]
              ) ]
        ...
        createObj
            [ "message",
              box (
                  createObj
                      [ "role", box "assistant"
                        "content", box [| createObj [ "type", box "text"; "text", box "a1" ] |] ]
              ) ]
...
let dtsFull () =
    let o =
        createObj [ "status", box "done"; "output", box "r"; "error", box ""; "input", box "i" ]

    match decodeToolState o with
...
let dtsNull () =
    check "dts null→None" (decodeToolState null = None)
