module VibeFs.Tests.MuxAiSettingsIntegrationTests
...
let resolveDelegatedAgentAiSettingsWorkspaceMergeSpec () = promise {
    let configFile =
        createObj [
            "subagentAiDefaults",
            box (
                createObj [
                    "coder", box (agentEntry "openai:sub-default" "low")
                ]
            )
            "agentAiDefaults",
            box (
                createObj [
                    "coder", box (agentEntry "openai:agent-default" "medium")
                ]
            )
        ]
    ...
    let workspace =
        createObj [
            ...
            box (
                createObj [
                    "coder", box (agentEntry "anthropic:ws-coder" "xhigh")
                ]
            )
        ]
    ...
    let deps =
        createObj [
            ...
            "findWorkspaceEntry",
            box (System.Func<obj, string, obj>(fun (_cf: obj) (wid: string) ->
            ...
            box (System.Func<obj, obj, string, JS.Promise<obj>>(fun (_rt: obj) (_cwd: obj) (_agentId: string) ->
                Promise.lift (
                    createObj [
                        ...
                        box (
                            createObj [
                                "model", box "openai:from-fm"
                                "thinkingLevel", box "max"
                            ]
                        )
                    ]
                )))
...
let resolveDelegatedAgentAiSettingsLenientConfigSpec () = promise {
            ...
            box (System.Func<obj, obj, string, JS.Promise<obj>>(fun (_rt: obj) (_cwd: obj) (_agentId: string) ->
                Promise.lift (
                    createObj [
                        ...
                        box (
                            createObj [
                                "modelString", box "openai:descriptor-only"
                                "thinkingLevel", box "off"
                            ]
                        )
                    ]
                )))
...
let resolveDelegatedAgentAiSettingsFrontmatterRejectSpec () = promise {
    let deps =
        createObj [
...
let resolveDelegatedAgentAiSettingsDefaultsOnlyMergeSpec () = promise {
    let configFile =
        createObj [
            "subagentAiDefaults",
            box (
