...
let replayReviewAlwaysSyncActivatesFromTexts () =
    let store = createReviewStore ()
    let activate =
        frontMatterPrompt [ yamlBlockField taskField "from-replay" ] "body"
...
let replayReviewIfStoreEmptyActivatesWhenEmpty () =
    let store = createReviewStore ()
    let activate = frontMatterPrompt [ yamlBlockField taskField "empty-store-task" ] "body"
...
let replayReviewIfStoreEmptySkipsWhenActiveButAlwaysSyncUpdates () =
    ...
    replayReviewIfStoreEmpty store "s4" [ frontMatterPrompt [ yamlBlockField taskField "ignored" ] "body" ]
    equal "if-store-empty skips active session" (Some "held") (store.getReviewTask "s4")
    let update = frontMatterPrompt [ yamlBlockField taskField "synced" ] "body"
