...
test('PTY_API_bytes_encodes_utf8', () => {
...
test('PTY_API_new_id_has_pty_prefix_and_eight_hex_chars', () => {
  const id = PtyId__get_Value(newId())
  assert.match(id, /^pty-[0-9a-f]{8}$/)
})

test('PTY_API_new_id_is_unique_per_call', () => {
...
test('PTY_API_abort_parent_invokes_every_registered_callback', () => {
...
test('PTY_API_abort_parent_with_unknown_id_is_a_noop', () => {
  abortParent('parent-never-registered')
})

test('PTY_API_unregister_removes_only_the_matching_token', () => {
...
test('PTY_API_unregister_last_callback_drops_the_parent_entry', () => {
...
test('PTY_API_unregister_with_unknown_parent_or_token_is_a_noop', () => {
...
test('PTY_API_tokens_are_monotonic_across_parents', () => {
  const t1 = registerParentAbort('parent-tok-1', () => {})
  const t2 = registerParentAbort('parent-tok-2', () => {})
  assert.ok(t2 > t1, `${t2} > ${t1}`)
})

test('PTY_API_throwing_abort_callback_does_not_block_the_rest', () => {
