...
const materialize = (store, events) => {
  const result = GitRaw.GitRawStore_materializeSnapshot(store, toList(events))
  return mustOk(result, 'materializeSnapshot')
}

const productionMerge = (store, snapshots) => {
  const input = Persist.MergeInputModule_ofList(toList(snapshots))
  return Merge.EventStoreMerge_merge(store, input)
}

const specMergeEvents = (events) => Merge.EventStoreMergeSpec_mergeEvents(toList(events))

const specMergeSnapshots = (store, snapshots) => {
  const input = Persist.MergeInputModule_ofList(toList(snapshots))
  return Merge.EventStoreMergeSpec_merge(store, input)
}
...
test('WriteBlob_matches_git_hash_object_sha1', () => {
  const store = createStore()
  const content = new TextEncoder().encode('{"a":1}\n')
  const oid = store.WriteBlob(content)
  assert.equal(Persist.GitObjectIdModule_value(oid), '0187f3b09d66cef7911083ad49f890f5a6589589')
  const roundTrip = store.ReadObject(oid)
  assert.equal(isSome(roundTrip), true)
  assert.deepEqual(Buffer.from(roundTrip), Buffer.from(content))
})

test('CompareAndSwapRef_Absent_then_expected_oid', () => {
  const store = createStore()
  const oid = store.WriteBlob(new TextEncoder().encode('x\n'))
  assert.equal(store.CompareAndSwapRef(Persist.StoreRef_canonical, undefined, oid), true)
  assert.equal(store.CompareAndSwapRef(Persist.StoreRef_canonical, undefined, oid), false)
  const other = store.WriteBlob(new TextEncoder().encode('y\n'))
  assert.equal(store.CompareAndSwapRef(Persist.StoreRef_canonical, oid, other), true)
  const current = store.ReadRef(Persist.StoreRef_canonical)
...
test('materializeSnapshot_payload_closure_and_missing_payload_fail_closed', () => {
  const store = createStore()
  const payloadOid = store.WriteBlob(new TextEncoder().encode('large-body\n'))
  ...
  const snapshot = materialize(store, [withPayload])
  const names = mustOk(GitRaw.GitRawStore_listPayloadNames(store, snapshot.RootOid))
  ...
  const missing = GitRaw.GitRawStore_materializeSnapshot(store, toList([dangling]))
...
test('merge_production_associative_commutative_idempotent_deterministic', () => {
  const store = createStore()
  const a = envelope({ id: 'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa', payload: { n: 1 } })
  const b = envelope({ id: 'bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb', payload: { n: 2 } })
  const c = envelope({ id: 'cccccccccccccccccccccccccccccccccccccccc', payload: { n: 3 } })

  const SA = materialize(store, [a])
  const SB = materialize(store, [b])
  const SC = materialize(store, [c])

  const merge2 = (x, y) => mustOk(productionMerge(store, [x, y]))
  const merge3 = (xs) => mustOk(productionMerge(store, xs))

  // commutative
  assert.equal(snapshotOid(merge2(SA, SB)), snapshotOid(merge2(SB, SA)))

  // associative
  const left = merge2(merge2(SA, SB), SC)
  const right = merge2(SA, merge2(SB, SC))
  assert.equal(snapshotOid(left), snapshotOid(right))

  // idempotent
  assert.equal(snapshotOid(merge2(SA, SA)), snapshotOid(SA))
  const ABC = merge3([SA, SB, SC])
  assert.equal(snapshotOid(merge2(ABC, ABC)), snapshotOid(ABC))
  ...
  const orders = [
    ...
    [SB, SC, SA],
    [SA, SC, SB],
  ].map((order) => snapshotOid(merge3(order)))
  for (const oid of orders) assert.equal(oid, orders[0])

  const spec = mustOk(specMergeSnapshots(store, [SA, SB, SC]))
  const blobs = mustOk(GitRaw.GitRawStore_listEventBlobs(store, ABC.RootOid))
  assert.equal(listItems(blobs).length, 3)
  assert.equal(listItems(spec).length, 3)
})

test('merge_identity_collision_fail_closed', () => {
  const store = createStore()
  const left = envelope({
  ...
  const SL = materialize(store, [left])
  const SR = materialize(store, [right])

  const merged = productionMerge(store, [SL, SR])
...
test('merge_production_matches_materialize_of_union', () => {
  const store = createStore()
  const payloadOid = store.WriteBlob(new TextEncoder().encode('shared\n'))
  const payloadHex = Persist.GitObjectIdModule_value(payloadOid)
  ...
  const SA = materialize(store, [a])
  const SB = materialize(store, [b])
  const merged = mustOk(productionMerge(store, [SA, SB]))
