// tests/unit/Journal/envelope.test.mjs — PERSIST-001/002/005/008.
//
...
test('PROMPT_011_RuntimeStarted_advances_a_workspace_watermark_not_every_session', () => {
  // PROMPT-011 counts plugin starts as recovery attempts, but the start is a
  // workspace fact: rewriting every session map would be O(starts × sessions)
  // on replay. Claims store the watermark at registration; later starts only
  // move AgentProjections.RuntimeStartCount.
