...
let submitKnowledgeGraphRejectsSecondCallSpec () = promise {
    ...
    // History starts with just the marker; first call should succeed.
            ...
            "messages", box (System.Func<obj, JS.Promise<obj>>(fun _ ->
                promise {
                    if not calledOnce then
                        // First call: marker only in history.
                        return box {| data = [| userTextMessage sessionID marker |] |}
                    else
                        // Second call: marker + completed return_bookkeeper tool in history.
                        let toolMsg =
                            box {|
                                info = createObj [
