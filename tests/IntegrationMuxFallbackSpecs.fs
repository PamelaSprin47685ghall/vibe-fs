...
let muxSessionErrorTriggersFallbackContinueSpec () = promise {
    let! tmpDir = mkdtempAsync "mux-fb-error-"
    do! writeFileAsync (tmpDir + "/AGENTS.md") "---\nmodels:\n  default:\n    - openai/gpt-5\n    - anthropic/claude-4\n---\n"

    let sessionID = "mux-fb-error-ws"
    let nudges = ResizeArray<string>()
    let deps =
        muxDepsWithNudgeCapture sessionID tmpDir
    ...
    check "Fallback consumed the error and dispatched one continue nudge via the fallback action executor" (nudges.Count = 1)
    let prompt = nudges.[0]
    check "Nudge text carries 'continue openai/gpt-5' (SendContinue against chain[0])"
...
let muxStreamEndToolCallAsTextTriggersFallbackSpec () = promise {
    ...
    let assistantMsg =
        createObj [
            ...
            "parts", box [|
                box {| ``type`` = "text"; text = "<tool_call><name>read</name></tool_call>" |}
            |] ]
    ...
    let ev = createObj [
        ...
        "properties", box (createObj [
            ...
            "parts", box [| box {| ``type`` = "text"; text = "incomplete" |} |] ]) ]

    do! (eventHook $ (ev, helpers)) |> unbox<JS.Promise<unit>>
    do! Promise.sleep 50

    check "Exactly one nudge fires (fallback RecoverWithPrompt), not a second from NudgeRuntime" (nudges.Count = 1)
    check "Nudge carries the FallbackMessageCodec recovery prompt ('You produced the tool call as raw text')"
