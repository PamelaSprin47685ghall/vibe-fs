...
let mergeSubagentBeforeAgent () =
    let configFile =
        createObj
            [ "subagentAiDefaults", box (createObj [ "explore", box (defaultsEntry "openai:subagent" "xhigh") ])
              "agentAiDefaults", box (createObj [ "explore", box (defaultsEntry "openai:agent" "medium") ]) ]
...
let modelStringKeyOnConfigDefaults () =
    let fromModelStringScalars =
        decodeAgentAiEntryScalars (
            unbox (createObj [ "modelString", box "anthropic:from-key"; "thinkingLevel", box "low" ])
        )

    equal
        "modelString key"
        (Some "anthropic:from-key")
        (fromModelStringScalars.Model |> Option.orElse fromModelStringScalars.ModelString)

    let fromModelScalars =
        decodeAgentAiEntryScalars (unbox (createObj [ "model", box "openai:from-model"; "modelString", box "ignored" ]))

    equal
        "model beats modelString"
        (Some "openai:from-model")
        (fromModelScalars.Model |> Option.orElse fromModelScalars.ModelString)

let blankModelSkipped () =
    let configFile =
        createObj
            [ "subagentAiDefaults", box (createObj [ "explore", box (defaultsEntry "" "medium") ])
              "agentAiDefaults",
