...
let decodeTaskCreateResultSuccessTrue () =
    let createResult =
        createObj [
            "success", box true
            "data", box (createObj [ "taskId", box "task-42" ])
        ]
    match decodeTaskCreateResult createResult with
    | Ok r ->
...
let decodeTaskCreateResultSuccessNoData () =
    ...
    | Error (InvalidIntent ("delegate.create", "taskId", "missing or empty")) ->
        check "create success=true no data" true
...
let decodeTaskReportEmpty () =
    ...
    | Error (InvalidIntent ("delegate.report", "reportMarkdown", "missing or empty")) ->
        check "report empty trimmed" true
