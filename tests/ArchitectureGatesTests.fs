...
/// .fsproj Compile order must mirror real layers (Kernel→Runtime→Hosts→tests),
/// not migration history. Hosts stay contiguous per host; integration/ is one block.
let private checkFsprojCompileOrder (cwd: string) =
    ...
    for path in includes do
        ...
        if layer >= 0 && layer <= 2 then
            failIf (layer < maxProd) (sprintf "production layer regression at %s (layer %d after %d)" path layer maxProd)
            if layer > maxProd then
                maxProd <- layer

    // After first non-src entry, no more src/ production files
    ...
    // Hosts: Mux, OpenCode, Omp each one contiguous block; host order Mux→OpenCode→Omp
    let hostFiles =
        includes
        |> List.choose (fun p ->
    ...
    failIf (List.ofSeq hostOrder <> expectedHosts) (sprintf "Hosts order must be Mux→OpenCode→Omp, got %A" (List.ofSeq hostOrder))

    // integration/ Compile entries form exactly one contiguous span
    let integIdx =
        includes
    ...
    // Every production .fs under src/ must appear in fsproj
