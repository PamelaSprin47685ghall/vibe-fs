...
let private mkChain () =
    [ mkModel "a" "m1"; mkModel "b" "m2"; mkModel "c" "m3" ]
...
let scan_k1_startsFromZero () =
    let s1, _ =
        transition (mkState FallbackPhase.Idle 1 0 0) (SessionError authErr) (mkCfg ()) (mkChain ())
...
let scan_k2_startsFromCurrent () =
    let s1, _ =
        transition (mkState FallbackPhase.Idle 2 1 0) (SessionError authErr) (mkCfg ()) (mkChain ())
...
let scan_k4_startsFromZero () =
    let s1, _ =
        transition (mkState FallbackPhase.Idle 2 3 0) (SessionError authErr) (mkCfg ()) (mkChain ())
...
let hallucination_belowThreshold () =
    ...
    let _, action =
        transition (mkState FallbackPhase.Idle 0 0 2) (SessionError retryErr) cfg (mkChain ())
...
let sessionIdle_idlePhase_scanToolCallAsText () =
    let _, action =
        transition (mkState FallbackPhase.Idle 0 0 0) SessionIdle (mkCfg ()) (mkChain ())
...
let exhausted_propagates () =
    ...
    let s1, action =
        transition (mkState (FallbackPhase.Scanning(0, 0)) 0 1 0) (SessionError retryErr) (mkCfg ()) chain
