...
let private mkReviewSession id st ver =
    { empty id nowVal with
        state = st
        version = ver }

let private regFromList sessions =
    sessions
    |> List.fold (fun r (s: ReviewSession) -> Map.add s.id s r) emptyRegistry
...
let hasActiveReviewStateActive () =
    check
        ...
        (Wanxiangshu.Kernel.ReviewSession.Query.hasActiveReviewState
            (regFromList [ mkReviewSession "s" (ReviewState.Active "t") 0 ])
            "s")
...
let hasActiveReviewStateLocked () =
    check
        "Locked is active"
        (Wanxiangshu.Kernel.ReviewSession.Query.hasActiveReviewState
            (regFromList [ mkReviewSession "s" (ReviewState.Locked("t", "r")) 0 ])
            "s")
...
let hasActiveReviewStateInactive () =
    check
        ...
        (not (
            Wanxiangshu.Kernel.ReviewSession.Query.hasActiveReviewState
                (regFromList [ mkReviewSession "s" ReviewState.Inactive 0 ])
                "s"
        ))
...
let hasActiveReviewStateAccepted () =
    check
        ...
        (not (
            Wanxiangshu.Kernel.ReviewSession.Query.hasActiveReviewState
                (regFromList [ mkReviewSession "s" ReviewState.Accepted 0 ])
                "s"
        ))

let hasActiveReviewStateNeedsRevision () =
    check
        "NeedsRevision is active"
        (Wanxiangshu.Kernel.ReviewSession.Query.hasActiveReviewState
            (regFromList [ mkReviewSession "s" (ReviewState.NeedsRevision "bad") 0 ])
            "s")
...
let stateOfPresent () =
    match
        Wanxiangshu.Kernel.ReviewSession.Query.stateOf
            (regFromList [ mkReviewSession "s" (ReviewState.Locked("t", "r")) 0 ])
            "s"
    with
...
let canTransitionTrue () =
    check
        "Inactive+Activate valid"
        (Wanxiangshu.Kernel.ReviewSession.Query.canTransition
            (regFromList [ mkReviewSession "s" ReviewState.Inactive 0 ])
            "s"
            (ReviewCommand.Activate "t"))
...
let canTransitionMissing () =
    check
        "missing can't transition"
        (not (Wanxiangshu.Kernel.ReviewSession.Query.canTransition emptyRegistry "nope" (ReviewCommand.Activate "t")))
...
let versionOfPresent () =
    equal
        "versionOf 5"
        (Some 5)
        (Wanxiangshu.Kernel.ReviewSession.Query.versionOf
            (regFromList [ mkReviewSession "s" ReviewState.Accepted 5 ])
            "s")
...
let reduceIfVersionMissingSessionReturnsNone () =
    match
        Wanxiangshu.Kernel.ReviewSession.Query.reduceIfVersionMatches
            ...
            "nope"
            1
            (RegistryAction.Activate("nope", "t", nowVal))
    with
