module Wanxiangshu.Tests.Wanxiangzhen.ExtendedFfDecisionTests
...
let entries () : (string * (unit -> unit)) list =
    [ ("FfDecision.ffResultLabel Merged returns 'merged'",
      ...
      ("FfDecision.ffResultLabel RebaseNeeded returns 'rebase_needed'",
       fun () ->
           let label = ffResultLabel (RebaseNeeded "abc123")
           equal "rebase_needed" label)

      ("FfDecision.ffResultLabel StaleCommit returns 'stale_commit'",
       fun () ->
           let label = ffResultLabel StaleCommit
           equal "stale_commit" label)

      ("FfDecision.ffResultLabel CoordinatorNotReady returns 'coordinator_not_ready'",
       fun () ->
