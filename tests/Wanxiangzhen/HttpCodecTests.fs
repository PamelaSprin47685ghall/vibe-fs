module Wanxiangshu.Tests.Wanxiangzhen.HttpCodecTests
...
let private taskA: SquadTask =
    { Id = "squad-a1b2"
      ...
      Status = Wanxiangshu.Kernel.Wanxiangzhen.SquadTask.Running
...
let entries () : (string * (unit -> unit)) list =
    [
      ...
      ("HttpCodec.encodeTaskDetail: all fields",
       fun () ->
      ...
      ("HttpCodec.encodeTaskDetail: empty dependsOn",
       fun () ->
      ...
      ("HttpCodec.encodeStateSnapshot: empty dag",
       fun () ->
           let dag = Wanxiangshu.Kernel.Wanxiangzhen.Dag.empty "s1" ""
      ...
      ("HttpCodec.encodeStateSnapshot: one task",
       fun () ->
           let dag =
               Wanxiangshu.Kernel.Wanxiangzhen.Dag.empty "s1" "req"
               |> Wanxiangshu.Kernel.Wanxiangzhen.Dag.addTask taskA
      ...
      ("HttpCodec.encodeFfResponseBody: Merged sha",
       fun () ->
           let o =
               encodeFfResponseBody (Wanxiangshu.Kernel.Wanxiangzhen.FfDecision.Merged "sha1")
               |> unbox<obj>

           equal "merged" (str o "result")
           equal "sha1" (str o "masterSha"))

      ("HttpCodec.encodeFfResponseBody: RebaseNeeded sha",
       fun () ->
           let o =
               encodeFfResponseBody (Wanxiangshu.Kernel.Wanxiangzhen.FfDecision.RebaseNeeded "sha2")
               |> unbox<obj>

           equal "rebase_needed" (str o "result")
           equal "sha2" (str o "masterSha"))

      ("HttpCodec.encodeFfResponseBody: StaleCommit no extra",
       fun () ->
           let o = encodeFfResponseBody StaleCommit |> unbox<obj>
           equal "stale_commit" (str o "result")
           checkBare (isNullish (get o "masterSha")))

      ("HttpCodec.encodeFfResponseBody: CoordinatorNotReady reason",
       fun () ->
      ...
      ("HttpCodec.encodeResult: label",
       fun () ->
           let o = encodeResult "ok" |> unbox<obj>
           equal "ok" (str o "result"))

      ("HttpCodec.decodeFfResult: Merged",
       fun () ->
           let body = createObj [ "result" ==> "merged"; "masterSha" ==> "s1" ]

           match decodeFfResult body with
           | Some(Wanxiangshu.Kernel.Wanxiangzhen.FfDecision.Merged sha) -> equal "s1" sha
           | _ -> checkBare false)

      ("HttpCodec.decodeFfResult: RebaseNeeded",
       fun () ->
           let body = createObj [ "result" ==> "rebase_needed"; "masterSha" ==> "s2" ]

           match decodeFfResult body with
           | Some(Wanxiangshu.Kernel.Wanxiangzhen.FfDecision.RebaseNeeded sha) -> equal "s2" sha
           | _ -> checkBare false)

      ("HttpCodec.decodeFfResult: StaleCommit",
       fun () ->
           let body = createObj [ "result" ==> "stale_commit" ]
           isSome (decodeFfResult body))

      ("HttpCodec.decodeFfResult: CoordinatorNotReady",
       fun () ->
           let body = createObj [ "result" ==> "coordinator_not_ready"; "reason" ==> "dirty" ]

           match decodeFfResult body with
           | Some(Wanxiangshu.Kernel.Wanxiangzhen.FfDecision.CoordinatorNotReady r) -> equal "dirty" r
           | _ -> checkBare false)

      ("HttpCodec.decodeFfResult: NotSubmittable",
       fun () ->
      ...
      ("HttpCodec.decodeSubmitBody: with commitSha",
       fun () ->
      ...
      ("HttpCodec.decodeSubmitBody: without commitSha",
       fun () ->
           let body = createObj []
           isNone (decodeSubmitBody body))

      ("HttpCodec.decodeRegisterBody: int pid",
       fun () ->
      ...
      ("HttpCodec.decodeRegisterBody: without pid",
       fun () ->
           let body = createObj []
           isNone (decodeRegisterBody body))

      ("HttpCodec.decodeLogBody: with message",
       fun () ->
      ...
      ("HttpCodec.decodeLogBody: without message",
       fun () ->
           let body = createObj []
