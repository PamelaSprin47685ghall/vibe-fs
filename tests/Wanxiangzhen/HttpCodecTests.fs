...
let entries () : (string * (unit -> unit)) list = [
    ...
    ("HttpCodec.encodeStateSnapshot: one task", fun () ->
        let dag = Wanxiangshu.Kernel.Wanxiangzhen.Dag.empty "s1" "req"
    ...
    ("HttpCodec.decodeFfResult: Merged", fun () ->
        ...
        | Some (Wanxiangshu.Kernel.Wanxiangzhen.FfDecision.Merged sha) ->
            equal "s1" sha
        | _ -> checkBare false)
    ...
    ("HttpCodec.decodeFfResult: RebaseNeeded", fun () ->
        ...
        match decodeFfResult body with
        | Some (Wanxiangshu.Kernel.Wanxiangzhen.FfDecision.RebaseNeeded sha) ->
            equal "s2" sha
        | _ -> checkBare false)
    ...
    ("HttpCodec.decodeFfResult: CoordinatorNotReady", fun () ->
        ...
        match decodeFfResult body with
        | Some (Wanxiangshu.Kernel.Wanxiangzhen.FfDecision.CoordinatorNotReady r) ->
            equal "dirty" r
        | _ -> checkBare false)
    ...
    ("HttpCodec.decodeFfResult: NotSubmittable", fun () ->
        ...
        match decodeFfResult body with
        | Some (Wanxiangshu.Kernel.Wanxiangzhen.FfDecision.NotSubmittable s) ->
            equal "done" s
        | _ -> checkBare false)
