module Wanxiangshu.Tests.Wanxiangzhen.SlaveRuntimeTests
...
let entries () : (string * (unit -> unit)) list =
    [ ("SlaveRuntime.slaveToolDefs has submit_to_squad",
       fun () ->
           let defs = slaveToolDefs testConfig
           checkBare (has defs "submit_to_squad"))

      ("SlaveRuntime.slaveToolDefs has query_squad",
       fun () ->
           let defs = slaveToolDefs testConfig
           checkBare (has defs "query_squad"))

      ("SlaveRuntime.slaveToolDefs submit description non-empty",
       fun () ->
           let defs = slaveToolDefs testConfig
      ...
      ("SlaveRuntime.slaveToolDefs query description non-empty",
       fun () ->
           let defs = slaveToolDefs testConfig
      ...
      ("SlaveRuntime.slaveToolDefs query has args schema",
       fun () ->
           let defs = slaveToolDefs testConfig
           let queryDef = get defs "query_squad"
           checkBare (not (isNullish (get queryDef "args"))))

      ("readSlaveConfig without env returns None",
       fun () ->
           let env = getEnv ()
           ...
           // restore
           if orig <> "" then
               setKey env "SQUAD_COORDINATOR_URL" (box orig))

      ("readSlaveConfig with env returns Some config",
       fun () ->
           let env = getEnv ()
...
let entriesAsync () : (string * (unit -> JS.Promise<unit>)) list =
    [ ("SlaveRuntime.submitToSquad returns LocalGitError for invalid worktree",
       fun () ->
           promise {
               let cfg =
                   { testConfig with
