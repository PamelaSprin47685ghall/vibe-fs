...
let entries () : (string * (unit -> unit)) list =
    [ ("e2e_behavior_coverage.registry",
       fun () ->
           let mockLen =
               Wanxiangshu.Tests.Wanxiangzhen.MockE2eTests.entriesAsync () |> List.length

           let openLen =
               Wanxiangshu.Tests.Wanxiangzhen.OpencodePluginE2eTests.entriesAsync ()
               |> List.length

           let extLen =
               Wanxiangshu.Tests.Wanxiangzhen.ExtendedMockE2eTests.entriesAsync ()
               |> List.length
