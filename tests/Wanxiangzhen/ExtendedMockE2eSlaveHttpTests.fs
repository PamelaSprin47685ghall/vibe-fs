module Wanxiangshu.Tests.Wanxiangzhen.ExtendedMockE2eSlaveHttpTests
...
let testSlaveSubmitMerged () : JS.Promise<unit> =
    promise {
        ...
        try
            ...
            let! r =
                fetchJson
                    (server.Url + "/task/squad-a1b2/submit")
                    (createObj
                        [ "method", box "POST"
...
let testSlaveSubmitTaskNotFound () : JS.Promise<unit> =
    promise {
        ...
        try
            let! r =
                fetchJson
                    (server.Url + "/task/does-not-exist/submit")
                    (createObj
                        [ "method", box "POST"
                          "headers", box {| Authorization = box ("Bearer " + rt.Token) |}
                          "body", box (JSON?stringify(createObj [ "commitSha", box "deadbeef" ])) ])
...
let entriesAsync () : (string * (unit -> JS.Promise<unit>)) list =
    [ ("ExtendedMockE2e.slave_submit_merged", testSlaveSubmitMerged)
      ...
      ("ExtendedMockE2e.slave_query_squad_task_detail", testSlaveQuerySquadTaskDetail) ]
