...
let pluginJsResolvesWithParentFallback () =
    ...
    chk
        "e2e.harness.PLUGIN_JS targets Wanxiangzhen plugin"
    ...
    chk
        ...
        (code.Contains("existsSync")
         && (code.Contains("../..") || code.Contains("'..', '..', 'build'")))

/// E2e harness must use .wanxiangzhen-e2e.lock for exclusive run serialization.
let lockFileIsE2eLock () =
    let code = harnessBundle ()
    chk "e2e.harness uses .wanxiangzhen-e2e.lock" (code.Contains("wanxiangzhen-e2e.lock") || code.Contains("E2E_LOCK"))

/// E2e harness dispose must clean up .wanxiangshu.ndjson event log.
let disposeCleansEventLog () =
    let code = harnessBundle ()
    chk "e2e.harness.dispose references .wanxiangshu.ndjson" (code.Contains(".wanxiangshu.ndjson"))

/// E2e harness must expose runCommand or session command path.
let exposesRunCommand () =
    let code = harnessBundle ()
    chk "e2e.harness exposes runCommand or session command" (code.Contains("runCommand") || code.Contains("/session/"))
...
let harnessUsesSpinUntilNoDelayPoll () =
    ...
    chk "e2e.harness no delay( polling function" (not (code.Contains "delay("))

    let countSub (s: string) (sub: string) =
    ...
    chk "e2e.harness setTimeout only in opencode serve timeout" (setTimeouts <= opencodeTimeouts)
...
let supportsInProcessMode () =
    ...
    chk "e2e.harness honors WANXIANGZHEN_E2E_INPROCESS env" (code.Contains "WANXIANGZHEN_E2E_INPROCESS")
