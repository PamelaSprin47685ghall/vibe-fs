module Wanxiangshu.Tests.Wanxiangzhen.DagTests
...
let entries () : (string * (unit -> unit)) list =
    [ ("Dag.empty",
      ...
      ("Dag.topologicalOrder branching",
       fun () ->
           let result =
               topologicalOrder [ ("d", [ "b"; "c" ]); ("c", [ "a" ]); ("b", [ "a" ]); ("a", []) ]

           checkBare (Result.isOk result))

      ("Dag.detectCycle none", (fun () -> isNone (detectCycle [ ("a", []); ("b", [ "a" ]); ("c", [ "b" ]) ])))

      ("Dag.detectCycle self", (fun () -> isSome (detectCycle [ ("a", [ "a" ]) ])))

      ("Dag.detectCycle mutual", (fun () -> isSome (detectCycle [ ("a", [ "b" ]); ("b", [ "a" ]) ])))

      ("formatDag shows sessionId",
       fun () ->
           let d = empty "squad-session-001" "req"
           let s = formatDag d
           checkBare (s.Contains "squad-session-001"))

      ("formatDag shows (no tasks) when empty",
       fun () ->
           let d = empty "s1" ""
           let s = formatDag d
           checkBare (s.Contains "(no tasks)"))

      ("formatDag shows task line with id/status for single task",
       fun () ->
      ...
      ("formatDag shows deps when present",
       fun () ->
           let d = empty "s1" "" |> addTask (mkTask "squad-a1b2" [ "squad-x9y8" ] Pending)
           let s = formatDag d
           checkBare (s.Contains "squad-x9y8"))

      ("formatSquadUpdateOutcome Success",
       fun () ->
           let s = formatSquadUpdateOutcome (Success)
           equal "Tasks created successfully." s)

      ("formatSquadUpdateOutcome DependencyErrors",
       fun () ->
      ...
      ("formatSquadUpdateOutcome InvalidInput",
       fun () ->
