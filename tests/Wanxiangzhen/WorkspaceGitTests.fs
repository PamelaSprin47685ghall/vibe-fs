module Wanxiangshu.Tests.Wanxiangzhen.WorkspaceGitTests
...
let entries () : (string * (unit -> unit)) list =
    [ ("hasCommits on empty repo returns false",
       fun () ->
           let dir = initEmptyRepo ()
           equal false (hasCommits dir)
           cleanup dir)

      ("hasCommits on repo with commit returns true",
       fun () ->
           let dir = initRepo ()
           equal true (hasCommits dir)
           cleanup dir)

      ("revParseHead returns non-empty sha",
       fun () ->
           let dir = initRepo ()
      ...
      ("revParseBranch returns branch name",
       fun () ->
           let dir = initRepo ()
      ...
      ("isDetached on branch returns false",
       fun () ->
           let dir = initRepo ()
           equal false (isDetached dir)
           cleanup dir)

      ("statusIsClean on clean repo returns true",
       fun () ->
           let dir = initRepo ()
           equal true (statusIsClean dir)
           cleanup dir)

      ("statusIsClean on dirty repo returns false",
       fun () ->
           let dir = initRepo ()
      ...
      ("mergeBaseIsAncestor with ancestor returns true",
       fun () ->
           let dir = initRepo ()
      ...
      ("mergeBaseIsAncestor non-ancestor returns false",
       fun () ->
           let dir = initRepo ()
           equal true (mergeBaseIsAncestor dir "main" "main")
           cleanup dir)

      ("showRefExists with existing branch returns true",
       fun () ->
           let dir = initRepo ()
           equal true (showRefExists dir "main")
           cleanup dir)

      ("showRefExists with non-existent branch returns false",
       fun () ->
           let dir = initRepo ()
           equal false (showRefExists dir "nonexistent-branch-12345")
           cleanup dir)

      ("worktreeAdd then worktreeRemoveForce cycle works",
       fun () ->
           let dir = initRepo ()
           ...
           try
               ...
               try
                   rmSync wtPath (createObj [ "recursive", box true; "force", box true ])
               with _ ->
                   ()

           cleanup dir)

      ("branchDeleteForce deletes branch",
       fun () ->
           let dir = initRepo ()
           let suffix = uniqueSuffix ()
