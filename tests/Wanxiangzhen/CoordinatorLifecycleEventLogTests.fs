...
let entries () : (string * (unit -> JS.Promise<unit>)) list = [
    ("handleSquadKill append fail → task remains Running not Cancelled", fun () ->
        promise {
            ...
            let now = rt.Deps.Now ()
            let task = Wanxiangshu.Kernel.Wanxiangzhen.SquadTask.create "squad-a1b2" "Task A" "Desc A" [] now
    ...
    ("handleSquadKill Ok → Pending and Running both Cancelled (foldEvent parity)", fun () ->
        promise {
            ...
            let now = rt.Deps.Now ()
            let pending = Wanxiangshu.Kernel.Wanxiangzhen.SquadTask.create "squad-pend" "P" "d" [] now
            let running = Wanxiangshu.Kernel.Wanxiangzhen.SquadTask.create "squad-run" "R" "d" [] now
