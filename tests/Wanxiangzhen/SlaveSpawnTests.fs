...
let private checkBareCmd (expectedCmd: string) (cmd: string) = equal expectedCmd cmd
...
let private standardEntries () =
    [ ("buildSlaveCommand.alacritty",
       fun () ->
           let cmd, args = buildSlaveCommand "alacritty" wt prompt
           checkBareCmd "alacritty" cmd
           let expected = [ "--hold"; "--working-directory"; wt; "-e"; "opencode"; "--prompt"; prompt ]
           checkBareArgs expected args)

      ("buildSlaveCommand.kitty",
       fun () ->
           let cmd, args = buildSlaveCommand "kitty" wt prompt
           checkBareCmd "kitty" cmd
           checkBareArgs [ "--directory"; wt; "opencode"; "--prompt"; prompt ] args)

      ("buildSlaveCommand.gnome-terminal",
       fun () ->
           let cmd, args = buildSlaveCommand "gnome-terminal" wt prompt
           checkBareCmd "gnome-terminal" cmd
           checkBareArgs [ "--working-directory=" + wt; "--"; "opencode"; "--prompt"; prompt ] args)

      ("buildSlaveCommand.konsole",
       fun () ->
           let cmd, args = buildSlaveCommand "konsole" wt prompt
           checkBareCmd "konsole" cmd
           checkBareArgs [ "--workdir"; wt; "-e"; "opencode"; "--prompt"; prompt ] args) ]

let private extraEntries () =
    [ ("buildSlaveCommand.wezterm",
       fun () ->
           let cmd, args = buildSlaveCommand "wezterm" wt prompt
           checkBareCmd "wezterm" cmd
           checkBareArgs [ "start"; "--cwd"; wt; "--"; "opencode"; "--prompt"; prompt ] args)

      ("buildSlaveCommand.wt",
       fun () ->
           let cmd, args = buildSlaveCommand "wt" wt prompt
           checkBareCmd "wt.exe" cmd
           checkBareArgs [ "-d"; wt; "opencode"; "--prompt"; prompt ] args)

      ("buildSlaveCommand.iterm2",
       fun () ->
      ...
      ("buildSlaveCommand.headless",
       fun () ->
           let cmd, args = buildSlaveCommand "headless" wt prompt
           checkBareCmd "opencode" cmd
           checkBareArgs [ "--prompt"; prompt ] args)

      ("buildSlaveCommand.unknown-terminal-falls-back-to-alacritty",
       fun () ->
           let cmd, args = buildSlaveCommand "foot" wt prompt
           checkBareCmd "foot" cmd
           checkBareArgs [ "--working-directory"; wt; "-e"; "opencode"; "--prompt"; prompt ] args) ]

let entries () : (string * (unit -> unit)) list =
    standardEntries () @ extraEntries ()
