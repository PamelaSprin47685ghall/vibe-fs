module Wanxiangshu.Tests.Wanxiangzhen.TaskTests
...
let entries () : (string * (unit -> unit)) list =
    [ ("Task.status round-trip",
       fun () ->
           ...
           |> List.iter (fun s -> equal (Some s) (s |> statusToString |> statusFromString)))

      ("Task.status fromString unknown", (fun () -> isNone (statusFromString "bogus")))
      ...
      ("Task.withStatus valid transition",
       fun () ->
      ...
      ("Task.tryWithStatus ok on valid",
       fun () ->
           let t = create "t1" "t" "d" [] "now"
      ...
      ("Task.tryWithStatus error on invalid",
       fun () ->
           let t =
      ...
      ("Task.withReconciledStatus overrides state machine",
       fun () ->
           let t =
               { (create "t1" "t" "d" [] "now") with
                   Status = Running }
      ...
      ("Task.taskIdPrefix is squad-", (fun () -> equal "squad-" taskIdPrefix))

      ("Task.create always generates Pending",
       fun () ->
           let t = create "squad-a1b2" "title" "desc" [] "2024-01-01"
           equal Pending t.Status)

      ("Task.withStatus throws on invalid transition",
       fun () ->
           let t = create "t1" "t" "d" [] "now"
