module Wanxiangshu.Tests.WebToolsCodecTests
...
let decodeWebsearchMissingWhatToSummarize () =
    ...
    | Error(InvalidIntent("websearch", "what_to_summarize", "required")) ->
        check "websearch missing what_to_summarize" true
    | _ -> check "websearch missing what_to_summarize" false

let decodeWebsearchOk () =
    let args =
        createObj [ "query", box "q"; "what_to_summarize", box "sum"; "numResults", box 3 ]
