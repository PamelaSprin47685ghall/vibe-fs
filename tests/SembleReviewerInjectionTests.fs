...
let testSembleInjectsForReviewer () =
    promise {
        let mockClientObj = createObj [
            "callTool", box (fun req -> Promise.lift (box {| content = [| box {| text = "{\"results\": []}" |} |] |}))
        ...
        try
            ...
            let encoded = [|
                ...
                box (createObj [
                    "info", box (createObj [ "id", box "m1"; "role", box "assistant"; "sessionID", box sessionID ])
                    "parts", box [||]
                ])
            |]
