...
let loopMessagesShared () =
    ...
    let parsedLoopMsg =
        parseFrontMatterScalars (buildLoopMessage multilineTask [ intro ])

    equal
        "loop message multiline task round-trips through block front-matter"
        (Some multilineTask)
        (Map.tryFind "task" parsedLoopMsg)
...
let yamlFrontMatterRoundTrip () =
    ...
    let mixedFm =
        frontMatter [ yamlField "task" "ship"; yamlSeqField "files" [ box "x.fs" ] ]
    ...
    let specialFm =
        frontMatter [ yamlField "task" "line1\n: colon\n# hash\n\"quoted\"" ]
