...
let opencodeSearchToolsUsesFuzzyToolsCodec () =
    ...
    check "arch: Opencode SearchTools fuzzy decode uses wireDecodeFailure" (search.Contains "wireDecodeFailure toolName")
...
let opencodeSearchToolsUsesWebToolsCodec () =
    ...
    check "arch: Opencode SearchTools websearch must not inline Dyn.str args query" (not (search.Contains "Dyn.str args \"query\""))
    check "arch: Opencode SearchTools webfetch must not inline Dyn.str args url" (not (search.Contains "Dyn.str args \"url\""))
...
let opencodeSearchToolsUsesToolCopy () =
    ...
    check "arch: Opencode SearchTools fuzzy session uses toolRequiresActiveSession" (search.Contains "toolRequiresActiveSession toolName")
    check "arch: Opencode SearchTools fuzzy uses fromOpencode for session/directory"
        ((search.Contains "fromOpencode context") && (search.Contains "runtime.Execution.SessionId") && (search.Contains "runtime.Execution.Directory"))
    check "arch: Opencode SearchTools fuzzy must not Dyn.str context sessionID" (not (search.Contains "Dyn.str context \"sessionID\""))
    check "arch: Opencode SearchTools must not inline requires an active session" (not (search.Contains "requires an active session"))
    check "arch: Opencode SearchTools web uses pluginDirectoryFromCtx" (search.Contains "pluginDirectoryFromCtx")
    check "arch: Opencode SearchTools must not Dyn.str ctx directory" (not (search.Contains "Dyn.str ctx \"directory\""))