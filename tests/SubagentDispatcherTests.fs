...
let fakeAdapter (response: SubagentResponse) : IHostAdapter =
    { new IHostAdapter with
        ...
        member _.ContinueSubagent(_, _) = Promise.lift response
    ...
    promise {
        ...
        let fakeAdapterWithSpy =
            { new IHostAdapter with
                ...
                member _.ContinueSubagent(childID, prompt) =
        ...
        let nextIter =
            ...
            if cleanRes.Contains("iterator:") then
                ...
                let rawIter =
                    if endIdx = -1 then remaining
                    else remaining.Substring(0, endIdx)
                rawIter.Replace("\"", "").Replace("'", "").Trim()
            else
                "failed"

        let args = box {| iterator = nextIter; prompt = "continue standard" |}
        let! continueResult =
            dispatch
                Opencode
                fakeAdapterWithSpy
        ...
        let nextArgs =
            box {| iterator = nextIter2; prompt = "second continue" |}
        let! nextResult =
            dispatch
                Opencode
                fakeAdapterWithSpy
