...
let spec_applyContextBudget_fiveConsecutiveTodos () =
    promise {
        ...
        for i in 1 .. 5 do
            ...
            let currentPlan = { plan with GetContextUsage = getUsageFun }
            
            // 1. Run below threshold
            // b=200000, s=currentTokens, c=0. F trigger: 2*a >= b + s. -> a >= 100000 + s/2
            let boundary = 100000L + currentTokens / 2L
            let belowTokens = boundary - 5000L
            let planBelow = { currentPlan with GetContextUsage = (fun _ -> Promise.lift (Some (int belowTokens))) }
            let messages = [ { info = { id = "msg-below"; sessionID = sessionID; role = User; agent = ""; isError = false; toolName = ""; details = null; time = null }; parts = []; source = Native; raw = null } ]
            ...
            let planTrigger = { currentPlan with GetContextUsage = (fun _ -> Promise.lift (Some (int triggerTokens))) }
            ...
            let nextPlan = { plan with GetContextUsage = (fun _ -> Promise.lift (Some (int currentTokens))) }
