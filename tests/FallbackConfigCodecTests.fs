...
let parseModelEntry_object () =
    let o : obj = createObj [ "id" ==> box "openai/gpt-5.5"
                              "temperature" ==> box 0.7
                              "maxTokens" ==> box 4096 ]
...
let parseModelEntry_withReasoningEffort () =
    let o : obj = createObj [ "id" ==> box "openai/gpt-5.5"; "reasoningEffort" ==> box "high" ]
    let e = parseModelEntry o
...
let extractFallbackConfig_agentsKeyNormalized () =
    ...
    let fm : obj =
        createObj [ "models" ==> box (createObj [ "agents" ==> box (createObj [ "Sisyphus - Ultraworker" ==> box ([ "openai/gpt-5.5" ] : obj list) ]) ]) ]
...
let extractFallbackConfig_defaultChain () =
    let fm : obj =
        createObj [ "models" ==> box (createObj [ "default" ==> box ([ "anthropic/claude-sonnet-4" ] : obj list) ]) ]
    match extractFallbackConfig fm with
    | Some cfg ->
...
let private mkModel (pid: string) (mid: string) (variant: string option) : FallbackModel =
    { ProviderID = pid; ModelID = mid; Variant = variant; Temperature = None; TopP = None; MaxTokens = None; ReasoningEffort = None; Thinking = false }
...
let applyOverride_matchesSpacedAgentName () =
    ...
    let fm : obj =
        createObj [ "models" ==> box (createObj
            [ "agents" ==> box (createObj [ "Sisyphus - Ultraworker" ==> box ([ "oai/gpt5:high" ] : obj list) ]) ]) ]
    let agentObj : obj = createObj [ "Sisyphus - Ultraworker" ==> createObj [ "model" ==> box "old" ] ]
    match extractFallbackConfig fm with
    | Some cfg ->
        let overrides = buildAgentModelOverrides cfg
