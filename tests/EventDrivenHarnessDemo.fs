...
let run () : JS.Promise<unit> =
    promise {
        ...
        bus2.emit "a" |> ignore
        bus2.emit "b" |> ignore
        assertEventSequence "seqMatch" bus2 [
            (fun s -> s = "a")
            (fun s -> s = "b")
        ]
        ...
        let fakeReg =
            createObj [ "eventHook",
                box (System.Func<obj, obj, JS.Promise<unit>>(fun ev helpers ->
        ...
        assertEventSequence "streamEndLoopNudge" nudgeBus [
            (fun ev -> ev.kind = LoopNudge)
        ]
