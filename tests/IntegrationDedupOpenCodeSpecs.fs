...
let opencodeDedupInPlaceSpec () =
    promise {
        ...
        let readPartA =
            createObj [ "type", box "tool"; "tool", box "read"; "state", box readStateA ]

        let readPartB =
            createObj [ "type", box "tool"; "tool", box "read"; "state", box readStateB ]
        ...
        do!
            ...
            $ (box {| sessionID = "dedup-session" |}, dedupInPlace)
            |> unbox<JS.Promise<unit>>

        let msgs = unbox<obj[]> dedupMessagesRef

        check
            "opencode dedup keeps messages array ref"
            (obj.ReferenceEquals(msgs, unbox<obj[]> (get dedupInPlace "messages")))
        ...
        check
            "opencode dedup replaces exact duplicate"
            (str stateB "output" = Wanxiangshu.Kernel.ToolOutputInfo.noChangeEnvelope ())

        let supersetContent = stableContent + String.replicate 8 "new content\n"
        let supersetState = createObj [ "output", box supersetContent ]

        let supersetPart =
            createObj [ "type", box "tool"; "tool", box "read"; "state", box supersetState ]
        ...
        do!
            (get p "experimental.chat.messages.transform")
            $ (box {| sessionID = "dedup-session2" |}, dedupSuperset)
            |> unbox<JS.Promise<unit>>
...
let opencodeDedupIgnoresBacklogFoldedReadsSpec () =
    promise {
        ...
        do!
            (get p "experimental.chat.messages.transform")
            $ (box {| sessionID = sessionID |}, messages)
            |> unbox<JS.Promise<unit>>

        let msgs = unbox<obj[]> messagesRef

        check
            "opencode dedup ignores backlog folded reads: keeps messages ref"
            (obj.ReferenceEquals(msgs, unbox<obj[]> (get messages "messages")))
