...
let formatDomainErrorExecutorExecutableMissing () =
    equal "ExecutorExecutableMissing bash" "executable not found: bash"
        (formatDomainError (ExecutorExecutableMissing "bash"))

let formatDomainErrorParseError () =
    equal "ParseError" "parse error in json: unexpected end of input"
        (formatDomainError (ParseError("json", "unexpected end of input")))

let formatDomainErrorToolNotPermitted () =
    equal "ToolNotPermitted" "tool 'write' not permitted for agent 'reader'"
        (formatDomainError (ToolNotPermitted("reader", "write")))

let formatDomainErrorInvalidIntent () =
    equal "InvalidIntent" "invalid arg for tool 'eval': missing"
        (formatDomainError (InvalidIntent("eval", "arg", "missing")))

let formatDomainErrorUpstreamTimeout () =
    equal "UpstreamTimeout" "upstream timeout after 30s"
        (formatDomainError (UpstreamTimeout 30))

let formatDomainErrorUpstreamRefused () =
    equal "UpstreamRefused" "upstream refused: connection reset"
        (formatDomainError (UpstreamRefused "connection reset"))

let formatDomainErrorSystemPanic () =
    equal "SystemPanic" "system panic: invariant violated"
        (formatDomainError (SystemPanic "invariant violated"))
...
let isAbortFalseForAllOthers () =
    ...
    check "ExecutorExecutableMissing not abort"
        (not (isAbort (ExecutorExecutableMissing "bash")))
    check "ParseError not abort"
        (not (isAbort (ParseError("ctx", "d"))))
    check "ToolNotPermitted not abort"
        (not (isAbort (ToolNotPermitted("a", "t"))))
    check "InvalidIntent not abort"
        (not (isAbort (InvalidIntent("t", "f", "d"))))
    check "UpstreamTimeout not abort"
        (not (isAbort (UpstreamTimeout 5)))
    check "UpstreamRefused not abort"
        (not (isAbort (UpstreamRefused "r")))
    check "SystemPanic not abort"
        (not (isAbort (SystemPanic "m")))
...
let classifyErrorLeafAbortByTag () =
    equal "MessageAborted tag → MessageAborted" MessageAborted
        (classifyErrorLeaf "SomeError" "MessageAborted" "nope")

let classifyErrorLeafSessionBusyByName () =
    equal "SessionBusyError name → SessionBusy" SessionBusy
        (classifyErrorLeaf "SessionBusyError" "SomeTag" "nope")

let classifyErrorLeafSessionBusyByTag () =
    equal "SessionBusy tag → SessionBusy" SessionBusy
        (classifyErrorLeaf "SomeError" "SessionBusy" "nope")

let classifyErrorLeafBackgroundedByName () =
    equal "ForegroundWaitBackgroundedError name → TaskWaitBackgrounded" TaskWaitBackgrounded
        (classifyErrorLeaf "ForegroundWaitBackgroundedError" "SomeTag" "nope")

let classifyErrorLeafBackgroundedByTag () =
    equal "TaskWaitBackgrounded tag → TaskWaitBackgrounded" TaskWaitBackgrounded
...
let classifyErrorLeafFallbackNoAbort () =
    ...
    equal "fallback no abort → UnknownJsError"
        (UnknownJsError msg)
        (classifyErrorLeaf "UnknownError" "SomeTag" msg)
...
let trySessionIdSuccess () =
    equal "trySessionId success" (Some "x")
        (trySessionId "x" |> Option.map sessionIdValue)
...
let tryAgentIdSuccess () =
    equal "tryAgentId success" (Some "agent-1")
        (tryAgentId "agent-1" |> Option.map agentIdValue)
...
let reduceEmptyNoChildren () =
    ...
    let s = reduce empty (ChildRegistered(cid1, { agent = "a"; parentSessionId = None }))
...
let reduceIdempotentUnregister () =
    let ghostId =
        ...
        | Ok id -> id
        | Error msg -> failwith ("childId parser failed: " + msg)
    equal "unregister on empty is idempotent" empty
        (reduce empty (ChildUnregistered ghostId))
