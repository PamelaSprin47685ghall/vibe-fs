...
let getOrLoadCapsFilesIsolatesDistinctSessions () = promise {
    let scope = create ()
...
let getOrLoadCapsFilesIsolatesDistinctDirectories () = promise {
    let scope = create ()
    ...
    check "caps cache same session different directory different list ref" (not (obj.ReferenceEquals(box first, box second)))
}

let getOrLoadCapsFilesReusesAfterDirectoryRoundTrip () = promise {
    let scope = create ()
    ...
    check "caps cache third load same list ref as first after dir detour"
        (obj.ReferenceEquals(box first, box third))
}

let getOrLoadCapsFilesNormalizesDirectoryAlias () = promise {
    let scope = create ()
