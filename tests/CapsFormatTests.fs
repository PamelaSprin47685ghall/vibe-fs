...
let formatReadOutputSingleLine () =
    let result = formatReadOutput "/path/to/file.txt" "hello" 1 None
...
let formatReadOutputMultiLine () =
    let content = "line1\nline2\nline3"
    let result = formatReadOutput "/f" content 1 None
...
let formatReadOutputEmptyContent () =
    let result = formatReadOutput "/f" "" 1 None
    check "empty content still has content tag" (result.Contains "<content>")

    check
        "empty content footer shows 1 line (empty string splits to 1 element)"
        (result.Contains "(End of file - total 1 lines)")

let formatReadOutputClosingTags () =
    let result = formatReadOutput "/f" "x" 1 None
    check "closing content tag" (result.Contains "</content>")

let formatReadOutputWithTotalLines () =
    let result = formatReadOutput "/path/to/file.txt" "hello\nworld" 1 (Some 150)
    check "has custom EOF footer" (result.Contains "(End of file - total 150 lines)")
...
let run () =
    ...
    formatReadOutputWithTotalLines ()
