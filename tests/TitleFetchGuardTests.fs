...
let signature () =
    equal
        "title probe signature pinned to opencode ensureTitle string"
        "Generate a title for this conversation"
        titleRequestSignature

let wrap () =
    let wrapped = wrapForTitle "重构用户服务"
    check "wrap embeds original inside do-not-exec tag" (wrapped.Contains "<do-not-exec>重构用户服务</do-not-exec>")
...
let detect () =
    ...
    let systemMention =
        box
            "{\"system\":\"Generate a title for this conversation is done elsewhere\",\"messages\":[{\"role\":\"user\",\"content\":\"你好\"}]}"

    check "system mention not detected" (not (isTitleRequestBody systemMention))

    let historyMention =
        box
            "{\"messages\":[{\"role\":\"user\",\"content\":\"你好\"},{\"role\":\"user\",\"content\":\"Generate a title for this conversation\"}]}"
    ...
    let realTitleBody =
        box
            "{\"messages\":[{\"role\":\"system\",\"content\":\"You are a title generator.\"},{\"role\":\"user\",\"content\":\"Generate a title for this conversation:\\n\"},{\"role\":\"user\",\"content\":\"真实需求\"}]}"
...
let tryWrapString () =
    ...
    equal
        "string content round-trips text"
        (wrapForTitle "你好")
        (tryWrapStringContent (box "你好") |> Option.defaultValue "")
