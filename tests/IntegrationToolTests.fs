...
let investigatorToolMissingClientSpec () = promise {
    let! workspaceDir = mkdtempAsync "investigator-missing-client-"
    let! p = plugin (box {| directory = workspaceDir |})
    let investigator = get (get p "tool") "investigator"
    let! result = (get investigator "execute") $ (createObj [ "intents", box [| sampleInvestigatorIntent "find investigator registration" |] ], createObj [ "directory", box workspaceDir; "sessionID", box "investigator-test" ]) |> unbox<JS.Promise<string>>
