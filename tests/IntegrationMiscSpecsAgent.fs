...
let disableMimoMemoryAndCheckpointSpec () =
    promise {
        let cfg = createObj []
...
let disableMimoMemoryAndCheckpointPreservesUserAgentSpec () =
    promise {
        let cfg =
            createObj
                [ "agent",
...
let applyAgentConfigForMimoDisablesWorkflowSpec () =
    promise {
        let cfg = createObj []

        let next =
            applyAgentConfigFor Wanxiangshu.Kernel.HostTools.mimocode cfg (createObj [])
...
let pluginConfigHookDisablesMimoMemoryAndCheckpointSpec () =
    promise {
        ...
        let thresholds = get checkpoint "thresholds"
        check "config hook checkpoint.thresholds empty" (isArray thresholds && (unbox<obj[]> thresholds).Length = 0)
