        ...
        let mockCreate (_: string) : JS.Promise<Result<FinderLike, string>> =
            promise {
                let finder =
                    { new FinderLike with
                        member _.fileSearch(_, _) = box null
                        member _.grep(_, opts) = capturedOpts <- Some opts; box {| ok = true; value = box {| items = [||]; totalMatched = 0 |} |}
                        member _.destroy() = ()
                        member _.isDestroyed = false }
                return Ok finder
            }
                ...
                let finder =
                    { new FinderLike with
                        member _.fileSearch(_, _) =
                            ...
                            box {| ok = true; value = box {| items = items; totalMatched = null; totalFiles = 0 |} |}
                        member _.grep(_, _) = box null
                        member _.destroy() = ()
                        member _.isDestroyed = false }
                return Ok finder
