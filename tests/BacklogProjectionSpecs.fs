...
let projectBacklogForMimocodeUsesTask () =
    ...
    check
        "mimocode project: latest task reports present"
        (allJson.Contains("Report 3a") && allJson.Contains("Report 3b"))
...
let projectBacklogDropsFoldedUserMessages () =
    ...
    check
        "backlog fold: uses front matter projection summary"
        (text.StartsWith("---\n")
...
let projectBacklogPrefixStaysStableWhenGrowing () =
    ...
    let projected3 = projectBacklog msgs3 backlog3 FoldStrategy.FoldAfterSecond "test"
...
let buildBacklogTextTest () =
    ...
    check
        "backlog text: stores reports in front matter"
        (text.Contains("user_message")
         && text.Contains("aha_moments")
    ...
    check
        "backlog text: empty body still explains folded work"
        (empty.Contains("Completed work from folded turns. File changes are already on disk."))

let buildBacklogTextWithErrorTest () =
    let text =
        buildBacklogTextWithError [ backlogEntry 1 "Did work" ] [] (Some "bad todo state")

    check
        "backlog text with error: error moves to body"
...
let buildCompactionAnchorPromptOrdersHistoryAnchorsBeforeBacklog () =
    let prompt =
        buildCompactionAnchorPrompt [ backlogEntry 1 "progress so far" ] (fun () ->
            [ "---\ncommand: with-review\ntask: ship feature\n---" ])
    ...
    check
        "history task/command anchor precedes backlog projection"
        (anchorIdx >= 0 && backlogIdx >= 0 && anchorIdx < backlogIdx)
