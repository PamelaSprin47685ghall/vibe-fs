              ...
              box (
                  createObj
                      [ "Object", box (fun (p: obj) -> createObj [ "type", box "object"; "properties", box p ])
                        ...
                        "Enum",
                        box (fun (values: obj array) (o: obj) -> createObj [ "type", box "enum"; "values", box values ])
                        "Array",
                        box (
                            System.Func<obj, obj, obj>(fun (items: obj) (opts: obj) ->
                                let result = createObj [ "type", box "array"; "items", box items ]

                                if not (Dyn.isNullish opts) then
                                    let mi = Dyn.get opts "minItems"

                                    if not (Dyn.isNullish mi) then
                                        result?("minItems") <- mi

                                    let d = Dyn.get opts "description"

                                    if not (Dyn.isNullish d) then
...
let invokeHandler (h: PiHarness) (event: string) (eventObj: obj) (ctx: obj) =
    emitJsExpr (eventHandler h event, eventObj, ctx) "Promise.resolve($0($1, $2))"
...
let todoPhaseEntries () : obj array =
    ...
    let entry =
        createObj [ "customType", box "todo-phases"; "content", box [| phase |] ]
