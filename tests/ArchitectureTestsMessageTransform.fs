...
let opencodeMessageTransformUsesProjectionPolicy () =
    ...
    check
        "arch: Opencode MessageTransform uses shouldExcludeAgentFromProjection"
        (code.Contains "shouldExcludeAgentFromProjection")

    check
        "arch: Opencode MessageTransform no defaultExcludedAgents Set.contains"
...
let muxMessageTransformUsesProjectionPolicy () =
    ...
    check
        "arch: Mux MessageTransform uses shouldExcludeAgentFromProjection"
        (code.Contains "shouldExcludeAgentFromProjection")

    check "arch: Mux MessageTransform no shouldExcludeMuxAgent" (not (code.Contains "shouldExcludeMuxAgent"))

let muxMessageTransformNoLocalCapsBuilder () =
    let code = requireFile "src/Mux/MessageTransform.fs" |> nonCommentCode
    check "arch: Mux MessageTransform no local buildCapsMessages" (not (code.Contains "let private buildCapsMessages"))
    check "arch: Mux MessageTransform delegates CapsCodec" (code.Contains "Mux.CapsCodec")

let muxMessageTransformUsesShellCapsCache () =
    let code = requireFile "src/Mux/MessageTransform.fs" |> nonCommentCode
    check "arch: Mux MessageTransform opens MessageTransformHostHooks" (code.Contains "MessageTransformHostHooks")
    check "arch: Mux MessageTransform uses loadCapsForScope" (code.Contains "loadCapsForScope")

    check
        "arch: Mux MessageTransform must not inline getOrLoadCapsFilesForScope"
        (not (code.Contains "getOrLoadCapsFilesForScope"))

    check "arch: Mux MessageTransform no local CapsFileCache" (not (code.Contains "module private CapsFileCache"))
    check "arch: Mux MessageTransform no direct findCapsFiles" (not (code.Contains "findCapsFiles"))

let muxMessageTransformUsesCommonExtractTexts () =
    let code = requireFile "src/Mux/MessageTransform.fs" |> nonCommentCode
    check "arch: Mux MessageTransform no local extractTexts" (not (code.Contains "let private extractTexts"))

    check
        "arch: Mux MessageTransform uses Shell extractTextsFromEncodedMessages"
...
let messagingPartCodecExists () =
    ...
    check
        "arch: MessagingPartCodec defines toolOutputAndErrorFromHostOutput"
        (code.Contains "let toolOutputAndErrorFromHostOutput")
...
let opencodeMessagingCodecUsesMessagingPartCodec () =
    ...
    check
        "arch: Opencode MessagingCodec no inline operation action from input"
...
let muxMessagingCodecUsesMessagingPartCodec () =
    ...
    check "arch: Mux MessagingCodec no private decodeToolStatus" (not (code.Contains "let private decodeToolStatus"))

let muxMessageTransformUsesMuxWorkspaceCodec () =
    let code = requireFile "src/Mux/MessageTransform.fs" |> nonCommentCode
    check "arch: Mux MessageTransform opens MuxWorkspaceCodec" (code.Contains "MuxWorkspaceCodec")
    check "arch: Mux MessageTransform uses isChildWorkspace from codec" (code.Contains "isChildWorkspace")

    check
        "arch: Mux MessageTransform must not local findWorkspaceEntry"
