...
let sessionStatusIdleDoesNotTriggerNudgeSpec () = promise {
    let messages = [|
        box {| info = box {| role = "assistant"; agent = "manager"; finish = "tool_calls"; time = box {| completed = 1 |} |}
               parts = [| box {| ``type`` = "text"; text = "still working" |} |] |}
    |]
    let promptCalls = ResizeArray<obj>()
    let mkClient () =
...
let sessionErrorWithoutFallbackTriggersNudgeSpec () = promise {
    ...
    let mkEvent typ props =
        box {| event = box {| ``type`` = typ; properties = props |} |}
    do! eventHook $ (mkEvent "session.error" (box {| sessionID = sessionID; error = box {| name = "APIError"; message = "Quota exceeded"; isRetryable = false |} |})) |> unbox<JS.Promise<unit>>
    do! yieldMicrotask ()
