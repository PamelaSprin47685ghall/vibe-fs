...
let sessionPostErrorSpec () = promise {
    let promptCalls = ResizeArray<obj>()
    let mkClient () =
        createObj [ "session", box (createObj [
            "prompt", box (System.Func<obj, JS.Promise<unit>>(fun arg ->
    ...
    let input = box {| sessionID = "post-err-session"; agentID = "reviewer"; outcome = "error"; error = "Quota limit reached" |}
...
let sessionUserQueryPostErrorSpec () = promise {
    let promptCalls = ResizeArray<obj>()
    let mkClient () =
        createObj [ "session", box (createObj [
            "prompt", box (System.Func<obj, JS.Promise<unit>>(fun arg ->
    ...
    let input = box {| sessionID = "query-err-session"; agentID = "reviewer"; step = 1; error = "Rate limit hit" |}
    let output = createObj []
...
let sessionInterruptedEventSpec () = promise {
    let promptCalls = ResizeArray<obj>()
    let mkClient () =
        createObj [ "session", box (createObj [
            "prompt", box (System.Func<obj, JS.Promise<unit>>(fun arg ->
            ...
            "messages", box (System.Func<obj, JS.Promise<obj>>(fun _ ->
                promise {
                    return box {| data = [|
                        box {| info = box {| role = "assistant"; agent = "reviewer"; model = box {| providerID = "openai"; modelID = "gpt-5" |} |}
                               parts = [| box {| ``type`` = "text"; text = "<function=bash>exit 0</function>" |} |] |}
                    |] |}
                }))
            "abort", box (System.Func<obj, JS.Promise<unit>>(fun _ -> Promise.lift ()))
        ]) ]
    ...
    // First, set agent name by session.status busy
    do! eventHook $ (box {| event = box {| ``type`` = "session.status"; properties = box {| info = box {| sessionID = sid |}; status = box {| ``type`` = "busy"; agent = "reviewer" |} |} |} |}) |> unbox<JS.Promise<unit>>
    
    // Then send session.interrupted
    do! eventHook $ (box {| event = box {| ``type`` = "session.interrupted"; properties = box {| info = box {| sessionID = sid |} |} |} |}) |> unbox<JS.Promise<unit>>
    do! yieldMicrotask ()
    
    // User interrupt should NOT continue
