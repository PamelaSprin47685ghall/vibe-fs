# Proposal: Student & Teacher

未裁决候选。不是当前规范。条款前缀 `LEARN-` 在此文内仅作候选编号，正式裁决前不得被实现当作合同；接受或推进后必须原子分发到正式层并将本 proposal 移至 `status/` 跟踪差距，代码实现完全对齐规范并检查 proof 通过后方可删除 `status/` 中的 proposal 文件；拒绝后删除正文并把长期理由写入 `why/`。
