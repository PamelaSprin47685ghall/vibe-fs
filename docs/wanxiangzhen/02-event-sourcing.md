...
> **逻辑 SSOT**（事件种类、`foldEvent`、先盘后内存）以本文件与 `Kernel/Wanxiangzhen` 为准。  
> **物理文件（当前实现）**：与万象术共用 `[workspace]/.wanxiangshu.ndjson` + `.wanxiangshu.ndjson.lock`（`Shell/EventLogCodec.fs`）；`squad_*`/`task_*` 行的 `session` 字段为万象阵 session id。下文若写 `.wanxiangzhen.ndjson`，视为规格别名，**以 `src/` 为准**。与旧「master session 对话历史 + yaml frontmatter 重放」叙事冲突时，以 NDJSON fold 为准。
