...
DAG 当前状态不是进程内存能可靠承诺的——coordinator 会重启、hook 会打断。**SSOT** 是项目根 `.wanxiangzhen.ndjson`（见 [02-event-sourcing.md](./02-event-sourcing.md)）：意图不落盘，已发生事实 append；内存 DAG 是对 NDJSON 按 `session` fold 的投影，启动时重放。宿主 master session 对话 **不作** DAG 真相。
...
DAG 状态变更以增量事件 **append** 至 `.wanxiangzhen.ndjson`（决策 2.2/2.4）。NDJSON 是唯一 durable SSOT（`02-event-sourcing.md`）；`session.prompt` 仅在需要 LLM 行动时（/squad 拆解、nudge、孤儿告警）显式发起，后台状态变更默认静默。
...
4. `commitEvent(SquadCreated)`：**先 append** `squad_created` 事件到 `.wanxiangzhen.ndjson`（事实落盘）；成功后更新内存 DAG。
...
描述: Submit task decomposition for the current squad session.
     ...
     and appends a single tasks_created event to .wanxiangzhen.ndjson.
...
3. 构造 `TasksCreated` → `commitEvent` **先 append** `.wanxiangzhen.ndjson`（成功后才 fold/add 到内存 DAG）；`squad_cancelled` → `handleSquadKill`
4. 触发 Scheduler.tick()
...
| 场景 | 行为 |
|------|------|
| context compaction 删除 squad 事件消息 | DAG SSOT = `.wanxiangzhen.ndjson`，与 session 历史解耦（[02-event-sourcing.md](./02-event-sourcing.md)）。compaction 只影响 LLM 上下文窗口，不影响事件流重放。无需 `.squad/state.json` 备份兜底 |
| NDJSON 事件行损坏（无法解析的非空行）| 读取 `.wanxiangzhen.ndjson` 时在损坏处**截断**（该行及之后丢弃），不跳过坏行继续 fold（`02-event-sourcing.md` §2）|
| 展示 frontmatter 解析失败 | 仅影响 LLM 可见文本/调试诊断；不影响 `.wanxiangzhen.ndjson` durable 重放（frontmatter 是展示层非 SSOT）|
...
- DAG 事实只 append `.wanxiangzhen.ndjson`（`02-event-sourcing.md`）。
...
- [x] 重启重放：`.wanxiangzhen.ndjson` fold + git 二次校正（§5.4 / [02-event-sourcing.md](./02-event-sourcing.md)）
...
| SSOT | 工作区 `.wanxiangshu.ndjson`（../05-event-sourcing.md）| `.wanxiangzhen.ndjson`（`02-event-sourcing.md`）|
| 事件格式 | yaml frontmatter + prose | yaml frontmatter + prose（同构）|
| 重放 | `inferReviewTaskFromTexts` 扫 frontmatter 标量 | `replayFromEventLog` 读 `.wanxiangzhen.ndjson` fold + git 二次校正（§5.4 / [02-event-sourcing.md](./02-event-sourcing.md)）|
...
```
replayFromEventLog(rt):
  events = ReadAllSquadEvents(projectRoot)              # 读 .wanxiangzhen.ndjson（02-event-sourcing.md）
