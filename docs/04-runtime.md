...
| 事件日志 | `EventStore/`（`EventStore.fs`、`EventLogCodec.fs`、`EventLogFile.fs`、`EventLogRuntimeStore.fs`、`EventLogRuntimeSync.fs`、`NudgeEventWriter.fs`、`SessionEventWriter.fs` 等） | NDJSON、锁、缓存、sync 与 projection |
...
| Tree-sitter | `TreeSitterShell.fs` | 可选语法能力 |
| 网络 | `WebSearchApi.fs`、`WebFetch.fs` | 搜索与抓取 |
| 动态类型 | `Dyn.fs`、`DynField.fs` | `obj` 安全访问 |
| 并发 | `Execution/PromiseQueue.fs`、`SerialStateHolder.fs`、`LivelockGuard.fs` | 串行队列与守卫 |
...
| Fallback | `Fallback/Coordinator.fs`、`FallbackCoordination.fs`、`RuntimeStore.fs`、`SessionRuntime.fs`、`Continuation*` | 降级规则、租约、continuation |
| Caps | `Workspace/CapsFileCache.fs`、`CapsFormat.fs` | caps 文件、格式 |
...
| Wanxiangzhen | `Wanxiangzhen/`（15 文件） | 协调器副作用 |
