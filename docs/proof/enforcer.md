...
## 领域与 codec / tip（ENFORCER-011/020..024）

| 证明 | 证据路径 |
|------|----------|
| 工具名 `chronicle`；旧名 `blog` 非法、无 alias | ENFORCER-011；codec / tip-v2 / cycle-protocol（code phase 随 rename 改断言） |
| 必填仅 `entry` + `tip`；**无** `evidence` 字段 | ENFORCER-020；`tests/unit/enforcer/codec.test.mjs`（`ENFORCER_020_*`、`ENFORCER_022_*`） |
...
| exact tip → exact RuleId；trim 后查找；RuleId = FieldName = TipName | `tests/unit/enforcer/codec.test.mjs`（`ENFORCER_021_*`） |
...
| tip 枚举与 tool enum 一致（runtime = package directory scan） | `tests/unit/enforcer/tip-v2-contract.test.mjs`（`ENFORCER_TIP_01`、`ENFORCER_TIP_02_and_16`） |

## Cycle 与多调用归并（ENFORCER-025/042/043/061）

| 证明 | 证据路径 |
|------|----------|
| `entry` 按 `PartOrdinal` 稳定合并（`"\n\n"`）；无独立 evidence 通道 | `tests/unit/enforcer/cycle-nudge.test.mjs`（`ENFORCER_042_text_merges_in_part_ordinal_order`）；`src/Wanxiangshu/Domain/EnforcerCycle.fs`（`mergeCalls`） |
...
| 有效 cycle 要求非空 `entry` | `tests/unit/enforcer/cycle-nudge.test.mjs`（`ENFORCER_043_valid_cycle_requires_nonempty_text`）；`EnforcerCycle.fs` `isValidCycle` |
| 空 `entry` tool gate 拒绝 | `tests/unit/enforcer/enforcer-cycle-protocol.test.mjs`（`ENFORCER_061_*`；旧名 `blog_tool` → `chronicle`） |
...
| 合并 `entry` >512 KiB UTF-8 → fail closed | `tests/unit/enforcer/bounds.test.mjs`（`ENFORCER_042_merged_text_over_512KiB_fails_closed`）；`EnforcerHost.validateCycle` + `SyntheticToml.byteCount` → `Diagnostic.fatal "enforcer-cycle-failed"` |
| **删除**独立 evidence 合并 / `MaxEvidenceBytes` | ENFORCER-020/156；旧 `ENFORCER_042_merged_evidence_*` / evidence dedupe 不得复活 |

## 原子提交 / 恢复（ENFORCER-045/152/153/154）

| 证明 | 证据路径 |
|------|----------|
| 一个 normal `chronicle` cycle 恰好追加一个 frame 并推进 RecordCoverage | `tests/unit/enforcer/enforcer-cycle-protocol.test.mjs`（`ENFORCER_host_completed_*` live request / second window） |
| duplicate ProviderRun 不产生第二条 | `tests/unit/enforcer/enforcer-cycle-protocol.test.mjs`（same-run idempotent）；`tests/unit/enforcer/tip-v2-contract.test.mjs`（`ENFORCER_TIP_09_replay_preserves_tip`） |
| unowned 完成的 chronicle 不发明 `BlogEntryCommitted` | `tests/unit/enforcer/enforcer-cycle-protocol.test.mjs`（without live request = noop） |
...
## Main Full / Identity（ENFORCER-071：交付前沿 ≠ 语义覆盖）

两轴不得压成单一 durable bool：

```text
TipDeliveryFrontier     occurrence 单调；ContextReanchored **不**重置
...
| occurrence ∈ Frontier ∧ Coverage 仍可恢复 → `IdentityOnly`（`tip: <name>`） | `tests/unit/enforcer/tip-guidance-delivery.test.mjs`（Identity 重复用例） |
| Full 且 occurrence ∉ Frontier → append `TipGuidanceDelivered`，推进 Frontier | `tests/unit/enforcer/tip-guidance-delivery.test.mjs`；`TipDeliveryProjection` / `Fold.fs` |
| semantic restoration（Coverage 丢失后再 Full）**不**推进 Frontier | ENFORCER-071；tip-guidance + TipDelivery fold |
| IdentityOnly 不得写成「全文永久可恢复」durable bool | `TipDeliveryProjection.apply`（IdentityOnly）；对应 tip-guidance 测试 |
...
| `ContextReanchored` 清空 TipSemanticCoverage，**不**重置 TipDeliveryFrontier；Coverage 不可恢复时必须再 Full | `tests/unit/enforcer/tip-guidance-delivery.test.mjs`；`TipDeliveryProjection.applyReanchor`；`docs/what/persist.md` ContextReanchored |
...
| tool failure / repair 耗尽进入统一 Fallback，主 cursor 经单一 writer 推进；Persona / language / system 字节不变（FALLBACK-014） | `tests/unit/enforcer/enforcer-cycle-protocol.test.mjs`（`ENFORCER_068_*`、`ENFORCER_065_*`） |
| abort 清理残留只注入 repair，不推进主 cursor / 不消耗预算 | `tests/unit/enforcer/enforcer-cycle-protocol.test.mjs`（`LOOP_006_interrupted_*`）；`tests/e2e/cases/fallback-aabb-trace.test.mjs`（`FallbackCursorAdvanced` 恰好 4 次） |
...
| 旧工具名 `blog` / 字段 `evidence` / 回执 `result="OK"` 不得复活 | ENFORCER-011/020/156；§18 inventory |
| TipDeliveryFrontier 与 TipSemanticCoverage 不得压成单一 durable bool | ENFORCER-071 |
...
Blogger 路径 canary：工具必须调用 `chronicle`、busy skip 不推进 coverage、成功提交推进 RecordCoverage。  
Main tip 路径 canary：首次 Full main.md、重复 Identity、compaction reanchor 后再 Full（Frontier ⊥ Coverage）。  
Observation 路径 canary：提交 tip 入 RecentTips；squash tip co-move；Companion 配对 unit。  
与 CTX/HOST 交叉：compaction 后 reanchor 不把 Host 摘要当 BlogFrame；TipDeliveryFrontier 不因 reanchor 重置；TipSemanticCoverage 与 Blog/Prefix 同原子清空。
