...
| 门 | 要求 |
|---|---|
| `scripts/checks/dsl-ownership.mjs --threshold=0` | 声明式 `DSL-MUTABLE`、跨文件 `dup-cases`、large-DU（≥10 case 无 `/// DSL-class:`）fail、`/// DSL-class: ControlState` 硬判 `program-counter`、Process 不再整目录豁免 `bool-loop`。**组合状态结构检测未实施**（`BloggerRuntimeCell.State+Drain` 双轴正交状态未被 gate 捕获） |
...
| 门禁自身 | 故意破坏门禁应变红 | `tests/unit/verify/dsl-ownership.test.mjs` 含声明式 mutable / 跨文件 dup-cases / 门禁注册门用例（已落地）。**组合状态结构检测未实施**，故组合状态 fixture 尚未落地；改名/声明类 canary 若已有可保留 |
