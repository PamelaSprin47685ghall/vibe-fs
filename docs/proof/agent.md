...
| warm-start normalization | CRLF/LF、trim、blank、stable case-sensitive dedupe；8-keyword cap | AGENT-032 |
| bounded parallel search | 独立 queries 同一 wave；每 query top_k=4；失败独立 fail-open；merge 恢复 keyword ordinal/local rank | AGENT-032 |
| prompt safety/bounds | ≤24 unique hints；≤64KiB；只删完整 entry；hostile strings 仍解析为 data；zero-keyword byte-exact Charge | AGENT-032 |
| role gate | snippets 只到 Coder/Inspector/DevOps；其它 role 非空 keywords fail；commission 无 keywords | AGENT-032 |
| NEEDHELP collaboration | fast→deep 同 Life；deep→真实 consultation child；finite/single-flight/no-recursion/cancel-no-resurrection | AGENT-031 |
