...
| C Language Parity | 每个 provider semantic resource：EN + zh-CN 皆存在；叶对现行；placeholder structural parity + semantic-anchor parity 后续（PROMPT-019） | 资源成对存在；缺语言 fail；invariant 标识不译 | HOST-026、PROMPT-017/019；`proof/host.md`、`proof/prompt.md` |
