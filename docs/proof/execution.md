...
| SyncDelegate WorkRecord：仅四段标题；答案在 Closing report；无 `answer`/`inspector_id`/`coder_id`/`completion_text` magic | EXEC-031、GLORY-025、COMPANION-003 → tests/unit/session/sync-delegate-runtime.test.mjs（`EXEC_031_bounded_work_record_answers_in_closing_report_not_raw_message`：Recent work + Closing report、答案在 Closing report、无 Opening 回传）；四段标题渲染 → tests/unit/context/lifecycle-work-record.test.mjs |
