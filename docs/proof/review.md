...
| 证明 | 期望 | 条款 |
|------|------|------|
| RequestKind 分型 | Process 一次 verdict terminal；Finality 才走 challenge/双 PERFECT；禁止 pendingChallenge 猜测混用 | REVIEW-013 |
| Accepted 与 Rk 1:1 | 每个 `TodoWriteAccepted` 恰好一个 process obligation / TodoReviewId | REVIEW-013、TODO-006 |
| VerdictKnown 不放行下一 Tk | 仅 verdict 时 T(k+1)/suicide 仍阻塞 | REVIEW-014、TODO-006 |
...
| includeOpening=false | Opening 不经 LWR 复制；OpeningRaw 旁路 | REVIEW-016、TODO-001、GLORY-004 |
...
| 替换后上下文 | 新 session 含 OpeningRaw + 有界 Manager LWR + 既往 WorkRecordRef | REVIEW-019、TODO-008 |
...
| Manager 不可见 | 不能 fork/list/join/inspect；surface 无 barrier/witness/2N/roster | REVIEW-015、TODO-013、GLORY-002 |
