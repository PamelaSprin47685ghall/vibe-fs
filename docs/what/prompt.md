...
`NeedHelpEscalation` 唯一允许的 binding 变化是 fast owner 请求对应 deep peer；`NeedHelpAdvice` 必须绑定触发 consultation 的原 deep agent。Cursor Pair Hint 只作为 HOST-013 provider projection 附着到真实 terminal tool result 的终态文本，不创建 user/system/assistant synthetic role，因此不参与 PromptIngress/AuthorityRoot/OpeningMaterial。
