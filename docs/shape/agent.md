...
## Inquiry 边界（见 AGENT-025 / AGENT-030）

Inquiry 正式工具面 = SyncDelegate `inspect` + Host MCP `sphinx_*`。所有权边界：
...
1. Host-final permission 与 ToolRegistry 两层都必须拒绝 Inquiry 的 `read` / `glob` / `grep` /
   ...
   `sphinx_*` 仅经 `ToolPermission.Sphinx` 放行（AGENT-030）；仍禁止 filesystem 直读。
