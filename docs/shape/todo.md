...
行为：`what/todo.md`（TODO-001..014）。程序：`how/todo.md`。证明：`proof/todo.md`。
...
| Opening Blogger/Y floor | `WorkRecordStart`（自 `LifeOpened`/Opening cursor 纯推导） | 禁止把 Opening protection 绑回 `WorkActivated`；禁止平行 stage floor（TODO-001、TODO-012） |
| Process-review 工作证据 | 既有 canonical LWR + `RecordCoverage` | 禁止第二 renderer / `TodoProcessReviewEvidenceProjection`；禁止 session-head LWR（TODO-008、TODO-012） |
| Lag-1 prefix 可替换性 | 既有 `PrefixCoverage` + `PrefixRebaseCommitted`（`EvidenceKind=TodoCheckpoint`）→ `ActivePrefixEpoch` | 禁止 todo-only 第二套 ActivePrefixEpoch；禁止用 RecordCoverage/RawGap 做 prefix replacement（TODO-008、TODO-009、TODO-012） |
| Desired lag-1 cutoff 事实源 | Accepted checkpoint 链纯推导 | 禁止 `NeedRebase`/`RebaseRequested` Stage；Accepted **不**直接 commit epoch（TODO-006、TODO-009、TODO-012） |
| Dedicated process reviewer 逻辑身份 | `DedicatedTodoReviewerEnlisted`（+ proven-loss `Replaced`） | 每 Life 一个 logical id；物理 session retention ≠ Finality graduate（TODO-008、TODO-010） |
...
| Manager-visible process surface | enriched `todowrite` tool result + safety-sealed ProcessReviewLWR；`MagicTodoManagerGuideline` | 允许 checkpoint review / PERFECT / REVISE / report；禁止泄漏 reviewer session / barrier / witness / 2N（TODO-013；GLORY-030/SURFACE-005 窄例外指向本条） |
...
```text
Domain/        TodoCheckpoint · TodoIdentity · TodoMerge · TodoReview · TodoPrompt
...
MagicTodoProjection              → canonical todo（TODO-007）
...
| PlanningTail / ManagerWorkActivation / 新 WorkActivated 业务资格 | 删除；Opening floor 改 `WorkRecordStart`（TODO-001） |
...
- **GLORY**：删除 Activation 业务路径（TODO-001）；`suicide` 前序接 TODO-010 drain；Dedicated Finality enlist/graduate 见 TODO-010；Manager 面泄漏边界收窄为 TODO-013（GLORY-030/SURFACE-005 例外 = process report 词面，非 reviewer 编排）。
...
- **HOST**：membrane canary、tool 身份、pair-programming 通用 marker 仍 HOST-*；MagicTodoManagerGuideline 为 Manager-only fragment（TODO-013），不得并入全局 pair 正文 owner。admission/V2 门禁语义 TODO-004。
- **PROMPT / PROJ**：只交叉引用 TODO-013 表面与 TODO-007 投影边界，不复制 settlement/cadence 语义。
