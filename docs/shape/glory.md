...
本文件定义 GLORY 机制的模块所有权、数据流边界与禁止的泄漏路径。条款正文见 `docs/what/glory.md`（GLORY-001..076、SURFACE-001..006）。Magic Todo 所有权见 `docs/shape/todo.md` 与 `TODO-*`。OpeningMaterial / WorkRecord 四段标题的表示合同见 COMPANION-003/014/015；本文件只定 Manager Life 侧所有权与 floor。
...
| LWR journal 物化 | `Application/Finality/LifecycleWorkRecordProjection.fs` | XTrace/`chronicle`/terminal → opaque work record；四标题 `Opening / Chronicle / Recent work / Closing report`（GLORY-025；COMPANION-003）；与 `XTraceCapture` 分居（TODO-008） |
...
→ last_words 成为 Closing report → 完成 handle
...
4. Reviewer / process-review 工作记录只由既有 canonical LWR 物化产生，request-range bounded，`includeOpening=false`；四标题为 `Opening / Chronicle / Recent work / Closing report`（GLORY-004/025/049；TODO-008；COMPANION-003）。旧标题 `Opening task / Work log / Uncompressed tail / Final output` 已删除。
