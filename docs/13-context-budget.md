...
**核心修复**：phase reset 时以折叠后的 stable projection bytes 通过当前 calibration 重新估算 `stableTokens`，不向 Host 伪装成任意 encoded 的 token 测量。这样 phase baseline 对应折叠后的候选，而不是上一轮未折叠 outbound 的 usage。

phase 边界 = todowrite 成功 = backlog 发生变化。旧 usage 只能用于 calibration，不能直接成为新 phase 的 $P$；没有可用 calibration 时退回 bootstrap bytes/2，并由硬安全水位保护高风险冷启动。

| 条件 | 新 $P$ | 新 $c$ |
| :--- | :--- | :--- |
| `State.IsNone && backlog.IsEmpty` | 0 | 0 |
| `State.IsNone && backlog ≠ []` | `stableTokens` | 重算 |
| `State.IsSome && backlog ≠ LastBacklog` | stable projection 估算值 | 重算 |
...
3. `classifyPressure`（传入 `foldAfterFirst=false`，$N=3$）为 `RequireTodoWriteEmergency` → 最终 encoded 追加 synthetic **User** 文本：
...
OpenCode 生产路径由 `resolveMaxInputTokens` → `tryGetMaxInputTokensAsyncDetailed` 调用 `session.get` 取当前会话的 `model.metadata`，直接提取 `limit.input`（优先）或 `limit.context`，无 `provider.list` 回退。

1. `session.get({ sessionID, directory })` 取 `res.data.model`。
2. `extractLimitFromModelDetailed` 读 `model.limit.input`（优先）或 `model.limit.context`，两者均为可选字段。
3. 若无模型元数据或 limit 字段缺失，沿 `modelFamilyLimit` → host config → 保守默认 8192 依次降级。
4. `tryObserveLatestUsage` 读 `session.messages` 取最近一条 assistant 的 `tokens.input + tokens.cache.read`，用于校准估算字节。
