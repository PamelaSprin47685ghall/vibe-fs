...
### 2026-09-04 — upstream `ed48cfc91` PR前同步

- PR前从`upstream/master`取得`bd30349f2`与`ed48cfc91`两笔Sphinx correctness提交。双方业务源码无交集；upstream的epistemic-reasoning规范、测试、Sphinx runtime、generic durability/integrator与EventStore集成原样保留，本分支未修改其产品语义或断言。
- 真实路径交集只有`published-contracts.json`、`release-closure-nodes.json`、`semantic-owners.json`与总`Wanxiangshu.fsproj`。前三者保留双方新增记录，总工程文件保留双方compile item及各自顺序。唯一人工冲突是`semantic-owners.json.total`：共同基线696，本分支净增1，upstream净增2，独立并集为699；正文路径集合由Git自动形成并集。
- 合并验证：JSON parse、`semantic-owners.mjs`（742 files / 49 owners）、Fantomas check与完整`node scripts/check.mjs`全绿；最终owner census为742 production files、792 contracts、193 localities、1,916 ProjectReferences/DAG、824 WHAT/4,144 tests。
- 合并后按最终staged tree重跑production fresh scan：1,916 ProjectReferences、4,472 actual source edges、973,806 capability facts、513 JavaScript traversals、189,708 Unknown、0 extraction diagnostic，digest=`sha256:775257a52468bce84c1a400b8d15256b412b857305744f5bc0504a4d37699fd8`。missing closure为15条；相对合并前既有12条，新增3条均由upstream `bd30349f2`新增的`Sphinx/GenericDurability.fs`产生，分别指向`eventstore-model-contract`、`foundation-identity`与`sphinx-event-vocabulary-contract`。compiler-resolved report-only analyzer逐边确认该归因。
- 本节点不通过给整个`sphinx-runtime`添加三个owner-wide ProjectReference来伪造闭合；这会扩大旧ACL并掩盖M6.3c需要原子切换的真实边。三条新增边与既有12条一起保留为v2 report-only deferred closure，不影响当前v1 release authority；不启动Fatal、Process/PTY、Delegation G6/G7或B7-C1。
- 最终release sink的唯一unit失败来自既有`EMR-007`源码断言把Fantomas单行排版当成语义。production的`isMessageUpdated`仍保留null guard、exact `message.updated`判定与`tryDecodePhysicalExecutionEnd`调用链，公开Host Surface行为proof继续全绿。测试只把赋值两侧空白改为format-invariant `\s*`；不删除条件、不放宽事件类型、不修改production。
- 第二次release sink通过4,096 unit、全部integration与275-case harness后，真实Long Stroke在Host health阶段暴露`signal-subscribe-failed: InvalidInput`。OpenCode 1.18.18由`createOpencodeClient`返回`OpencodeClient` class instance；B3把嵌套SDK client误当plain JSON record，因prototype拒绝真实carrier。HOST-BOUNDARY-028现区分顶层/events数据与opaque client capability：前者仍须plain record，后者只接受ordinary object（拒绝primitive、array、boxed scalar、Date与throwing carrier）；class client无legacy events走`LocalEventHook`，有合法events才取得opaque disposer。永久RED在旧实现稳定18/19失败；修复后host-signal-adapter focused compile、aggregate build、subscription 19/19、完整fast gate与真实Long Stroke 60 steps/7.0s全绿。
