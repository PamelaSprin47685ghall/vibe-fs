...
**换 Peer ≠ 换人。** Fallback 只改写 `EffectiveAgent` / 模型绑定。SessionPersona、system prompt 字节、`ProviderLanguage` 在 Life 内保持不变。身份是「这个人是谁」；执行绑定是「下一刻由哪颗心承接」。把失败切边写成新人格或新办公室，会拆前缀缓存、撕自我连续性，并把机器恢复策略泄漏进经验层。Strength 换 fast peer 同理：只换执行者，不换 Persona / prompt / language。

## 备选与被拒

**Offset 表示：byte/int 计数 vs DU，以及反序列化失败处理。** 拒 byte：0–255 皆可构造，`side` 对非法字节无分支，非法态在类型层就能溜进来。拒绝在 `ofByte` 遇到非法字节时抛出 `invalidOp`：持久化数据损坏是可预见失败，抛异常会把数据校验变成程序事故。选择 `Result<FallbackOffset, FallbackOffsetDecodeError>`，让 Journal load/fold 明确拒绝损坏 envelope；该错误不冒充 Append `CommitUnknown`。
...
**切边：重绑 Persona/prompt/language vs 只换 EffectiveAgent。** 拒随 fallback 重写 Persona、system prompt 字节或 session 语言：办公室与自我模型是 Life 常量；重写即伪造新身份并打碎 KV-cache 前缀。拒把 cursor / SideA / SideB / Offset / ConsecutiveFailureCount 暴露给 provider：那是机器预算代数，不是参与者该解码的世界态。选仅改 `EffectiveAgent`（及对应模型绑定）；Authority 与 Persona 不动。
