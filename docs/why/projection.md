...
**Intent 冲突解决：隐式注册顺序选边 vs canonical order + 显式合并/冲突。** 拒绝依靠注册顺序：同一 intent 集会因装配顺序产出不同前缀。选择稳定总序；真实可合并的组合定义规则，其余返回 `ProjectionConflict`。不强迫有序追加操作满足虚假的交换律。
...
**DSL 是否承载生命周期。** 拒：启动 Agent、等待 provider、写 Journal 属 Coordinator 职责；投影层若长第二套编排即翻回 Program AST 反模式（PROJ-007）。Service 层只映射输入快照→确定性投影。
