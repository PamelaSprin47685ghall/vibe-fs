...
**推论**：稳定规则进 `src/Kernel/`；三套宿主（OpenCode/Mimocode、Mux、OMP）只做薄适配。判断法则：去掉 Node 与宿主 `obj` 后仍成立的逻辑 → Kernel。
...
- **事实只追加**：修正靠补偿事件，非覆盖旧行。
- **内存 = fold**：`ReviewStore`、nudge 表 = 对该 `session` 事件行的纯 fold（可加进程内缓存，磁盘为先）。
...
宿主侧：compaction 压缩 LLM 窗口即可。万象术侧：每次 **校验通过** → append 带有 `openTodosJson` 的 `assistant_completed` 事件，携带**全量** `todos`（及 `select_methodology`）。
...
- 单进程关键路径用 **`PromiseQueue.SerialQueue`**（`src/Runtime/Subsession/PromiseQueue.fs`）
