...
**推论**：IO 与动态对象编解码集中在 `src/Runtime/` 与各 `*Codec`；Kernel 写成可单测的纯函数（无 `Dyn`、不直接绑 Node API）。复杂流程在 Runtime 或宿主层编排，不在 Kernel 堆副作用。
...
万象术侧：每次 `todowrite` / `task` **校验通过** → append `work_backlog_committed`，携带**全量** `todos`（及 `select_methodology`）。**不再**依赖 compaction 后注入 anchor prompt，再从历史 tool 消息 fold backlog。
...
- 单进程关键路径用 **Promise 串行队列**（如 `Runtime.PromiseQueue.SerialQueue`）
...
**交付语义**：**At-least-once delivery + idempotent host operation + correlation / reconciliation**。为每个外部操作保留稳定身份（`EffectId`、`RunId`、`TurnId`、`IdempotencyKey`）。

**资源身份纪律**：可跨重启恢复的资源（如 Turn Deadline）必须使用**稳定 Key** 标识复用资格、持久化**绝对到期时间**（`DeadlineAt`）而非相对时长，避免重启后超时无限延后。
