...
- Expanded both `fast-inspector` and `deep-inspector` from `{executor}` to the exact model-visible tool set `{read, glob, grep, executor}`. Direct file inspection no longer requires shell commands; write/edit, delegation, PTY, verdict, build, test, and runtime execution boundaries remain denied, with role prompts, Host permission contracts, tests, and normative docs updated together.
...
- Fixed idle reconcile swallowing `TurnNeedsContinuation` / `TurnInProgress` after classifier fixes. Public work agents such as `deep-devops` that stop after reasoning-only or formal text containing XML markup (including broken tags) now dispatch one `InteractionRepair` continuation without completing the child run or advancing durable fallback; trailing idle signals that arrive mid-reconcile are preserved for a follow-up pass.
...
- Fixed the DevOps `coder` callback the same way and replaced its unsupported Fable `TrySet*` completion calls with a guarded `SetResult`/`SetException` bridge, so successful one-shot Coder runs return normal tool output instead of red errors.
...
- Strengthened the reviewer and manager-loop canaries to reproduce two `PERFECT` calls in one physical user turn, keep REVISE/confirmation edges strictly ordered instead of reusable, and assert exact tool outcomes, distinct provider/tool identities, one shared authority root, and durable confirmed idle.
