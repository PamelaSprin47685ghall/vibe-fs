...
- Long Stroke 剧本与 oracle 对齐现行产品合同（release 层 e2e 仍红，剩余为冷边界契约迁移）：`long-stroke.toml` 把 Manager 会话的 `todowrite` 声明改为现行 `assume`（update/todos）并同步工具列表，`manager-reopened-loop` 的 user 片段改为现行 `runtime/manager-assess` 文案，补上后继迭代的 `[[epoch]] manager-loop` 冷边界声明与 1/2 步阶梯；`long-stroke-oracles.mjs` 的 `bindManagerLoopSequence` 为后继族补齐 1/2 步映射，`assertHumanRootManagerLoop` 与 `014.test.mjs` 的 preflow 改为「初始迭代走 authority 族、后继迭代走 assess-resource 族」各一次交付。实测 Long Stroke 因此从 preflow 一路推进到 orchestrator shell → Manager 1~4 迭代 → conflict 修复 fork/join，随后停在 `RetirementCommitted ≥ 5`：conflict 修复迭代的 fork 之后没有 wake 让 Manager 收割并退休。另：`cold-boundary.js` 的 `manager-loop` 准入规则与三个 harness gate case（含派生序列 case）仍按旧合同（「下一迭代丢弃退休流量」）编码，与 WHAT[relay-context-projection-001]「保留完整历史」相矛盾；该契约迁移未完成，故 `verify:release` 的 e2e 仍红（在 HEAD 上同样红）。
