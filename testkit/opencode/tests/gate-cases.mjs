/**
 * gate-cases.mjs — Behavior test cases for testkit/opencode quality gates.
 */
...
async function runProcessHostDisposeContract() {
  const hostCode = fs.readFileSync(new URL('../process-host.js', import.meta.url), 'utf8');
  const checksCode = fs.readFileSync(new URL('../process-host-checks.js', import.meta.url), 'utf8');
...
async function runScenarioStrictDefault() {
  const stateCode = fs.readFileSync(new URL('../strict-mock-state.js', import.meta.url), 'utf8');
