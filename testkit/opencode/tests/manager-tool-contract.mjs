...
test('manager permission denies global executor tool and executes mailbox path', async () => {
  ...
  try {
    ...
    assert.equal(typeof config.agent.coder.prompt, 'string');
    assert.match(config.agent.coder.prompt, /Coder edits/);
    assert.match(config.agent.coder.prompt, /Surgical Precision/);
    assert.match(config.agent.coder.prompt, /inspector\(prompts\)/);
    assert.equal(typeof config.agent.devops.prompt, 'string');
    assert.match(config.agent.devops.prompt, /DevOps executes/);
    assert.match(config.agent.devops.prompt, /fork-pty/);
    assert.match(config.agent.devops.prompt, /No Direct File Modification/);
    assert.equal(typeof config.agent.inspector.prompt, 'string');
    assert.match(config.agent.inspector.prompt, /Investigative Inspector/);
    assert.match(config.agent.inspector.prompt, /executor[`'"\s]*ONLY/i);
    assert.match(config.agent.inspector.prompt, /Strict Read-Only Invariant/);
    assert.equal(typeof config.agent.reviewer.prompt, 'string');
    assert.match(config.agent.reviewer.prompt, /Uncompromising Reviewer/);
    assert.match(config.agent.reviewer.prompt, /Double-PERFECT/);
    assert.match(config.agent.reviewer.prompt, /verdict\("PERFECT"\)/);
    assert.match(config.agent.reviewer.prompt, /verdict\("REVISE"\)/);
    assert.equal(typeof config.agent.browser.prompt, 'string');
    assert.match(config.agent.browser.prompt, /Information Navigator/);
    assert.match(config.agent.browser.prompt, /`network`/);
    assert.match(config.agent.browser.prompt, /do \*\*not\*\* have/i);
    assert.equal(config.agent.browser.permission.read, 'allow');
    assert.equal(config.agent.browser.permission.glob, 'allow');
    assert.equal(config.agent.browser.permission.grep, 'allow');
    assert.equal(config.agent.browser.permission.network, 'allow');
    assert.equal(config.agent.browser.permission.executor, 'deny');
    assert.equal(config.agent.browser.permission.write, 'deny');
    assert.equal(typeof config.agent.meditator.prompt, 'string');
    assert.match(config.agent.meditator.prompt, /Architectural Strategist/);
    assert.match(config.agent.meditator.prompt, /Transparent Trade-Off Evaluation/);
    assert.match(config.agent.meditator.prompt, /inspector\(prompts\)/);
    assert.equal(config.agent.meditator.permission.read, 'allow');
    assert.equal(config.agent.meditator.permission.glob, 'allow');
    assert.equal(config.agent.meditator.permission.grep, 'allow');
    assert.equal(config.agent.meditator.permission.inspector, 'allow');
    assert.equal(config.agent.meditator.permission.write, 'deny');
    assert.equal(config.agent.meditator.permission.fork, 'deny');
    assert.equal(config.agent.meditator.permission.network, 'deny');
    assert.equal(typeof config.agent.orchestrator.prompt, 'string');
    assert.match(config.agent.orchestrator.prompt, /Multi-Worktree Director/);
    assert.match(config.agent.orchestrator.prompt, /fork-manager/);
    assert.match(config.agent.orchestrator.prompt, /Host-owned Dual PERFECT/);
    ...
    assert.equal(config.agent.orchestrator.permission.read, 'deny');
    assert.equal(config.agent.orchestrator.permission.executor, 'deny');
    assert.equal(typeof config.agent.executor.prompt, 'string');
    assert.match(config.agent.executor.prompt, /Command Output Summarizer/);
    assert.match(config.agent.executor.prompt, /AgentRole\.Executor/);
    assert.match(config.agent.executor.prompt, /Tool Capability: \[\] \(NONE\)/);
    assert.equal(config.agent.executor.permission['*'], 'deny');
    assert.equal(config.agent.executor.permission.executor, 'deny');
    assert.equal(config.agent.executor.permission.fork, 'deny');
    assert.equal(config.agent.executor.permission.read, 'deny');
    assert.equal(typeof config.agent.blogger.prompt, 'string');
    assert.match(config.agent.blogger.prompt, /Work Log Blogger/);
    assert.match(config.agent.blogger.prompt, /AgentRole\.Blogger/);
    assert.match(config.agent.blogger.prompt, /Tool Capability: \[\] \(NONE\)/);
    assert.match(config.agent.blogger.prompt, /Self-Compression/);
    assert.equal(config.agent.blogger.permission['*'], 'deny');
    assert.equal(config.agent.blogger.permission.executor, 'deny');
    assert.equal(config.agent.blogger.permission.read, 'deny');
