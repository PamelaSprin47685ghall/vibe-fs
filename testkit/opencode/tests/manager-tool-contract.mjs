...
test('manager permission denies global executor tool and executes mailbox path', async () => {
  ...
  try {
    ...
    assert.match(config.agent['fast-manager'].prompt, /Do not ask a Coder to obtain any of those results through Inspector/);
    assert.match(config.agent['fast-manager'].prompt, /Once its edits are complete, the Coder is done/);
    ...
    assert.match(config.agent['fast-coder'].prompt, /Use Inspector only for a genuinely necessary static investigation/);
    assert.match(config.agent['fast-coder'].prompt, /inspector\(agent: "fast-inspector", prompts\)/);
    assert.match(config.agent['fast-coder'].prompt, /Editing Is the Completion Boundary/);
    assert.match(config.agent['fast-coder'].prompt, /Do not check whether the code compiles or works/);
    assert.match(config.agent['fast-coder'].prompt, /DO NOT use `inspector` to bypass that boundary/);
    assert.match(config.agent['fast-coder'].prompt, /Never ask Inspector to run, reproduce, check, or diagnose compilation, builds, typechecks, linters, tests, programs, or runtime behavior/);
    assert.match(config.agent['fast-coder'].prompt, /After the final required file edit, stop working/);
    ...
    assert.match(config.agent['fast-inspector'].prompt, /DO NOT compile, build, typecheck, lint, format, test, benchmark, run repository programs/);
