...
const RUNNER = 'scripts/run-canary-staggered.mjs';

/** The harness and its scripts: where a canary registry and its consumers live. */
const SCOPE = [
...
export const singleSourceCases = [
...

  {
    ...
    fn: () => {
      ...
      // (`STATUS/design-script-forest.md:630`).
      const holds = (source) => {
...

  {
    ...
    fn: () => {
      ...
      assertTrue(/from\s*['"][^'"]*canary-manifest/.test(runner), `${RUNNER} must import the manifest`);

      const pasted = runner
        .split('\n')
        .map((text, at) => ({ line: at + 1, text }))
        .filter(({ text }) => new RegExp(`['"\`][^'"\`]*${CANARY_SUFFIX}`).test(text));

      assertEq(
        pasted.length,
        0,
        `${RUNNER} names canary files directly: ${pasted.map(({ line, text }) => `${line} ${text.trim()}`).join(' | ')}`,
      );
    },
  },
...

  {
    ...
    fn: () => {
      // The silent-omission hazard one level up from a stale array. `foo_canary.mjs` and
      // `foo-canary.test.mjs` both look like canaries to a reader and register with nothing, so
      // the suite would be smaller and still green — invisible in exactly the direction that
      // matters. Reported rather than thrown at import: a narrower suite is a failing gate case,
      // while taking every canary run down over an unluckily named helper is not proportionate.
      const claims = ['real-canary.mjs', 'canary-driver.mjs', 'underscore_canary.mjs', 'dotted-canary.test.mjs'];

      withTree(Object.fromEntries(claims.map((name) => [name, 'export const x = 1;\n'])), (dir) => {
        assertEq(
          JSON.stringify(nonConformingCanaryNames(dir)),
          JSON.stringify(['dotted-canary.test.mjs', 'underscore_canary.mjs']),
          'a stem ending in canary must be spelled with the suffix; a file merely ABOUT canaries claims nothing',
        );
        assertEq(JSON.stringify(readCanaryTests(dir).map((file) => basename(file))), JSON.stringify(['real-canary.mjs']));
      });
