...
export const readableRepositoryTexts = (root = defaultRoot) => {
  ...
  for (const relPath of paths) {
    if (relPath.endsWith('LoopDetectorConstants.fs')) continue
    try {
...
export const generateLoopDetectorConstantsSource = (calibration) => {
  return `namespace Wanxiangshu.Execution.Session

// Generated automatically before build by scripts/generate-loop-constants.mjs.
// Calibration corpus: repository tracked strict UTF-8 files.
// Fitted distribution: Beta(alpha, beta) over bounded support [1.0, 1/(1-lambda)].
[<RequireQualifiedAccess>]
module LoopDetectorConstants =

    let VocabularySize: int = ${calibration.vocabularySize}
    let HalfLife: float = ${calibration.halfLife.toFixed(1)}
    let Lambda: float = ${calibration.lambda.toFixed(16)}
    let MaxSupport: float = ${calibration.maxSupport.toFixed(14)}
    let DistributionMean: float = ${calibration.mean.toFixed(14)}
    let DistributionVariance: float = ${calibration.variance.toFixed(14)}
    let DistributionStd: float = ${calibration.std.toFixed(14)}
    let BetaAlpha: float = ${calibration.betaAlpha.toFixed(14)}
    let BetaBeta: float = ${calibration.betaBeta.toFixed(14)}
    let ConfidenceLevel: float = ${calibration.confidenceLevel.toFixed(2)}
    let ConfidenceQuantile: float = ${calibration.confidenceQuantile.toFixed(2)}
    let BetaQuantileU: float = ${calibration.betaQuantileU.toFixed(16)}
    let NormalWeightedDistinctCount: float = ${calibration.normal.toFixed(14)}
    let TheoreticalLoopWeightedDistinctCount: float = ${calibration.theoreticalLoop.toFixed(1)}
    let LoopWeightedDistinctThreshold: float = ${calibration.threshold.toFixed(14)}
`
}

export const generateLoopConstantsFile = (root = defaultRoot) => {
  const calibration = calibrateFromRepository(root)
  const source = generateLoopDetectorConstantsSource(calibration)
