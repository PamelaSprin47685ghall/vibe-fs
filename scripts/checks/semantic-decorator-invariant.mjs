...
export const PHYSICAL_LISTENER_CONTRACTS = Object.freeze([
  ...
  Object.freeze({
    ...
    binding: 'exitListeners',
    sites: ['let exitListeners = ResizeArray<PtyExitEvent -> unit>()', 'exitListeners.Add listener'],
    owner: 'process-execution',
    what: 'PROC-003',
    traceRelation: 'R_physical_pty_exit_listener=each-first-close-once-per-registered-listener',
    ...
    failurePolicy: 'listener failures are isolated so remaining physical listeners receive the close event',
    cancelPolicy: 'PTY abort is delivered as a typed close event without changing listener registration',
...
export const TRACE_CHANGE_CONTRACTS = Object.freeze([
  ...
  Object.freeze({
    ...
    ], owner: 'managed-session-lifecycle', what: 'MANAGED-SESSION-013',
