...
const scanRetiredIdentityTokens = (text, file, failures) => {
  for (const token of ['SessionPersona', 'PersonaBinding', 'SessionSurface']) {
    scanPattern(
      text,
      file,
...
export const run = (root = process.cwd()) => {
  ...
  console.log('participant-identity-boundary: OK — opaque owner, IdentitySeed authority boundary, and zero parallel identity owners')
  return 0
}
