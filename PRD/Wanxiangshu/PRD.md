...
DAG 状态变更以增量事件 **append** 至 `.wanxiangzhen.ndjson`（决策 2.2/2.4）。NDJSON 是唯一 durable SSOT（`PRD/EventSourcing.md`）；`session.prompt` 仅在需要 LLM 行动时（/squad 拆解、nudge、孤儿告警）显式发起，后台状态变更默认静默。
...
| NDJSON 事件行损坏（无法解析的非空行）| 读取 `.wanxiangzhen.ndjson` 时在损坏处**截断**（该行及之后丢弃），不跳过坏行继续 fold（`PRD/EventSourcing.md` §2）|
...
| SSOT | 工作区 `.wanxiangshu.ndjson`（`PRD/02-event-sourcing.md`）| `.wanxiangzhen.ndjson`（`PRD/EventSourcing.md`）|
...
事件以 NDJSON 行落盘（`v/session/kind/at/payload`，见 `PRD/EventSourcing.md` §3）。`encodeEvent` 生成的 frontmatter + prose 文本仅用于 `output.parts` 展示（如 `/squad` 拆解指令），不是事实载体。
