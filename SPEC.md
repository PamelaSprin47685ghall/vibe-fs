...
* ~~session 的全部权威状态只能存在于一个 aggregate~~ ✅（`FallbackRuntimeStore` 唯一权威 aggregate；`ProjectionCache` 为事件投影缓存）
* ~~删除剩余 `*Transitions.fs` 薄包装~~ ✅（`Runtime/Fallback/*Transitions.fs` 已删；调用方直用 `SessionRuntime*Pure`；`Kernel/SessionControl/LeaseTransitions.fs` 是事件折叠非薄包装）
* ~~continuation / nudge / compaction 统一 episode 身份与迟到事件规则~~ ✅
  * `OwnerEpisodeState` + `EpisodeStage`（`Kernel/SessionControl/State.fs`、`EventEnvelope.fs`）
  * `foldEpisode` / `Projection.foldOwnerAndLease` 折叠 owner+lease+ordinal+stage
  * `isLateEvent`（`FoldApply.fs`）按 continuation/nudge/compaction/human-turn ordinal 丢弃迟到事件；`Fold.applyEvent` 迟到直接保旧态
...
* `Runtime/EventStore/EventLogRuntime.fs` — ✅ 已删除（原唯一纯 re-export 门面）；调用方直 open `EventLogRuntimeStore` / `Nudge` / `Recovery` / `Sync` 等子模块
* 以下含独有业务逻辑，**不是**纯门面，保留：
  * `Hosts/OpenCode/HookExecute.fs` — patch 归一化 + UI label + Gateway
  * `Runtime/Fallback/FallbackConfigCodec.fs` — parse/extract/load 独有
  * `Runtime/Execution/BacklogProjectionBuild.fs` — compaction 编排
  * `Hosts/OpenCode/SubsessionHostAdapter.fs` — dispatcher 编排
  * `Runtime/Messaging/OpencodeSessionEventCodec.fs` — todos/nudge/last-assistant
  * `Runtime/Search/FuzzySearch.fs` — fuzzyContinue 分发

迁移纪律仍有效：生产调用方 → 测试 → 再删兼容层。测试服从架构。
...
* [x] 无未引用生产源文件。 ✅ 已验证（src/ 下 585 个 `.fs`，均由 fsproj 收录）
...
**最优先剩余：Fallback Coordinator 副作用闭环（F-01 残差）、F-05 旧 ActionExecutor 主路径裁决、全宿主真实 E2E。** 文件名整理不优先。
...
## 三、进行第二轮自然边界拆分 ✅ 阶段目标已达成

**已完成：**
- `checkProductionLineLimits`：>250 = 0；200–250 阶段上限曾为 50，现收紧目标 ≤25 已达成
- >200 行业务编排启发式检测（`let rec`/`do!`/`let!`）
- 250–299 长文件拆分：`EventStore`、`NudgeEffect`、`CommandProcessor`、`OpenCodeModelResolution`、`DecisionObserve`、`LeaseValidation`、`FuzzySearchGrep`、`Fold`、`LeaseIdentity*` 等
- **磁盘现状（2026-07-20）**：`src` 下 **0** 文件 >250 行；**22** 文件在 200–250 行（≤25 目标 ✅；旧文 44/46 为过期计数）
- `Helpers` 清理：`ModelResolutionHelpers.fs` → `ModelResolutionCatalog.fs`
- ~~Fallback 状态收口~~ ✅ aggregate + 薄包装 + episode/late-event 均已落地

**可选长尾：** 22→更少属品味优化，非门禁阻断。

## 六、Fallback 不能停留在“文件拆完了” ✅ 状态模型已收口
...
* ~~删除剩余 `*Transitions.fs` 薄包装~~ ✅
* ~~continuation / nudge / compaction 统一 episode + 迟到事件规则~~ ✅（`OwnerEpisodeState` / `foldEpisode` / `isLateEvent`）

验收时不再接受“这个字段比较特殊，所以单独保存”。

**运行路径残差（非状态模型）：** Fallback `Coordinator.enqueueRelevantEvent` 仍在 mailbox 决定后 `Promise.start` 跑 continuation intent（F-01）；靠 `RetryDispatchGovernor.stillValid` + `handleDispatchComplete` lease 校验缓解，尚未做到 effect 结果强制回投同一 actor。
...
| 功能                 | 当前判断（对照磁盘） | 核心残差 / 已修要点 |
| ------------------ | ----------- | ------------- |
| Nudge              | N-01…N-06 代码已收口；整体仍待真实 E2E 压实 | 假成功/nonce/Dispatched 时序/异常伪装/stale lease/owner 静默 → 已有生产修复+单测 |
| Fallback Continue  | 状态模型收口；F-01 残差 + F-05 主路径未删净 | generation-only 匹配已禁；Governor 真队列；Coordinator 仍 `Promise.start` intent |
| Sub-session        | S-01…S-08 代码已收口 | Actor+receipt+stale abort 防杀新 turn；仍待全链路 E2E |
| Reviewer 子会话       | R-01…R-03 运行时已补；未迁 SubsessionService | `promptWithAbort` 会 `physicalAbort`；父 abort→child；`performCleanup` finally |
| OMP prompt 路径      | 部分契约入码；表级 E2E 未齐 | `CancelPendingDispatch` 生效；有 id 不伪造 ordered marker；双 abort API 仍在 |
| Mux nudge/continue | receipt 校验已落地 | boolean true→`AcceptanceUnknown`；无 abort→`AbortUnavailable` 显式降级 |
| 万象阵通知 prompt       | 假成功已修；幂等/去重未齐 | API 缺失 raise+error log；防重复发送仍待 |
...
`NudgeTriggerOps.resolveOwner`：非 test 且 runtime `NoOwner` 时**不**猜测 owner；调用 `appendNudgeOwnerUnknownOrFail` 写 `nudge_owner_unknown` 事件并打诊断日志，返回 `NoOwner`（nudge 不触发）。测试：`NudgeOwnerDiagnosticTests`。符合“不许猜 owner 发 prompt，但不可静默”。

---

## 4.2 Fallback Continue

### F-01：dispatch side effect 在状态队列之外 ⚠️ 部分完成

**已有：**

* Subsession 路径：`SessionDispatcher` 串行 mailbox；Dispatch/Cancel/Complete/Fail/OnSessionClosed 同队列；abort 结果回投后再按 generation/turn 提交（`SessionDispatcherLifecycle.fs`）
* Fallback 缓解：`runWithRetryGovernor` + `stillValid`（`verifyLease`/`ensureActiveAndOwner`）；`handleDispatchComplete` 校验 pending lease 后才写 Dispatched

**残差（诚实）：** `Fallback.Coordinator.enqueueRelevantEvent` 仍在 queue 内决定后 `Promise.start` 跑 `ContinuationExecution.run`（intent 出队裸跑）。未满足“effect 结果强制回投同一 actor 再改态”的完整模型。P0 竞态窗口缩小但未关闭。

### F-02：generation 相同被当作缺失 continuation ID 时的匹配依据 ✅ 已完成

`LeaseValidationRules.classifyContinuationMatch` 仅：

1. `parentID` = lease.HumanTurnID → `MatchedByParentId`
2. host run ID = lease.HumanTurnID → `MatchedByHostRunId`
3. continuation marker = lease.ContinuationID → `MatchedByMarker`
4. 否则 `UnmatchedStatusHint`（**无** generation equality 回退）

空 continuationId 的 `isContIdMatch=true` 仅作 status-hint 语义，不把事件匹配为当前 continuation。单测：`FallbackLeaseValidationRulesTests`。PR1 亦已勾“禁止 generation-only matching”。
...
`RetryDispatchGovernor` 现为真双层 `SerialQueue`：

* 外层 `SessionKey{Workspace,SessionID}` 串行 → 每物理 session 至多一活跃 dispatch
* 内层 `ProviderKey{Workspace,Provider,Model,Variant}` 限速窗
* `stillValid` 在入队后/睡眠后再次检查；`CancelledBeforeDispatch` 路径明确
* 单测：`RetryDispatchGovernorTests`（并发串行、限速、取消）

进程级单例仍在（`ContinuationDispatchOps.retryGovernor`）；语义已拆 session 串行 vs provider 限速，不再是“注释撒谎的空壳”。
...
### S-08：abort 是物理 session 级，而不是 turn 级 ✅ 已完成（主路径）

`SessionDispatcher.CancelByTurn`：

1. mailbox 内标记 CancelRequested；HostAccepted/RunObserved 时捕获 `Generation` 并启动 `physicalAbort`
2. abort Promise 完成后**回投**同一 mailbox
3. 仅当 `ReferenceEquals(active,target)` ∧ logicalTurn 匹配 ∧ `Generation` 未变 ∧ 未 closed 才 `AbortSent`；否则 `AlreadyTerminal Superseded/SessionClosed`

OpenCode `SubsessionHostAdapter.Abort` 经 dispatcher；OMP Abort 校验 `ActiveTurnId` + actor `GetCurrentTurn` + `AbortSent` 幂等。单测：`DispatchRegistryTests`（stale abort、close-during-abort、abort-fail）。

残差：Fallback `cancelAfterDispatch` 仍直接 `executor.AbortRun`（无 dispatcher generation 回投）；属 Fallback 路径，非 Subsession S-08 主缺口。
...
`SubagentSpawnTransport.promptWithAbort`：`Promise.race` 若 abort 胜出 → 调用 `physicalAbort session childID` 再 reject `AbortError`；信号已 aborted 时同样先 physical abort。不再只停本地等待。

### R-02：外部 abort 没有完整传入 child abort ✅ 已完成

`ReviewerLoopOps.runLoopWithCleanup`：父 `abortSignal` → `childAbort.abort()`（已 aborted 立即传；否则 `addEventListener`）；`setAbortSuppressor` 绑同一 child controller。

### R-03：完成和异常路径缺乏统一 finally 清理 ✅ 已完成（运行时）

`runLoopWithCleanup`：`try` loop / `with` 记错 → **总是** `performCleanup`（去 listener、unlockReview、CleanupSession、`abortAndUnregister`）→ 再 rethrow。

**架构残差：** Reviewer **未**迁入 `SubsessionService` owner/policy；仍自建 loop。子代理路径已用 `SubsessionService`（`SubagentRunExec`/`SubagentIoRun`）。统一生命周期属后续，不否定 R-01…R-03 运行时修复。
...
* `CancelPendingDispatch` → `HostReceiptWaiterRegistry.cancelByTurn` + 按 turn Abort（非空实现）
* `OmpSubsessionHostHelper.checkMessages`：有 user/continuation message id → `UserMessageObserved`；**仅**无 id 时才 `OrderedTurnMarkerObserved`（注释明确禁止无 id 时伪造）
* 契约注释写在 `OmpSubsessionHost`：resolve = 入有序流 barrier；当前 turn error/idle 不得早于 resolve（**宿主假设仍在，缺表级 E2E 证伪/证实**）
* Abort：turn 所有权检查 + `AbortSent` 幂等；仍可能并行调 `session.abort` 与 `pi.session.sessionAbort`

**仍开放（不可发明完成）：**

| 契约 | 状态 |
| --- | --- |
| prompt 返回时刻语义 | 注释假设，无独立契约探针矩阵闭环 |
| idle 早于 resolve | 未用真实 OMP 版本证明 |
| model omit vs `""` | 未收敛为唯一省略语义 |
| summarizer waitForIdle 读到旧 idle | 未在本轮对照关闭 |
| 双 API abort 重复 | 代码仍 `Promise.all` 两路 |

未验证前，业务状态机不得把注释当定理。

---

## 4.6 Mux ✅ receipt / 降级已落地

`Hosts/Mux/Fallback/Executor.fs`：

* `validateMuxReceipt`：校验 sessionId + dispatchId/nonce/continuationId + 非空 messageId/receiptId
* boolean `true` → reject `AcceptanceUnknown`（**禁止**把裸 true 当发送成功）
* boolean `false` / nullish / 字段不匹配 → Failed
* `AbortRun` → reject `AbortUnavailable`（显式能力降级，非 silent no-op）

集成单测：`IntegrationMuxFallbackSpecs`（nudge/continue boolean true → AcceptanceUnknown）。

**残差：** compaction 零宽 correlation、全事件进 session actor、restart reconciliation 全矩阵 — 未宣称全部 E2E 绿。
...
* `SessionIo.promptSession`：API/prompt 函数缺失 → `wanxiangzhen_session_api_missing` raise + `console.error`（假成功已灭）
* 调用失败路径同样 error log 后抛出
* `CoordinatorReplay` orphan warning `Promise.catch` 防崩溃

**待完成（诚实）：**

* 幂等控制（同 intent 不重复物理 prompt）
* 防重复发送 / restart acceptance-unknown 策略
* 与统一 DispatchId 生命周期对齐的完整 trace
...
1. ~~同一 physical session 最多一个插件主动 prompt in-flight~~ ✅ `SessionDispatcher.reserveRecord`
2. ~~无法识别来源时不自动取消、不自动归属~~ ✅ F-02 Unmatched；无 identity idle 不瞎结算（设计+部分实现）
3. ~~禁止 generation-only matching~~ ✅ F-02
4. ~~宿主 API 缺失必须显式失败~~ ✅ N-01 / 万象阵 SessionIo
5. 为 nudge、fallback、sub-session、review 分别提供 kill switch — ⚠️ 未在本轮对照逐项核实
6. ~~ambiguous receipt 进入 AcceptanceUnknown，禁止立即重发~~ ✅ Mux/Subsession 路径
7. ~~所有 session abort 前重新校验 active dispatch~~ ✅ S-08 SessionDispatcher
8. ~~增加结构化 trace~~ ✅ CommandProcessor TelemetryStateTransition（PR1）

Phase 0 目标：不再误杀和重复发送 — 代码面大部达成；全宿主 E2E 压实仍开放。

## Phase 1：确定唯一架构 ⚠️ 未完成

* Subsession：CommandProcessor + SessionDispatcher + receipt = 事实主路径
* Fallback：仍 `IActionExecutor` + Coordinator（F-05 开放）
* 统一 PromptDispatch 类型/接口已存在于 `Kernel/Dispatch` + `Runtime/Dispatch`，**未**覆盖全部 Fallback continue
* Review 未复用 SubsessionService

验收条件仍有效：从 OpenCode Hook 追踪一次 continue，只能找到一条生产执行路径 — **当前未满足**。

## Phase 2：建立 Session Actor ✅ 骨架已有 / 覆盖未满

**已有 mailbox：** `SubsessionActor`、`SessionDispatcher`（每物理 session）、Fallback `Coordinator` per-session `SerialQueue`。
...
* ~~session 单 synthetic prompt 限制~~ ✅ 已完成（`SessionDispatcher` / `reserveRecord`：同物理 session 已有 active dispatch 则拒绝再入）；
* ~~人类消息去重前置~~ ✅ 已完成；
...
## PR 2：统一 PromptDispatch 与 Session Actor ⚠️ 基础设施已有，Fallback 主路径未并入

**已落地（代码+单测）：**

* ~~标准 dispatch identity~~ ✅ `Kernel/Dispatch/Identity.fs`
* ~~标准 receipt / HostReceiptWaiter~~ ✅ `Runtime/Dispatch/HostReceiptWaiter*`
* ~~每 session mailbox~~ ✅ `SessionDispatcher` + `SubsessionActor` + Fallback per-session `SerialQueue`
* ~~effect result 回投（Subsession abort/complete）~~ ✅ `SessionDispatcherLifecycle`
* ~~exactly-once waiter~~ ✅ S-01…S-03
* ~~session close 清理~~ ✅ S-07

**未完成：**

* Fallback continuation **未**改走 SessionDispatcher；仍 Coordinator+`Promise.start` intent（F-01 残差）
* “新 continuation 主路径接入”对 Fallback 而言未完成（F-05）

## PR 3：Nudge 与 Fallback 迁移 ⚠️ 大部代码完成，E2E/删旧路径未齐

**已落地：** N-01…N-06；F-02/F-03/F-04；episode fold；OpenCode parentID/marker correlation（`LeaseValidationRules` + metadata codec）

**未完成：** 删除旧 ActionExecutor 唯一化；真实乱序 E2E 全绿；F-01 完整闭环

## PR 4：Sub-session 与 Reviewer ⚠️ 运行时修复完成，架构统一未完成

**已落地：** S-01…S-08；R-01…R-03 运行时；receipt cancel；stale evidence 删除；scoped abort（dispatcher）

**未完成：** Reviewer 迁入 SubsessionService；child session 全链路真实 E2E 签字

## PR 5：OMP、Mux、万象阵 ⚠️ 部分

* Mux receipt + AbortUnavailable 降级 ✅
* OMP CancelPending + 有 id 不伪造 marker ⚠️ 契约表未闭环
* 万象阵假成功 ✅；幂等/去重 ❌
