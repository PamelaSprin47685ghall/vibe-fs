...
* ~~continuation、nudge、compaction 必须共享统一的 episode 身份与迟到事件规则。~~ ✅ 已完成（`EpisodeIdentity` 统一 epoch/id 规则；settle/late idle/lease transition 共用 `disposeLateEvent` / `canSettleCompaction` / `continuationLeaseIsCurrent` / `nudgeLeaseIsCurrent`）。
...
**待完成：**
- 200–250 行长尾收敛到 ≤25
- ~~Fallback 状态收口~~ ✅ aggregate、薄包装、episode 身份统一均已完成
...
* `NudgeTriggerOps.resolveOwner` 生产路径禁止猜测；`NoOwner` 时写 `nudge_owner_unknown` 事件（`feature/session/event/reason`）并 `console.warn` 结构化诊断；
* 测试模式仍可从宿主 messages 末条 role 推断 Human，不污染生产；
* 回归：`tests/NudgeOwnerDiagnosticTests.fs`。
...
* `SessionDispatcher.CancelByTurn`：phase1 只标记 `CancelRequested`；phase2 **持有 mailbox** 二次校验（dispatchId/generation/ownership/AbortSent/IsClosed）后才调用 `physicalAbort`，避免宿主 abort 与新 turn Reserve 交错；
* stale 路径写 `DispatchStaleAbort` 事件，不调用宿主；
* OpenCode host `AlreadyTerminal` → `console.warn` stale_abort；OMP host 按 active turn / actor turn / AbortSent 门控并结构化 warn；
* 回归：`tests/DispatchRegistryTests.fs`（holds mailbox / stale log-only / duplicate abort）+ `tests/SubsessionPhysicalIsolationTests.fs`（OMP scoped abort）。
...
- `SessionIo.promptSession`：API 缺失 raise；参数/调用失败写结构化 `wanxiangzhen_prompt_session_failed` 日志
- `OrphanNotify`：稳定 `idempotencyKey`（orphan task id 排序）+ `warning_sent` / `prompt_failed` 事件载荷
- `CoordinatorReplay.warnOrphans`：best-effort（失败不中断 replay）；发送前预留 key 防双发；仅成功保留 + 落盘 `wanxiangzhen_warning_sent`；失败释放 key 可重试；空 MasterSessionId 不永久压制
- 重放 `recoverSentKeys` 与内存预留 union，并发 replay 不丢 in-flight 预留
