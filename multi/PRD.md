# PRD: Squad — Multi-Agent Opencode Coordinator

> 本文档为保姆级实现规格。所有 opencode/vibe-fs API 引用均经源码核实（见 DEV_TALK 轮次 5），出处以 `packages/...` 或 `src/...` 标注。凡与原始设想冲突处，已按真实 API + 最佳实践修正，并在正文标注「修正」。

## 0. 一句话定义

Squad 是一个 opencode 插件。正常启动的 opencode 进程（coordinator）加载插件后自起一个本地 HTTP server，把用户需求经自身 LLM 拆解为 DAG 任务图；对每个就绪任务从最新 master 创建 git worktree、用终端模拟器启动一个独立 slave opencode 进程在隔离 worktree 中开发；slave 完成后经工具调用通知 coordinator，coordinator 以 ff-only 协议把分支线性合并回 master，永不产生 merge commit、永不解决冲突（冲突由 slave rebase 自行消化）。
...
Squad 与 vibe-fs 是两个独立插件，可同时安装：

- vibe-fs 提供 `/loop`（With-Review Mode）：slave 在 worktree 中完成开发后走 review。
- Squad 不自实现 review，依赖 vibe-fs 的 `/loop`。未安装 vibe-fs 时 slave 降级为不 review 直接提交。
- 两插件互不 import、互不感知，仅在 prompt 层 / slash-command 层协同（核实 5.3）。Squad 把 slave 初始 prompt 包成 vibe-fs 的 `task:` frontmatter 格式触发 With-Review，或经 `session.command` 程序化触发 `/loop`。
...
- 所有 master git 操作经单一 `SerialQueue` 串行化（核实：复用 vibe-fs `Shell.PromiseQueue.SerialQueue`）。
...
| 角色 | 定义 |
|------|------|
| **Coordinator** | 用户直接交互的 opencode 进程。加载 Squad 插件后在随机端口启动本地 HTTP server。它是运行时协调者与唯一写入入口，不是 SSOT 承载体；SSOT 是 master session 对话历史（事件流），git refs 负责合并事实校正。负责 DAG 拆解、调度、ff 合并、worktree/slave 生命周期。 |
...
| **masterBranch** | 集成分支。**不硬编码 `master`**（修正，核实 5.9）：Squad 启动时 `git rev-parse --abbrev-ref HEAD` 取主仓库当前分支为默认值，可被 AGENTS.md frontmatter `squad.masterBranch` 覆盖。只有 coordinator 能 ff 推进它。 |
...
```
事件类型（共 7 类，详见附录 D.1）:
  squad_created     // Squad Session 创建（含原始需求）
...
```
┌──────────────────────────────────────────────────────┐
│  Coordinator (用户的 opencode 进程 + Squad 插件)        │
│                                                        │
│  ┌──────────┐ ┌────────────┐ ┌──────────────────────┐ │
│  │ Squad    │ │ Scheduler  │ │ Git Executor         │ │
...
│ opencode tui  │  │ opencode tui  │
│ Squad 插件    │  │ Squad 插件    │  (同一插件，env 区分 slave 模式)
...
| 能力 | 真实 API | 出处 |
|------|----------|------|
| 插件入口 | `(input: PluginInput, options?) => Promise<Hooks>`；vibe-fs 形式 `pluginFor host ctx : Promise<obj>` 返回 hook 字典 | `packages/plugin/src/index.ts`；`src/Opencode/Plugin.fs` |
...
| 拿 opencode server URL | `input.serverUrl: URL`（仅供参考，Squad 自起独立 server） | 同上:62 |
| Bun shell | `input.$: BunShell`（可跑 git，Squad 选 child_process） | `packages/plugin/src/shell.ts` |
...
2. config = readSquadConfig(input.worktree/AGENTS.md)  (§7)
...
复用 vibe-fs nudge 机制（决策 1.2）：LLM 回复了但没调 `squad_update` → 注入提示：

```
你还没提交拆解结果。请调用 squad_update 工具提交任务拆解（events 数组）。
```

nudge 上限 3 次（同 vibe-fs `maxNudges`），超限放弃并向用户说明。
...
- 无参：杀所有 Squad Session 的所有 running/submitted slave 进程。
...
有 vibe-fs（包成 /loop 触发，决策 1.9 + 核实 5.3）——两种实现路径，MVP 取路径 i：

- 路径 i（frontmatter 包装，零耦合）：把任务描述包成 vibe-fs 识别的 `task:` frontmatter（vibe-fs 的 `todowrite`/`task` 工具语义），让 slave LLM 自然进入 With-Review。**首选**，因为不依赖 vibe-fs 内部命令名。
- 路径 ii（程序化 slash command）：slave 插件用 `client.session.command({ sessionID, command:"loop", arguments })` 触发 `/loop`（核实 5.3）。需 vibe-fs 确实注册了 `loop` 命令，耦合稍强，作为备选。

有 vibe-fs 的初始 prompt（路径 i）：

```
vibe-fs /loop（With-Review Mode）可用。请按 review 流程开发：
...
### 6.3 vibe-fs 检测（核实 5.3）

slave 插件加载时检测 vibe-fs 是否安装。**修正**：opencode 插件间无标准互查 API，用以下任一启发式（实现时择稳）：
- 检测 opencode 配置中 vibe-fs 插件包是否列出（读 opencode config 的 plugin 列表）。
- 或检测 vibe-fs 暴露的全局/文件锚点。
...
描述: |
  ...
  - If vibe-fs /loop is available, you have passed review (PASS).
...
4. review PASS（或无 vibe-fs，跳过 review）
...
**修正**（核实 5.7）：rebase 目标统一为本地 `{masterBranch}`（worktree 共享主仓库 .git，coordinator ff 后本地 masterBranch ref 即最新）。仅当项目本身有 origin remote 且需要时才 fetch；纯本地 Squad 流程不需要。
...
核实 5.5：vibe-fs 的轻量 frontmatter 解析器只认标量字段（用于 review 重放锚点），**不解析数组/嵌套**。但 `sharedDirs` 是数组、`squad:` 是嵌套对象。故 Squad 配置解析走 Shell 层的完整 `yaml` 包（vibe-fs 依赖已含 yaml），不复用那个轻量标量解析器。事件 frontmatter（§5.4/附录 D）才用得上轻量重放扫描，但其数组字段（如 `depends_on`）也需注意编码（见附录 D）。

### 8.5 配置缺失与降级

- 无 `AGENTS.md` 或无 `squad:` 段 → 全用默认值（`maxConcurrent=3`、terminal 平台探测、masterBranch=当前分支、sharedDirs=[]）。
- 单字段缺失 → 该字段取默认（合并语义，同 vibe-fs DEFAULT 合并）。
...
[5] slave LLM 开发：读 schema、写 migration、改 model
    → vibe-fs 可用（SQUAD_VIBEFS=1）→ /loop review → reviewer PASS（§6.6）
...
| 事件 frontmatter 损坏（首行坏）| 重放时该条跳过 + git 二次校正补真相（§5.3）。截断语义同 vibe-fs（坏处止）|
...
- 插件 F# + Fable → JS，与 vibe-fs 一致；可复用 vibe-fs Kernel/Shell 既有模块（§3.x 分层）。
...
- Git 操作经 `$`（BunShell）或 `child_process.execSync`，与 vibe-fs Shell.Executor 一致同步串行。
- 异步原语：全程 `Promise`（Fable 编译 JS 环境开除 Async/Task，核实宝典）。SerialQueue 复用 vibe-fs `Shell.PromiseQueue.SerialQueue`（§5.7）。
...
直接复用 vibe-fs `Shell.PromiseQueue.SerialQueue`：局部可变 `tail` 锁队尾，内部 try-with 捕获防断链，异步变更强制排队 → 无锁保护 masterBranch git 操作原子性。ff 检查 + 合并作为单个 Enqueue 单元（§7.3 不变式）。
...
| 内置 review | 依赖 vibe-fs `/loop`，两插件 prompt 层协同，互不 import（决策 1.9、§6.6）| 公理 1 关注点分离 |
...
目标：单 Squad Session，能拆解 → 起 slave → ff 合并 → 清理，崩溃不卡死 DAG。
...
- [ ] vibe-fs `/loop` 检测 + slave prompt 注入分支（裸版 / 有 vibe-fs 版，§6.6）
...
| 术语 | 定义 |
|------|------|
| Coordinator | 用户交互的 master opencode 进程，加载 Squad 插件后无 `SQUAD_COORDINATOR_URL`。运行时协调者与唯一写入入口；SSOT 承载于 master session 对话历史，git refs 负责合并事实校正 |
...
| Squad Session | 一次 `/squad` 创建的执行上下文，含一个 DAG |
| SSOT | Single Source of Truth；此处 = coordinator 所持 master session 对话历史（frontmatter 事件流）；git refs 作第二真理源。|
| SerialQueue | vibe-fs `Shell.PromiseQueue.SerialQueue`，串行锁队尾保 masterBranch git 操作原子（§5.7）|
...
| 维度 | vibe-fs | Squad |
|------|---------|-------|
| 架构 | Kernel(纯函数) + Shell(副作用) + 多 Host | Coordinator + Slave + HTTP，复用 vibe-fs Kernel/Shell |
...
| Review | 内置 submit_review + reviewer loop | 不内置，依赖 vibe-fs `/loop`（决策 1.9）|
...
- 标量字段（session_id / task_id / title / master_sha / status）平铺 frontmatter 顶层，重放只读 scalar，不做全文 NLP（同 vibe-fs front-matter 锚点）。
- 数组字段（depends_on）用 yaml 序列。**核实 5.x：vibe-fs Kernel frontmatter 解析器只认标量不认数组**——故 Squad 重放须走 Shell 层全量 yaml 解析（依赖已含 yaml 包），不能复用 Kernel 轻量标量解析器（§8.4）。
