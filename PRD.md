...
`amend` 字段已在代码中移除（commit 78531ac），不再作为控制字段处理。
...
`filterAmendFromArgs` 已随 `amend` 功能一起移除。
...
控制字段（`warn`/`warn_tdd`/`warn_reuse`）应进入 sidecar/control envelope，而不是重新塞回普通 enumerable 参数。已移除的 `amend` 不再适用。
...
`ControlEnvelope` 中保存三个 warn 值，各 Host 的 before hook 通过 `ToolHookRuntime.saveCompliance` 将 envelope 持久化到按 session/toolCallID 索引的 transient store，供 after hook 恢复。
...
* 核心业务参数结构无效；
* 安全或权限禁止；
* 已移除的 `amend` 等残留字段，或任何会破坏事件历史的控制参数。
...
# 附录 VI：子会话架构与模型指令（代码已实现）

## 一、设计目标

主会话（host session）里的 `coder` / `investigator` / `browser` / `meditator` 等子代理，需要一个与主会话生命周期隔离的**子会话执行单元**。它必须满足：
