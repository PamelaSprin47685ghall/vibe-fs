...
- If a wrapper merely calls the spike, replace the wrapper/spike pair with one production implementation.

## Common Wrong Fixes
- Keep both “prototype” and “production wrapper” paths where the wrapper merely calls the spike.
- Rename the spike module to `prod` without changing assumptions.
- Add a few tests of the demo happy path and declare it production-ready.
- Leave hard-coded credentials, global state, or skipped failures “until later.”

## Verification
Exercise the production boundary, failure paths, and lifecycle rather than only the successful demo scenario that motivated the spike. The invariant is: shipped structure follows production contracts; the prototype contributes knowledge, not scaffolding.
