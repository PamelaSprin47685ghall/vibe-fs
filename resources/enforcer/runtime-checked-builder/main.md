...
- If some constraints are truly dynamic, keep one atomic constructor/result type and do not expose a mutable incomplete object.
- If an internal accumulator is unavoidable, confine it so it cannot masquerade as the domain value.

## Common Wrong Fixes
- Add more `isValid` checks to the same mutable builder.
- Throw from every setter while still allowing `build` on a reused instance.
- Freeze the object after `build` but leave incomplete instances publicly constructible.
- Document “call these setters in order” instead of making order/type the API.

## Verification
Attempt to omit or reorder required stages and construct contradictory combinations. Static structure or the single constructor boundary should reject them before an invalid domain instance escapes. The invariant is: no escaped value is incomplete or contradictory relative to the construction contract.
