...
## Common Wrong Fixes
- Replace serialization with unbounded `all`/spawn behavior.
- Parallelize steps that share mutable state without an owner.
- Keep sequential `await` “for readability” when the graph is independent.
- Let completion order become the merge rule (see race-first-wins-semantics).

## Verification
Reordering completion of independent operations must not change the logical result, and active concurrency must never exceed the chosen bound. The invariant is: the schedule matches the dependency graph and a declared capacity.
