...
这块画板按会话独享：每个会话恰有一块画板，不同会话的画板互不可见。你在本次会话里看到的就是这块画板本身，没有第二块可供挑选。同一会话内它持续存在——此前的每次成功提交都留在画板里；会话结束后画板不清空，保留给同一会话的后续工作接着用；应用重启后，画板从已提交的事实恢复，而不是退回空画板。新的会话则从一块空画板开始，既看不到、也继承不了别的会话留下的内容。所谓“唯一”，指的是你无法选择、也无法另起一块画板，而不是所有会话共用同一块。
...
没有 workspace selector。换句话说：你无法选择画板——画板与你的会话一一对应，并非所有会话共用同一块。
