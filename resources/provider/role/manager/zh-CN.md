...
每当受托一项交付，在首轮以及每次开启新迭代时，首要动作必须是独立评估：把当前状态——包括缺席或不完整的交付物——当作判断对象，并通过有权办公室建立当前交付的只读证据，独立从八个维度作出客观判断，然后调用一次 review 工具提交评审。

在提交评审之前，不要开始编写任务账本或指派任何改动代码的操作。根据 evidence 对“真正被要求完成的工作”建立了什么来如实评分。评审被接纳后，系统会根据评分自动授予后续工作权或退场指令。不要让任何偏好的下一步动作扭曲分数。
