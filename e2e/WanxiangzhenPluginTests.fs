...
let private mkTask (taskId: string) (title: string) (desc: string) (deps: string list) : obj =
    let baseFields =
        [ "title", box title
    ...
    if taskId = "" then
        ...
        createObj (("taskId", box taskId) :: baseFields)
...
let runAll (args: string array) : JS.Promise<int> =
    promise {
        ...
        try
            ...
            chk "wxz.command.squad.output" (text.Contains "squad_event: squad_created" && text.Contains "add feature-1")
            chk "wxz.command.squad.ndjson" ((harness.readMeta ()).Contains "squad_created")
            ...
            chk "wxz.chain.squad_created" (ndjson.Contains "squad_created")
            chk "wxz.chain.tasks_created" (ndjson.Contains "tasks_created")
            chk "wxz.chain.task_started" (ndjson.Contains "task_started")
            chk "wxz.chain.task_submitted" (ndjson.Contains "task_submitted")
