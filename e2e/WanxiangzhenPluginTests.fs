...
let runAll (args: string array) : JS.Promise<int> =
    promise {
        ...
        let updateArgs = createObj [
            "events",
            box [|
                mkTasksCreated [ mkTask "task-e2e-01" "T" "D" [] ]
            |]
        ]
        ...
        // 3. scheduler trigger & wait — task should transition to
        //    Running
        do! harness.waitForScheduler "task-e2e-01"
        ...
        let regStatus = unbox<int> (get regRes "status")
        chk "wxz.register.status" (regStatus = 200)

        // 5. submit — POST /task/{taskId}/submit with commitSha
        //    Configure mocks so the ff-only merge path succeeds
