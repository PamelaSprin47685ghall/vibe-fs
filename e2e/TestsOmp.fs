...
let runAll (_args: string array) : JS.Promise<int> =
    promise {
        ...
        chk
            "e2e-omp.methodology.responded"
            ((jsonStringify methodologyResult).Contains "methodology analysis output"
             && not ((jsonStringify methodologyResult).Contains "error"))
        ...
        chk
            "e2e-omp.methodology.risk_analysis.responded"
            ((jsonStringify riskResult).Contains "risk analysis output"
             && not ((jsonStringify riskResult).Contains "error"))
        ...
        let! thoughtResult =
            h.triggerTool
                "methodology"
                (box
                    {| methodology = "thought_experiment"
                       note = String.replicate 1100 "n"
                       background = String.replicate 1100 "b"
                       intent = String.replicate 1100 "i" |})
                sessionId
        ...
        chk
            "e2e-omp.methodology.thought_experiment.responded"
            ((jsonStringify thoughtResult).Contains "thought experiment output"
             && not ((jsonStringify thoughtResult).Contains "error"))
