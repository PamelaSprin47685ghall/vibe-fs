/**
 * p0-canary.js — P0 canary tests using shared-host 一条龙 mode.
 *
...
const tests = [
  ...
  // ── OC-FUZZY-003 ───────────────────────────────────────────────────────────
  {
    ...
    fn: async (t) => {
      fs.writeFileSync(`${t.host.workDir}/grep_target.txt`,
  ...
  // NOTE: /session/{id}/command for /loop is a streaming endpoint (never ends).
  // We fire it with a short timeout and verify activation via NDJSON/events.
  {
    ...
    fn: async (t) => {
      ...
      // Fire /loop via raw fetch (streaming endpoint, response never ends)
      ...
      // Check NDJSON for activation
      const ndjsonPath = `${t.host.workDir}/.wanxiangshu.ndjson`;
      let found = false;
      if (fs.existsSync(ndjsonPath)) {
        const ndjson = fs.readFileSync(ndjsonPath, 'utf8');
        found = ndjson.includes('loop_activated');
      }

      // Check events too
      if (!found) {
        found = t.events.allEvents.some(e =>
          e.type === 'loop_activated' || e.properties?.type === 'loop_activated'
        );
      }
