...
const exitCode = await runScenario({
...
}, [
  {
    ...
    fn: async (t) => {
      ...
      console.log('\n=== raw events ===');
      for (const e of events.filter((e) => e.type.startsWith('message') || e.type === 'session.idle')) {
        console.log(`seq=${e.seq} type=${e.type}`);
        console.log(JSON.stringify(JSON.parse(e.raw), null, 2).slice(0, 800));
      }
      console.log('==================');
    },
  },
