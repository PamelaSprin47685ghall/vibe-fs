...
let runAll (args: string array) : JS.Promise<int> =
    promise {
        ...
        let! plugin = importDefault<obj> ("../build/src/Opencode/PluginMimoTui.js") |> Promise.lift
        ...
        let buildMockApi (todoResult: obj) (messages: obj[]) (parts: obj[]) (childrenData: obj[]) =
            ...
            let sessionState =
                createObj [ "todo", box originalTodo; "messages", box (fun (_: string) -> box messages) ]
            ...
            let apiObj =
                createObj
                    [ "state",
                          ...
                          createObj
                              [ "session",
                                box (
                                    createObj
                                        [ "get",
                                          box (fun _ -> Promise.lift (box {| data = {| parentID = "parent-id" |} |}))
                      ...
                      box (
                          createObj
                              [ "current",
                                ...
                                "navigate",
                                box (
                                    System.Action<string, obj>(fun _ opts ->
                                        navigationTarget <- dynStr opts "sessionID")
                                ) ]
                      )
                      "command", box (createObj [ "register", box (fun (fn: obj) -> capturedCmdRegister <- Some fn) ])
                      "ui",
                      box (
                          createObj
                              [ "dialog",
                      ...
                      "lifecycle", box (createObj [ "onDispose", box (fun (cb: obj) -> disposeCb <- Some cb) ]) ]
        ...
        let taskSuccessMsg = [| box (createObj [ "id", box "msg-1" ]) |]
                                       ...
                                       createObj
                                           [ "todos",
                                             box
                                                 [| box (
                                                        createObj
                                                            [ "content", box "recovered"; "status", box "pending" ]
                                                    ) |] ]
                                   ) ]
                         ) ]
               ) |]
        ...
        let (api5, _, _, sess5) =
            buildMockApi (box [||]) taskSuccessMsg taskSuccessPart [||]
        ...
        let taskErrorMsg = [| box (createObj [ "id", box "msg-2" ]) |]
        ...
        let taskErrorPart =
            [| box (
                   createObj
                       [ "type", box "tool"
                         "tool", box "task"
                         "state", box (createObj [ "status", box "error" ]) ]
               ) |]
        ...
        let (api7, triggerDispose7, origTodo7, sess7) =
            buildMockApi (box [||]) [||] [||] [||]
            ...
            [| box (
                   createObj
                       [ "id", box "child-agent-1"
                         "title", box "child-1"
                         "time", box (createObj [ "created", box 1000.0; "updated", box 2000.0 ]) ]
               )
               box (
                   createObj
