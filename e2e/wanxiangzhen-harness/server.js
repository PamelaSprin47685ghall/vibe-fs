...
export function assembleHarness(ctx, runtime, hooks, tmpDir, state) {
  return {
    ...
    callSlavePlugin: (sCtx, url, tid, wt, mb, tok) =>
      runHelpers.callSlavePlugin(sCtx, url, tid, wt, mb, tok),
    dispose: () => runHelpers.dispose(runtime, tmpDir, state.makeDeps(runHelpers.shapeWanSquadLine)),
    ...setters,
  };
}
...
export async function startInProcess(opts) {
  ...
  const mod = await import(PLUGIN_JS);
  const pluginWithDeps = mod.pluginWithDeps;
  ...
  const deps = makeDeps(shapeWanSquadLine);
