...
module ArchitectureGates =
    ...
    let private checkTestsNextFiles (files: seq<string>) (violations: List<string>) =
        for file in files do
            if file.EndsWith(".fs") && not (file.EndsWith("ArchitectureGates.fs")) && not (file.EndsWith("ArchitectureGateSupport.fs")) then
                let text = NodeFsGatesSupport.readFileSync (file, "utf-8")
                for token in forbiddenTokens do
                    if containsForbiddenToken text token then
                        violations.Add(sprintf "File '%s' contains forbidden token '%s'" file token)
            if file.EndsWith(".fsproj") then
                ...
                if text.Contains("ProjectReference") then
                    let allowed = text.Contains("../next/Wanxiangshu.Next.fsproj") || text.Contains(@"..\next\Wanxiangshu.Next.fsproj")
                    let hasLegacyWanxiangshu = text.Contains("wanxiangshu.fsproj") && not (text.Contains("Wanxiangshu.Next.fsproj"))
                    let forbidden = hasLegacyWanxiangshu || text.Contains("../src") || text.Contains(@"..\src") || text.Contains(@"\src")
    ...
    let ``Next_has_no_legacy_tokens`` () =
        ...
        Assert.True(NodeFsGatesSupport.existsSync testsNextDir, sprintf "Directory 'tests-next' does not exist at %s" testsNextDir)
        let nextFiles = List<string>()
        for f in collectFsFiles nextDir do nextFiles.Add(f)
        let propsPathNext = NodeFsGatesSupport.pathJoin (nextDir, "Directory.Build.props")
        if NodeFsGatesSupport.existsSync propsPathNext && isDir propsPathNext then nextFiles.Add(propsPathNext)
        let testsNextFiles = List<string>()
        for f in collectFsFiles testsNextDir do testsNextFiles.Add(f)
        let propsPathTests = NodeFsGatesSupport.pathJoin (testsNextDir, "Directory.Build.props")
        if NodeFsGatesSupport.existsSync propsPathTests && isDir propsPathTests then testsNextFiles.Add(propsPathTests)
    ...
    let ``Next_fsproj_has_no_legacy_project_reference`` () =
        let repoRoot = findRepoRoot ()
        let fsprojPath = NodeFsGatesSupport.pathJoin (repoRoot, NodeFsGatesSupport.pathJoin ("next", "Wanxiangshu.Next.fsproj"))
        Assert.True(NodeFsGatesSupport.existsSync fsprojPath, sprintf "File 'Wanxiangshu.Next.fsproj' does not exist at %s" fsprojPath)
    ...
    let ``Next_recursive_scan_includes_subdirectory_files`` () =
        ...
        Assert.True(List.length files >= 10, sprintf "Expected recursive scan to return >= 10 files, got %d" (List.length files))
        let fromSubdir (token: string) (leaf: string) = files |> List.exists (fun f -> f.Contains(token) && f.EndsWith(leaf))
    ...
    let ``Test_runner_timeout_is_strictly_1s`` () =
        let repoRoot = findRepoRoot ()
        let runnerPath = NodeFsGatesSupport.pathJoin (repoRoot, NodeFsGatesSupport.pathJoin ("tests-next", "runner.js"))
        ...
        Assert.True(content.Contains("runTestInWorker") && content.Contains("1000"), "runner.js must enforce rigid 1000ms (1s) process timeout limit for all tests")
        Assert.True(content.Contains("detached: true") && content.Contains("SIGKILL"), "runner.js must hard-stop the isolated test process tree on timeout")
    ...
    let ``No_sleep_or_setTimeout_allowed_in_tests_or_production`` () =
        ...
        for file in allFiles do
            if file.EndsWith(".fs") && not (file.EndsWith("ArchitectureGates.fs")) && not (file.EndsWith("ArchitectureGateSupport.fs")) then
                let text = NodeFsGatesSupport.readFileSync (file, "utf-8")
                if containsForbiddenToken text "sleepJs" then violations.Add(sprintf "File '%s' contains forbidden sleep token 'sleepJs'" file)
                if containsForbiddenToken text "sleep" then violations.Add(sprintf "File '%s' contains forbidden sleep token 'sleep'" file)
        Assert.Empty(violations)
