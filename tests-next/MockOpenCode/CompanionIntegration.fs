...
module CompanionIntegration =
    ...
    let private blogModel =
        Ok { providerID = "test"; modelID = "blogger"; variant = Some "fast" } : Result<OpencodeModel, string>
    ...
    let ``Companion creates blogger through mock port`` () =
        task {
            ...
            state.SendHandler <- Some (fun sId text opts ->
                task {
                    let mid = MessageId.create ("c-" + Guid.NewGuid().ToString("N").Substring(0, 6))
                    eventPort.NotifyTerminal sId (TerminalOutcome.Completed mid) |> ignore
                    return Delivered mid
                })

            let sid = SessionId.create "comp-primary"
            let companion =
                new CompanionHost(sid, sessionPort, ?bloggerModel = Some blogModel)
            ...
            trueThat (state.Created.Length > 0)
                "Companion must create a blogger child session via MockOpenCodePort"
            trueThat (state.Sent.Length > 0)
                "Companion must send prompt via MockOpenCodePort"
        }
