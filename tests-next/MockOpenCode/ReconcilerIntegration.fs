    ...
    let ``Reconciler triggers guard nudge through mock port`` () =
        withTempDir (fun directory ->
            task {
                ...
                use journal =
                    AgentJournal.create
                        directory (RuntimeId.create "recon-test") 1 DateTimeOffset.UtcNow
                    ...
                    { new ISessionSnapshotPort with
                        member _.GetMessages _ =
                            Task.FromResult(Ok [
                                ...
                                msg "a1" "assistant" (Some "reviewer") (Some "stop")
                                    [| textPart "review done" |] None
                            ]) }
                ...
                trueThat (state.Sent.Length > 0)
                    "Reconciler flow must produce guard nudge via mock port"
            })
            ...
            let snapshot =
                { new ISessionSnapshotPort with
                    member _.GetMessages _ =
                        Task.FromResult(Ok [
                            msg "u1" "user" None None [||] None
                            msg "a1" "assistant" (Some "coder") (Some "stop")
                                [| textPart "done" |] None
                        ]) }
