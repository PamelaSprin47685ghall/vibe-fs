...
module RolesTests =
    ...
    let ``Manager_role_permission_matrix`` () =
        ...
        Assert.True(Roles.isAllowed Role.Manager ToolPermission.Pty)
        ...
        Assert.False(Roles.isAllowed Role.Manager ToolPermission.Blogger)

    [<Fact>]
    let ``Coder_role_permission_matrix`` () =
        let allowed = set [ ToolPermission.Inspector; ToolPermission.Blogger ]
        ...
        Assert.True(Roles.isAllowed Role.Coder ToolPermission.Blogger)

        Assert.False(Roles.isAllowed Role.Coder ToolPermission.Fork)
        Assert.False(Roles.isAllowed Role.Coder ToolPermission.Exec)
        Assert.False(Roles.isAllowed Role.Coder ToolPermission.Read)
    ...
    let ``Meditator_and_Reviewer_role_permission_matrix`` () =
        ...
        Assert.Equal<ToolPermission Set>(allowed, Roles.permissions Role.Reviewer)
