...
- production-bound evidence 从每个 active、single-primary WHAT callback 独立出发，沿 exact lexical local call 求文件内执行闭包。闭包承认直接 helper/IIFE/async helper；局部 higher-order helper 只在函数体调用对应形参时传递该 call-site callback。每个根保留自己的形参环境，禁止其他 WHAT 的 callback 串借。
- property callback 只从 exact `fast-check` default/namespace binding 注册；`fc.property`/`fc.asyncProperty` 必须被可达的 exact `fc.assert`/`fc.check` 消费。async property runner 必须被 `await` 或 `return`，未等待的 Promise 不取得证明权威。
- shadow、dead helper、未消费 property、其他 law callback、只导入不用与静态不可达 call edge 仍被拒绝。遍历以 callable + call-site environment 为 visited state，递归/互递归闭包必须终止且稳定。consumer package authority、law owner、源码编译与 dist 产物检查仍分别 fail-closed。该图只证明 production Surface 处于 proof 的静态执行闭包，不充当 assertion 数据流证明。
...
### 6. ESM 模块链接门禁（`js-module-linkage`）
