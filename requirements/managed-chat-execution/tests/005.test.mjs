...
test('WHAT[CHATEXEC-005] ProviderStarted enforces acceptance provider run and terminal fences', () => {
  ...
  const accepted = acceptManagedChat('run-5-1', 'msg-root-5-1', 'HumanRoot', {
    kind: 'RootSelection',
    ownerSession: null,
  ...
  const start = providerStarted(key, 'run-5-1-p', 'use-committed-epoch', 'work-main')
...
test('WHAT[CHATEXEC-005] equal start and terminal duplicates are semantic no-ops', () => {
  ...
  const accepted = acceptManagedChat('run-5-2', 'msg-root-5-2', 'HumanRoot', {
    kind: 'RootSelection',
    ownerSession: null,
    ownerLogicalRun: null,
    ownerAuthorityRoot: null,
  ...
  const start = providerStarted(key, 'run-5-2-p', 'use-committed-epoch', 'work-main')
...
test('WHAT[CHATEXEC-005] ProviderStarted before Accepted rejects', () => {
  ...
  const start = providerStarted(key, 'run-5-3-p', 'use-committed-epoch', 'work-main')
...
test('WHAT[CHATEXEC-005] exact public assistant observation alone establishes provider start', () => {
  ...
  const accepted = acceptManagedChat('run-5-4', 'msg-root-5-4', 'HumanRoot', {
    kind: 'RootSelection',
    ownerSession: null,
    ownerLogicalRun: null,
    ownerAuthorityRoot: null,
  ...
  const start = providerStarted(key, 'run-5-4-p', 'use-committed-epoch', 'work-main')
