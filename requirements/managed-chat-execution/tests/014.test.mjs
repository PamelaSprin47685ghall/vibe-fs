...
test('WHAT[CHATEXEC-014] capture canonicalizes facts and preserves only immutable owner evidence', async () => {
  const captured = await captureEvidence(captureInput(), surfaces)
  const projection = fold(captured.execution.facts)
  const status = queryFacts(captured.execution.facts, captured.execution.key.sessionId, captured.execution.key.physicalUserMessageId)

  assert.equal(projection.ok, true)
  assert.deepEqual(captured.execution.projection, projection.value)
  assert.deepEqual(captured.execution.status, status.status)
  assert.deepEqual(captured.capacity.reconciliation, { kind: 'NoOp' })
  assert.equal(Object.isFrozen(captured), true)
  assert.deepEqual(JSON.parse(serializeEvidence(captured)), captured)
})
...
test('WHAT[CHATEXEC-014] capture redacts known failures and rejects payload or stack fields', async () => {
  ...
  assert.match(captured.diagnostics[0].participant, /\[REDACTED\]/)
  assert.equal('effectiveAgent' in captured.diagnostics[0], false, 'diagnostic view carries no EffectiveAgent')
...
test('WHAT[CHATEXEC-014] agent-028 session-only binding is hostile; current owners fold exact keys with fixed participant', async () => {
  ...
  const recoveryRes = await recoverScenarios([agent028.currentModel.recoveryScenario])
  assert.deepEqual(recoveryRes.decisions, [agent028.currentModel.expectedDecision])
})
