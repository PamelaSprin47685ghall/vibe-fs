...
test('WHAT[CHATEXEC-006] terminal replay performs no acceptance or capacity effect', async () => {
  const res = await admission.runManagedAdmissionScenario('terminal-replay')
...
test('WHAT[CHATEXEC-006] same key terminal conflict', () => {
  ...
  const accepted = chatExecution.acceptManagedChat('run-6-1', 'msg-root-6-1', 'HumanRoot', {
    kind: 'RootSelection',
    ownerSession: null,
  ...
  const start = chatExecution.providerStarted(key, 'run-6-1-p', 'use-committed-epoch', 'work-main')
  const term1 = chatExecution.terminal(key, 'run-6-1-p', 'Completed')
  const term2 = chatExecution.terminal(key, 'run-6-1-p', 'Cancelled')
...
test('WHAT[CHATEXEC-006] Terminal directly after Accepted is rejected', () => {
  ...
  const accepted = chatExecution.acceptManagedChat('run-6-2', 'msg-root-6-2', 'HumanRoot', {
    kind: 'RootSelection',
    ownerSession: null,
    ownerLogicalRun: null,
    ownerAuthorityRoot: null,
  ...
  const term = chatExecution.terminal(key, 'run-6-2-p', 'Completed')
...
test('WHAT[CHATEXEC-006] each terminal disposition is durable after provider start', () => {
  ...
  const accepted = chatExecution.acceptManagedChat('run-6-3', 'msg-root-6-3', 'HumanRoot', {
    kind: 'RootSelection',
    ownerSession: null,
    ownerLogicalRun: null,
    ownerAuthorityRoot: null,
  ...
  const start = chatExecution.providerStarted(key, 'run-6-3-p', 'use-committed-epoch', 'work-main')
  const term = chatExecution.terminal(key, 'run-6-3-p', 'Completed')
...
test('WHAT[CHATEXEC-006] provider terminal before ProviderStarted rejects', () => {
  ...
  const term = chatExecution.terminal(key, 'run-6-4-p', 'Completed')
...
test('WHAT[CHATEXEC-006] conflicting terminal rejects without a second write', () => {
  ...
  const accepted = chatExecution.acceptManagedChat('run-6-5', 'msg-root-6-5', 'HumanRoot', {
    kind: 'RootSelection',
    ownerSession: null,
    ownerLogicalRun: null,
    ownerAuthorityRoot: null,
  ...
  const start = chatExecution.providerStarted(key, 'run-6-5-p', 'use-committed-epoch', 'work-main')
  const term1 = chatExecution.terminal(key, 'run-6-5-p', 'Completed')
  const term2 = chatExecution.terminal(key, 'run-6-5-p', 'Failed')
...
test('WHAT[CHATEXEC-006] each uncertain Terminal append leaves projection provider-started', () => {
  ...
  const accepted = chatExecution.acceptManagedChat('run-6-6', 'msg-root-6-6', 'HumanRoot', {
    kind: 'RootSelection',
    ownerSession: null,
    ownerLogicalRun: null,
    ownerAuthorityRoot: null,
  ...
  const start = chatExecution.providerStarted(key, 'run-6-6-p', 'use-committed-epoch', 'work-main')
