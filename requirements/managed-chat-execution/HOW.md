...
`admissionCrashPointScenarios` 的 recovery decision 与 port invocation 走 production runtime；A–I 的 durable prefix 另由 production transaction/lifecycle Surface 验证。原先按 cut label 构造的 `beforeRestart` / `afterRestart` snapshot 与对应假测试已删除，因为它们未执行真实 OS process crash、同一 durable backing reopen 与 public snapshot。

CHATEXEC-009 的 accepted-phase canary 由 parent integration test 启动两个 Node OS process。进程 A 经真实 plugin `chat.message` 完成 durable `Accepted` 与 exact admission，使用同步 marker 固定 Status、SessionBinding、ModelRouting 三个正式 Surface 的退出前观测，证明本场景确实建立 binding、token、custody、execution 与 derived owner；同 session、不同 physical message 的 decoy 必须保持未接纳且无 binding，拒绝 session-only lookup。随后直接 `process.exit(86)`，不调用 dispose。进程 B 以同一 workspace/journal 新建真实 plugin；正式 Surface 必须同时显示 exact `Accepted` 留存、decoy 仍为空、exact binding 为零，并证明公开 capacity snapshot 的 ledger、token、custody、execution、waiter、owner、lineage 与 activeCount 全部为零。该 proof 与 durable codec exclusion proof 共同闭合 accepted phase 的 CHATEXEC-009；不宣称本阶段从未建立的 waiter 被 crash 清除，不覆盖未公开 registry，也不宣称 A–I 每个 cut 的物理 reboot、accepted-message replay 或自动 recovery 已闭合。
...
| CHATEXEC-009 | `requirements/managed-chat-execution/tests/facts-roundtrip.test.mjs::WHAT[CHATEXEC-009] durable execution fact round-trip excludes process-local artifacts`；`requirements/managed-chat-execution/tests/integration/process-restart-canary.test.mjs::WHAT[CHATEXEC-009] abrupt process exit retains exact Accepted and empties local admission capacity`（只闭合 accepted phase；A–I physical reboot、accepted-message replay 与自动 recovery 不在本 proof 宣称范围） |
