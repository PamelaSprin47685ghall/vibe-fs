...
CHATEXEC-009 的 accepted-phase canary 由 parent integration test 启动两个 Node OS process。进程 A 经真实 plugin `chat.message` 完成 durable `Accepted` 与 exact admission，使用同步 marker 固定 Status、SessionBinding、ModelRouting 三个正式 Surface 的 crash 前观测，随后直接 `process.exit(86)`，不调用 dispose。进程 B 以同一 workspace/journal 新建真实 plugin；正式 Surface 必须同时显示 `Accepted` 留存、exact binding 为零、exact capacity owner/token/custody/execution/waiter 均不存在。该 proof 与 durable codec exclusion proof 共同闭合 accepted phase 的 CHATEXEC-009；不宣称 A–I 每个 cut 的物理 reboot、accepted-message replay 或自动 recovery 已闭合。
