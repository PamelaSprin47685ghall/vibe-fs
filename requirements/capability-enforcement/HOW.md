...
- `enough/abandon/defer` 的行为语义 → `attention-regulation`（DEPENDS ON）；本包只投影可见性/执行 gate。
- `subscribe/publish` 的 mailbox/路由语义 → `concern-routing`（DEPENDS ON）；本包不拥有通信事实。
- `celebrate/regret` 与 Enhancer 的学习语义 → `institutional-learning`（DEPENDS ON）；本包不拥有 rule birth。
...
| ENF-006 | 现有：`tests/agent-permission-gate.test.mjs` `ROLE_ALLOW` + `HOST_skill_remains_allowed_for_every_managed_role` + `ASSUME_is_a_non_authority_utility_for_interactive_roles_only`；`tests/auto-injected-tool.test.mjs` `AUTOINJ_skill_wire_stays_host_owned_and_is_not_plugin_registered` / `AUTOINJ_active_empty_skill_call_is_denied_without_touching_real_skill_names`。待 GAP：七个新增 utility 对 WorkMain 交互角色可见、对 Blogger/Distiller/Bookkeeper 不扩权，并且不进入 `Roles.permissions`。 | MOVE + NEW + GAP | `node --test requirements/capability-enforcement/tests/agent-permission-gate.test.mjs requirements/capability-enforcement/tests/auto-injected-tool.test.mjs` |
