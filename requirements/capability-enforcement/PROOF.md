...
| ENF-006 | `tests/agent-permission-gate.test.mjs` `ROLE_ALLOW`（Distiller: []、Blogger: [chronicle]）+ `requirements/capability-enforcement/tests/auto-injected-tool.test.mjs` `AUTOINJ_rolePredicate_allows_work_roles_and_denies_blogger_distiller` | MOVE + REUSE | 分别 `node --test` |
