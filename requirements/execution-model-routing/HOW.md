...
decorator 额外维护 capacity-only `MainSession ↔ BloggerSession` association 与 exact execution 的 actual lender。普通 Main requester 仍按 session lineage 找 credit；Blogger requester 不沿 Main ancestry找 token，而是先找到 owner Main 当前 borrowed execution 的 actual lender Main，再映射到该 lender 的 Blogger。只有这枚平行 Blogger token 可作为 companion credit。Blogger admission 后把 actual companion lender 冻结到 Blogger exact execution；Main 随后完成、recall 或改成 own token，都不把正在进行的 Blogger execution 半途改绑。Blogger 自己的 owner recall/step fence 仍走同一 token state machine。
...
每个 provider request 前，messages transform 先进入 capacity decorator；无 token 就在 transform 前 await。assistant error 或任意 completed assistant（包括 `finish="tool-calls"`）结束 provider step 并归还 step token；**assistant error 永不单独结束 physical execution binding**。普通 physical terminal 必须同时满足 assistant、无 error、completed、存在非 `tool-calls` finish。step start 同时提交当前 wire 已可见 assistant run 集合作 fence；旧 terminal 不得释放新 step，下一 transform 的新增 run 可补证漏掉的 terminal。
...
| EMR-010 lineage-local preemptive provider capacity | `tests/model-routing-runtime.test.mjs`：`EMR_010_lineage_credit_is_free_only_to_descendants_not_global_waiters`、`EMR_010_ancestor_recall_waits_for_descendant_step_end_without_overbooking`、`EMR_010_late_old_terminal_cannot_release_a_new_provider_step`、`EMR_010_confirmed_pre_dispatch_suppression_returns_the_step_token`、`EMR_010_recalled_child_may_take_new_ordinary_capacity_for_exact_target`、`EMR_010_owner_priority_beats_multiple_children_and_nested_borrowers`、`EMR_010_credit_never_crosses_provider_boundary`、`EMR_010_multi_provider_credit_requires_one_token_attribution`、`EMR_010_blogger_borrows_the_lender_blogger_when_main_is_borrowed`、`EMR_010_blogger_gets_no_companion_credit_when_main_did_not_borrow`；`tests/routing-authority-boundary.test.mjs`：`EMR_010_borrowing_complexity_is_owned_only_by_the_capacity_decorator` 锁定 ledger/decorator 所有权且 MJS 无借贷状态；`requirements/host-boundary/tests/host001-fragment-events.test.mjs`：`finish="tool-calls"` 结束 provider step 但不结束 physical execution，final assistant 同时提供两层 evidence | NEW + CONTRACT | `node --test requirements/execution-model-routing/tests/model-routing-runtime.test.mjs requirements/execution-model-routing/tests/routing-authority-boundary.test.mjs requirements/host-boundary/tests/host001-fragment-events.test.mjs` |
...
- **GAP-024**（provider assistant error 被误当 physical terminal，导致 same-physical retry 丢 execution binding）— CLOSED. RED 证据：旧实现使 `requirements/host-boundary/tests/host001-fragment-events.test.mjs` 的 failed assistant 被错误解码为 `PhysicalExecutionEnd`。关闭证据：`HostEventCodec.physicalExecutionTerminalInfo` 仅接受无 error、completed、明确非 `tool-calls` finish；`requirements/host-boundary/tests/provider-retry-host-edge.test.mjs` 永久重放 failed step 后 same-physical retry；execution-model-routing 46/46、host-boundary 196/196 focused 全绿，`npm run format-build-test` 全阶梯通过（authoritative semantic 3350/3350、verification harness 275/275、Long Stroke e2e、package integration、dry-run pack）。
