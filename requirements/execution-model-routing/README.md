...
`execution-model-routing` 拥有 Wanxiangshu 的物理模型调度边界：模型策略从哪里来、scheduler 看到什么 occupancy、`null` 如何形成事件驱动等待、root/worktree 插件实例如何共享占用，以及 session/system invocation 何时释放 ModelTarget。
...
- `host-boundary`：提供 plugin load、root/worktree 多实例与 Host prompt/config/system-call 适配边界。
...
- Host SDK/hook 是否真的允许 request model/reasoning mutation、title/compaction model override → `host-boundary` canary。
