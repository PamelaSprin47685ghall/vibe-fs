...
assistant `error` 同样只属于 provider-step terminal evidence：它必须归还当前 step token，但**不得**作为 physical execution terminal 删除 exact binding。原因不是错误是否“可重试”的字符串分类，而是 Host retry 仍可复用同一个 `PhysicalUserMessageId`；adapter 在看到后续 execution 之前没有资格猜测这次 error 已经永久终结该 material。普通 physical terminal 因此必须是无 error 的 completed assistant，且 `finish` 明确存在并非 `tool-calls`。缺少该 witness 时保持 lease；后续新 physical material、session delete/scope cleanup/plugin shutdown 仍能精确收敛旧 occupancy。
