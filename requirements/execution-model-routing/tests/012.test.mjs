...
test('WHAT[EMR-012] capacity lifecycle admits every legal fenced transition', async () => {
  ...
  const observed = {
    ...
    role: 'coder',
    participant: 'alice',
    target: executionAdmissionTarget(runtime, acquisition.lease),
  }
  const first = commitExecutionAdmission(runtime, acquisition.lease, observed)
  assert.equal(first.kind, 'Applied')

  const duplicate = commitExecutionAdmission(runtime, acquisition.lease, observed)
  assert.equal(duplicate.kind, 'AlreadyApplied')
})
...
test('WHAT[EMR-012] capacity lifecycle rejects every illegal edge and opposite terminal', async () => {
  ...
  const foreign = {
    sessionId: 'session-2',
    physicalUserMessageId: 'physical-2',
    role: 'coder',
    participant: 'alice',
    target: executionAdmissionTarget(runtime, acquisition.lease),
  }
  const wrong = commitExecutionAdmission(runtime, acquisition.lease, foreign)
...
test('WHAT[EMR-012] same physical retry preserves the capability and a newer generation stales it', async () => {
  const runtime = createRuntime(() => target())
  const first = await acquireExecutionAdmission(
    runtime,
    'session-1',
    'physical-1',
    'coder',
  ...
  const second = await acquireExecutionAdmission(
    runtime,
    'session-1',
    'physical-2',
    'coder',
  ...
  const observed = {
    ...
    role: 'coder',
    participant: 'alice',
    target: executionAdmissionTarget(runtime, first.lease),
  }
  const stale = commitExecutionAdmission(runtime, first.lease, observed)
