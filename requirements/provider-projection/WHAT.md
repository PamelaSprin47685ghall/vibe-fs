...
禁止各功能直接接收并任意修改 `Message list`（`docs/what/projection.md` PROJ-001）。
...
**证据**：PROJ-001、`docs/why/projection.md`「投影形态」被拒方案。
...
「不同型」的结构事实归本包。
**证据**：PROJ-003、`docs/why/projection.md`「投影分层」被拒方案。
...
wire bytes、生成 digest/seal）（`docs/shape/projection.md` PROJ-004）。

**含义**：副作用只发生在 Coordinator；Planner/Renderer 是同入同出的纯函数。
**边界**：三层是当前实现合同；若未来证明其它分层同样满足本包命题，是 HOW 变化。
**证据**：PROJ-004、`docs/how/projection.md`「装配：三层」。
...
**证据**：PROJ-005、`docs/shape/projection.md` PROJ-005。
...
**证据**：PROJ-006、`docs/why/projection.md`「Intent 冲突解决」被拒方案。
...
**证据**：PROJ-007、`docs/why/projection.md`「DSL 是否承载生命周期」被拒方案。
...
机制归本包。
**证据**：ARCH-010、`changes/completed/js-tools-toml-result.md`、`docs/why/synthetic-toml.md`。
...
（`changes/completed/corrective.md` §1）。
...
**证据**：ARCH-011、`changes/completed/cursor-pair-hint.md` §8、corrective.md §8。
...
**证据**：COMPANION-007/012、`docs/how/projection.md`「Canonical digest」。
...
归本包。
**证据**：ARCH-010、`docs/why/synthetic-toml.md`。
