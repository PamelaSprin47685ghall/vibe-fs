...
|---|---|---|---|
| PROVIDER-PROJECTION-001（投影是代数；禁直接改 Message list） | REUSE `tests/unit/context/projection-algebra.test.mjs` `PROJ_004_renderer_maps_intents_to_writeback_instructions`（intent → writeback 指令，无直改路径） | REUSE | `node --test tests/unit/context/projection-algebra.test.mjs` |
...
| PROVIDER-PROJECTION-007（DSL 不负责生命周期） | REUSE 同上 `PROJ_002_the_snapshot_is_the_attempt_local_input_contract`（纯快照进→投影出，无生命周期输入）；结构性证明见 `docs/how/projection.md`（Coordinator 外置副作用） | REUSE | 同上 |
...
| PROVIDER-PROJECTION-009（instruction/data plane 由消费语义决定） | REUSE `tests/unit/codec/join-result-renderer.test.mjs` `MISC_join_render_batch_agent_completed_natural_language_and_work_record`（work record → entry-local comment）/ `MISC_join_render_batch_empty_work_record_no_comment`；REUSE `tests/unit/context/blogger-toml.test.mjs` `CTX_013_*`（data-only delta） | REUSE | `node --test tests/unit/codec/join-result-renderer.test.mjs` / `node --test tests/unit/context/blogger-toml.test.mjs` |
| PROVIDER-PROJECTION-010（representation 不反向创造 authority/state） | `requirements/provider-projection/tests/synthetic-toml.test.mjs` `ARCH_011_renderer_exposes_no_parser`（NEW：结构上无读回通道）；REUSE `tests/unit/host/pair-thought-anchored.test.mjs` `H13_05_missing_anchor_pair_is_omitted_not_relocated`（synthetic pair 不冒充真实消息） | NEW + REUSE | 见各文件行 |
| PROVIDER-PROJECTION-011（semantic equality ≠ wire equality；digest 从 Semantic 算） | REUSE `requirements/context-compression/tests/companion-projection.test.mjs`（SPLIT@cutover 后归本包）`COMPANION_007_canonical_digest_uses_semantic_projection_not_toml`；REUSE `tests/unit/context/projection-algebra.test.mjs` `PROJ_004_the_wire_view_and_the_written_back_bytes_decode_to_the_same_digest_input` | REUSE | `node --test requirements/context-compression/tests/companion-projection.test.mjs`（当前物理位置） |
...
- `tests/unit/context/projection-algebra.test.mjs`：
...
- `requirements/context-compression/tests/companion-projection.test.mjs`（当前物理在
  ...
  - `COMPANION_007_canonical_digest_uses_semantic_projection_not_toml` 断言归属
    ...
    本包 tests/。
- `tests/unit/codec/join-result-renderer.test.mjs`：join renderer 的 work-record 语义 →
  `work-record`；「entry-local comment 分类」→ provider-projection（009）。
- `tests/unit/host/pair-thought-anchored.test.mjs`：prefix 字节/anchor replay → 
