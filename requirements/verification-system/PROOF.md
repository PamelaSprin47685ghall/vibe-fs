...
| VERIFICATION-SYSTEM-002 | `requirements/verification-system/tests/proof-ladder.test.mjs`（test: VERIFY_001_l4_has_exactly_one_e2e_entry_in_the_ladder）；REUSE `tests/unit/verify/g4r-freeze.test.mjs`（G4R_FREEZE_case_ceiling_rejects_growth / G4R_FREEZE_top_level_entry_required_exactly_one_when_present）；REUSE `tests/unit/verify/e2e-event-ceiling.test.mjs`（long-stroke.toml declares theoretical exact event ceilings） | NEW+REUSE | node --test requirements/verification-system/tests/proof-ladder.test.mjs |
| VERIFICATION-SYSTEM-003 | REUSE `tests/unit/verify/g4r-freeze.test.mjs`（case ceiling=0 机器证明无并行 canary 通道）；REUSE `tests/unit/verify/e2e-event-ceiling.test.mjs`（event 天花板精确）；「物理契约论证」人工裁决面由 VERIFY-002 文本 + review 承接 | REUSE | node --test tests/unit/verify/g4r-freeze.test.mjs |
...
| VERIFICATION-SYSTEM-006 | `requirements/verification-system/tests/e2e-watchdog-feed.test.mjs`（E2E_WATCHDOG_FEED_case_files_do_not_feed_watchdog_directly）；REUSE `tests/unit/verdict-feed.test.mjs`（VERIFY_004_a_verdict_renews_the_silence_window / VERIFY_004_bytes_moving_is_recorded_and_does_not_renew） | MOVE+REUSE | node --test requirements/verification-system/tests/e2e-watchdog-feed.test.mjs |
| VERIFICATION-SYSTEM-007 | REUSE `tests/unit/domain.meta.test.mjs`（deadline verdict does not depend on the ambient timezone）；temporal 层 virtual-time 断言归 `time-capability`（tests/unit/temporal/**，REUSE 边界注） | REUSE | node --test tests/unit/domain.meta.test.mjs |
| VERIFICATION-SYSTEM-008 | REUSE `tests/unit/guide-contract.test.mjs`（VERIFY_008_the_published_plugin_entrypoint_loads / VERIFY_008_every_emitted_module_actually_loads / VERIFY_008_the_contract_and_the_facade_read_the_same_build）；REUSE `tests/unit/domain.meta.test.mjs`（facade 元测试：utcOffset / deadline comparisons）；REUSE `tests/unit/run.mjs`（staleness gate——陈旧产物 fail closed） | REUSE | node --test tests/unit/guide-contract.test.mjs |
...
| VERIFICATION-SYSTEM-010 | REUSE `tests/unit/verify/g4r-freeze.test.mjs`（G4R_FREEZE_case_ceiling_may_only_decrease / G4R_FREEZE_timeout_ceiling_rejects_inflation） | REUSE | node --test tests/unit/verify/g4r-freeze.test.mjs |
| VERIFICATION-SYSTEM-011 | REUSE `tests/unit/run.mjs`（--coverage 阈值门禁，run-inner COVERAGE_LINE_THRESHOLD）；SPLIT@cutover：覆盖门禁拆分计划见 HOW.md | REUSE | node tests/unit/run.mjs --coverage |
...
- `g4r-freeze.mjs`（+ 其回归）是迁移期 One World ratchet（PROOF-MAP 标 DELETE）；cutover
  后由永久 One World 门承接，断言强度不缩水（只收紧）。
- 覆盖门禁（VERIFY-011）：当前载体 `tests/unit/run.mjs --coverage`（MECHANISM）；cutover
  后按「分母完整 + 阈值即红 + 无豁免」重写为可独立单跑的 oracle 或包内测试。
- **PROOF-MAP 归属分歧（cutover 按 assertion 复核）**：`tests/unit/verdict-feed.test.mjs`
  （VERIFY-004 watchdog 分类器）、`tests/unit/domain.meta.test.mjs`（VERIFY-008 facade
  元测试）、`tests/unit/guide-contract.test.mjs`（VERIFY-005/008 契约面）按内容属
