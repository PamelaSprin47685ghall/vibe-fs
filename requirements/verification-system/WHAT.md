...
**规范陈述**：Gate 只阻断语义违规，不阻断尺寸；文件长度既不硬阻断也不告警，也不做
任何机械行数检查（含 advisory——2026-08-15 起 Kolmogorov size advisory 及其 baseline
一并删除）。行数是症状不是病因；用行数代理会产生反向激励（机械拆分 `*Helpers.fs` /
`*Fields.fs` / `*Core.fs`）。机械后缀命名仍需显式 allowlist——这才是防拆分逃逸的真门禁。

**含义/动机**：真正要禁的是样板、框架礼仪、错误抽象、重复知识——由语义门禁直接命中。
advisory 本身没有失败价值：不判红的「检查」只会让人以为有保护，是假门；删除机械行数
检查，把「大小是否合理」留给语义 review 判断。
