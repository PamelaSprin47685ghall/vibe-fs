...
下列任何现象的出现均视为严重门禁退化，等同于发布阻断：
- 将 wall-clock 绝对总超时作为唯一挂死判据。
- 允许原始底层传输流量或连接噪声续期看门狗。
- 允许背景车道的进展续期看门狗。
- 移除看门狗的诊断状态转储，仅保留粗暴的退出码。
- 让看门狗计时器持有事件循环，导致正常结束仍需等待静默窗口耗尽。
- 存在无任何超时或看门狗保护的裸等待时间窗。
- 声明了断言心跳机制但未在实际代码中接线生效。
- 使用固定 sleep 代替明确的因果就绪同步。
- 将就绪超时或就绪前异常退出误判为测试通过。
- 门禁演变为「最多尝试 N 次」或「重试直到碰巧通过」。
- 静态门禁的文件路径判据指向不存在的目录或文件。
- 随意延长静默窗口或测试超时阈值以掩盖底层并发竞态。
