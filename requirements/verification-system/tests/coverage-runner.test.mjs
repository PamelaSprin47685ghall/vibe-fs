// requirements/verification-system/tests/coverage-runner.test.mjs — CLEAN-E2E §11.7
// 9 test cases covering all requirements for dedicated c8 coverage runner:
// 1. 一份被测文件、一份未导入文件: 两者都在分母，未导入文件为零
// 2. 未导入文件顶层抛错: 不执行该顶层代码，仍作为未覆盖文件出现
// 3. 同名文件分处两个目录: 报表两条独立记录
// 4. 主测试与一个正常完成的子进程覆盖不同分支: 合并后的同一文件包含两条实际路径
// 5. 测试断言失败: 命令失败，即使已写出报告
// 6. 内层 runner 在测试启动前崩溃: 明确 infrastructure error，不接受全零报告当成功
// 7. raw 数据为空、JSON 损坏或错误 run-id: 命令失败，不读取历史数据兜底
// 8. 分母与 build receipt 对等 (source map / dist mismatch)
// 9. 测试运行期间生产文件被改写: INPUT_CHANGED，不授予本次报告当前性
...
import { runCoverage } from '../../../scripts/coverage.mjs'
import {
