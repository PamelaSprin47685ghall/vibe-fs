/**
 * manager-tool-surface-canary-plugin.mjs — test-only OpenCode plugin wrapper.
 *
...
export const managerToolSurfaceCanaryPluginPath = fileURLToPath(import.meta.url);
...
const writeJson = (name, value) => {
  if (!directory) throw new Error('manager tool surface canary directory unset');
...
export const collectManagerProviderToolEvidence = (scenario, opts = {}) => {
  ...
  const assumeAdvertised = unionTools.includes('assume');
  ...
  return {
    ...
    unionTools,
    assumeAdvertised,
    // Recorded, never asserted: the retired ledger tool must be absent, and the
    // canary below proves it rather than trusting this flag.
    todowriteAdvertised,
 ...
 * Manager must advertise the production `assume` cognitive surface and must NOT
 * advertise the retired `todowrite` ledger; a missing surface, a resurrected
 * legacy tool, alias drift, or incomplete carrier evidence all fail closed.
 *
 * @param {string} dir
 * @param {{ managerProviderWire?: ReturnType<typeof collectManagerProviderToolEvidence> | null, xTraceParts?: object[] }} [opts]
 */
export const assertManagerToolSurfaceCanariesAEGH = (dir, opts = {}) => {
  const managerProviderWire = opts.managerProviderWire ?? null;
  if (!managerProviderWire || managerProviderWire.requestCount === 0) {
    throw new Error('MANAGER_TOOL_SURFACE_CANARY: missing Manager provider-wire evidence');
  }
  if (!managerProviderWire.assumeAdvertised) {
    throw new Error(
      `MANAGER_TOOL_SURFACE_CANARY: Manager provider wire must advertise the production assume cognitive surface: ${JSON.stringify(managerProviderWire)}`,
    );
  }
  if (managerProviderWire.todowriteAdvertised) {
    throw new Error(
      `MANAGER_TOOL_SURFACE_CANARY: Manager provider wire must not advertise the retired todowrite ledger: ${JSON.stringify(managerProviderWire)}`,
    );
  }
  for (const required of ['fork', 'horizon', 'join', 'resume', 'assume', 'suicide']) {
    if (!managerProviderWire.unionTools.includes(required)) {
      throw new Error(`MANAGER_TOOL_SURFACE_CANARY: missing Manager tool ${required}`);
    }
  }
  ...
  if (failures.length > 0) {
    throw new Error(
      `manager tool surface canary recorded failure artifacts: ${JSON.stringify(failures[0])}`,
    );
  }
...
export default {
  id: 'wanxiangshu-e2e-manager-tool-surface-canary',
  async server(input) {
    if (!loadedAsOuterPlugin || !production) {
      throw new Error('manager tool surface canary requires directory and production plugin path');
    }
