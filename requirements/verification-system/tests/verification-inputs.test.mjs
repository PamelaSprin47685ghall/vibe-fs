...
test('collectVerificationInputs collects expected relative paths and diff detects mutations', () => {
...
test('verify detects mid-flight inputs change via runStep perturbation and halts with fail', async () => {
  const fixture = setupFixtureRepo()
...
test('verify with isolated logDirectory creates run dir and latest link without touching repo root', async () => {
  const fixture = setupFixtureRepo()
