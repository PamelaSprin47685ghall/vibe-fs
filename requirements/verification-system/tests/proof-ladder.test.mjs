...
test('VERIFY_001_format_build_test_ladder_pins_the_five_layers_in_order', () => {
...
test('VERIFY_001_l4_has_exactly_one_e2e_entry_in_the_ladder', () => {
  const { scripts } = JSON.parse(read('package.json'))
  const command = scripts['format-build-test']
...
test('VERIFY_001_every_ladder_step_target_exists', () => {
...
test('VERIFY_001_checks_directory_is_wired_plus_allowlist_only', () => {
  const checkSource = read('scripts/check.mjs')
  const wired = wiredGates(checkSource)
...
test('VERIFY_001_check_mjs_propagates_nonzero_fail_closed', () => {
  const checkSource = read('scripts/check.mjs')
  assert.match(
    checkSource,
...
test('VERIFY_005_fail_closed_propagates_a_failing_gate_exit_code', () => {
...
test('VERIFY_005_fail_closed_treats_an_unspawnable_gate_as_failure', () => {
