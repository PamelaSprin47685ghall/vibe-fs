...
import * as policy from '../../../dist/Context/Companion/CompressionSurface.js'
...
test('WHAT[CONTEXT-COMPRESSION-002] HOST_006_prevention_layer_names_every_setting_that_must_be_off', () => {
  ...
  assert.deepEqual(policy.compactionSettingPaths, ['compaction.auto', 'compaction.prune', 'compaction.autocontinue'])

  for (const setting of policy.compactionSettings) {
...
test('WHAT[CONTEXT-COMPRESSION-002] COMPANION_009_prune_is_listed_and_says_why_containment_cannot_save_it', () => {
  ...
  const prune = policy.compactionSettings.find((s) => s.path === 'compaction.prune')
...
test('WHAT[CONTEXT-COMPRESSION-002] HOST_006_autocontinue_is_answered_false_rather_than_left_to_the_default', () => {
  ...
  assert.equal(policy.compactionAutoContinueEnabled, false)
})

test('WHAT[CONTEXT-COMPRESSION-002] HOST_006_a_setting_that_cannot_be_written_fails_startup_with_its_reason', () => {
  const verdict = policy.compactionJudgeFirstTurn({ unavailable: 'compaction.auto', session: 'ses_x', pseudoRuns: 0 })
...
test('WHAT[CONTEXT-COMPRESSION-002] HOST_006_startup_probe_passes_when_settings_are_off_and_the_first_turn_is_clean', () => {
  const verdict = policy.compactionJudgeFirstTurn({ unavailable: undefined, session: 'ses_x', pseudoRuns: 0 })
...
test('WHAT[CONTEXT-COMPRESSION-002] HOST_006_a_compaction_on_the_first_turn_means_a_second_implementation', () => {
  ...
  const verdict = policy.compactionJudgeFirstTurn({ unavailable: undefined, session: 'ses_probe', pseudoRuns: 1 })
...
test('WHAT[CONTEXT-COMPRESSION-002] HOST_006_the_setting_check_takes_precedence_over_the_turn_observation', () => {
  ...
  const verdict = policy.compactionJudgeFirstTurn({ unavailable: 'compaction.auto', session: 'ses_x', pseudoRuns: 3 })
...
test('WHAT[CONTEXT-COMPRESSION-002] HOST_006_containment_keys_on_the_folded_predicate_not_raw_fields', () => {
  ...
  assert.equal(policy.compactionIsContainable(true), true)
  assert.equal(policy.compactionIsContainable(false), false)
})
...
test('WHAT[CONTEXT-COMPRESSION-005] CTX_005_containment_does_not_discriminate_by_source', () => {
  ...
  assert.equal(policy.compactionIsContainable.length, 1)
})
...
test('WHAT[CONTEXT-COMPRESSION-002] HOST_006_the_newest_unhandled_compaction_is_the_one_to_reanchor', () => {
  ...
  assert.equal(policy.compactionNextReanchor(['msg_c1', 'msg_c2', 'msg_c3'], []), 'msg_c3')
})

test('WHAT[CONTEXT-COMPRESSION-002] HOST_006_at_most_one_reanchor_is_emitted_per_observation', () => {
  const single = policy.compactionNextReanchor(['msg_c1', 'msg_c2', 'msg_c3', 'msg_c4'], [])
...
test('WHAT[CONTEXT-COMPRESSION-002] HOST_006_a_new_compaction_after_a_handled_one_is_still_caught', () => {
  ...
  assert.equal(policy.compactionNextReanchor(['msg_c1', 'msg_c2'], ['msg_c1']), 'msg_c2')
})
