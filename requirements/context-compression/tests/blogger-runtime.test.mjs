...
// Busy = host HasFlight; material route also preserves one durable open producer.
...
test('WHAT[CONTEXT-COMPRESSION-018] ENFORCER_047_idle_plus_material_starts', () => {
  assert.equal(rt.decideMaterial(false, false, false, main()), 'Start')
})
...
test('WHAT[CONTEXT-COMPRESSION-018] ENFORCER_047_inflight_plus_material_skips_without_queue', () => {
  ...
  assert.equal(rt.decideMaterial(true, false, true, main2()), 'Skip')
})

test('WHAT[CONTEXT-COMPRESSION-018] ENFORCER_047_open_producer_between_steps_stages_material', () => {
  assert.equal(rt.decideMaterial(true, false, false, main2()), 'Offer')
})
...
test('WHAT[CONTEXT-COMPRESSION-018] ENFORCER_047_idle_plus_parked_waiter_offers', () => {
  assert.equal(rt.decideMaterial(false, true, false, main2()), 'Offer')
})
