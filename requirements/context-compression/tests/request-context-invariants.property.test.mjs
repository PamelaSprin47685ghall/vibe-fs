// WHAT[CONTEXT-COMPRESSION-027] Blogger request construction authority (W6 / proposal §4.2).
//
// Every successful BloggerMain/Squash construction satisfies the §4.2 invariants
// (nextIngested > prevIngested, DeltaDigest = SHA256(Toml), coherent epochs,
// squash count = carried digests) with coverage/epoch binding; corrupt /
// version-mismatched / I-O-unavailable durable input each yields its own
// distinct typed rejection; an old-epoch staged request cannot claim current
// commit authority.
//
// The test calls the compiled production constructor (`BloggerRequestMaterial`
// via `CompanionRuntimeSurface.contextOfJs`) and the compiled production
// recovery path (`EnforcerFrameRecovery.tryReloadRequestContextDetailed` and
// the `resolveCycleContext` fallback contract). Fixed seeds; no sleeps — the
// journal is driven through the real compiled surfaces.
...
// ── honest descriptor builders (what a correct owner would stage) ───────────
//
// The descriptor only carries domain inputs. The canonical identities stay
// production-computed: `contextOfJs` re-derives them inside the owner
// boundary, so the oracle below re-derives the same values independently
// with node:crypto (same algorithm, independent code path).
