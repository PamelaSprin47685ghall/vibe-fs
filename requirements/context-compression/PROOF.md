...
| CONTEXT-COMPRESSION-009（候选未提交不是事实） | `tests/probe-selection.test.mjs`：`CTX_010_the_probe_records_the_epoch_it_was_built_from`；`requirements/prefix-stability/tests/prefix-epoch.test.mjs`（prefix-stability 包）`CTX_010_a_failed_probe_leaves_no_trace_to_undo`；REUSE：`tests/unit/context/attempt-plan.test.mjs` `CTX_010_a_discarded_probe_leaves_the_committed_epoch_in_place` | MOVE + REUSE | `node --test requirements/context-compression/tests/probe-selection.test.mjs` |
