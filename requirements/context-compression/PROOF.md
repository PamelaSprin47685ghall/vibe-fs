...
| CONTEXT-COMPRESSION-013（诊断不是控制输入） | `tests/ctx014.test.mjs`：`CTX_014_forbidden_field_names_never_appear_in_production_source`、`CTX_014_diagnostic_emit_accepts_only_whitelisted_fields`、`CTX_014_diagnostic_emit_is_silent_by_default_and_observable_on_demand` | MOVE | `node --test requirements/context-compression/tests/ctx014.test.mjs` |
...
| CONTEXT-COMPRESSION-017（Opening floor） | `tests/ctx-opening-floor.test.mjs`（NEW）：`CTX_016_pre_t1_floor_is_the_xtrace_head_not_an_activation_cursor`、`CTX_016_work_activated_is_inert_and_does_not_move_the_floor`、`CTX_016_blogger_effective_start_is_max_of_record_coverage_and_floor`；跨包：`tests/x-trace-capture-hardening.test.mjs`（semantic-trace 包）`COMPANION_003_capture_opening_takes_authoritative_requirements` | NEW + 跨包 | `node --test requirements/context-compression/tests/ctx-opening-floor.test.mjs` |

## 2. 本包拥有的测试文件（全部单跑绿）

| 文件 | 来源 | 状态 |
|---|---|---|
| `tests/blog-projection.test.mjs` | MOVE `tests/unit/context/blog-projection.test.mjs` | 已跑绿（20 pass） |
| `tests/companion-projection.test.mjs` | MOVE `tests/unit/context/companion-projection.test.mjs` | 已跑绿（27 pass） |
| `tests/blogger-delta.test.mjs` | MOVE `tests/unit/context/blogger-delta.test.mjs` | 已跑绿（19 pass） |
| `tests/probe-selection.test.mjs` | MOVE `tests/unit/context/probe-selection.test.mjs` | 已跑绿（13 pass） |
| `tests/recovery-slot.test.mjs` | MOVE `tests/unit/context/recovery-slot.test.mjs` | 已跑绿（20 pass） |
| `tests/host-compaction-policy.test.mjs` | MOVE `tests/unit/context/host-compaction-policy.test.mjs` | 已跑绿（14 pass） |
| `tests/ctx014.test.mjs` | MOVE `tests/unit/context/ctx014.test.mjs` | 已跑绿（7 pass） |
| `tests/terminal-validity.test.mjs` | MOVE `tests/unit/context/terminal-validity.test.mjs` | 已跑绿（6 pass） |
