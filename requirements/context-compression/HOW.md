...
| CONTEXT-COMPRESSION-019（Y 后辅助注入 coverage 归零，历史 occurrence 保留） | `tests/injected-context-reanchor.test.mjs`：ordinary/Cursor pair old synthetic 不 replay + 新 occurrence fresh call id；ordinary/Cursor requirement old reads 不 replay + 同 digest 可重新 request。tip coverage 的同构语义由 GD-005 独立拥有。 | NEW | `node --test requirements/context-compression/tests/injected-context-reanchor.test.mjs` |
