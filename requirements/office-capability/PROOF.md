...
| OFF-005 | `tests/office-capability-integrity.test.mjs` `OFF_005_each_office_consequence_hits_manager_law_and_fork_description_in_both_locales` + `requirements/office-capability/tests/office-capability-gate.test.mjs` `gate_f_catalog_names_five_forkable_offices` / `gate_f_office_capability_fixture_is_green` / `gate_f_missing_manager_coder_projection_is_red` | NEW + REUSE（SPLIT：language-parity-gate = provider-language 结构 parity + office-capability 语义；Gate F 断言归本包，SPLIT@cutover） | 分别 `node --test` |
| OFF-006 | `tests/office-capability-integrity.test.mjs` `OFF_006_offices_are_not_interchangeable_general_purpose_agents` + `requirements/office-capability/tests/eval/provider-office-boundary/office-boundary-eval.test.mjs`（4 oracle 全绿） | NEW + REUSE（SPLIT：eval family = office-capability + participant-horizon；office oracle 归本包） | `node --test requirements/office-capability/tests/eval/provider-office-boundary/office-boundary-eval.test.mjs` |
| OFF-007 | `scripts/checks/semantic-anchors.mjs` `ROLE_SEMANTIC_ANCHORS.manager.no-personal-repository-witness`（经 `requirements/office-capability/tests/office-capability-gate.test.mjs` `gate_c_semantic_anchor_parity_*` 双语文档命中） | REUSE（锚点 id 本包拥有；命中验证共享 Gate C 机制） | `node --test requirements/office-capability/tests/office-capability-gate.test.mjs` |
...
| OFF-014 | `tests/office-capability-integrity.test.mjs`（inquiry consequence 双语文档命中）+ `requirements/capability-enforcement/tests/inquiry-permissions.test.mjs`（Inquiry 工具面，REUSE） | NEW + REUSE | 分别 `node --test` |
