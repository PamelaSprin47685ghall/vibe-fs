...
“尚未见过”不是不变量。协议修复耗尽、旧回调、传输拒绝与存储不可用都能正常到达；把它们合并成 fatal 会把一个请求的失败扩大成所有会话的进程死亡。保留的致命入口必须有明确前提和生产代码证据，不能由测试另写一套决策再证明自己。结算后的 fatal 与先 fatal 后结算也不等价：真正退出的进程不会执行后续语句。
