// EXECFAIL-010 gate fixture tests: prove the fatal-inventory scanner itself.
//
// The gate must (a) fail on an unregistered fatal/trip/ReportFatalDiagnostic
// call site, including one-hop aliases; (b) fail when an inventory row's
// source symbol moved away (proof stale); (c) fail when a FixedWithRegression
// row names a test that does not exist. Positive case: the exact current
// inventory + current tree scan GREEN.
//
// These fixtures exercise scanFatalInventory/scanFatalSites directly (pure,
// in-memory) — not the on-disk tree — so they stay deterministic while
// siblings edit production files around them.
...
// ── gate (a): unregistered site fails ────────────────────────────────────
...
// ── gate (c): missing regression test fails ──────────────────────────────
...
// ── gate positive: covered tree is green ─────────────────────────────────
