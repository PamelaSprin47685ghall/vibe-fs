...
| 命题 | 唯一落点测试 |
|---|---|
| EXECFAIL-001 | `requirements/execution-failure-policy/tests/policy.test.mjs::WHAT[EXECFAIL-001] observes every closed failure and persistence commitment variant`；`requirements/execution-failure-policy/tests/host-codec.test.mjs::WHAT[EXECFAIL-001] Host adapter returns closed typed failures from structural evidence`；`requirements/execution-failure-policy/tests/provider-mapping.test.mjs::WHAT[EXECFAIL-001] adapter returns typed ProviderTransient`；`requirements/execution-failure-policy/tests/provider-mapping.test.mjs::WHAT[EXECFAIL-001] provider adapter preserves permanent kind and exact attempt identity`；`requirements/execution-failure-policy/tests/provider-mapping.test.mjs::WHAT[EXECFAIL-001] first-token evidence maps interruption without transparent retry classification` |
