...
- **残留与原因**：`delegation-fold` 仍直接引用 `composition-durable-projection`，因为它就是 durable aggregate fold，需要 `AgentProjection.update` 与 `FoldRejection`（27 处符号使用）；它还调用 authority 域的 `PromptAuthorityRun.closeCompletedAgentOwnerChildWork`（13 处）。这两条是「aggregate fold 按域切片、各自拥有迁移」的未完成工作，不是引用改名能解决的。- DELEG-029 关于 PTY adapter（不得读取 `HostForkRuntime`、Fork runtime、Process implementation、Gate、`Dictionary`、registry、TCS）的一半仍未实现：`Execution/Delegation/Fork/Host/Pty.fs` 目前扩展 `HostForkRuntime` 并使用 `Dictionary`，该半句没有对应证明，缺口见本文件 GAP 段。
