...
- `DELEG-029`（CLOSED）：PTY 边界半句（PTY adapter 不得读 `HostForkRuntime`/`Dictionary` 等）已由源码重构闭合。新增 delegation-owned 纯契约分片 `delegation-pty-port`（定义 `DelegationPtyCapability` 窄能力），PTY 物理执行收回为 `HostForkRuntime` 内部方法并暴露 `PtyCapability` 属性；`delegation-pty-adapter` 剥离对 `delegation-host-adapter` 与 `execution-delegation-fork-runtime` 的依赖并移除 `HostForkRuntime` 扩展方法；PtyTool/JoinTool 仅消费窄能力。测试落点见 `requirements/delegation/tests/029.test.mjs`（三层静态门禁断言）及 DELEG-028 ratchet 调整。
