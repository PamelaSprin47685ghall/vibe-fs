...
| DELEG-005 机器拓扑不进委托面 | `requirements/delegation/tests/fork-tool.test.mjs`（`FORK_deep_devops_returns_after_enqueue_even_when_host_prompt_promise_never_settles`：真实 fork 在 Host run Promise 永不 settle 时仍返回；`FORK_calling_creates_machine_agent_but_returns_only_byname`、`FORK_unknown_byname_does_not_echo_internal_identity`）；交叉 `requirements/dispatch-protocol/tests/fire-and-forget.test.mjs::PROMPT_007_detached_returns_even_when_session_send_task_never_settles`；`requirements/delegation/tests/join-v2-wire.test.mjs` | REUSE | `node --test requirements/delegation/tests/fork-tool.test.mjs` |
