...
| DELEG-002 calling 只差 persona/depth 不差 authority | anchor `persona-not-authority`（fork 组，双语言命中）；`tests/unit/agent/catalog.test.mjs`（`participant-identity` 交叉，persona/binding 分离） | REUSE | `node scripts/checks/semantic-anchors.mjs` |
...
| 源 | 目标 | 结果 |
|----|------|------|
| `tests/unit/execution/fork-child-payload.test.mjs` | `requirements/delegation/tests/fork-child-payload.test.mjs` | `node --test` 绿（21/21；import 深度已适配） |
