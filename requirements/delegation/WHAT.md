...
## [001] 委托 = 语义 charge + entitled office + 逻辑 owner + bounded 返回后果
...
## [002] 同一 Office 的 authority 不变，calling 别名不具有独立路由权威

属于同一 Office 的不同 calling 别名（支持 fast 与 deep 别名，仅作为向后兼容参数保留，不具有独立路由权威与额外权限）不改变该 Office 的权能与权限；每个 Role 对应单一确定 Persona，不以 calling 别名扩权或形成多重档位。

## [003] 独立 road 与 same-road continuation 硬区分，各占独立工具
...
## [006] fork 成功仅 Byname 承接 charge；续做沿用已绑定 binding
...
## [009] serialization key = immediate caller ReuseScope；同 key 至多一个 active batch

同步委托的串行化作用域为直接调用方的 ReuseScope。同一 key 下同时至多存在一个活跃批次；在前一批次完成前到达的新请求直接拒绝。不同层级的嵌套委托各占本层 scope，互不阻塞。

## [010] delegate 模型绑定由系统调度与有效配置决定，禁止自选 target

委派绑定的模型与执行配置由系统调度统一映射（支持 tier 及 fast/deep 别名，仅作为向下兼容参数保留，不赋予额外权限与模型选择权），模型不可自选目标 target。复用既有 child 时严格沿用其已绑定 managed agent。

## [011] 无 return 通道；ordinary completion 结束 batch
...
## [025] work unit completion 由 causal identity 决定，不由订阅时刻决定
...
## [026] effect truth 只能沿 direct CE 单向前进
...
## [027] 新 assignment 不得伪装成 busy nudge
...
## [028] Delegation contract/runtime 编译闭包必须按 effect 边界分层
...
`Delegation.Contract` 的 transitive production `.fs` 不得超过 100。fold/runtime/sync/fork localities 的 transitive production `.fs` 以 185 为 target；adapter locality（`Delegation.Host.Adapter`、`Delegation.Pty.Adapter`）按其 charter 必须组装 durable spine、wait、host signal 与 managed-agent 物理 vocabularies，closure 由此被上游共享 spine 主导，其闭包以全仓 production `.fs` 的 60% 为 hard ceiling（超过即 full fallback，不伪装为 focused compile），实测规模作为回归 ratchet——任何增长必须伴随本条修订；fold/runtime localities 的闭包由 delegation 自有 contract 分片与少量 foundation/identity vocabulary 构成。2026-09-25：adapter locality 随 durable spine 装配的 `AgentFact.Cognition` 一并纳入认知 runtime surface——新增 `Participant/Cognition/RuntimeSurface.fs` 经 `composition-durable-fact → participant-cognition-workspace` 进入 `delegation-host-adapter` 闭包，该 locality 的 ratchet 由 306 增至 307（实测 307/307，仍远低于 60% hard ceiling）；此增长是结构演进的显式记账，非例外豁免。owner compile 必须把选定 locality 的 transitive ProjectReference 扁平化为唯一编译计划，单次调用的 Compile Include 必须保持拓扑偏序；`build.mjs` 严禁退回目录通配。

2026-09-25（记账二）：durable spine 与 prefix 分片把 `context-prefix-epoch` 词表改为显式 ProjectReference 声明——`Persistence/Journal/{Envelope,FactCodec,PromptFactCodec,EventStoreJournalCodec,ProjectionState}`、`Composition/Durable/{Fold,Projection,AgentJournalPortAdapter}` 与 `Context/Prefix/{Wire,WirePort}` 直接消费 `ActivePrefixEpoch`／`PhaseWindow`／`PrefixEpochProjection`。该 shard 携带 `Context/Prefix/{ProbeSelection,PhaseWindow,Projection,Epoch,Surface}.fs` 五源，经共享 spine 进入 `delegation-host-adapter` 闭包，ratchet 由 307 增至 312（实测 312/312，仍远低于 60% hard ceiling）；这是以前仅由 aggregate 隐含可达的词表被显式声明的结果，不是新的耦合方向。

## [029] Delegation runtime、PTY 与 durable outer routing 必须单向注入
...
## [030] delegation fatal escalation 只消费 mandatory injected fuse
...
## [031] reusable completion checkpoint 以 closed settlement 收口，不以 string 失败
...
## [032] Engineer 完成即返回，禁止跨角色与向后委托差遣
