...
`Delegation.Contract` 的 transitive production `.fs` 不得超过 100。fold/runtime/sync/fork localities 的 transitive production `.fs` 以 185 为 target；adapter locality（`Delegation.Host.Adapter`、`Delegation.Pty.Adapter`）按其 charter 必须组装 durable spine、wait、host signal 与 managed-agent 物理 vocabularies，closure 由此被上游共享 spine 主导，其闭包以全仓 production `.fs` 的 60% 为 hard ceiling（超过即 full fallback，不伪装为 focused compile），实测规模（2026-09-12：host 289、pty 290、recovery 45；较 278/279 的增长全部来自本轮新增的窄 contract 端口源文件与真实声明补齐；orchestrator 适配器拆分为 `change-orchestrator-port-adapter` 分片后不再经共享 hub 拉入 Change 子树）作为回归 ratchet——任何增长必须伴随本条修订；fold/runtime localities 的闭包由 delegation 自有 contract 分片与少量 foundation/identity vocabulary 构成，recovery 已收缩到 47，185 仍是其上限目标而非现状。composition locality 豁免规模预算。owner compile 必须把选定 locality 的 ProjectReference closure 合并为 aggregate-order、零 ProjectReference 的单一 flat project，并只启动一次 Fable。
