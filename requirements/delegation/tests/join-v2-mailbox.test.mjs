...
const source = readFileSync(new URL('../../../src/Wanxiangshu/Execution/Session/Wait/CompletionMailbox.fs', import.meta.url), 'utf8')
...
test('WHAT[DELEG-019] JOIN_MAILBOX_operator_abort_is_distinct', () => {
  ...
  assert.match(source, /OperatorAbort/)
})
test('WHAT[DELEG-019] JOIN_MAILBOX_user_message_signal_wakes_current_join', () => {
  ...
  assert.match(source, /UserMessageArrived/)
})
