...
test('WHAT[DELEG-013] JOIN_V2_failed_agent_is_natural_language', () => {
  const wire = join.renderBatch('english', [{ kind: 'failed', agentId: 'a2', agentName: 'deep-reviewer', role: 'Reviewer', runId: 'run-a2', code: 'E', message: 'failed' }])
...
test('WHAT[DELEG-016] JOIN_V2_malformed_role_run_or_kind_is_rejected_without_success_wire', () => {
  const base = { kind: 'completed', agentId: 'a1', agentName: 'fast-coder', role: 'Coder', runId: 'run-a1', workRecord: 'ok' }
  ...
  assert.equal(join.renderBatch('english', [{ kind: 'abandoned', agentId: 'a1', agentName: 'fast-coder', role: 'UnknownRole', reason: 'gone' }]), '')
