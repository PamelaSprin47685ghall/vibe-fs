...
- **fork-manager DTO**（`archive/changes/completed/orchestrator-e2e-timeout.md` 考古、`archive/docs/why/orchestrator.md` 备选节）：
  旧 `agent=fast-manager|job_id` + `worktree` + `reused=true` 把机器拓扑当世界语言。拒因：双轨期间旧面与新面并存，
  测试与 prompt 永远对齐泄漏面；一次断，旧符号删。
- **`return` 双 await**（`archive/docs/why/execution.md` SyncDelegate 节；`archive/changes/completed/universal.md` §14–16）：
  ...
  bounded WorkRecord。
- **Student–Teacher**（`archive/changes/completed/universal.md`、`ce-student-teacher-collapse.md`）：
  生命周期 cell + handoff + pending slot 合并把调用栈位置藏进可变字段，terminal handler 必须猜下一步。
  已 clean-break 删除；SyncDelegate 是后继，不继承该拓扑。
- **microtask 猜批次**（`archive/docs/why/execution.md` Sync batch 备选节）：用「先 drain 一次、创建 child 后再
...
- **NEEDHELP 当失败**（`archive/changes/completed/increase-strength.md` §3.1/§6）：把求助当 provider failure /
