...
| REVIEW-ASSURANCE-009（同 snapshot、排他 frontier、事件驱动、waiter fail-closed 恢复语义） | `tests/consumable-review.test.mjs` → 无 verdict + producer permanent loss 才 fail closed；`CompletedAwaitingJoin` 可继续 record-ready；**durable verdict 已存在时 Retired 也不得被误判 producer loss**；`REVIEW_014_only_..._consumable` 的 `ReviewerRecordFrontier` 断言 | NEW + REWRITE | 同上 |
