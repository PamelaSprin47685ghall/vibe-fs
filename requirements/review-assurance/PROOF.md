...
| REVIEW-ASSURANCE-008（VerdictKnown vs ConsumableReview 两段式、typed verdict identity 只来自积分 projection） | `tests/consumable-review.test.mjs` → `REVIEW_014_a_durable_verdict_alone_never_makes_the_review_consumable`、`REVIEW_014_only_todo_review_concluded_marks_the_review_consumable`、`REVIEW_017_process_verdict_identity_comes_from_the_integrated_projection_not_a_judge_tool_call_trace`（无 `judge tool_call` XTrace 仍以 projection 的 ProviderRun/ToolCallId Concluded） | NEW + REWRITE | `node --test requirements/review-assurance/tests/consumable-review.test.mjs` |
| REVIEW-ASSURANCE-009（同 snapshot、排他 frontier、事件驱动、waiter fail-closed 恢复语义） | `tests/consumable-review.test.mjs` → projection-verdict 回归同时要求同 snapshot LWR；无 verdict + producer permanent loss 才 fail closed；`CompletedAwaitingJoin` 可继续 record-ready；**durable verdict 已存在时 Retired 也不得被误判 producer loss**；`REVIEW_014_only_..._consumable` 的 `ReviewerRecordFrontier` 断言；源码结构为 sample revision → `tryConclude` → presence → `awaitChangeFrom sampledRevision` | NEW + REWRITE | 同上 |
