...
| DURABLE-EVENTS-021 | `event-store-append.test.mjs`：`semantic_failure_writes_cut_tail_reset_and_the_same_feature_can_succeed_next`（底层 cut/reset 可供下一进程恢复）+ `every_live_semantic_cut_boundary_trips_process_fatal_instead_of_returning_a_normal_error`（AgentJournal / Casebook / js transaction / Strength prepared+turn live boundaries 全部 process fatal）；`requirements/managed-session-lifecycle/tests/satellite-runtime.test.mjs::HOST_015_direct_companion_repoint_trips_process_fatal_on_semantic_cut`；`canonical-integrator.test.mjs` source gate | NEW + REUSE |
