...
| DURABLE-EVENTS-021 | `event-store-append.test.mjs::WHAT[DURABLE-EVENTS-021] semantic_failure_writes_cut_tail_reset_and_the_same_feature_can_succeed_next`（底层 EventStore：bad fact + ProjectionCutTail durable、typed cut receipt、storage 本身未 poison、reopen replay）；`requirements/managed-session-lifecycle/tests/satellite-runtime.test.mjs::HOST_015_direct_companion_repoint_trips_process_fatal_on_semantic_cut`（live AgentJournal cut receipt 先触发 `journal-semantic-cut` fatal；node:test 仅屏蔽物理 kill）；`canonical-integrator.test.mjs` source gate（PlanCut/ApplyCut + one full replay budget） | NEW + REUSE |
