...
test('WHAT[KNOWLEDGE-REUSE-006] CASE006_synthesis_refresh_publishes_refreshed_with_revised_a', async () => {
  ...
  try {
    ...
    assert.equal((await bookkeeper.refreshStale(handle, dir, 's-mech-1')).value, false)
    ...
    const refreshed = await bookkeeper.refreshStale(handle, dir, 's-mech-1')
...
test('WHAT[KNOWLEDGE-REUSE-006] CASE006_mechanical_refresh_no_case_is_noop', async () => {
  const { dir, handle, cleanup } = sandbox()
  try {
    const result = await bookkeeper.refreshStale(handle, dir, 'missing')
...
test('WHAT[KNOWLEDGE-REUSE-006] CASE006_mechanical_refresh_missing_file_still_publishes', async () => {
  ...
  try {
    ...
    bookkeeper.setSessionPort(port)
    const result = await bookkeeper.refreshStale(handle, dir, 's-gone')
    assert.equal(result.ok, true)
