...
Bookkeeper child 生命周期（`fast-bookkeeper`/`deep-bookkeeper`、Clerk/Curator Persona、InternalLeaf + Attached）由 Session/Process 侧持有（`docs/shape/casebook.md` Bookkeeper 身份边界）；本包只消费 `BookkeeperRequest` 契约（KNOWLEDGE-REUSE-006）。
...
### 被拒方案（详见 `changes/completed/perm-inspector.md`、`docs/why/casebook.md`）
...
- digest synthesizer：`changes/completed/perm-inspector.md` 曾规划 LearningCompiler / CaseSynthesizer；G6 Product Exit 明确 **synthesizer gone**——「不新建 LearningCompiler/CaseSynthesizer/StudentReplacement」是当前 absence（KNOWLEDGE-REUSE-006 边界），无合成器可迁移。
