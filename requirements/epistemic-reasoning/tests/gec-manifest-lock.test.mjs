...
test('WHAT[EPI-020] schema_hash_mismatch_rejects_observation_or_content_drift_passes_silently', async () => {
  ...
  drifted[1].observation.schema.hash = 'schema-hash-tampered';
  ...
  renamed[1].observation.schema.id = 'sphinx.probe.open/input@2';
...
test('WHAT[EPI-020] mid_run_plugin_swap_is_rejected_or_lock_is_advisory', async () => {
  ...
  swapped[1].observation.pluginLock = [manifest({ release: '9.9.9', abiHash: 'abi-canon-999' })];
  ...
  added[1].observation.pluginLock = [...lock, manifest({ id: 'late', release: '1.0.0', abiHash: 'abi-late-001' })];
