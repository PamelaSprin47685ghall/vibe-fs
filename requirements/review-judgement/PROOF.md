...
|---|---|---|---|
| REVIEW-JUDGEMENT-001（judge 工具形态：typed verdict、无描述字段、不 echo；Reviewer-facing provider 指令只命名 judge，禁止已删除的 verdict 工具名） | `requirements/review-judgement/tests/judge-tool-contract.test.mjs` → `REVIEW_001_*`（schema 只许 verdict / parse 精确 / spec 单参数 / 三类 Reviewer provider 指令双语均指向 judge / 回执不 echo） | NEW | `node --test requirements/review-judgement/tests/judge-tool-contract.test.mjs` |
