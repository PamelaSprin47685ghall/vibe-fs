...
- **production-bound verification surface**：`JudgeSurface.interruptAfterSubmittedJudgement` 只把 `JournalHandle`、当前 physical message、JS background scheduler 与 JS physical port 转为 production 所需类型，然后调用 `JudgeTool.interruptAfterSubmittedJudgement`。surface 不读取 projection、不判断 run/call、不追加 closure、不自行决定是否 interrupt。行为 proof 用真实 journal 同时放入 exact tool result 与 wrong-run/wrong-call decoy，捕获 `runBackground` work，在 production entry 返回后先断言物理 port 尚未调用，再显式运行 work。append failure 直接来自真实 writer，并证明 scheduler 未收到 work。
...
| REVIEW-JUDGEMENT-008 | `requirements/review-judgement/tests/process-review-judgement.test.mjs::WHAT[REVIEW-JUDGEMENT-008] REVIEW_013_a_single_verdict_never_becomes_a_confirmed_witness_by_itself`；`requirements/review-judgement/tests/process-review-judgement.test.mjs::WHAT[REVIEW-JUDGEMENT-008] REVIEW_013_wrong_run_and_call_decoys_cannot_authorize_terminal_interrupt`；`requirements/review-judgement/tests/process-review-judgement.test.mjs::WHAT[REVIEW-JUDGEMENT-008] REVIEW_013_exact_tool_result_closes_before_background_interrupt`；`requirements/review-judgement/tests/process-review-judgement.test.mjs::WHAT[REVIEW-JUDGEMENT-008] REVIEW_013_real_append_failure_never_schedules_interrupt` |
