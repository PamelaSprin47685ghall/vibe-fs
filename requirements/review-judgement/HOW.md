...
- `src/Wanxiangshu/Infrastructure/OpenCode/Tools/JudgeTool.fs`：
  ...
  - TodoProcessReview 成功回执走 `tool/judge/received` 文案（`Your judgment has been received, please conclude the conversation.` / `你的判断已被收下，请你结束对话。`），**不 echo verdict**。Finality first PERFECT 不是 terminal judgement delivery：`Challenge()` 只渲染 `resources/provider/review/challenge`，禁止再拼 `Path.Received`；REVISE / second judgement 才 Accept。再次调用 process judge 走 `tool/judge/already-judged`。
...
- `src/Wanxiangshu/Mission/Review/Barrier/Reverify.fs` 与 process `ReviewerWorkflow` 是两条独立 F# CE：前者拥有 Finality challenge / normal-terminal→nudge / next judgement；后者拥有 process 一次 judge→record-ready→terminal。共享 `ReviewJudgement` / inbox 是物理数据边界，不共享 lifecycle control flow。
...
| REVIEW-JUDGEMENT-008（过程评审一次 durable judge 即 terminal；Finality first PERFECT 只 challenge；两条 CE 不合并） | `requirements/review-judgement/tests/process-review-judgement.test.mjs` 全部 `REVIEW_013_*`；`requirements/review-judgement/tests/judge-tool-contract.test.mjs` → `REVIEW_013_process_receipt_and_finality_challenge_have_disjoint_terminal_semantics`；`requirements/review-judgement/tests/verdict-tool.test.mjs` → process request dedupe cases | NEW | `node --test requirements/review-judgement/tests/process-review-judgement.test.mjs requirements/review-judgement/tests/judge-tool-contract.test.mjs requirements/review-judgement/tests/verdict-tool.test.mjs` |
