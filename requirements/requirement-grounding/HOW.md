...
- **解析逻辑**：`requirements/<package>/**` 内部路径无条件触发本包 self coverage；包外路径按 `<package>/APPLIES-TO` 声明的 glob 规则求值。`GroundingCatalog.matches` 直接调用既有 `JsGlobFs.matchesPathPattern` typed 合同，非法模式仍为 `invalid APPLIES-TO pattern` 错误；model 分片静态引用 `repository-programming-js-capability`，无动态 GlobFs 加载、无编译器 union 数值解码。
...
- **js-* effect**：transaction 在 preflight 后把显式 read set 与完整 mutation effect set 一并交给同一 observation port；该 port 的错误被吞掉，commit 资格只由 repository-programming 自身事务规则决定。preflight→observation→fresh preflight 的冲突闭包由 `REPOSITORY-PROGRAMMING-014` 拥有；本包只证明完整 effect set 被观察且 observation 失败不阻断合法提交。语义测试面（`RequirementGroundingRepositorySurface`）直接调用既有 `JsGeneratorSurface.typedRole("Coder", "en")` 与 `JsToolWorkflow.runWithFileAccessObservation` typed 合同并对 `JsToolOutcome`／`JsFailure` 做类型匹配，以既有失败词汇报告失败码；保留 2000ms 沙箱 deadline、60s epoch 超时、1MiB 输出上限、`None` 持久化、created 路径与 pending 包／材料语义，以及 Coder 面缺失与 observation 失败的错误语义。runtime 分片静态引用 `opencode-tools-filemutationtools` 与 `repository-programming-js-capability`，无动态加载、无编译器 union 数值解码、无缺失模块静默成功。失败码保持既有可观察词汇（`invalid_program`／`program_failed`／`program_timeout`，其余为 `unknown`），仅改由 `JsFailure` 类型匹配投影，不再经编译器 union 数值解码。
...
- 投影边界：`RequirementGroundingTransform` 直接消费 `PairProgrammingThoughtTransform` 的既有 typed 合同（provider 判别、placement Result 语义、后缀拼/剥），由 `Wanxiangshu.Owner.requirement-grounding.opencode-host-requirementgrounding-runtime.fsproj` 静态引用 `guidance-delivery.enforcer-guidance-tip` 声明；无动态模块加载、无缺失模块静默成功 fallback，placement 错误经 `projectOrTerminate` fail-closed。012 验证 journal 关闭/重开后仍重放冻结结果字节，并验证新 digest 只追加，不将同进程重开冒充 OS crash。生产 Surface 的临时 smoke 另验证无 transcript address 时明确拒绝、补齐地址后 pending grounding 仍可投影；它不证明 `projectOrTerminate` 的物理终止效果。删除仅匹配入口符号的源码伪证明，typed 依赖由真实 consumer focused Fable 编译验证。
...
| REQUIREMENT-GROUNDING-007 | `requirements/requirement-grounding/tests/opencode-gate.test.mjs::WHAT[REQUIREMENT-GROUNDING-007] ordinary providers replay anchored read call-result pairs while Cursor appends NUL-BOM result-only bytes after the pseudo-skill with stable source-path attributes`；`requirements/requirement-grounding/tests/opencode-gate.test.mjs::WHAT[REQUIREMENT-GROUNDING-007] grep match files do not trigger APPLIES-TO before an explicit read` |
...
| REQUIREMENT-GROUNDING-012 | `requirements/requirement-grounding/tests/grounding-delivery.test.mjs::WHAT[REQUIREMENT-GROUNDING-012] freezes result-only terminal bytes across restart replay while changed digests append without rewriting the provider prefix` |
