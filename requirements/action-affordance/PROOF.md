...
运行：`node --test requirements/action-affordance/tests/<file>`；全量由 `node tests/unit/run.mjs` 并入。
...
| 002 | `tests/action-affordance.test.mjs::AA_prompt_020_high_risk_verbs_have_semantic_anchor_catalog`；REUSE：`tests/unit/verify/language-parity-gate.test.mjs::gate_c_tool_description_anchor_catalog_requires_high_risk_verbs`（SPLIT@cutover：本包 + `provider-language` 机制面） | NEW + REUSE | 同上 / `node --test tests/unit/verify/language-parity-gate.test.mjs` |
...
| 008 | REUSE：`tests/unit/verify/tool-referential-integrity.test.mjs::gate_a_duplicate_tool_name_is_red`（same name = 唯一 semantic contract；schema/name 执行面 SPLIT@cutover → `capability-enforcement`）；`tests/action-affordance.test.mjs::AA_arch_006_007_distinct_semantics_have_distinct_names_and_contracts` | REUSE + NEW | `node --test tests/unit/verify/tool-referential-integrity.test.mjs` |
| 009 | `tests/action-affordance.test.mjs::AA_prompt_020_fork_contract_answers_whom_work_is_entrusted_to`（两个 calling 名只差 persona/深度）；REUSE：`tests/unit/verify/language-parity-gate.test.mjs::gate_f_catalog_names_five_forkable_offices`（canonical 面归 `office-capability`） | NEW + REUSE | 同上 |
...
tests/unit/verify/tool-referential-integrity.test.mjs
