...
历史教训（docs/why/prompt.md「Tool description：tooltip vs 调用合同」）：
...
| 症状 | 历史出处 |
|---|---|
| `inspect` 只说正向（"Ask an Inspector to establish a repository fact"），无因果只读/不修码负边界 → Coder 把修复写进 charge | docs/why/prompt.md；office-boundary eval case `coder-inspect-ownership` |
...
- **DRY 掉调用方合同。** 拒绝：`Single semantic ownership does not require single presentation.`
  机器已知的 office ontology 必须完整成为 participant 能够据以行动的世界知识（docs/why/prompt.md）。
