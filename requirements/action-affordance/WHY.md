...
- **线性上下文绑架非线性工作**：复杂写作、研究、设计与规划最终要输出 token 序列，但中间认知状态并不天然线性。若工具面只允许“继续说下去”或预先规定 `claim/evidence/draft` 等 ontology，参与者会把暂时结构误当最终结构，后到信息难以反向重排已有认识。`assume` 因此提供一个无预定义 schema 的持久 JSON 画板，并直接复用 jq 作为查询与重构语言；语义结构由参与者自己创造、推翻、重建，工具只拥有 JSON 持久化与 jq 执行边界。
