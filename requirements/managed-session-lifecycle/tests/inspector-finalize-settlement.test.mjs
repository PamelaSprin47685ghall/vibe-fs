// MANAGED-SESSION-021/022: inspector finalize settles to a closed union and
...
const sandbox = () => {
  const dir = mkdtempSync(join(tmpdir(), 'wxs-inspector-settle-'))
...
test('WHAT[MANAGED-SESSION-021] INSPECTOR_SETTLE_identity_retention_is_owner_driven_not_finally', async () => {
  ...
  assert.match(deletion, /InspectorFinalizeCommitment\.Finalized/)
  assert.match(deletion, /InspectorFinalizeCommitment\.NothingToFinalize/)
  assert.match(deletion, /InspectorFinalizeCommitment\.NotCommitted/)
  assert.match(deletion, /InspectorFinalizeCommitment\.Unknown/)
  assert.match(deletion, /InspectorFinalizeCommitment\.PhaseConflict/)
})
