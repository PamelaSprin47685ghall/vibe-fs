...
const hostMemberOf = (name) =>
  hostRuntimeModule[`HostForkRuntime__${name}`] ??
  Object.entries(hostRuntimeModule).find(([k]) => k.startsWith(`HostForkRuntime__${name}_`))?.[1]
...
const cancelAndDrain = hostMemberOf('CancelAndDrain')
...
test('MANAGED_SESSION_009_shutdown_cancel_drains_durable_abandon_before_return', async () => {
  ...
  const run = installRun(liveCtx.runtime, 'ag8', sessionId('ses_c8'), Role.Coder)
  adoptChild(liveCtx.runtime, 'ag8', sessionId('ses_c8'))

  await cancelAndDrain(liveCtx.runtime)

  const outcome = await run.Source.get_Task()
  assert.equal(caseOf(outcome), 'AgentAbandoned', 'parent cancel must settle the in-memory waiter as Abandoned')
  assert.equal(outcome.fields[1], 'ParentCancelled')
  const projection = agentJournal.handleProjection(liveCtx.journal, PARENT)
  const record = handleProjection.tryFind(handleId.agent('ag8'), projection)
  assert.equal(handleProjection.lifecycleOf(record), 'Abandoned', 'awaitable cancel must settle durable HandleAbandoned')
  assert.deepEqual(liveCtx.sessions.calls.filter(([name]) => name === 'AbortSession'), [['AbortSession', 'ses_c8']])
  liveCtx.cleanup()
})
