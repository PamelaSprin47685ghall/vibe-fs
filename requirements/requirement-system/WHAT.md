...
**规范陈述**：requirements/ 树只含 INDEX 列出的包目录；当前 52 个包每个都有
...
**边界**：包数量（当前 52）是设计期结果不是稳定 API；但「树 == 索引」的封闭性永久成立。
...
**含义/动机**：依赖骨架（`requirements/INDEX.md`，当前 130 edge / 0 cycle，按邻接 code block 逐项机器计数）是唯一来源；
