...
1. 存在无 owner、双重 owner 或互相矛盾的 normative 命题。
2. 规范命题脱离正式编号与落点，散落于非规范散文中。
3. `requirements/` 目录下出现未经索引登记的外来包，或包文档结构残缺。
4. 包声明了超出依赖骨架定义的非法依赖关系。
5. 已声明的 HOW 证明引用悬空、归属错误或伪造 active test 权威；迁移清单、合同册、proof-level 登记或文件存在性不得代替真实证据边。

缺少证明是未完成工作，不是已有测试失败。HOW 与 GAP 台账保留 OPEN/PARTIAL，追踪器继续列出缺口；已有测试全绿不等于规范已全部证明。把两者混成一个失败状态，会迫使开发者伪造证明或削弱断言来“变绿”。
