...
//      中当前全部 46 个包都有 requirements/<pkg>/{README,WHY,WHAT,HOW,PROOF}.md；
...
// 中途红有两种，均为预期：缺包（当前 46 包未全落地）与已落地但尚不完整
...
//   - `全量迁移状态`：当前 46 包 × 5 文档 + 无 INDEX 外目录。迁移中途必然红，
...
test('meta verifier: 全量迁移状态（INDEX 当前 46 包 × 5 文档，无 INDEX 外目录）', () => {
  ...
  assert.equal(allNames.length, 46, `expected 46 packages in INDEX, found ${allNames.length}`)
