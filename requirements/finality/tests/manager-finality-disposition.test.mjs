...
// JS lifecycle events applied one at a time by FinalitySurface.applyEvent,
// then classifyEnding / admitLabor
...
/** Apply each JS event through the production one-event fold. */
...
test('WHAT[FINALITY-025] a completed Life replays as AlreadyCompleted, never restarts', () => {
  const world = project([lifeOpened(), finalityRequested(), finalityReviewerEnlisted(), finalityBlessed(), lifeCompleted()])
...
test('WHAT[FINALITY-003] an open request resumes the same ToolCallId replay', () => {
  const world = project([lifeOpened(), finalityRequested()])
...
test('WHAT[FINALITY-003] a request already in motion waits for the current cohort', () => {
  const world = project([lifeOpened(), finalityRequested(), finalityReviewerEnlisted()])
...
test('WHAT[FINALITY-014] rejection keeps the same Life and a new suicide begins fresh Finality', () => {
  const world = project([lifeOpened(), finalityRequested(), finalityReviewerEnlisted(), finalityRejected()])
...
test('WHAT[FINALITY-016] a blessing leaves the Life open until the second suicide', () => {
  const world = project([lifeOpened(), finalityRequested(), finalityReviewerEnlisted(), finalityBlessed()])
...
test('WHAT[FINALITY-022] a new Life inherits no blessing/roster/request and starts fresh Finality', () => {
  const first = project([lifeOpened(), finalityRequested(), finalityReviewerEnlisted(), finalityBlessed(), lifeCompleted()])
  assert.equal(finality.archivedLivesView(first).length, 1)

  const reopened = finality.applyEvent(first, {
    kind: 'life-opened',
    sessionId: SESSION,
    lifeId: 'life-2',
...
test('WHAT[FINALITY-002] finality eligibility is the combination of commitment, request, and experience typing', () => {
  ...
  const planned = project([lifeOpened()])
  ...
  const inFlight = project([lifeOpened(), finalityRequested(), finalityReviewerEnlisted()])
  assert.deepEqual(classify(inFlight, 'call-2', true), { kind: 'wait-for-current-request' }, 'open request owns the cohort')

  const blessed = project([lifeOpened(), finalityRequested(), finalityReviewerEnlisted(), finalityBlessed()])
