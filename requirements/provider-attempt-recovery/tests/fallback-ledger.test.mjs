...
const {
  cursor,
  acceptHumanRoot: fallbackAcceptHumanRoot,
...
test('WHAT[PAR-001] PAR_FALLBACK_001_no_active_run_advances_nothing_and_writes_no_fact', async () => {
  ...
  try {
    ...
    const outcome = await recordConfirmedFailure(
      journal,
...
test('WHAT[PAR-003] PAR_FALLBACK_003_same_failure_observed_twice_advances_once', async () => {
  ...
  try {
    ...
    const first = await recordConfirmedFailure(
      journal,
      cursor.defaultBudget,
      SESSION,
      'msg_asst_1',
    ...
    const second = await recordConfirmedFailure(
      journal,
      cursor.defaultBudget,
      SESSION,
      'msg_asst_1',
...
test('WHAT[PAR-005] PAR_FALLBACK_005_twelfth_failure_admission_is_recovery_exhausted', async () => {
  ...
  try {
    ...
    for (let i = 1; i <= 11; i += 1) {
      const advanced = await recordConfirmedFailure(
        journal,
        cursor.defaultBudget,
        SESSION,
        `run-${i}`,
    ...
    const thirteenth = await recordConfirmedFailure(
      journal,
      cursor.defaultBudget,
      SESSION,
      'run-13',
...
async function admit(journal, providerRunName) {
  const recorded = await recordConfirmedFailure(
    journal,
    cursor.defaultBudget,
    SESSION,
    providerRunName,
