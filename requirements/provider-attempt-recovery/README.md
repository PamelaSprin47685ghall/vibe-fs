...
DEPENDS ON：`participant-identity`（换执行者 ≠ 换人；身份字节 guarantee 由 identity 包提供）、
`interaction-authority`（continuation 的 wire/authority 语义）。理由：PAR-013 的「不换身份」与
PAR-014 的「continuation 只在该 Run 内」分别消费这两个包的 guarantee。
