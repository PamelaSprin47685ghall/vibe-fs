...
| `feature-ablation` | 巡检与渐进验收需要正式消融开关与 DAG，使下游未审机制可零影响关停而不改源码。 |
...
| `execution-model-routing` | 固定 Role 身份与物理模型策略必须分离；唯一 MJS scheduler 以 `role + running` 决定 ModelTarget，runtime 只维护 lease occupancy。 |
