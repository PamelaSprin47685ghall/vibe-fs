...
| `degeneration-guard` | 尚未结束的 attempt 若已进入病态重复，应在污染更多历史前主动终止并交给正常 recovery。 |
...
| `requirement-grounding` | 代码路径一旦被阅读或修改，适用 requirement package 的当前文档与测试必须像模型自己调用 `read` 一样进入开发上下文；相同内容版本不重复。 |
...
- 新增 `requirement-grounding`：代码访问与 requirement package 的自动上下文关联是独立产品能力；它消费 requirement-system 的包结构，但不改变任何 package 的 semantic ownership。
