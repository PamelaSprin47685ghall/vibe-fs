...
|---|---|---|---|
| CAUSAL-001 观测非权威 | `tests/causal-wait.test.mjs` — `RED_8_application_observer_enter_only_snapshot_via_reader`（observer/reader 分面）+ REUSE：`scripts/checks/causal-wait-boundary.mjs`（六条静态扫描；经 check.mjs 运行，可红） | MOVE + REUSE | `node --test requirements/causal-wait/tests/causal-wait.test.mjs`；`node scripts/check.mjs` |
