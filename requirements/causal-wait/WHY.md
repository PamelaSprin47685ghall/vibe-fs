...
在复杂的分布式或并发工作流中，跨 Owner 的等待是必然存在的（如 Manager 等待评审或子代理交付、Retirement 等待递归资源终态、Orchestrator 等待子任务交付）。如果这些等待**不可诊断**，当系统阻塞挂死时只能观察到模糊的「Task pending」，无法回答五个关键的因果诊断问题（CCE 五问）：
1. **Owner**：哪个业务工作流在等待？
