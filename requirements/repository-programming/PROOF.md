...
| `REPOSITORY-PROGRAMMING-008` | `js-tools-fs.test.mjs` → `JS007_glob_bounded_deterministic_enumeration` / `JS007_glob_gitignore_skips_git_and_ignored`；交叉 `js-bindings.test.mjs` → `JS007_bindings_path_boundary_denies_escape` / `JS007_bindings_glob_lists_matching_paths` | MOVE | `node --test requirements/repository-programming/tests/js-tools-fs.test.mjs requirements/repository-programming/tests/js-bindings.test.mjs` |
