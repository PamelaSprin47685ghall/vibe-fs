...
- **结果面曾经固化缺陷**（`changes/completed/js-tools-toml-result.md`）：JS-016 声称走 Synthetic TOML，实际把 `run()` 的 JSON `stringify` 进一个字符串字段——`status = "ok"` / `result = "{...}"` / 逗号拼接 `written`，两套语法叠信封，路径含逗号不可消歧。`tests/unit/js-tools/js-workflow.test.mjs` 的旧 golden 曾把缺陷锁成正确形状。本 Change 重做结果面（REPOSITORY-PROGRAMMING-016），`status`/`result`/`written`/`created` 是已拒绝方向。
