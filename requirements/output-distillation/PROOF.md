...
| DISTILL-009 Distiller 私有 runtime | `tests/unit/session/distiller-ownership.test.mjs` `EXEC_014_distiller_fork_is_host_owned_hidden_and_parent_invisible` | REUSE（SPLIT：hidden handle→`managed-session-lifecycle`） | `node --test tests/unit/session/distiller-ownership.test.mjs` |
...
| 源 | 目标 | 类型 | 结果 |
|----|------|------|------|
| `tests/unit/execution/executor-summarize.test.mjs` | `requirements/output-distillation/tests/executor-summarize.test.mjs` | MOVE | `node --test` 绿（9 断言） |
...
- `tests/unit/session/distiller-ownership.test.mjs`：Distiller 私有 runtime 语义归本包（DISTILL-009）；
