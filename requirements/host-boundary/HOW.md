...
|---|---|---|---|
| HOST-BOUNDARY-001 | `tests/host001-fragment-events.test.mjs` `HOST_001_fragment_events_die_at_earliest_boundary` + `HOST_001_terminal_message_identity_is_physical_capacity_evidence_not_a_business_signal`（failed assistant 只结束 provider step，不结束 physical execution）；`tests/provider-retry-host-edge.test.mjs` `HOST_001_failed_provider_step_keeps_same_physical_execution_binding_for_host_retry`（acquire → step → failed assistant → Host release decision → same-physical retry step）；`tests/signals.test.mjs` `MISC_signals_router_loop_delta_bypasses_adapt` | NEW + MOVE | `node --test requirements/host-boundary/tests/host001-fragment-events.test.mjs requirements/host-boundary/tests/provider-retry-host-edge.test.mjs` |
