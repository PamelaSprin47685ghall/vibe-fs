...
|---|---|---|---|
| HOST-BOUNDARY-001 | `tests/host001-fragment-events.test.mjs` `HOST_001_fragment_events_die_at_earliest_boundary` + `HOST_001_terminal_message_identity_is_physical_capacity_evidence_not_a_business_signal`（terminal `message.updated` 仍不成为 `HostSignal`；failed assistant 只结束 provider step，不结束 physical execution）；`tests/provider-retry-host-edge.test.mjs` `HOST_001_failed_provider_step_keeps_same_physical_execution_binding_for_host_retry`（acquire → step → failed assistant → Host release decision → same-physical retry step）；交叉 `requirements/intra-participant-parallelism/tests/fission-runtime.test.mjs` `exact Fission terminal bridge reconciles a completed lane even when session idle is missing`（仅 durable Fission lane + current physical match 可用 exact successful terminal edge 打开 snapshot occasion，outcome 仍来自完整 snapshot）；`tests/signals.test.mjs` `MISC_signals_router_loop_delta_bypasses_adapt` | NEW + MOVE + REUSE | `node --test requirements/host-boundary/tests/host001-fragment-events.test.mjs requirements/host-boundary/tests/provider-retry-host-edge.test.mjs requirements/intra-participant-parallelism/tests/fission-runtime.test.mjs` |
...
| HOST-BOUNDARY-005 | `tests/reconcile-idle-early.test.mjs`（NEW：旧 terminal 不得跨 current physical fallback；exact terminal `message.updated` edge 只驱动一次额外 snapshot read；edge/pending race 不丢 wake；IdleWake 的 QuiescencePermit 跨 projection re-kick 保留；FailureWake + snapshot exact current assistant 即刻物化 `TurnFailed`，无需等待 terminal projection/idle；current assistant 尚不可见时仍 pending；无 counter-driven reread）；交叉 `requirements/intra-participant-parallelism/tests/fission-runtime.test.mjs` `exact Fission terminal bridge reconciles a completed lane even when session idle is missing`（先记录 edge epoch，再 opening `RetryWake`，没有 idle 也实际 publish `TurnCompleted` snapshot observation）；`tests/reconcile-supervisor.test.mjs` `EXEC_reconcile_*`（SnapshotError/NoTurn 不自轮询；只有 IdleWake 可把普通 nonterminal current assistant 交给 repair；terminal 仍 Publish；single-flight / generation fence / ClearSession） | NEW + MOVE + REUSE | `node --test requirements/host-boundary/tests/reconcile-idle-early.test.mjs requirements/host-boundary/tests/reconcile-supervisor.test.mjs requirements/intra-participant-parallelism/tests/fission-runtime.test.mjs` |
