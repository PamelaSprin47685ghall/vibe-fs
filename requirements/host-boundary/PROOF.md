...
| HOST-BOUNDARY-019 | 本包全部 canary（MOVE/NEW 表）；`tests/host-capability-observation.test.mjs`（HostContractUnsupported 显式失败）；REUSE 本文件 Magic Todo membrane canary 清单（H/A/C 未落地 → GAP，见下）；managed request model mutation canary → `execution-model-routing` GAP-016 | NEW + REUSE + GAP-016 | 见各行 |
