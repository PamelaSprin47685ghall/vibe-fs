...
| HOST-BOUNDARY-014 | `tests/host-hooks.test.mjs`（`HOST_009_every_registered_hook_has_a_fixture_here` / `HOST_009_every_hook_accepts_its_arguments_positionally` / `HOST_009_the_tool_registry_is_a_registry_not_a_triggered_hook`：仅现有 Hook；`HOST_009_hook_invariant_exceptions_cross_a_fatal_membrane_before_rethrow`：escaping hook exception 先 Diagnostic.fatal）；`requirements/managed-session-lifecycle/tests/satellite-runtime.test.mjs::HOST_015_direct_companion_repoint_trips_process_fatal_on_semantic_cut`（低层 live journal cut 不依赖 Host catch，直接 process fuse）；`scripts/checks/architecture.mjs` | MOVE + REUSE | `node --test requirements/host-boundary/tests/host-hooks.test.mjs requirements/managed-session-lifecycle/tests/satellite-runtime.test.mjs` / `node scripts/checks/architecture.mjs` |
