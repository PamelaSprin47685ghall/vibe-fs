...
node tests/unit/run.mjs                                                  # 全量
```

本包 1 个 NEW 测试文件（4 断言）单独跑绿；其余落点 REUSE 原位/跨包。
...
| EFFECT-ACCOUNTING-002 | `tests/unit/execution/join-missing-final-report.test.mjs::EXEC_join_MissingFinalReport_Failed_keeps_run_pending_not_failed`、`EXEC_join_empty_Completed_keeps_run_pending_not_failed`、`EXEC_join_real_Failed_still_claims_run`（unknown ≠ failure；物理缺失 ≠ 未发生）+ `requirements/change-integration/tests/job.test.mjs::PERSIST_009_request_alone_is_not_created`（Requested-only 如实投影） | REUSE | 各文件 `node --test` |
| EFFECT-ACCOUNTING-003 | `requirements/change-integration/tests/job.test.mjs::PERSIST_009_worktree_request_then_created_marks_identity_created`（Requested→Created 顺序事实）+ `tests/unit/orchestrator/runtime.test.mjs::ORCH_007_NeedsReview_preserves_the_active_worktree`（worktree 保留断言；PERSIST-009 顺序注释见文件头） | REUSE | 各文件 `node --test` |
...
| EFFECT-ACCOUNTING-005 | `tests/unit/temporal/orchestrator-conflict-confluence.test.mjs::THEOREM_publish_claimed_three_branch_order_is_fixed`（先查物理 head）+ `tests/unit/reconciliation/magic-todo-membrane.test.mjs::TODO-006 T1 accept succeeds then T2 prepare is a lag-1 wait, not a fail-closed Admission`（证据门决定等待/重试） | REUSE | 各文件 `node --test` |
| EFFECT-ACCOUNTING-006 | `requirements/durable-events/tests/event-store-journal-writer.test.mjs::append_failure_poisons_and_returns_CommitUnknown`（跨包 REUSE：写失败 → CommitUnknown，不假装 committed，不盲重试）+ `requirements/change-integration/tests/job.test.mjs::PERSIST_009_request_alone_is_not_created` | REUSE | 各文件 `node --test` |
| EFFECT-ACCOUNTING-007 | `tests/unit/execution/join-aborted-not-terminal.test.mjs`：`P0_RECOVERY_JOIN_001_aborted_alone_is_not_terminal`、`P0_RECOVERY_JOIN_001_aborted_observed_never_joinable`、`P0_RECOVERY_JOIN_001_tryFromDurableCompleted_rejects_cancelled`、`P0_RECOVERY_JOIN_001_joinable_completion_has_no_fromAborted_export`、`P0_RECOVERY_JOIN_001_aborted_times_n_projection_unchanged`（在 crash-reconciliation 包测试内，见 SPLIT 清单）+ `tests/unit/verify/p0-recovery-join-gate.test.mjs::P0_RECOVERY_JOIN_GATE_positive_clean_break_shapes_present`（completion-blob-schema-v2 / legacy-false-abort-decode / false-completion-rejected-fact 正向规则）+ `P0_RECOVERY_JOIN_GATE_production_sources_are_green` | REUSE | 各文件 `node --test` |
| EFFECT-ACCOUNTING-008 | `requirements/change-integration/tests/job.test.mjs::PERSIST_009_*`（5 锚点：Worktree typed 对）+ `tests/unit/journal/blogger-request-materialized.test.mjs::C5_entry_commit_records_receipt_and_clears_open_request`（Blogger Request→Entry）+ `requirements/obligation-ledger/tests/magic-todo-event-store.test.mjs::TODO-012 persists typed prepared identity through AgentJournal and EventStore boot`（Todo Prepared→Accepted） | REUSE | 各文件 `node --test` |
| EFFECT-ACCOUNTING-009 | `requirements/effect-accounting/tests/effect-facts.test.mjs::publish_claimed_recovery_three_branch_order_is_fixed`（BackfillPublished/AttemptPublish/RebaseAndReviewAgain/FailClosed 四态）+ `::publish_claim_without_durable_rebase_witness_is_rejected` + `tests/unit/temporal/orchestrator-conflict-confluence.test.mjs::THEOREM_publish_claimed_three_branch_order_is_fixed`、`THEOREM_publish_claimed_without_prior_rebase_is_rejected` | NEW + REUSE | 各文件 `node --test` |
| EFFECT-ACCOUNTING-010 | `tests/unit/journal/fact-codec.test.mjs::PERSIST_005_pre050_marker_refuses_with_migration_message`（DurableEffectRequested/Accepted 拒绝）+ `requirements/effect-accounting/tests/effect-facts.test.mjs::typed_effect_facts_replace_the_generic_durable_effect_union` | REUSE + NEW | 各文件 `node --test` |
| EFFECT-ACCOUNTING-011 | `requirements/obligation-ledger/tests/magic-todo-projection.test.mjs::TODO-004 rejects Accepted when it names another Prepared envelope`（PreparedFactRef 精确指名）+ `TODO-005 Accepted supersedes Current immediately and REVISE conclusion cannot roll it back`（Accepted 不折回）+ `requirements/obligation-ledger/tests/magic-todo-event-store.test.mjs::TODO-012 persists typed prepared identity through AgentJournal and EventStore boot` | REUSE | 各文件 `node --test` |
| EFFECT-ACCOUNTING-012 | `tests/unit/reconciliation/magic-todo-membrane.test.mjs`：`TODO-006 T1 accept succeeds then T2 prepare is a lag-1 wait, not a fail-closed Admission`、`TODO-006 T2 prepare succeeds once T1 process review is Concluded (lag-1 wait resolves, no invalidOp)`、`TODO-005 REVISE is feedback only: next checkpoint sees the report and Current never rolls back` + `requirements/effect-accounting/tests/effect-facts.test.mjs::publish_claim_without_durable_rebase_witness_is_rejected` | REUSE + NEW | 各文件 `node --test` |

## 统计

- 命题 12 条；落点行 12；NEW 1 文件（`effect-facts.test.mjs`，4 断言）+ REUSE 12 个现有
  文件（`tests/unit/execution/join-missing-final-report.test.mjs`、
  `tests/unit/execution/join-aborted-not-terminal.test.mjs`、
  `tests/unit/verify/p0-recovery-join-gate.test.mjs`、
  `tests/unit/temporal/orchestrator-conflict-confluence.test.mjs`、
  `tests/unit/orchestrator/runtime.test.mjs`、
  `tests/unit/reconciliation/magic-todo-membrane.test.mjs`、
  `tests/unit/journal/fact-codec.test.mjs`、`tests/unit/journal/blogger-request-materialized.test.mjs`、
  ...
  `requirements/durable-events/tests/event-store-journal-writer.test.mjs`）。
- GAP：0。

## SPLIT@cutover 清单

1. `tests/unit/execution/join-aborted-not-terminal.test.mjs`：单-owner（本包）但**暂不物理
...
4. `requirements/change-integration/tests/job.test.mjs` PERSIST-009 小节、
   `tests/unit/orchestrator/runtime.test.mjs` 顺序断言：`change-integration` 已在其
   SPLIT@cutover 清单中把 PERSIST-009 事实顺序断言划归本包；cutover 时物理拆分。
5. `tests/unit/reconciliation/magic-todo-membrane.test.mjs`：lag-1 证据门断言
