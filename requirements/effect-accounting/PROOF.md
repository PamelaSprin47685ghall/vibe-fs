...
| EFFECT-ACCOUNTING-002 | `requirements/effect-accounting/tests/join-missing-final-report.test.mjs::EXEC_join_MissingFinalReport_Failed_keeps_run_pending_not_failed`、`EXEC_join_empty_Completed_keeps_run_pending_not_failed`、`EXEC_join_real_Failed_still_claims_run`（unknown ≠ failure；物理缺失 ≠ 未发生）+ `requirements/change-integration/tests/job.test.mjs::PERSIST_009_request_alone_is_not_created`（Requested-only 如实投影） | REUSE | 各文件 `node --test` |
