...
test('WHAT[behavior-diagnosis-006] CHRONICLE_canonical_text_trims_and_rejects_empty', () => {
...
test('WHAT[behavior-diagnosis-006] CHRONICLE_missing_tip_returns_rulebook_consequence', () => {
...
test('WHAT[behavior-diagnosis-006] CHRONICLE_live_cycle_decision_is_typed_completed', () => {
  assert.deepEqual(chronicleExecutionContract(true), { kind: 'Completed', value: 'provider-result' })
})
test('WHAT[behavior-diagnosis-006] CHRONICLE_no_live_cycle_decision_is_typed_before_host_encoding', () => {
  assert.deepEqual(chronicleExecutionContract(false), { kind: 'NoLiveCycle' })
})
test('WHAT[behavior-diagnosis-006] CHRONICLE_no_live_cycle_aborts_then_host_adapter_exposes_sdk_error', async () => {
...
test('WHAT[behavior-diagnosis-006] ENFORCER_023_missing_tip_fails', () => {
  const result = enforcer.decodeCall({ text: 'work log entry' })
  assert.equal(result.ok, false)
  assert.equal(result.error, enforcer.missingTipError)
  assert.equal(result.error, 'missing required argument: tip')
})
test('WHAT[behavior-diagnosis-006] ENFORCER_023_empty_tip_fails', () => {
...
test('WHAT[behavior-diagnosis-006] ENFORCER_020_text_is_trimmed_empty_becomes_none', () => {
...
test('WHAT[behavior-diagnosis-006] ENFORCER_022_text_and_evidence_are_reserved_not_tips', () => {
  const field = firstField()
...
test('WHAT[behavior-diagnosis-006] ENFORCER_022_has_valid_text_requires_nonempty_text', () => {
  const field = firstField()
  const empty = enforcer.decodeCall({ text: '   ', tip: field })
...
test('WHAT[behavior-diagnosis-006] ENFORCER_064_empty_text_returns_public_tool_error', () => {
...
test('WHAT[behavior-diagnosis-006] ENFORCER_TIP_05_missing_tip_fails', () => {
