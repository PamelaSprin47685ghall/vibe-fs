...
test('WHAT[behavior-diagnosis-007] CHRONICLE_unknown_tip_is_repaired_at_runtime', () => {
...
test('WHAT[behavior-diagnosis-007] ENFORCER_023_nonempty_unknown_tip_resolves', () => {
...
test('WHAT[behavior-diagnosis-007] ENFORCER_024_misspelled_tip_maps_to_nearest_rule', () => {
...
test('WHAT[behavior-diagnosis-007] ENFORCER_021_valid_field_maps_exact_rule_id', () => {
...
test('WHAT[behavior-diagnosis-007] ENFORCER_021_tip_trims_whitespace_before_lookup', () => {
  const field = firstField()
  const rule = firstRule()
...
test('WHAT[behavior-diagnosis-007] ENFORCER_TIP_06_unknown_tip_resolves_to_catalog', () => {
  const r = enforcer.decodeCall({ text: 'entry', tip: 'totally-unknown-field' })
  assert.equal(r.ok, true)
  assert.ok(fields.includes(r.value.tip.fieldName))
})
test('WHAT[behavior-diagnosis-007] ENFORCER_TIP_07_valid_field_maps_rule_id_exactly', () => {
