...
| BD-017 无效 cycle 有界协议修复（0/2+ chronicle、nudge/AABB exact-terminal 幂等、abort 不推进） | REUSE `requirements/behavior-diagnosis/tests/enforcer-cycle-protocol.test.mjs` `ENFORCER_060_pure_prose_first_issues_interaction_nudge_not_aabb` / `ENFORCER_060_pure_prose_same_terminal_reentry_does_not_aabb` / `ENFORCER_060_same_terminal_reentry_after_aabb_is_idempotent` / `ENFORCER_061_same_empty_text_terminal_reentry_after_aabb_is_idempotent` / `ENFORCER_042_multi_call_first_terminal_issues_one_nudge_and_does_not_commit` / `ENFORCER_042_second_multi_call_terminal_after_nudge_triggers_aabb` / `LOOP_006_interrupted_blog_repairs_without_advancing_primary_cursor`；`requirements/behavior-diagnosis/tests/enforcer-153-rejudge.test.mjs` `ENFORCER_153_snapshot_rejudge_recognizes_exactly_one_completed_chronicle` / `ENFORCER_153_snapshot_rejudge_uses_named_chronicle_toolparts` / `ENFORCER_153_cold_rejudge_never_invents_AabbRepairIssued` / `ENFORCER_153_hot_path_aabb_preserves_target_terminal_identity`；`requirements/behavior-diagnosis/tests/integration/blogger-nudge-plugin-repro.test.mjs` `REPRO_blogger_pure_prose_terminal_idle_should_nudge_without_another_transform` / `REPRO_blogger_second_prose_terminal_idle_spends_aabb_not_second_nudge`（真实 plugin 时序：transform-start → prose terminal → idle；durable AABB 同 terminal 重入幂等） | REUSE + ADD | `node --test requirements/behavior-diagnosis/tests/enforcer-cycle-protocol.test.mjs requirements/behavior-diagnosis/tests/enforcer-153-rejudge.test.mjs requirements/behavior-diagnosis/tests/integration/blogger-nudge-plugin-repro.test.mjs` |
