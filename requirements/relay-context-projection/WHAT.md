...
## [001] audit history 完整，provider history 按 retirement cut 切段
...
## [002] cut 覆盖 suicide request 与 tool result，并丢弃退休尾部

accepted retirement 的 cut frontier 必须覆盖前任最后 assistant part、suicide tool call、其 tool result 及同 causal batch 的内部 ack；不能用最近 N 条、时间戳或文本搜索 suicide 推断。retirement tool 之后、下一个 owner-admitted 迭代输入之前的 retired-run racing tail（closing、phantom join）全部丢弃。`runtime/manager-assess` 是新迭代的显式 owner resource，不是可剥离 transport wake。真实用户需求、typed authority 消息与新迭代 owner resource 永不过 cut。cut 只认位置、role 与 typed message identity，不认文本、不猜 run、不等物理 arrival。

## [003] provider 消息上下文只含 typed authority 内容与当前迭代消息

下一迭代的 provider 消息上下文只包含 typed authority 消息、exact owner-admitted `runtime/manager-assess` 资源与当前迭代消息：前任 chain-of-thought、原始长日志、secret 与 retired racing tail 一律不得进入。工作区是共享执行状态，Manager 直接检查它，projection 不转述它；当前动作由事实约束后的显式资源与 capabilities 表达。AuthorityRevision、SnapshotId 与 phase ID 永不序列化进 provider 消息。


## [004] 每一迭代使用同一 review-first 起点

所有迭代使用同一 review-first 结构：先独立评审当前完成情况和质量。首迭代只陈述当前权威需求，不伪造前任 commit、测试或结论；后续迭代保留相同权威前缀并追加 exact `runtime/manager-assess` 资源，不复述前任私有过程。工作区是共享执行状态，Manager 自行检查工作区，projection 不负责叙述工作区内容。

## [005] 物理 SessionId 与用户线程保持连续

迭代切换不得为了缩上下文创建新的用户聊天线程或删除历史。SessionId 可以跨迭代复用；IncumbencyId 和 provider context 不得复用。

## [006] projection 是确定性过滤，不生成合成列表
...
## [008] authority message chain 跨 cut 保留，普通前任消息与内部 wake 不借机穿透
