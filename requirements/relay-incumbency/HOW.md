...
- `src/Wanxiangshu/Mission/Relay/Contract.fs(.fsi)`：Road/Incumbency/phase/port vocabulary；`RetirementOutcome = Continue | Accepted of QualityCertificateId`、`ProjectionCut = { ProviderRunId; ToolCallId }`、`RetirementSummary = { Id; IncumbencyId; SnapshotId; AuthorityRevision; ProjectionCut; Outcome }`（快照字段类型仍为 WorkspaceSnapshotId，快照与修订是 load-bearing binding）；`AssessmentBinding` 携带 IncumbencyId；`ManagerLoopGate.gateKind` 前缀 `manager-loop:`；保留 `QualityCertificate`、`RetiredProviderRunIds` 与内部 phase 名；定义道路固定 DevOps 绑定与控制权转交合同。
- `src/Wanxiangshu/Mission/Relay/Facts.fs(.fsi)`：唯一 ID/事件合成点。`IncumbencyOpening = { RoadId; IncumbencyId; AuthorityRevision; Transaction }`；`IncumbencyOpening.initial(sha256, SessionId, PhysicalUserMessageId, WorkspaceSnapshotId)` 产出 RoadOpened + IncumbencyOpened；`IncumbencyOpening.next(sha256, RoadId, RetirementId, AuthorityRevision, WorkspaceSnapshotId)` 只产出 IncumbencyOpened；调用方不再自行合成这些 ID 与事件。Host crypto 由调用方注入（`sha256: string -> string`，verification-system-008 参数倒置）；不依赖 host-boundary。
