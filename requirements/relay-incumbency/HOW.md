...
- `src/Wanxiangshu/Mission/Relay/Facts.fs(.fsi)`：唯一 ID/事件合成点。`IncumbencyOpening = { RoadId; IncumbencyId; AuthorityRevision; Transaction }`；`IncumbencyOpening.initial(sha256, SessionId, PhysicalUserMessageId, WorkspaceSnapshotId)` 产出 RoadOpened + IncumbencyOpened；`IncumbencyOpening.next(sha256, RoadId, RetirementId, AuthorityRevision, WorkspaceSnapshotId)` 只产出 IncumbencyOpened；调用方不再自行合成这些 ID 与事件。Host crypto 由调用方注入（`sha256: string -> string`，VERIFY-008 参数倒置）；不依赖 host-boundary。
