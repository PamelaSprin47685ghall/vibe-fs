...
| GD-010 检测语料跨 family 冲突门 | REUSE `tests/unit/verify/enforcer-cross-family-collision.test.mjs` `enforcer_collision_documents_scoring_formula` / `enforcer_collision_scorePair_fail_warn_note_clean` / `enforcer_collision_temp_dir_fail_closed_on_extreme_non_siblings`（gate KEEP，本体在 `scripts/checks/enforcer-cross-family-collision.mjs`） | REUSE | `node --test tests/unit/verify/enforcer-cross-family-collision.test.mjs` |
...
- `tests/unit/enforcer/tip-v2-contract.test.mjs`：`ENFORCER_TIP_13/14`（previous
