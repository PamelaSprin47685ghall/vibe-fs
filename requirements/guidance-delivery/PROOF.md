...
| GD-012 新 occurrence 消费当前 calibration projection 后冻结 MarkerText | `tests/pair-calibration.test.mjs`（no-estimate omission / bilingual advisory craft / occurrence-by-occurrence remaining）+ REUSE `requirements/time-capability/tests/pair-session-elapsed.test.mjs`（fresh elapsed / historical immutable / tip→elapsed→estimate→guideline order） | NEW + REUSE（FROZEN 2026-08-14） | **按用户要求冻结后未执行**；实现后不改 oracle |
