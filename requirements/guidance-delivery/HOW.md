...
- `durable-events`：交付事实（TipGuidanceDelivered）与历史 pair 是 durable fold
  的输入。
- `concern-routing`：本包只在新 Pair Hint occurrence 组合其已决的 subscription/message fragment；mailbox 身份、路由、coverage 与何时算 delivered 归该 owner。
...
| GD-012 新 occurrence 消费当前 calibration/concern projection 后冻结 MarkerText | 现有：`tests/pair-calibration.test.mjs` `WHAT[GD-012] GD_012_DELEG_022_no_estimate_means_no_dynamic_fragment` / `WHAT[GD-012] GD_012_each_new_occurrence_can_render_a_new_remaining_without_rewriting_old_text` / `WHAT[GD-012] GD_012_dynamic_fragment_is_between_tip_and_canonical_guideline` + REUSE `requirements/time-capability/tests/pair-session-elapsed.test.mjs`。待 GAP：subscription announcement / mailbox message 只在新 occurrence staging；pair placement 与 concern coverage atomic；failed placement zero-consume；一旦冻结后 replay byte-stable 且不重新消费。 | NEW + REUSE + GAP | 现有 frozen oracle 不改；新增 concern proof 由 GAP 建立 |
