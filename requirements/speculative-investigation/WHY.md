...
结果可稳定重放、错误方向的损失有界（changes/completed/strength.md §三.1）。
...
观察消费证据、再 promotion（changes/completed/strength.md §三.2）。
...
权限产生分叉（docs/shape/strength.md STRENGTH-013..019 逐条分配 owner）。
...
历史失败模式第一手考古：`changes/completed/strength.md` §二十二（存储收口）、§二十三
（崩溃矩阵）、§三十（明确拒绝的方向）、§三十一（最终不变量）。

## 3. 明确拒绝的方向（考古，不构成 WHAT）

以下方向在 changes/completed/strength.md §三十逐条拒绝，理由已吸收进对应命题：
