...
const publishRequest = (bundle, decision = 'd1', replica = 'replica-1') => {
  if (decision && decision.startsWith('replica-')) {
    return { ownerSessionId: 'owner', decisionId: 'd1', targetProviderRun: 'run-1', replicaSessionId: decision, budget: 'K1', anchorDigest: 'anchor-a', bundle }
  }
  return { ownerSessionId: 'owner', decisionId: decision, targetProviderRun: 'run-1', replicaSessionId: replica, budget: 'K1', anchorDigest: 'anchor-a', bundle }
}
...
test('WHAT[SPEC-INV-006] prepared_cut_legal_command_is_accepted_by_the_fold', async () => {
  ...
  try {
    const durability = Strength.durabilityCreate(local.store)
    const bundle = frame()
    const published = await Strength.durabilityPublishPrepared(durability, publishRequest(bundle, 'cut-d1'))
