...
- **全 Job 串行 vs 只锁 ref mutation**（`docs/why/orchestrator.md` 备选节）：曾考虑把 Integration Gate
...
- **e2e 超时先放大再查因**（`changes/completed/orchestrator-e2e-timeout.md`）：三 canary watchdog 超时
