...
`GIT_ff_merge_refuses_dirty_target_worktree`）；REUSE `tests/unit/orchestrator/runtime.test.mjs`
...
`ORCH_007_every_progress_case_yields_exactly_one_action`）；REUSE `tests/unit/orchestrator/runtime.test.mjs`
...
边界（`docs/why/orchestrator.md` 备选节）。
...
REUSE `tests/unit/orchestrator/runtime.test.mjs`（`ORCH_007_NeedsReview_preserves_the_active_worktree`）。
