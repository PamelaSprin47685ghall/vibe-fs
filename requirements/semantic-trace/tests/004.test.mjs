...
const appendRun = (projection, sequence, generation, run, turn = 0) => unwrap(trace.appendPart(projection, {
  sequence,
  role: 'assistant',
...
test('WHAT[SEMANTIC-TRACE-004] provider runs segment the ordered semantic projection', () => {
  let projection = trace.emptyProjection()
  projection = appendRun(projection, 1, 0, 'run-a')
  projection = appendRun(projection, 2, 0, 'run-b', 1)
...
test('WHAT[SEMANTIC-TRACE-004] a new provenance generation changes only the current-generation query', () => {
  let projection = trace.emptyProjection()
  projection = appendRun(projection, 1, 0, 'run-before')
  projection = appendRun(projection, 2, 1, 'run-after')
