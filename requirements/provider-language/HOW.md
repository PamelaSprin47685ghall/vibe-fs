...
- `ProviderProse.fs`（`Wanxiangshu.Resources`）：
  ...
  - `render` / `instructionLines` / `document` / `documentFor`：装载 + 替换 + 交
    `SyntheticToml` 布局。
- 装载装配顺序（`docs/how/prompt.md`）：Common Law → Role Law → Office Library；Tools
...
| 源 | 判定 | 说明 / 落点 |
|---|---|---|
| `changes/completed/PromptRestoration.md` | EVIDENCE | WHY 考古：Gate 0 前「system=zh-CN + tool=English」半 i18n 病灶；Class A/B/C；Gate E ratchet 580→0。落点：WHY.md 失败模式 + WHAT 005/009 |
| `docs/why/host.md` §21 | EVIDENCE | 被拒方案：每 attempt/child 重读全局；语言绑 Role/Agent。落点：WHY.md + WHAT 002/003/004 |
| `docs/why/prompt.md`「ProviderLanguage」节 | EVIDENCE | 被拒方案：运行中切语言、译 protocol id。落点：WHY.md + WHAT 004/011 |
