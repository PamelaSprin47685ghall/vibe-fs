...
| SESSION-ONTOLOGY-005 | REUSE `requirements/session-ontology/tests/session-association.test.mjs` `HOST_008_a_session_cannot_be_its_own_Companion` / `HOST_008_one_Companion_cannot_serve_two_work_sessions`（owner 唯一）；`HOST_008_orthogonal...`（blogger owner = work session） | REUSE + NEW | `node --test requirements/session-ontology/tests/session-association.test.mjs` |
...
| SESSION-ONTOLOGY-008 | REUSE `requirements/session-ontology/tests/session-association.test.mjs`：`HOST_008_linking_records_both_directions_at_once`、`COMPANION_002_a_Companion_is_structurally_a_leaf`、`COMPANION_002_giving_a_Companion_its_own_Companion_is_refused`、`COMPANION_002_a_second_different_Y_for_one_work_session_is_refused`、`HOST_008_one_Companion_cannot_serve_two_work_sessions`、`COMPANION_003_relinking_the_same_pair_is_idempotent` | REUSE | `node --test requirements/session-ontology/tests/session-association.test.mjs` |
| SESSION-ONTOLOGY-009 | REUSE `requirements/session-ontology/tests/session-association.test.mjs` `COMPANION_001_every_work_session_may_have_a_Y_regardless_of_role`（非资格白名单）+ `COMPANION_003_unlinking_frees_the_work_session_to_get_a_fresh_Y` | REUSE | 同上 |
...
| SESSION-ONTOLOGY-012 | REUSE `requirements/session-ontology/tests/sync-delegate.test.mjs` `HOST_008_AttachmentKind_bookkeeper_carries_transaction_id`；`tests/session-ownership-ratchet.test.mjs` `session_ownership_matrix_green_fixture`（Bookkeeper 行） | REUSE + MOVE | `node --test requirements/session-ontology/tests/sync-delegate.test.mjs` |
...
| SESSION-ONTOLOGY-014 | `tests/session-ownership-ratchet.test.mjs` `session_ownership_matrix_rejects_unexpected_kind`（注入 `Teacher` → `unexpected-kind`）；REUSE `requirements/managed-session-lifecycle/tests/satellite-runtime.test.mjs` `HOST_014_SatelliteKind_is_Companion_only` | MOVE + REUSE | `node --test requirements/session-ontology/tests/session-ownership-ratchet.test.mjs` |
