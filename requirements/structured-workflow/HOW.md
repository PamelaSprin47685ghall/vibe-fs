...
- `scripts/checks/owner-contracts.mjs` 与 `scripts/checks/owner-projects.mjs` 共同治理 owner locality 与跨 owner 引用：`owner-contracts` 校验 `published-contracts.json` 合同册、release-closure 状态与 exact symbol/consumer 承诺；`owner-projects` 校验 ProjectReference graph 为 DAG，foreign locality 只引用 published contract/physical-port/adapter，且 flattened Fable emit 的 compile set 与 owner locality source 并集一致。

### 5. Owner contract 与 compile-locality cutover

57.15 cutover 后，白盒 FCS 扫描已退休。`owner-symbol-uses.fsx`、`owner-dependencies.mjs`、`composition-root-invariant.mjs` 及其 FCS evidence/reuse pipeline 已整体删除。`owner-contracts.mjs` 与 `owner-projects.mjs` 作为继任 gate：`owner-contracts` 校验 `published-contracts.json` 合同册、release-closure 状态与 exact symbol/consumer 承诺；`owner-projects` 校验 ProjectReference graph 为 DAG、source coverage、单 owner、foreign contract-only reference，并检查 flattened Fable emit 的 compile set 与 owner locality source 并集一致。
