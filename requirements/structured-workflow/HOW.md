...
19. **业务/纯决策分片不再直读外部物理适配器（delegation 计时、enforcer 修复、relay 快照）；SCC 保持 18**：
   ...
   - 已判定的标签偏差（不修）：`enforcer/resources-runtimeresources(runtime) → resources/resources-promptresources(runtime)` 实际只消费纯数据（`PromptCatalog` 记录与纯格式化函数），不构成 013 违规；该引用的"runtime"分类是旧 locality 标签偏差，不是知识耦合。
