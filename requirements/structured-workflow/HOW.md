...
F# node classifier只把显式穷举且能力已由compiler-resolved symbol/dependency observation承接的结构kind判为irrelevant；未知active pattern、address-of、array/object construction、object expression、trait call、`this`与IL节点保持Unknown。scanner结合FCS `FullType`、`IsMutable`、`IsModuleValueOrMember`、declaring entity与field symbol：仅exact primitive immutable value可判irrelevant；function/interface/abstract member/function-field carrier保留`CapabilityValue`，array/含mutable field的carrier、module mutable与field mutation分别保留`runtime-cell`或`top-level-mutable`。FCS不能独立证明lexical mutable value没有逃逸到lambda/object，因此local read/write仍保持Unknown，不能用`IsModuleValueOrMember=false`冒充escape proof。external symbol只按closed FQN family或exact member分类，assembly本身不授权pure，mutable collection family不得整体判pure。FCS symbol occurrence先按ephemeral `(source range, symbol)`去重，再为每个exact `(source, anchor, raw case, payload)`独立分配`same_anchor_occurrence_ordinal`；无关observation插入不改变fact identity，同payload不同range仍保持两个ID。
