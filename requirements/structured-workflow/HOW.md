...
`locality-slice-report.mjs --write-fresh-worksheet`只允许覆盖全`undecided` worksheet。locality topology变化时，它先从正式owner inventory原子重建全部live ID，再执行依赖stage-0 generated input的fresh report；任何`decided` record仍拒绝覆盖。bootstrap不携digest、classification或授权力，只消除已删除project path阻塞。
...
`scripts/checks/locality-slice-report.mjs`组合inventory、compiler scan、dependency analysis、tracked generator、capability extractor与唯一canonical world。默认summary仍对完整world验schema、reference、partition与digest，只输出census、全部locality candidate/reason、按code聚合的finding及report-only Unknown census；`--full`只额外展开逐locality actual queries。Unknown census只在world验证成功后读取fact，按observation case与closed Unknown payload分组；完整locality/source set只进入distinct count与domain-separated digest，输出各保留前5个canonical representative。实现复核分组fact count等于canonical `unknown_count`，并以全部group row产生稳定top-level digest；它不回填world、worksheet或任何授权输入。`--write-fresh-worksheet`只从同一candidate projection生成178条closed、全`undecided`、无digest/授权力的migration worksheet，拒绝覆盖任何已裁决记录。report与worksheet未接入`check.mjs`、Wireit或release sink；M6.4前旧gate仍是唯一authority。
...
| STRUCTURED-WORKFLOW-014 | `requirements/structured-workflow/tests/capability-observation.test.mjs::WHAT[STRUCTURED-WORKFLOW-014] capability observations and JavaScript traversal fail closed with exact codes`；`requirements/structured-workflow/tests/capability-observation.property.test.mjs::WHAT[STRUCTURED-WORKFLOW-014] one capability or traversal mutation yields its exact violation`；`requirements/structured-workflow/tests/locality-slice-report.test.mjs::WHAT[STRUCTURED-WORKFLOW-014] report-only Unknown census is a bounded deterministic projection of canonical facts` |
