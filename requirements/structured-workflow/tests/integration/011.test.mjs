...
const BOUNDARY_FIXTURE = join(ROOT, 'requirements/structured-workflow/tests/fixtures/owner-project-boundary')
...
function compile(project) {
  ...
  return new Promise((resolveResult, reject) => {
    const child = spawn(
      'dotnet',
      ['tool', 'run', 'fable', '--', join(BOUNDARY_FIXTURE, project), '-o', outDir, '--noGitignore'],
...
test('WHAT[STRUCTURED-WORKFLOW-011] independent Fable checks enforce compile-shard input boundaries', async () => {
  const [
    green,
    red,
    merged,
...
test('WHAT[STRUCTURED-WORKFLOW-011] flat closure compilation compiles transitive closure green and keeps unreferenced sources red', async () => {
  const emitterPath = join(BOUNDARY_FIXTURE, 'Emitter.fsproj')
  ...
  try {
    const greenResult = await compileOwnerProject({
      projectPath: join(BOUNDARY_FIXTURE, 'LeakyConsumer.fsproj'),
    ...
    const redResult = await compileOwnerProject({
      projectPath: join(BOUNDARY_FIXTURE, 'RedConsumer.fsproj'),
      aggregatePath: emitterPath,
      scratchRoot,
      rootPropsPath,
      stdio: 'pipe',
  ...
  <ItemGroup>
    ...
    <Compile Include="${join(BOUNDARY_FIXTURE, 'RedConsumer.fs')}"/>
  </ItemGroup>
</Project>`,
      'utf8',
    )
