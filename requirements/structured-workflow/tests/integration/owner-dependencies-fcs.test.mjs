...
test('WHAT[STRUCTURED-WORKFLOW-011] one tagged production scan is reused fail-closed and consumer-filtered', async (context) => {
  mkdirSync(repositoryScratchRoot, { recursive: true })
  ...
  try {
    await context.test('WHAT[STRUCTURED-WORKFLOW-011] production scan emits tagged normalized evidence', () => {
    ...
    await context.test('WHAT[STRUCTURED-WORKFLOW-011] tagged evidence reuse is filtered and fails closed', () => {
      const normalized = JSON.parse(readFileSync(normalizedEvidence, 'utf8'))
    ...
    await context.test('WHAT[STRUCTURED-WORKFLOW-011] explicit fixture scan cannot consume production evidence', () => {
