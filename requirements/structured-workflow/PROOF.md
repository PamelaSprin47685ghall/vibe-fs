...
| STRUCTURED-WORKFLOW-012（Semantic Compression 必须有 proof） | `tests/semantic-vocabulary.test.mjs`：`SW_011_named_vocabulary_surface_exists_in_Application`（被压缩 Vocabulary 的存在面）；proof 义务表见 HOW.md §3.4（每个高阶 Vocabulary 必须有 temporal/behavioral proof，源自 `archive/docs/proof/dsl-structured-program.md` 高阶 Vocabulary 证明义务表） | NEW + HOW | `node --test requirements/structured-workflow/tests/semantic-vocabulary.test.mjs` |
