...
全仓禁止自建 FCS（`FSharp.Compiler.Service`）扫描。禁止直接或经 wrapper、反射、`.fsx` 调用 compiler service 提取 F# typed AST、symbol use、application use、推断类型或源码依赖图；全量、局部、owner/locality、fixture、report-only、CLI、CI、pre-build、缓存、snapshot、delta、外部 evidence 导入与复用均无豁免。禁止以“轻量”“精简”“只报告”“未来恢复”重新引入。正常 Fable 编译内部使用 compiler service 不属于独立扫描，但不得为提取扫描证据额外启动或插桩编译器。

ProjectReference 图必须是 DAG，并且是声明的源码输入与编译归属边界的唯一权威。F# 边界只由当前工程图、精确 compile closure、sibling `.fsi` 与正常 Fable 编译证明；不得要求 FCS-derived actual source edge 或另一套 symbol ACL。静态工程图门禁核对引用、归属、授权与输入并集，真实编译核对该输入集合内的类型、签名与 private 可见性。`Wanxiangshu.fsproj` 只是无语义的 flattened emitter：其 compile set 必须与 locality source 并集精确相等、不得引用 locality project、不得成为授权 provider。aggregate 编译成功不替代工程图门禁，ProjectReference 也不冒充 Fable source-merge 下的独立 assembly firewall。requirement dependency graph 与声明的 project graph 只共享 owner identity，不要求边集合相等。

每个可被其他 locality 使用的 provider locality 必须形成一个 contract slice，并由 sibling `.fsi` 作为唯一 export inventory；manifest 不得重复保存或声称实施 per-symbol/per-owner ACL。同一 slice 的全部 `.fsi` exports 对其 effective audience 可见；若该能力集合不可共同授权，必须拆 slice。module-local private binding 与 `.fsi` 未导出 symbol 的隐藏性必须由真实编译器边界反例证明，不得由扫描器或人工 symbol 清单模拟；不得把 Fable 的 `internal`、top-level private module 或 `DisableTransitiveProjectReferences` 当作未经证明的隔离边界。
...
manifest 只保存规范允许集合：每个 slice 的 provider locality、owner、exposure、`allowed_direct_consumers`、bounded slice 的 `allowed_effective_consumers`、`laws[]`、justification 与 semantic evidence。direct consumers 是当前 ProjectReference DAG 中直接引用 provider 的 locality，effective consumers 是其反向传递闭包删掉 provider 自身；本规范中的 actual audience 只指这份当前工程图的可达集合，不表示扫描得到的 symbol use。两者必须从当前工程图推导，不得写入 manifest 形成第二事实源。每条 cross-locality direct ProjectReference 必须恰有一个 slice grant；direct consumers 必须与 allowed direct consumers 精确相等。physical port、adapter 与 composition wiring 必须保留 exact `consumer locality → provider slice` capability relation及两端 module，不得降级为 owner pair、裸路径或仅证明 ProjectReference 存在。
...
门禁必须拒绝：unowned/duplicate owner 或 locality、重复编译归属、locality kind/exposure 非法、ProjectReference SCC、missing/stale/duplicate reference 或 grant、same-owner 未授权 edge、compile input 逃出声明 closure、private locality 被引用、contract closure 含非 Contract 或超过预算、shared 携带 effect authority、bounded effective audience 越界、effect 被非 composition 到达、composition 被普通 consumer 引用、emit/locality compile-set drift、`.fsi` 缺失或与 implementation 不兼容、伪造/悬空 semantic evidence，以及 physical port/adapter/composition relation 降级。真实 compiler canary 必须通过正常 Fable 编译固定：从 consumer 精确 closure 移除必需 provider 后编译为 RED；合法 direct/transitive 输入与 alias/open/generic/type/pattern 使用为 GREEN；module-local private binding 与 `.fsi` 未导出 symbol 的外部访问为 RED；implementation 与签名不兼容为 RED。canary 只断言编译成功/失败或公开行为，不读取 compiler AST、symbol table 或扫描报告。外部 package 引用与 production ProjectReference 分开核对。边界未被这些证据证明时保持未证明，禁止补空 evidence、伪造 source edge 或恢复 FCS 扫描来宣称通过。
...
production extractor必须先由`enumerateCapabilityObservationsV1`枚举完整`C(W)`，再由总函数`classifyCapabilityObservationV1`给每条 observation恰一个`Irrelevant | Classified | Unknown` disposition；`extractObservedCapabilityFactsV1`必须证明 observation keys 与 disposition keys 全等且三类两两不交。`C(W)`只覆盖直接来自可追溯源码字节的显式 Fable Import/Emit/emitJsExpr 与 JavaScript visitor 产出的 capability observations，包括 repository-generated artifact 的 JavaScript；不得包含或要求 FCS 提取的 F# typed AST、external symbol occurrence、推断类型或 public-signature classification。F# export visibility 由 `.fsi` 与真实编译器边界证明，F# 业务语义由注册的 owner 行为 proof 证明；不得通过类型形状递归扫描给其授予 pure/capability 标签。`Classified`必须保留runtime、authority、mutable resource与semantic class的完整多标签集合；metadata、kind、exposure、grant、generated proof与allowlist均不得增加、删除或降格 observation。无法从允许的字节输入解析的动态 interop 保持 Unknown；禁止启动 FCS 补足或用空数组冒充完整证据。
...
M6.3b report必须在canonical world验证后，从canonical capability facts确定性投影report-only Unknown census；它不是world、worksheet、manifest或cutover的输入，不取得授权力。每组identity精确等于`observation.case + unknown_class + syntax_kind + raw_identity`，组与组内locality/source均按canonical comparator排序；组内fact count总和必须精确等于world的`unknown_count`。每组只输出前5个locality/source representative，同时保留完整distinct count与完整集合digest；top-level digest覆盖全部canonical group row。输入fact顺序或完全相同的重复row不得改变分组、排序或digest。
禁止 `PublicSignatureExport` 扫描分类作为 capability fact；签名只界定公开类型与值，不证明其实现纯度或无副作用，禁止由“不是 capability-type”默认 PureRepresentation。
