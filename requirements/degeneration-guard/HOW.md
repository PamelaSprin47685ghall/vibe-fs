...
`HostSignalBootstrap` 通过 `LoopSensor.create` 静态构造并调用 `scope.AttachLoopSensor`，reset／observe／drop 使用 `ILoopSensor` 合同；continuation 资源直接取自 `LoopSensor.continuationPath`。无动态模块及 global fallback，缺失装配立即暴露。`host-boundary/tests/loop-sensor-wiring-owner.test.mjs` 经真实插件 fork 创建 managed child、投递订阅事件并观察 SDK abort：仅中断该 child 一次，root 与 foreign session 均豁免；完整 reconcile 后 continuation 的细分语义由下表 sensor owner 测试证明。
