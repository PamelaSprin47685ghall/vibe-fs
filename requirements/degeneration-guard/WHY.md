...
- **固定记忆尺度 + Repository SSOT vs 特例森林**：detector 以 `256` 个 `o200k_base` token 为固定 half-life；每次 build 从当前 strict UTF-8 仓库连续文本流即时派生正常 envelope。运行期不按角色、模型、语言动态放宽边界，也不存在第二份可手调配置。
