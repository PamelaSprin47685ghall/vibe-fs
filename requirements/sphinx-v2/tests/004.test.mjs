...
test('WHAT[sphinx-v2-004] an overrun is recorded rather than absorbed', () => {
  const specs = Core.listOfItems([{ Name: 'modelCalls', Kind: Core.resourceKindCreate('consumed', 'call'), AuthorizedLimit: 10 }])
  const oversettled = Core.mapOf([['modelCalls', 13]])

  assert.equal(Core.budgetObservedOverrun(specs, oversettled, Core.mapOf([]), 'modelCalls'), 3)
  // Availability clamps at zero rather than going negative.
