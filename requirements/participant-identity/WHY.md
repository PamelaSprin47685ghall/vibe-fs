...
Role 是本名词汇（Manager/Orchestrator/Engineer/DevOps/Blogger），每个 Role 在运行时恰有一个 Persona。Engineer 是独立真身份，拥有本地事实调查与源码读写实现权能，绝非其他角色的 Persona 别名或包装。活跃身份集合与调度路径仅包含合法活跃角色。Predictor 与 Bookkeeper 是内部机制专用角色（Predictor 用于 Strength 降级，Bookkeeper 用于案例维护），不参与普通调度、不进入工具门禁、不暴露给用户可见 fork 候选。

活跃身份解析与历史身份解析必须严格分开：历史事件与归档中的历史身份仅在历史数据边界隔离解码，严禁静默自动升级为具备写权限的 Engineer，亦严禁将 DevOps 误解析为具备 Fission 权能的 Engineer。Manager 的公开 fork 候选仅限 Engineer；DevOps 由合法 Runtime 绑定创建，模型在视界中仅见稳定 Byname，仅通过 resume 进行续做调用。
...
- **历史身份静默升权**：将历史记录中的只读历史身份在恢复或反序列化时自动套用为全权限 Engineer。
...
- 非合法活跃集合的身份通过活跃调度路径被拉起，或历史身份在重放时被赋予不应拥有的写入与 Fission 权限。
