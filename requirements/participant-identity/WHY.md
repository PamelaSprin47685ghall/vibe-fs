...
Role 是本名词汇（Manager/Orchestrator/Coder/Inspector/Browser/Inquiry/Reviewer/DevOps/Distiller/Blogger），每个 Role 在运行时恰有一个 Persona。Predictor 是内部机制专用角色，只为 Strength 降级指定廉价 provider/model，不参与普通调度、不进入工具门禁、不暴露给用户可见 fork 候选。
