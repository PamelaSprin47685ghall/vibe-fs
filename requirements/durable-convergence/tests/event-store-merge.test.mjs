...
const make = (id, parents = [], stream = 'merge/main', payload = {}) => new Domain.EventEnvelope(
  eventId(id), streamId(stream), 'JobRequested', FsList.ofArray(parents.map(eventId)), payload, FsList.ofArray([]),
)
const ids = (result) => listItems(result.toJSON()[1]).map((e) => idValue.event(e.EventId))
...
test('WHAT[DURABLE-CONVERGENCE-001] set union never drops distinct events', () => {
  const result = Merge.merge(FsList.ofArray([['writer-a', FsList.ofArray([make(A)])], ['writer-b', FsList.ofArray([make(B)])]]))
  assert.deepEqual(ids(result).sort(), [A, B].sort())
})

test('WHAT[DURABLE-CONVERGENCE-002] writer enumeration is commutative', () => {
  const a = ['writer-a', FsList.ofArray([make(A), make(C, [A])])]
  const b = ['writer-b', FsList.ofArray([make(B)])]
  assert.deepEqual(ids(Merge.merge(FsList.ofArray([a, b]))), ids(Merge.merge(FsList.ofArray([b, a]))))
})

test('WHAT[DURABLE-CONVERGENCE-002] duplicate stream input is idempotent by EventId', () => {
  const event = make(A, [], 'merge/main', { x: 1 })
  const result = Merge.merge(FsList.ofArray([['a', FsList.ofArray([event])], ['copy', FsList.ofArray([event])]]))
...
test('WHAT[DURABLE-CONVERGENCE-003] identity collision is fail closed not LWW', () => {
  ...
  const result = Merge.merge(FsList.ofArray([['left', FsList.ofArray([left])], ['right', FsList.ofArray([right])]]))
  assert.equal(result.toJSON()[0], 'Error')
})
