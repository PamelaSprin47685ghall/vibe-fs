...
（`tests/unit/persist/event-store-merge.test.mjs`、`event-store-converge.test.mjs`、
`tests/integration/persist/dumb-server.test.mjs`，以及已迁入 `durable-events/tests/`
的 fold/append 文件）。
...
|---|---|---|---|
| DURABLE-CONVERGENCE-001 | `requirements/durable-convergence/tests/replica-merge-laws.test.mjs::set_union_never_drops_concurrent_events`（两个并发 EventId 都存活）+ `tests/unit/persist/event-store-merge.test.mjs::merge_identity_collision_fail_closed`（同 id 异 bytes 之外的一切都保留） | NEW + REUSE | `node --test requirements/durable-convergence/tests/replica-merge-laws.test.mjs` / `node --test tests/unit/persist/event-store-merge.test.mjs` |
| DURABLE-CONVERGENCE-002 | `requirements/durable-convergence/tests/replica-merge-laws.test.mjs::merge_is_commutative_associative_idempotent`（commutative/associative/idempotent/deterministic 四律）+ `tests/unit/persist/event-store-merge.test.mjs::merge_spec_oracle_associative_commutative_idempotent_deterministic`、`merge_production_associative_commutative_idempotent_deterministic` | NEW + REUSE | 各文件 `node --test` |
| DURABLE-CONVERGENCE-003 | `requirements/durable-convergence/tests/replica-merge-laws.test.mjs::production_merge_matches_the_set_union_spec_oracle`（production ≡ materialize(union) ≡ spec oracle）+ `tests/unit/persist/event-store-merge.test.mjs::merge_production_matches_materialize_of_union`、`CompareAndSwapRef_Absent_then_expected_oid` | NEW + REUSE | 各文件 `node --test` |
...
1. `tests/unit/persist/event-store-merge.test.mjs` / `event-store-converge.test.mjs`：
...
2. `tests/integration/persist/dumb-server.test.mjs`：integration 本轮不迁；cutover 时随
