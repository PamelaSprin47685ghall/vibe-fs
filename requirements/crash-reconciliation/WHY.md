...
- **固定 DevOps 崩溃恢复单一权威与命令去重**：固定 DevOps 崩溃恢复后同一道路仅保留一个逻辑执行权威；未决的真实命令绝不自动重放，真实进程随会话生命周期彻底收束，恢复严格沿用原绑定模型/Persona。
...
- DevOps 崩溃恢复后自动重放未决命令，造成命令重复执行或双重副作用。
- DevOps 崩溃恢复时篡改绑定的模型或产生双重物理执行权威。
