...
- `GAP-017` —— **OPEN**：CRASH-018 现有 oracle 手工把 `command.execute.before.output.parts` 复制进 `chat.message`，没有证明真实 Host physical user material 能拿到 dynamic restart briefing，也没有证明同一 material 的 tool-result provider step 会绕过普通 semantic transforms。补法：`tests/explicit-continue.test.mjs` 改为 physical message 只携 command template，由 production `chat.message` 自行 materialize briefing，并断言 messages transform disclosure-only。
