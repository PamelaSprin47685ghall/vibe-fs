...
- **显式 `/continue`（CRASH-018）**：config 注册 command；`command.execute.before` 只在 command=`continue` 时读取 parent 的 durable handles，逐 child 用 `ISessionSnapshotPort.GetMessages` 判 physical 可访问；可访问 child 只调用 process-local adopt（Restore + BindChildSession + parent map），不 append fact、不 send prompt。hook 把 restart/broken-tool disclosure + surviving/unavailable child 清单作为带 `wanxiangshu_explicit_resume=true` metadata 的 visible text part 交给 LLM。messages transform 只检查 trailing user material 上这个 marker；不维护 SessionId suppression latch。`chat.message` 以 exact `(SessionId, PhysicalUserMessageId)` 登记 marker，并通过 `TurnBinding.BindPhysicalUserMaterial` 只推进 reconcile 的当前物理 material：不创建 HumanRoot、不登记 PromptKey continuation。否则旧 `ActiveLogicalRun` 会让 `/continue` 保持 `UnknownOrigin`，reconcile 继续落回旧 root，fresh idle permit 随后错误生成 repair。下一条普通 user material替换 marker 时，同一物理边界推进 TurnBinding 并清除 suppression；无需 idle/abort/delete 即恢复正常。真正 reopen handle/发送 charge 留给后续普通 fork reuse，因此 resume discovery 与业务 effect 分离。
...
| CRASH-018 explicit `/continue` | `tests/explicit-continue.test.mjs`：`CRASH_018_continue_registers_a_visible_command`、`CRASH_018_non_continue_command_is_a_noop`、`CRASH_018_continue_discloses_restart_without_minting_completion`、`CRASH_018_missing_session_is_visible_and_does_not_resume`、`CRASH_018_resume_briefing_keeps_unverified_children_visible`、`CRASH_018_marked_continue_material_suppresses_the_old_active_root_on_idle`（真实 plugin hooks：durable old HumanRoot + 无 process-local user binding；command marker → chat.message → messages.transform → idle；旧实现实际发 1 条 repair prompt，新实现 0） | NEW | `node --test requirements/crash-reconciliation/tests/explicit-continue.test.mjs` |
