...
test('P0_RECOVERY_JOIN_001_crash_after_aborted_observed_stays_active', () => {
...
test('P0_RECOVERY_JOIN_001_crash_before_handle_completed_append_has_no_completion', () => {
  const handles = handlesOf(foldFacts([handleFact.linked]))
  assert.equal(lifecycleOf(handles), 'Active')
...
test('P0_RECOVERY_JOIN_001_crash_after_completed_before_consume_is_awaiting_join', () => {
...
test('P0_RECOVERY_JOIN_001_crash_after_retired_is_idempotent', () => {
...
test('P0_RECOVERY_JOIN_001_crash_matrix_no_aborted_durable_fact', () => {
