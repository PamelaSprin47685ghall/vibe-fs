...
本包唯一存在理由是：**允许同一个 logical participant 临时拥有多个 coequal execution presents，同时保持 identity、authority、children、外部 responsibility 与最终 completion cell 不分裂。** Fission lane 的边界必须由结构化参数表达；用换行符在一个长字符串里编码 lane 会让多行 charge 与 lane 分隔符冲突，不能成为可靠协议。
