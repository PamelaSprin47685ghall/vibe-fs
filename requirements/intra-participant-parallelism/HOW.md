...
OpenCode transport 还存在另一条独立 liveness 边界：`promptAsync` 的最终 assistant `message.updated` 可以正常到达，而后续 `session.status=idle` / `session.idle` 在某些 transport 上丢失。Fission 不能因此永远停在零条 `FissionLaneMaterialized`。`HostSignalCodec.tryDecodePhysicalExecutionEnd` 已只接受 no-error、completed、non-`tool-calls` 的 exact physical terminal；`HostSignalBootstrap` 在该 edge 上先调用 `Scheduler.NotifyProjectionChanged` 记下 projection epoch，再验证 `(a)` Scheduler 当前 physical user 与 edge 的 `parentID` 一致、`(b)` durable `FissionProjection.tryMembershipOfLane` 命中，然后只对该 Fission lane `Kick(RetryWake)`。这不是把 `message.updated` 升级成业务 `HostSignal`：edge 不提供 terminal outcome，`ReconcilePass` 仍从完整 SDK snapshot 产生 `TurnCompleted`，随后 `FissionHost.observeLaneTurn` 才写 lane LWR/materialized fact。先记 edge、后 Kick 让 Scheduler 原有 edge-epoch 机制继续覆盖 snapshot 暂时滞后的 lost-wakeup race；迟到的 coarse idle 只形成幂等重复观察。
...
| INTRA-PARTICIPANT-PARALLELISM-009 | `tests/fission-domain.test.mjs::WHAT[INTRA-PARTICIPANT-PARALLELISM-009] convergence requires all lane records and all completion deliveries`；`tests/fission-domain.test.mjs::WHAT[INTRA-PARTICIPANT-PARALLELISM-009] ring successor wraps and forwards past already-closed lanes to the next live present`；`tests/fission-runtime.test.mjs::WHAT[INTRA-PARTICIPANT-PARALLELISM-009] observeLaneTurn and OrdinaryTurnWorkflow absorb Fission-replaced owner turns without sending continuations`；`tests/fission-runtime.test.mjs::WHAT[INTRA-PARTICIPANT-PARALLELISM-009] exact Fission terminal bridge reconciles a completed lane even when session idle is missing`；`tests/fission-source-ratchet.test.mjs::WHAT[INTRA-PARTICIPANT-PARALLELISM-009] Host convergence performs ring takeover before reporting the old logical owner` | complete set 才可进入 takeover；closed successor 按 ring 机械跳过；exact successful lane terminal 在 durable membership + current physical match 下可在 idle 丢失时打开 snapshot reconcile occasion，但 outcome 仍只来自完整 snapshot；takeover transport receipt 后立即 durable claim `PromptKey`，不得在 lane terminal observer 内等待未来 physical id；最终 present 以 AcceptedDispatch 精确匹配 handoff physical turn，并以真实 ordinary final prose 回填 old logical owner completion cell；旧 owner turn 继续静默吸收 |
