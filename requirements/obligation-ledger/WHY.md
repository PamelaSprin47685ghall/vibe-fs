...
3. **第二写者篡夺账本写权**：除当前 Manager 之外的写者决定哪份 account 生效，制造 accepted-but-not-current 半态与多 writer 冲突，破坏崩溃恢复的可重演性。
...
反之，若破坏「Accepted 立即 supersede CurrentObligations」、引入第二账本写者或加入 `status` 冷状态机，`relay-retirement` 的判定、崩溃恢复与义务账本将同时失效。这是一个独立的失败域。

## 3. 核心不变量与失败判定

系统在以下任一情况发生时判定为 RED：

- 当前 mission debt 失去单一真相源（Host 表、外部写者或内存状态可充当 current）。
