...
import { promptResources } from '../../../tests/unit/support/domain.mjs'
...
test('CE_prompt_015_one_system_prompt_per_role_not_per_tier', () => {
...
test('CE_prompt_015_canonical_composition_common_law_role_law_office_library', () => {
  const catalog = promptResources.loadForLanguage(LANGUAGE)
...
test('CE_prompt_015_system_prompt_does_not_enumerate_runtime_tool_surface', () => {
  const catalog = promptResources.loadForLanguage(LANGUAGE)
...
test('CE_prompt_016_library_ingress_teaches_knowledge_not_authority', () => {
...
test('CE_prompt_016_office_library_closing_keeps_books_subordinate_to_assignment', () => {
  for (const locale of ['en', 'zh-CN']) {
...
test('CE_role_law_cognition_anchors_present_in_both_locales', () => {
...
test('CE_role_law_is_enduring_self_model_without_tier_split_or_hidden_orchestration', () => {
...
test('CE_agent_031_pair_hint_teaches_needhelp_as_normal_collaboration', () => {
  for (const locale of ['en', 'zh-CN']) {
...
test('CE_pair_hint_teaches_parallel_wave_without_global_concurrency_number', () => {
  for (const locale of ['en', 'zh-CN']) {
    const text = read(`resources/provider/host/pair-programming-guideline/${locale}.md`)
