...
| 015 | `src/Wanxiangshu/OpenCode/Plugin/PluginTransforms.fs` + `src/Wanxiangshu/OpenCode/Host/SessionExecutionBinding.fs`；`requirements/cognitive-environment/tests/blogger-chronicle-thought.test.mjs` | Blogger-only optional ephemeral reasoning：从当前 exact provider-attempt binding 读取 `modelID`，仅 exact allowlist 命中才启用；allowlist 当前 `Set.empty`，故默认关闭。启用时 locale 取 session binding，每次 transform 先 strip 上轮同文案 reasoning，再按当前 frontier 注入一个裸 `reasoning.text`；不走 Journal/GuidelineProjection，不借 synthetic skill wire |
