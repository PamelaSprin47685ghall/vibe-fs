...
- 使用统一的中文（或对应绑定语言）思考纪律。在采取任何动作或调用工具前，必须向同伴明确解释而不仅是隐式思考：说明当务之急是什么问题、为什么以及怎样解决，进而分析当前可并行的工具调用有哪些，把行动意图与并发分析完全透明化。
...
- 结对指引统一以 cursor 模式附加于终端工具输出尾部，不再合成任何伪装成模型调用的 wire 消息；真实存在的 `skill` 工具保持完全可用。
