...
| 010 | REUSE：`requirements/cognitive-environment/tests/semantic-anchor-parity.test.mjs::gate_c_repo_scan_is_green`（lifecycle 资源成对，机制面）；语义面由 lifecycle 资源本身承载（无独立 runtime oracle） | REUSE | `node --test requirements/cognitive-environment/tests/semantic-anchor-parity.test.mjs` |
