...
本 Proposal 中的 `LEARN-*` 是冻结历史编号，不注册为正式 Clause 前缀。目标语义按当前治理归入
Agent、Prompt、Host、Execution、Persist、Architecture、Security 与相关 proof owner；正式定义
只位于 `docs/{what,shape,how,proof,why}`。

### Remaining work

- [x] 查证兄弟 OpenCode 源码与当前依赖，完成 Session 继续、idle/nudge、内部 Agent 可见性、
      request-specific tool schema 与 execution gate 的 Host canary。
- [x] 在正式 docs 裁决 Student/Teacher 角色、统一 Satellite Session、两种 request kind、QA 生命周期、
      Skill 写入、取消/恢复/清理与 Compatible-when-disabled 边界。
- [x] 实现公开 Student、私有 Teacher、统一 SatelliteRuntime、原子 QA writer、teacher/return 工具、
      学习/编译 profile、idle reconcile、恢复与删除后 terminal。
- [x] 添加角色/工具双门、Session 连续性、QA 原子顺序与损坏、idle/return、可见性、取消和重放回归。
- [x] 运行 spec、lint、build、unit、integration、e2e/release 相关门禁并复核规范—实现—proof 一致性；
      全局门禁中的非本 Change 例外如实记录于 Final outcome。

### Completion criteria

1. 只有 HumanRoot 显式选择 fast/deep Student 才启动学习；普通 Agent 路径零副作用。
2. 每个 Student run 恰有一个私有叶子 Teacher；复用、Replacement 与无法证明三路恢复均 fail closed。
3. StudentLearn、Teacher、StudentCompile 的 provider schema 与 runtime gate 原子一致，越权调用被拒绝。
4. QA 是唯一知识状态，逐字、原子、先落盘后效果；损坏拒绝，取消与最终 return 的清理顺序可证明。
5. 同一 Student 完成一个或多个自然边界 SKILL；编译 idle 不误完成，第一版不返回学习阶段。
6. 正式 docs、实现、proof 与兼容性裁决一致，所有相关检查通过且无未解决 blocker。

### Blockers

当前没有 Student/Teacher blocker。

已撤回一项过强解释：Proposal 没有禁止 Student 的最终 `return` 先形成普通 tool result，再由同一 Host
推理循环产生 Assistant completion；`return(message): never`、terminal 顺序与 canary 只要求最终 message
成为用户回复并终止 Student Run。因此，插件工具合同没有 successful-terminal API 本身不构成 GOV-009
阻断。

两项 Host canary 均已完成：

1. Student 最终 `return` 删除 QA 后，OpenCode 的后续 Assistant completion 提交逐字 final message；
2. Teacher `return` 只完成等待中的 `teacher` 工具、普通正文不成为回答，Teacher Session 可复用。

源码证据取自兄弟仓库 OpenCode `v1.18.14`（commit
`65cf14df16c191f3e9684f0d9a8bae69103ced6d`）；精确 1.18.14 binary 的真实 Student/Teacher canary 通过。

## Final outcome

### Delivered

- 正式 Agent、Prompt、Host、Execution、Persist、Architecture、Security 及 proof 层已注册 Student/Teacher
  语义；Proposal 的 `LEARN-*` 只保留为冻结历史编号。
- 新增公开 fast/deep Student、内部 fast/deep Teacher、统一 SatelliteRuntime、StudentLearn / Teacher /
  StudentCompile 原子工具面与双层 execution gate。
- 新增 Git-private 原子 QA writer、同一 Teacher Session 的同步问答、idle→compile、SKILL 路径限制、最终
  return、取消、重启复用、Replacement 与歧义 fail-closed。
- Student provider view 不进入 Companion decode、XTrace 或 Blogger；Learn→Compile 只在显式
  request-kind 冷边界更换 tools，message prefix 保持 append-only。

### Verification

- `npm run format:check`、`npm run build`、`git diff --check` 通过。
- unit：1024/1024 通过；integration 全套通过，其中 harness 273/273 通过。
- e2e 单轮完整套件：22/22 通过；精确 OpenCode 1.18.14 的 Student/Teacher Host canary 通过。
- architecture、DSL ownership threshold 0、DSL ratchet、P0 recovery/join 门禁通过。
- `npm run lint` 已运行；唯一失败来自未跟踪且不属于本 Change 的
  `changes/proposed/Predict & Reduce Strength.md:2696`（`PROJ-008` 标题）。该文件未被修改。
- 固定三轮全局 e2e 未整体通过：精确 1.18.14 的第 1 轮中既有 `reviewer-verdict` 在首个
  `manager.0` 请求等待 5 秒后超时；同轮 `student-teacher` 通过。未使用 repeat-until-pass，也未把该
  非本 Change flaky 解释为通过。

### Closure

本 Change 的六项 Completion criteria 均由正式规范、实现、永久回归与精确 Host canary 闭环；没有遗留
Student/Teacher blocker。上述两个全局例外已隔离到未修改的外部 Proposal与既有非 Student e2e，均未被
隐瞒或纳入本 Change 的成功断言。
