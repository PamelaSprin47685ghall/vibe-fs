...
- **Requested by**：用户。先要求「根据 changes/ 里面的语义精细确定真实意图」，随后明确补充：LLM 经常上来先写一条「我要做计划」的 meta todo；之后 live 实测又出现 disguised meta-todo 与 `[•] placeholder: planning` 占位逃逸；希望第一次 todowrite 产生「一考定终身」的心理约束，可暗示其初始提交不可逆，但不要求真的删除后续更新能力。另明确：`review` 失败属于语义失败；**红字只留给语法失败**；**不得存在万象术自身失败作为 tool error**；基础设施异常应**直接杀死整个 OpenCode 并报错，方便调试人员处理**。
...
- **T1 / meta todo 最终裁决**：第一次 `todowrite` = Planning Table 已完成后的**完整 obligation account 提交**。`make a plan / analyze the request / write todos / decide next steps / 先做计划` 等描述 Manager 自己认知过程的 meta-work 不是 mission debt。**调查动词也不能洗白 meta-work**：若 `survey repository / trace startup / find O(N^2) hotspots / understand architecture / inventory risks / 调查仓库结构 / 追踪启动路径 / 寻找热点` 的目的只是为了发现真正 obligations 应该是什么，它仍属于 Planning Table cognition。判定首先用 completion counterfactual：若该项完美完成而用户要求的世界状态/交付物仍未改变，只是 Manager 更理解、有清单、有计划或知道下一步，则不是 obligation。其次必须通过 **handoff completeness**：另一位称职 Manager 仅凭 `{name,work}` 就能判断欠的 outcome 与 closure evidence；`[•] placeholder: planning`、`TBD`、裸阶段名、slot-reserving label、deferred-content entry 都是未完成规划，不是较小 obligation。只有当调查/诊断/分析/审计/报告本身就是用户要求的交付物时才例外。用户实测反例 `survey-startup-and-complexity: 调查仓库结构、启动路径与可疑 O(N^2) 热点，写出可交付的完整义务账` 明确判为 disguised meta-todo。Provider guidance 对 T1 采用 **one-shot initial-commitment framing**：让 Manager 把第一次提交当成「对初始判断的一次不可逆考试 / 没有第二次第一次提交」；后续 living account 仍可因现实变化、新证据或纠偏更新，因此该 framing 是规范性承诺压强，不得伪造成 Host 事实上的不可修改限制。
...
- **验证记录（2026-08-13 最终复验 + placeholder follow-up）**：用户 live 实测先暴露 disguised meta-todo：`survey-startup-and-complexity: 调查仓库结构、启动路径与可疑 O(N^2) 热点，写出可交付的完整义务账`，随后又出现 `[•] placeholder: planning`。前者用 completion counterfactual 判定，后者新增 handoff-completeness 判据；两个 exact case 都进入 Planning Table / `todowrite` description / `{name,work}` schema field descriptions 与 invariant。T1 guidance 现采用「one irreversible examination of initial judgment / no second first submission」的规范性 framing，同时明确后续 living account 仍可因现实/证据变化纠偏，避免把暗示误实现成 Host 不可修改。相关定向 invariant + host codec + language parity `35/35` GREEN，prompt integration `7/7` GREEN；Host 源码静态断言继续禁止 placeholder/TBD 关键词分类器。此前全栈基线仍为 Fable `build ok`、全量 unit `2498/2498`、完整 integration（含 harness `275/275`）与 Long Stroke 63 steps GREEN / journal `636/636`。本 follow-up 的全仓 `node scripts/check.mjs` 当前被并行工作树中与 Todo 无关的 `ToolResultBound.fs` / `JsAnchorFs.fs` 5 个 DSL ownership finding 阻断，故不得把本 follow-up 误报为全仓 check GREEN；相关 language-parity/prompt ownership tests 已独立通过。
