...
- [ ] **G4R-3 The Long Stroke** — skeleton landed: `tests/e2e/entry.test.mjs` + `scenarios/long-stroke.toml` + `support/long-stroke-oracles.mjs`; freeze allows exactly-one entry when present; full adversity wiring + green run still open; **event ceilings** landed (`setup.maxJournalEvents=312` / `maxSseEvents=1800`, journal envelopes + SSE sans heartbeat; harness fail-closed via `support/event-ceiling.js`) — CandidateReady path still RED
