...
> 正文中「SSOT/13 — Projection Algebra」的引用按既有裁决（`STATUS/shock-anneal.md` 包 N、
> `STATUS/design-synthetic-toml.md`）统一指向 `SSOT/01` 的 `ARCH-010`（运行时合成文本 TOML
> Instruction/Data 记法）；「Projection DSL」的完整代数语义由本文档 STRENGTH-066…074 自洽承载。
> `SSOT/13` 编号保持空缺，不建文件。
