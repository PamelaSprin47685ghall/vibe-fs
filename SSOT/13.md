...
normal request 是 data-only：行为规则只在 system prompt 一处（COMPANION-004），不把 normal instruction 写进 delta comment，也不插入独立 instruction user message。squash request 是 instruction-only（CTX-012）。
