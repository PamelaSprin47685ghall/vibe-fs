...
* 归并时 `text` 按 tool-call 顺序拼接，同一规则评分取最大值，`evidence` 按顺序去重后拼接；
...
* 声明唯一工具是 `blog`；
* 解释 `# Working Record`、`# New Work To Record` 与最终 instruction 的消息层含义；
* 要求每个请求恰好调用一次 `blog`；
...
3. Scores are confidence 0..9; omit zero-valued rule fields.
4. Optional concise `evidence` may describe the strongest nonzero findings.
5. Do not write enforcement messages into `text`.
6. Do not output ordinary assistant prose instead of calling blog.
...
每轮最终 instruction 是独立 user message（COMPANION-005），不进 TOML：
...
Frame append、coverage 推进与 enforcement 数据是同一原子提交，不允许半提交。
...
EnforcementReport 必须作为扩展后的 `BlogEntryCommitted` 的结构化附属（ScoreVectorRef / EvidenceRef / ToolCallIds / ObservedPrefixEpochId）。
禁止独立 `EnforcementCycleCommitted` 业务事实。BlogEntry、coverage 与该次评分共享同一个 provider-cycle 身份（ENFORCER-045）。
