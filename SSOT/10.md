...
原理必须继承并发扬，任何重构不得丢弃或降级。现有实现（`testkit/opencode/watchdog.js`、`scripts/run-canary-staggered.mjs`、`tests-mjs/runner.mjs`、`stability-checker.js`）只是一次并不完善的落地，它不因先存在而获得权威。
...
同一 XTrace segment 不得由两套独立 parser/renderer 生成：XTrace 是 Y delta、LWR gap、terminal capture 的共同唯一 source（COMPANION-007、COMPANION-012）。
...
```text
src/Wanxiangshu.Next/**/*.fs           生产实现，唯一 .fs 领域
...
`.mjs` 测试消费 `build/next`。若产物早于 `src/Wanxiangshu.Next/**/*.fs`，测试运行器必须拒绝运行并报错，不得使用旧产物给出绿灯。
