...
原理必须继承并发扬，任何重构不得丢弃或降级。现有实现（`testkit/opencode/watchdog.js`、`scripts/run-canary-staggered.mjs`、`tests-mjs/runner.mjs`、`stability-checker.js`）只是一次并不完善的落地，它不因先存在而获得权威。原理与实现的偏差记录在 `STATUS/shock-anneal.md` 包 W，届时按第一性原理瀑布流重建。
...
同一次 provider request
  ...
  └─ ProviderSemanticProjection（去 ID，语义相等，跨会话）
        └─ BloggerDeltaProjection（去图片内容，TOML 文本，≤200 KiB 切块）
```
