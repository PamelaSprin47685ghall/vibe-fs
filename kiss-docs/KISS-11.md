...
- **启动**：Gateway 在该 Epoch **初始 Host turn 终态**后激活**本 Runtime** Driver（KISS-Driver）；非 Human 到达瞬间。  
...
Owner / Stage / 多 Generation / Lease / 复合 State / FallbackPhase / Nudge* → Driver、Epoch、while、尾递归、PromptProtocol。
...
|Generation 家族|本 Runtime Epoch + Host MessageOrigin|
...
无工作 Finish；Todo 轮；网错；模型链；Review 轮；Invalid 有界；取消；parent 关；重启；重复事件；迟到 Epoch；NoProgress；Plugin 消息不升 Epoch。
