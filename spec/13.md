# spec/13 — 运行时 LLM 可见合成 TOML 的解释规范

主条款位置：`spec/01.md` 的 `ARCH-010`
...
`ARCH-010` 是主规范。本文件不重新定义 `ARCH-010`，只固定其解释边界、例表、迁移判据与禁止实现清单。冲突时以 `spec/01.md` 为准。
...
先以 `spec/01.md` 的 `ARCH-010` 主条款冻结规则，再迁移生产 prompt。
