 ...
 * `Hosts/Omp/NudgeRuntime.fs`
  ...
  * 全局 stores map 已提供 `remove` / `tryRemove` / `clear` / `count` / `ids` 操作；
  * `RuntimeScopeForgetSession.forgetSession` 在 session close 时调用 `remove`，实现 workspaceRoot 级 store 释放；
  * `tests/RuntimeScopeLifecycleTests.fs` 以 1000 session 压力测试验证 store 数量回到基线。
