...
它不是单纯的工具集合，而是把一套多代理工作流能力落到两个不同宿主上：

- OpenCode 风格插件：`src/Opencode/`
- Mux 风格注册表：`src/Mux/`
...
- 搜索/读取/执行/审查/问答工具
- review loop、todo folding、wiki/bookkeeper、nudge、editplus
...
4. review、wiki、nudge、editplus 这类能力都跨多轮对话，且必须在重启后尽量恢复。
...
- review 是一个有限状态机
- nudge 是一个有限状态机
- wiki 是 append / daily rewrite / weekly rewrite 三种作业
...
- `src/Opencode/WikiRuntime.fs` 里的 `commandQueue` / `launchQueue` / `writeQueues`
...
### Opencode / Mux：两个宿主，两个适配面

因为两个宿主对插件的暴露方式不同，所以项目没有强行用一层巨型通用抽象吞掉差异，而是保留两个适配面：

- `src/Opencode/`：直接面向 OpenCode 插件 hook、tool schema、消息变换
...
2. 纯维护策略：何时 daily/weekly rewrite
3. 进程内 runtime 状态
...
### 6. editplus 为什么是 tag，不是行号

因为编辑中的行号天然不稳定。只要插入或删除一段文本，后续引用的绝对行号就会漂移。

所以 editplus 选择“会话内稳定 tag”这套协议：

- 读取时给行分配 tag
- 编辑时通过 tag 区间定位
- 变动区间产生新 tag，未变行尽量保留旧 tag
- 状态还能从消息历史 replay
...
### 7. 为什么大量提示词都用 YAML front-matter
...
- `pnpm build` 会执行 `dotnet fable vibe-fs.fsproj --outDir build`
- 构建产物位于 `build/`
- 包导出入口是 `build/index.js`
