...
- Mux 风格注册表：`src/Mux/`
- oh-my-pi（`@oh-my-pi`）Pi 扩展：`src/Omp/`（入口 `Plugin.fs` → `kunweiExtension`；npm 子路径 `vibe-fs/omp`）

## 项目要解决的真实问题

表面提供：子代理委派、文件读写、模糊搜索、执行器、审查循环、knowledge graph、nudge、53 个结构化方法论笔记本工具（`methodology_*`）、OpenCode / Mimocode / Mux / OMP 四宿主接线。
...
3. 工具/权限/意图元数据：`ToolCatalog`/`Config`/`Subagent`/`SubagentIntents`/`HostTools`（含 `Omp`）/`ToolPermission`/`ToolCopy`/`ToolResult`/`ToolArgs`/`ToolContext`/`ToolOutputInfo`/`OmpSessionTools`/`WebFetchGuard`
...
5. 纯算法或纯解析：`FuzzyQuery`/`FuzzyPath`/`FuzzyFormat`/`Executor`/`TreeSitterKernel`/`Domain`/`Methodology`（`select_methodology` 枚举与 `todowrite` 文案）
...
- `src/Omp/`：oh-my-pi `kunweiExtension`、Pi 事件钩子、`ChildSession` 子 workspace、工具 schema（`OmpToolSchema`）
...
共享同一个 Kernel；差异留在各层：schema 生成、tool execute 签名、消息对象形态、子代理启动（OpenCode `ChildAgentRegistry` / Mux delegate / OMP `taskService`+子 workspace）、`task`/`todowrite` 命名（`HostTools` 按 `Opencode|Mimocode|Mux|Omp` 分支）。`src/Omp/` 仅依赖 Kernel+Shell，禁止 `open` / 引用 `VibeFs.Opencode`、`VibeFs.Mux`。
...
8. `src/Opencode/KnowledgeGraphRuntime.fs`（对照 `src/Omp/KnowledgeGraphRuntime.fs`）
...
- KnowledgeGraph 日维护：OpenCode/Mux runtime 经 `Shell.KnowledgeGraphMaintenanceRun.runMaintenanceIfDue`；禁止在 Opencode/Mux 再抄一套未共享的 `launchIfDue` 调度（OMP 在 `Omp/KnowledgeGraphRuntime` 内维护，与 Shell 策略语义对齐）
...
- `src/Omp/` 禁止 `open` / 引用 `VibeFs.Opencode`、`VibeFs.Mux`、`engine/`
...
2. `src/Omp/Plugin.fs` + `PluginCore.fs`：OMP `kunweiExtension`、Pi 事件、子 workspace、KG/Magic/review 接线
...
src/
  ...
  Methodology/  53 个方法论 schema + Registry + Opencode/Mux 工具注册
  Opencode/     OpenCode / Mimocode 插件适配层与 TUI 扩展
  Mux/          Mux 注册与 wrapper 适配层
  Omp/          oh-my-pi 扩展适配层（仅 Kernel+Shell）
tests/          纯内核 + Shell + 集成 + 插件契约 + 架构边界探针（含 Omp*Tests）
