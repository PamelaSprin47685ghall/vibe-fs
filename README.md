...
3. 宿主会 compaction，对话历史不可靠；durable 工作状态须落在 `.wanxiangshu.ndjson`（`docs/05-event-sourcing.md`）。
...
`PromptFrontMatter` 仍解析多块 YAML（工具输出、用户可读锚点）。**不再**把 multi-frontmatter 或 compaction-anchor prompt 当作 review/todo 的真相源；durable 语义见 `docs/05-event-sourcing.md`。
...
全覆盖技术文档见 **[`docs/00-index.md`](./docs/00-index.md)**（`docs/01`–`docs/19`）。
