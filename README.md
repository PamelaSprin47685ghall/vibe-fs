...
4. 历史折叠与消息语义
   ...
   - `MagicCore.fs` + `MagicProjection.fs` + `MagicTodo.fs`
...
- 编解码边界：`SubagentIntentsCodec.fs`、`WebSearchCodec.fs`
- 运行时 store/queue：`PromiseQueue.fs`、`MagicSessionStore.fs`、`ChildAgentRegistry.fs`
- review/nudge runtime：`ReviewRuntime.fs`、`NudgeRuntime.fs`
