...
本轮已修：**Phase C** 双宿主 `SubagentTools` 并行 spawn 收拢 **`Shell.SubagentSpawn`**（`subagentToolsUseSubagentSpawn`）；**Phase C** `MessageTransformCore.backlogSessionOpsFrom` 消除双宿主内联 `BacklogSessionOps`（`messageTransformUsesBacklogSessionOpsFrom`）；**Phase C** Mux read 去重收拢 **`Shell.ReadDedupMuxPlugin`**（`muxMessageTransformUsesReadDedupMuxPlugin`）；**Phase E** **`RuntimeScope`** 删除模块级 projection 委托（`projectionOf` 与 `captureReport`/`takeReport`/`tryGetReport`/`storeBacklog`/`tryGetBacklog`），会话投影读写仅经注入 **`scope.Projection`**（`runtimeScopeNoModuleProjectionHelpers`）；**`ArchitectureTests`** 修括号/门禁登记。静态门禁见 `tests/ArchitectureTests.fs`（**4882** 绿）。

**已知技术债（诚实标注，待下一迭代）**：`IToolRuntimeContext` 未在除 **Mux** web/read/write/fuzzy/executor、`SubagentTools` bind（参数已 codec）、`ReviewToolsMux`、`Wrappers` syntax/todo、KG `StartBookkeeperAppend`/`KnowledgeGraphToolDefs`、**Opencode** SearchTools/Executor/`ReviewTools`/`SubagentTools`/`PluginCore`/`KnowledgeGraphTools`/部分 hook 外的路径普遍采用（双宿主 **`MessageTransform`** 消息体与工具 part、Delegate、`SubagentTools` spawn/schema 等仍 `Dyn`）；**`RuntimeScope.getDefault()`**、模块级 **`enqueuePerSession`** 仍为兼容退路。

**剩余主线**：收宿主 `Dyn`→codec（Phase B，双宿主 `MessageTransform` 消息/part 余量）；`MessageTransform` 宿主差异继续收敛（Phase C，`SubagentTools` 注册/schema 仍宿主侧，并行 spawn 已 `SubagentSpawn`）；`Result` 链与其余工具 codec（Phase D 余量）；收拢 `RuntimeScope.getDefault()` 等模块级退路（Phase E）；全工具 `IToolRuntimeContext` + 统一 `ToolArgs`（Phase G）与 §7 文案目录余量（hook/spawn 等未收拢字段）。
