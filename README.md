...
review/todo 状态都不是"当前进程内存"能可靠承诺的。进程会重启、hook 会打断、background session 延后返回。宿主的消息历史天然存在，应被视为事实来源；进程内 store 只能可重建投影。
...
更硬的纪律：`reviewStore` / `sessionManager` / 任意 runtime store 都只是缓存投影，不是 loop/nudge/replay 的真相源。凡是"当前 session 是否仍处于 with-review / loop mode"、"上一轮是否已经 nudge"、"子会话是否应继承 loop"这类状态机判断，一律先从消息历史 front-matter / durable tool output 纯重放，再把结果塞进快照交给 Kernel；禁止在 nudge 决策链里直接读取内存布尔态，否则重启、子会话、compaction、延迟持久化一来就会把假状态当事实。
...
补充铁律：nudge 的输入必须是历史重建出的 `SessionSnapshot`，不能把任何内存活跃查询、`alreadyNudged` 之类状态直接穿进 `Decision`。一旦某宿主想知道 loop 是否活跃，先 replay 历史，后做决策。

### 提示词 YAML front-matter

顶部结构化字段程序精确识别，下方 prose 模型友好。重启后 replay 只读 scalar 字段，不必全文 NLP。

### `multi-frontmatter` 作为等效事实源

同一条 `user`/`assistant` 消息可携带多个 YAML front-matter 块；`PromptFrontMatter` 按块顺序解析与合并。compaction 完成后，由一次真实 `prompt()` 发出固定正文 `See above for some messages before compaction.`，并把被折叠掉的 anchors（backlog、task、KG 等）以多块 front-matter 形式重新挂回普通历史，供 replay / review / KG 恢复。
...
- `NudgeEffect` / `NudgeSnapshot` / `Omp.NudgeHooks` 等 nudge 状态机入口禁止直接读任何内存活跃态查询；loop 事实必须经 `ReviewReplayPolicy`/历史文本重建
...
6. `src/Opencode/MessageTransform.fs` + `src/Mux/MessageTransform.fs` + `src/Omp/MessageTransform.fs`：caps prelude、KG prelude、Magic todo、read dedup、review replay
