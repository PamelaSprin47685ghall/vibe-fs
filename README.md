...
在 OpenCode、Mimocode、Mux、oh-my-pi 里用的**多代理开发插件**：子代理帮你写代码、查仓库、搜网页、跑命令；带审查循环、待办与进度交接、方法论笔记本，并在重启后仍能恢复工作状态。
...
| 能力 | 说明 |
| :--- | :--- |
| 子代理 | `coder`、`investigator`、`browser` 等；支持 `continue` 对同一会话追问 |
| 审查 | `/loop <任务>` 进入 With-Review；`submit_review` / 审查者 `return_reviewer` |
| 待办与交接 | `todowrite`（Mux / OpenCode / OMP）或 Mimocode 的 `task`；须带完整待办列表与五份工作报告 |
| 方法论 | 54 个 `methodology_*` 工具 + `meditator`；与待办里的 `select_methodology` 联动 |
| 工具箱 | 读写改、模糊搜索、网页搜索/抓取、执行器（含模式与输出上限约束）、OpenCode/Mimocode 的 PTY 等 |
...
工作区根目录会维护 **`.wanxiangshu.ndjson`**（及锁文件）；万象阵 `squad_*`/`task_*` 事件与万象术事件**共用**该文件。审查任务、待办快照、催促记录等**持久状态**写在这里，而不是只靠聊天历史。用户一般无需手改该文件。
...
5. **从源码参与开发**：需要 .NET（`net10.0`）、Fable、`npm run build-and-test`（完整构建+测试约数十秒）。日常改代码请遵守仓库 `AGENTS.md`（宿主上游改动范围、hook 写字段方式等）。

---

## 和万象阵的分工

- **万象术**：单工作区内的多代理、审查、事件日志、待办 SSOT。
...
**一句话**：万象术把「委派、审查、待办、持久进度」做成可插拔的多宿主插件；你要的是更稳的长程协作，而不是多几个零散工具名。